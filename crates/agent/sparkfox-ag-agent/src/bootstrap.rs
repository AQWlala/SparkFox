...
use sparkfox_ag_providers::LlmProvider;
...
impl sparkfox_ag_browser::extract::ExtractModel for SessionExtractModel {
...
#[cfg(feature = "browser-use")]
#[async_trait::async_trait]
impl sparkfox_ag_browser::visual_fallback::VisualLocator for SessionVisualLocator {
    async fn locate(
        &self,
        screenshot: &[u8],
        instruction: &str,
    ) -> Result<sparkfox_ag_browser::visual_fallback::VisualLocateResult, String> {
    ...
    async fn locate_labeled(
        ...
        instruction: &str,
        n_labels: usize,
    ) -> Result<sparkfox_ag_browser::visual_fallback::SomLabelResult, String> {
...
/// Parse the vision model's reply into a [`VisualLocateResult`](sparkfox_ag_browser::visual_fallback::VisualLocateResult).
///
...
fn parse_visual_locate_result(
    ...
    use sparkfox_ag_browser::visual_fallback::{PixelBox, VisualLocateResult};
...
/// Parse the vision model's SoM reply into a [`SomLabelResult`](sparkfox_ag_browser::visual_fallback::SomLabelResult).
///
...
fn parse_som_locate_result(
    ...
    use sparkfox_ag_browser::visual_fallback::SomLabelResult;
...
pub struct AgentBootstrap {
    ...
    /// the raw pieces (NOT the `sparkfox_ag_browser` type) so the field exists regardless of the
    ...
    /// (default) → fail-closed (current behavior). Feature-gated: the trait is in `sparkfox_ag_browser`.
    #[cfg(feature = "browser-use")]
    approval_gate: Option<Arc<dyn sparkfox_ag_browser::BrowserApprovalGate>>,
}
...
impl AgentBootstrap {
    ...
    pub fn approval_manager(mut self, mgr: Arc<sparkfox_ag_protocol::ToolApprovalManager>) -> Self {
    ...
    /// callers (`sparkfox-be-ai-agent`) need not depend on `sparkfox-ag-browser` to wire it. Omit it
    ...
    #[cfg(feature = "browser-use")]
    pub fn approval_gate(mut self, gate: Arc<dyn sparkfox_ag_browser::BrowserApprovalGate>) -> Self {
    ...
    pub async fn build(mut self) -> anyhow::Result<BootstrapResult> {
        ...
        tracing::info!(target: "sparkfox_ag_agent", workspace = %cwd, "agent bootstrap: workspace cwd resolved");

        let provider = self
            .provider
            .unwrap_or_else(|| sparkfox_ag_providers::create_provider(&self.config));

        let memory_dir = sparkfox_ag_memory::paths::auto_memory_dir(cwd_path);

        let file_cache = if self.config.file_cache.enabled {
            Some(Arc::new(std::sync::RwLock::new(
                sparkfox_ag_tools::file_cache::FileStateCache::new(&self.config.file_cache),
            )))
        } else {
            None
        };

        let mut registry = sparkfox_ag_tools::registry::ToolRegistry::new();
        ...
        registry.register(Box::new(sparkfox_ag_tools::read::ReadTool::new(
            file_cache.clone(),
            Some(cwd_path.to_path_buf()),
        )));
        registry.register(Box::new(
            sparkfox_ag_tools::write::WriteTool::new(file_cache.clone())
                .with_write_root(write_root.clone())
                .with_cwd(Some(cwd_path.to_path_buf())),
        ));
        registry.register(Box::new(
            sparkfox_ag_tools::edit::EditTool::new(file_cache.clone())
                .with_write_root(write_root.clone())
                .with_cwd(Some(cwd_path.to_path_buf())),
        ));
        registry.register(Box::new(
            sparkfox_ag_tools::apply_patch::ApplyPatchTool::new(file_cache)
                .with_write_root(write_root.clone())
                .with_cwd(Some(cwd_path.to_path_buf())),
        ));
        ...
        // one language server is configured (default off → no behaviour change).
        {
            ...
            if !lsp_map.is_empty() {
                registry.register(Box::new(sparkfox_ag_tools::lsp::LspTool::new(
                    lsp_map,
        ...
        let process_supervisor =
            sparkfox_sh_process_runtime::ProcessSupervisor::new(sparkfox_sh_process_runtime::SupervisorConfig::default());
        let process_capability = sparkfox_sh_process_runtime::CapabilityPolicy {
            cwd_roots: vec![cwd_path.to_path_buf()],
            sandbox: if self.config.tools.bash_sandbox {
                sparkfox_sh_process_runtime::SandboxPolicy::MacSeatbelt {
                    write_roots: vec![cwd_path.to_path_buf()],
                }
            } else {
                sparkfox_sh_process_runtime::SandboxPolicy::UnrestrictedLocalOwner
            },
        };
        ...
        registry.register(Box::new(sparkfox_ag_tools::bash::BashTool::new(
            Arc::clone(&process_supervisor),
            cwd_path.to_path_buf(),
            process_capability.clone(),
        )));
        registry.register(Box::new(sparkfox_ag_tools::grep::GrepTool::new(
            cwd_path.to_path_buf(),
        )));
        registry.register(Box::new(sparkfox_ag_tools::glob::GlobTool::new(
            cwd_path.to_path_buf(),
        )));
        ...
        let process_store = Arc::new(sparkfox_ag_tools::process_store::ProcessStore::new());
        registry.register(Box::new(sparkfox_ag_tools::exec_command::ExecCommandTool::new(
            Arc::clone(&process_supervisor),
            Arc::clone(&process_store),
            cwd_path.to_path_buf(),
            process_capability.clone(),
        )));
        registry.register(Box::new(sparkfox_ag_tools::write_stdin::WriteStdinTool::new(
            Arc::clone(&process_supervisor),
            Arc::clone(&process_store),
        )));
        ...
        let skills = sparkfox_ag_skills::loader::load_all_skills(
            cwd_path,
        ...
        let skill_checker = sparkfox_ag_skills::permissions::SkillPermissionChecker::new(
        ...
        let skill_invocation_runner = local_invocation_runner.as_ref().map(|runner| {
            Arc::clone(runner) as Arc<dyn sparkfox_ag_types::agent::AgentInvocationRunner>
        });
        registry.register(Box::new(
        ...
        if self.config.tools.computer.enabled {
            ...
            registry.register(Box::new(sparkfox_ag_computer::ComputerTool::new(
        ...
        if self.config.tools.browser.enabled {
            ...
            let mut browser_tool = sparkfox_ag_browser::BrowserTool::with_policy(
                ...
                self.browser_secret_source
                    .clone()
                    .map(|(vault_path, key)| sparkfox_ag_browser::BrowserSecretSource { vault_path, key }),
            );
            ...
            if self.config.tools.browser.site_memory {
                let sm_root = sparkfox_ag_config::config::app_config_dir()
                    .map(|d| d.join("browser-data").join("site-memory"))
                    .unwrap_or_else(|| std::env::temp_dir().join("sparkfox-ag-browser-data").join("site-memory"));
                let sink = sparkfox_ag_browser::site_memory::FileSiteMemorySink::new(sm_root);
                let store = std::sync::Arc::new(sparkfox_ag_browser::site_memory::SiteMemoryStore::new(
                    Box::new(sink),
                ));
        ...
        registry.register(Box::new(sparkfox_ag_tools::update_plan::UpdatePlanTool::new()));
        ...
        registry.register(Box::new(sparkfox_ag_tools::tool_search::ToolSearchTool::new(
            deferred_state,
        )));
        if let Some(manager) = &mcp_manager {
            sparkfox_ag_mcp::tool_proxy::register_mcp_tools(
                &mut registry,
                manager,
