...
use sparkfox_ag_types::message::{Message, TokenUsage};
...
impl SessionManager {
    ...
    fn update_index(&self, session: &Session) -> anyhow::Result<()> {
        ...
        let summary = session
            .messages
            .iter()
            .find(|m| m.role == sparkfox_ag_types::message::Role::User)
            .and_then(|m| {
                m.content.iter().find_map(|c| {
                    if let sparkfox_ag_types::message::ContentBlock::Text { text } = c {
