...
//! a concrete sink; standalone `sparkfox-ag-cli` passes `None` and these are not registered.
...
/// its `RequirementService`; `sparkfox-ag-agent` only depends on this trait.
#[async_trait]
