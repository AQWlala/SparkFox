...
use sparkfox_ag_config::compact::CompactConfig;
use sparkfox_ag_config::config::Config;
use sparkfox_ag_config::hooks::HookEngine;
use sparkfox_ag_protocol::events::ToolCategory;
use sparkfox_ag_providers::{LlmProvider, ProviderError, create_provider};
use sparkfox_ag_tools::registry::ToolRegistry;
use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
use sparkfox_ag_types::message::{ContentBlock, Message, Role, StopReason, TokenUsage};
use sparkfox_ag_types::skill_types::{ContextModifier, PlanModeTransition, effort_to_string};
...
/// guard. See docs/superpowers/specs/2026-06-21-sparkfox-ag-agent-overhaul-design.md §5 F0.3.
...
impl ToolEfficiencyStats {
    ...
    fn log(
        ...
        tracing::info!(
            target: "sparkfox_ag_agent::tool_efficiency",
            session_id,
            msg_id,
            agent_turns,
            stop_reason,
...
pub struct AgentEngine {
    ...
    thinking: Option<sparkfox_ag_types::llm::ThinkingConfig>,
    /// Resolved provider compat settings (for capability validation)
    compat: sparkfox_ag_config::compat::ProviderCompat,
    ...
    approval_manager: Option<Arc<sparkfox_ag_protocol::ToolApprovalManager>>,
    protocol_writer: Option<Arc<dyn sparkfox_ag_protocol::writer::ProtocolEmitter>>,
    ...
    compaction_level: sparkfox_ag_compact::CompactionLevel,
    toon_enabled: bool,
    ...
    process_supervisor: Option<Arc<sparkfox_sh_process_runtime::ProcessSupervisor>>,
...
impl AgentEngine {
    ...
    pub fn set_process_supervisor(
        &mut self,
        supervisor: Arc<sparkfox_sh_process_runtime::ProcessSupervisor>,
    ) {
        assert!(
    ...
    pub async fn shutdown_processes(&self) -> Option<sparkfox_sh_process_runtime::ShutdownReport> {
        let supervisor = self.process_supervisor.as_ref()?;
        Some(supervisor.shutdown().await)
    }

    pub fn compaction_level(&self) -> sparkfox_ag_compact::CompactionLevel {
    ...
    pub fn compat(&self) -> &sparkfox_ag_config::compat::ProviderCompat {
        &self.compat
    }
    ...
    pub fn init_session(
        ...
        if let Some(mgr) = &self.session_manager {
            ...
            tracing::info!(target: "sparkfox_ag_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub fn set_approval_manager(&mut self, mgr: Arc<sparkfox_ag_protocol::ToolApprovalManager>) {
        self.approval_manager = Some(mgr);
    }

    pub fn set_protocol_writer(&mut self, writer: Arc<dyn sparkfox_ag_protocol::writer::ProtocolEmitter>) {
    ...
    pub fn apply_config_update(
        ...
        if let Some(thinking_str) = thinking {
            if !self.compat.supports_thinking() {
                ...
                match thinking_str.as_str() {
                    "enabled" => {
                        ...
                        self.thinking = Some(sparkfox_ag_types::llm::ThinkingConfig::Enabled {
                            budget_tokens: budget,
                        });
                        changes.push(format!("thinking: enabled (budget: {budget})"));
                    }
                    "disabled" => {
                        self.thinking = Some(sparkfox_ag_types::llm::ThinkingConfig::Disabled);
            ...
            && let Some(sparkfox_ag_types::llm::ThinkingConfig::Enabled { budget_tokens }) =
                &mut self.thinking
        {
        ...
        if let Some(ref level_str) = compaction {
            match level_str.parse::<sparkfox_ag_compact::CompactionLevel>() {
                Ok(new_level) => {
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            tracing::info!(target: "sparkfox_ag_agent", last_input_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                tracing::info!(target: "sparkfox_ag_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        if let Ok(runtime) = tokio::runtime::Handle::try_current() {
            ...
            let _ = std::thread::Builder::new()
                .name("sparkfox-engine-process-cleanup".to_owned())
                .spawn(move || {
...
mod set_config_tests {
    ...
    use sparkfox_ag_tools::{Tool, registry::ToolRegistry};
    use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
    use sparkfox_ag_types::message::{ContentBlock, Role};
    ...
    impl LlmProvider for NullProvider {
        async fn stream(
            ...
            tx.send(LlmEvent::Done {
                stop_reason: sparkfox_ag_types::message::StopReason::EndTurn,
    ...
    impl LlmProvider for LoopProvider {
        async fn stream(
            ...
            let _ = tx
                .send(LlmEvent::Done {
                    stop_reason: sparkfox_ag_types::message::StopReason::ToolUse,
                    usage: Default::default(),
                })
                .await;
            Ok(rx)
        ...
        async fn stream(
            ...
            tx.send(LlmEvent::Done {
                stop_reason: if self.as_deltas {
                    sparkfox_ag_types::message::StopReason::EndTurn
                } else {
                    sparkfox_ag_types::message::StopReason::ToolUse
                },
                usage: Default::default(),
            })
            .await
    ...
    fn make_engine(model: &str) -> super::AgentEngine {
        super::AgentEngine {
            ...
            thinking: None,
            compat: sparkfox_ag_config::compat::ProviderCompat::anthropic_defaults(),
            ...
            allow_list: vec![],
            current_reasoning_effort: None,
            compact_config: sparkfox_ag_config::compact::CompactConfig::default(),
            compact_state: super::CompactState::new(),
            plan_state: Default::default(),
            plan_active_flag: None,
            cache_detector: super::CacheBreakDetector::new(),
            compaction_level: sparkfox_ag_compact::CompactionLevel::default(),
            toon_enabled: false,
            max_recent_images: 3,
            commands: crate::commands::default_registry(),
            goal: None,
    ...
    async fn provider_error_preserves_completed_tool_pair_but_strips_its_image() {
        let mut engine = make_engine("vision-model");
        engine.messages = vec![
            sparkfox_ag_types::message::Message::new(
                Role::User,
            ...
            sparkfox_ag_types::message::Message::new(
                ...
                vec![ContentBlock::ToolResult {
                    ...
                    is_error: false,
                    images: vec![sparkfox_ag_types::tool::ToolImage {
                        media_type: "image/png".into(),
                        data: "A".repeat(1024),
                    }],
                }],
    ...
    async fn execute_turn_with_content_rejects_forged_tool_blocks() {
        let mut engine = make_engine("vision-model");
        engine.messages.push(sparkfox_ag_types::message::Message::new(
            Role::User,
    ...
    async fn provider_error_after_autocompaction_restores_content_checkpoint() {
        ...
        engine.messages = vec![sparkfox_ag_types::message::Message::new(
            Role::User,
            vec![ContentBlock::Text {
    ...
    fn rewind_last_turn_truncates_to_marker() {
        use sparkfox_ag_types::message::{ContentBlock, Message, Role};
    ...
    async fn repeated_tool_search_is_bounded_by_stagnation_guard() {
        ...
        engine
            .tools
            .register(Box::new(sparkfox_ag_tools::tool_search::ToolSearchTool::new(
                deferred_state,
            )));
    ...
    async fn provider_turn_accepts_exactly_128_tool_calls() {
        ...
        assert_eq!(result.stop_reason, sparkfox_ag_types::message::StopReason::EndTurn);
    ...
    fn set_config_changes_effort() {
        let mut engine =
            make_engine_with_compat("m", sparkfox_ag_config::compat::ProviderCompat::openai_defaults());
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(sparkfox_ag_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(sparkfox_ag_types::llm::ThinkingConfig::Enabled {
            budget_tokens: 8000,
        });
        let changes = engine.apply_config_update(None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(sparkfox_ag_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_all_fields_at_once() {
        let compat = sparkfox_ag_config::compat::ProviderCompat {
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(sparkfox_ag_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(sparkfox_ag_types::llm::ThinkingConfig::Disabled) => {}
...
mod phase6_tests {
    ...
    use sparkfox_ag_tools::registry::ToolRegistry;
    use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
    use sparkfox_ag_types::skill_types::{ContextModifier, EffortLevel};
...
mod compact_tests {
    ...
    use sparkfox_ag_tools::registry::ToolRegistry;
    use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
    use sparkfox_ag_types::message::{ContentBlock, Message, Role, StopReason};
    use serde_json::json;
...
mod plan_mode_tests {
    ...
    use sparkfox_ag_tools::registry::ToolRegistry;
    use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
    use sparkfox_ag_types::skill_types::{ContextModifier, PlanModeTransition};
...
mod tool_efficiency_tests {
    ...
    use sparkfox_sh_process_runtime::{CapabilityPolicy, ProcessSupervisor, SupervisorConfig};
    use sparkfox_ag_tools::{
