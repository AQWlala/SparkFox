                ...
                *ctx.messages = result.messages;
                ...
                if let Some(boundary) = ctx.messages.first_mut() {
                    for block in &mut boundary.content {
                        if let sparkfox_ag_types::message::ContentBlock::Text { text } = block
                            && text.starts_with(auto::BOUNDARY_PREFIX)
                        {
                            let metadata = sparkfox_ag_types::compact::CompactMetadata {
...
mod tests {
    ...
    async fn compact_already_compact_guard() {
        ...
        let config = sparkfox_ag_config::compact::CompactConfig::default();
