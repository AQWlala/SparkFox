...
//! through a `CompanionMemorySink` trait object. The backend (sparkfox-be-companion)
...
/// by `sparkfox-be-companion` over its `CompanionStore`; `sparkfox-ag-agent` only depends on this.
#[async_trait]
...
/// `sparkfox-be-companion` over its store + `skill_service`; `sparkfox-ag-agent` only depends
