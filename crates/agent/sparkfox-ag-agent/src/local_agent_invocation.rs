...
use sparkfox_ag_tools::bash::BashTool;
use sparkfox_ag_tools::edit::EditTool;
use sparkfox_ag_tools::glob::GlobTool;
use sparkfox_ag_tools::grep::GrepTool;
use sparkfox_ag_tools::read::ReadTool;
use sparkfox_ag_tools::registry::ToolRegistry;
use sparkfox_ag_tools::write::WriteTool;
use sparkfox_ag_types::message::TokenUsage;
...
impl LocalAgentInvocationRunner {
    ...
    async fn invoke_with_effective_scope(
        ...
        tracing::info!(target: "sparkfox_ag_agent", cwd = %self.cwd.display(), "delegated Agent invocation started");
    ...
    pub(crate) async fn execute_fanout(
        ...
        let workspace_plan = FanoutWorkspacePlan::for_scopes(
            &effective_scopes,
            sparkfox_ag_tools::worktree::is_git_repo(&self.cwd),
        );
        ...
        let worktree_baseline = if workspace_plan.isolate_mutating {
            ...
            Some(
                match tokio::task::spawn_blocking(move || {
                    sparkfox_ag_tools::worktree::WorktreeBaseline::capture(&cwd)
                })
                .await
                {
    ...
    async fn invoke_isolated_one(
        ...
        contributor: Option<Arc<dyn ContextContributor>>,
        effective_scope: ToolScope,
        baseline: Arc<sparkfox_ag_tools::worktree::WorktreeBaseline>,
    ) -> AgentInvocationOutput {
    ...
    ///
    /// The path is accepted only after [`sparkfox_ag_tools::worktree::Worktree`]
    ...
    fn clone_for_isolated_worktree(
        &self,
        worktree: &sparkfox_ag_tools::worktree::Worktree,
    ) -> Result<Self, String> {
        let source = self
            .cwd
            .canonicalize()
        ...
        if let sparkfox_sh_process_runtime::SandboxPolicy::MacSeatbelt { write_roots } =
...
fn build_tool_registry(
    ...
    for descriptor in CHILD_TOOL_CATALOG {
        ...
        let tool: Box<dyn sparkfox_ag_tools::Tool> = match descriptor.kind {
...
mod phase7_tests {
    ...
    use async_trait::async_trait;
    use sparkfox_sh_process_runtime::{CapabilityPolicy, ProcessSupervisor, SandboxPolicy, SupervisorConfig};
    use sparkfox_ag_config::compat::ProviderCompat;
    use sparkfox_ag_config::config::{Config, ProviderType};
    use sparkfox_ag_providers::{LlmProvider, ProviderError};
    use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
    use sparkfox_ag_types::message::{StopReason, TokenUsage};
    use sparkfox_ag_types::agent::{AgentInvocationInput, AgentToolPolicy};
    ...
    fn test_registry(
        ...
        cwd: &std::path::Path,
        capability: CapabilityPolicy,
    ) -> Result<sparkfox_ag_tools::registry::ToolRegistry, String> {
        build_tool_registry(
            scope,
            cwd,
            &capability,
    ...
    fn delegated_and_synthesis_agents_never_inherit_executable_parent_hooks() {
        ...
        config.hooks.post_tool_use.push(sparkfox_ag_config::hooks::HookDef {
        ...
        config.hooks.stop.push(sparkfox_ag_config::hooks::HookDef {
    ...
    fn isolated_worktree_translates_authority_to_the_exact_verified_worktree() {
        ...
        let worktree =
            sparkfox_ag_tools::worktree::Worktree::create(&nested).expect("create worktree");
