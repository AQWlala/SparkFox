...
//! bound bases; standalone `sparkfox-ag-cli` passes `None` and the tool is absent.
//!
//! Mirrors `requirement_tools.rs`: trait here, impl in `sparkfox-be-ai-agent`.
...
/// `KnowledgeService`; `sparkfox-ag-agent` only depends on this trait.
#[async_trait]
...
/// `KnowledgeService::write_file`; `sparkfox-ag-agent` only depends on this trait.
///
...
/// Backend seam for knowledge write-back. Implemented by the backend over its
/// `KnowledgeService::write_document`; `sparkfox-ag-agent` only depends on this trait.
