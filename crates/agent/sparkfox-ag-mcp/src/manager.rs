    ...
    pub async fn connect_all(configs: &HashMap<String, McpServerConfig>) -> Result<Self, McpError> {
        ...
        for (name, config) in configs {
            match tokio::time::timeout(MCP_CONNECT_TIMEOUT, Self::connect_server(name, config)).await {
                Ok(Ok(server)) => {
                    tracing::info!(target: "sparkfox_ag_mcp", server = %name, tools = server.tools.len(), resources = server.supports_resources, "mcp server connected");
                    servers.insert(name.clone(), server);
                }
                Ok(Err(e)) => {
                    // Non-fatal: continue with other servers
                    tracing::warn!(target: "sparkfox_ag_mcp", server = %name, error = %e, "mcp server connection failed");
                }
                Err(_) => {
                    // Non-fatal: a hung handshake must not block the other
                    // servers or the agent bootstrap. Skip this server.
                    tracing::warn!(target: "sparkfox_ag_mcp", server = %name, timeout_secs = MCP_CONNECT_TIMEOUT.as_secs(), "mcp server connection timed out");
                }
            }
        }
    ...
    pub async fn shutdown(&self) {
        for (name, server) in &self.servers {
            if let Err(e) = server.transport.close().await {
                tracing::warn!(target: "sparkfox_ag_mcp", server = %name, error = %e, "error closing mcp server");
            }
        }
    }
