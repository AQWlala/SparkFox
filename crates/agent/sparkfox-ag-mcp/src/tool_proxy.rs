...
use sparkfox_ag_types::tool::{JsonSchema, ToolImage, ToolResult};
...
/// `sparkfox-ag-tools` `read.rs` `MAX_IMAGE_BYTES` (5 MiB). We estimate the decoded
...
pub fn register_mcp_tools(
    registry: &mut sparkfox_ag_tools::registry::ToolRegistry,
    manager: &Arc<McpManager>,
...
mod tests {
    ...
    use sparkfox_ag_config::config::TransportType;
    use serde_json::json;
    ...
    fn static_mcp_always_uses_origin_stable_canonical_namespace() {
        ...
        registry.register(Box::new(
            sparkfox_ag_tools::tool_search::ToolSearchTool::new(state),
        ));
    ...
    async fn tool_search_finds_mcp_by_original_tool_name_and_server_alias() {
        ...
        let mut registry = sparkfox_ag_tools::registry::ToolRegistry::new();
        let state = registry.deferred_state();
        let search = sparkfox_ag_tools::tool_search::ToolSearchTool::new(state.clone());
        let registrations =
    ...
    async fn restored_mcp_activation_and_display_routing_follow_the_same_origin() {
        ...
        let mut original = sparkfox_ag_tools::registry::ToolRegistry::new();
        register_single_server_tools(&mut original, &manager_a, "A", true).unwrap();
        assert_eq!(original.tool_names(), vec![a_alias.clone()]);
        let search = sparkfox_ag_tools::tool_search::ToolSearchTool::new(original.deferred_state());
        ...
        let mut resumed = sparkfox_ag_tools::registry::ToolRegistry::new();
    ...
    fn provider_display_names_do_not_change_when_dynamic_registration_order_reverses() {
        ...
        let mut a_then_b = sparkfox_ag_tools::registry::ToolRegistry::new();
        ...
        let mut b_then_a = sparkfox_ag_tools::registry::ToolRegistry::new();
    ...
    fn dynamic_mcp_allowlist_requires_the_stable_canonical_provider_name() {
        ...
        let mut raw_name_policy = sparkfox_ag_tools::registry::ToolRegistry::new();
        ...
        let mut canonical_policy = sparkfox_ag_tools::registry::ToolRegistry::new();
