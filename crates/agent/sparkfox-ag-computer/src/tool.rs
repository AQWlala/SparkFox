...
impl ComputerTool {
    ...
    fn engine(&self) -> Result<Arc<dyn A11yEngine>, String> {
        ...
        if guard.is_none() {
            *guard = Some(sparkfox_ag_a11y::create_engine().map_err(|e| e.to_string()));
        }
    ...
    async fn do_observe(&self) -> ToolResult {
        ...
        if snap.entries.len() < OCR_FUSION_AX_THRESHOLD {
            ...
            let ocr = tokio::task::spawn_blocking(move || sparkfox_ag_a11y::ocr_screenshot(&img, &langs))
                .await
        ...
        *self.last_snapshot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(SnapshotCache {
        ...
        sparkfox_ag_a11y::overlay::draw_set_of_marks(&mut shot.image, &display);
...
fn ax_rect_to_pixel(r: sparkfox_ag_a11y::Rect, g: &CaptureGeometry) -> sparkfox_ag_a11y::Rect {
    let sx = g.img_w as f64 / g.logical_w.max(1) as f64;
    let sy = g.img_h as f64 / g.logical_h.max(1) as f64;
    sparkfox_ag_a11y::Rect {
    ...
    fn describe(&self, input: &Value) -> String {
        ...
        let detail = match action {
            ...
            "set_element_value" => {
                ...
                format!("set element [{r}] = {:?}", sparkfox_ag_tools::truncate_utf8(text, 40))
            }
            "right_click_element" | "double_click_element" => {
                let r = input.get("ref").and_then(|v| v.as_u64()).unwrap_or(0);
            ...
            "launch" => {
                ...
                match input.get("app").and_then(|v| v.as_str()) {
                    Some(app) => format!("launch {:?} with {app:?}", sparkfox_ag_tools::truncate_utf8(target, 60)),
                    None => format!("launch {:?}", sparkfox_ag_tools::truncate_utf8(target, 60)),
                }
            }
            ...
            "type" => {
                let text = input.get("text").and_then(|v| v.as_str()).unwrap_or("");
                format!("type {:?}", sparkfox_ag_tools::truncate_utf8(text, 40))
            }
            "key" => {
