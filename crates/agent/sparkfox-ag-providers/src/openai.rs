...
use sparkfox_ag_config::compat::{self, ProviderCompat};
...
use sparkfox_ag_types::tool::{ToolDef, truncate_deferred_description};
...
fn tool_images_user_message(
    tool_use_id: &str,
    images: &[sparkfox_ag_types::tool::ToolImage],
    supports_image: bool,
) -> Option<Value> {
...
impl LlmProvider for OpenAIProvider {
    async fn stream(
        ...
        let (response, headers, body) = loop {
            ...
            tracing::debug!(target: "sparkfox_ag_providers", body = %serde_json::to_string_pretty(&body).unwrap_or_default(), "outgoing request");
    ...
    while let Some(chunk) = stream.next().await {
        ...
        while let Some(line_end) = buffer.iter().position(|byte| *byte == b'\n') {
            ...
            if let Some(data) = line.strip_prefix("data:").map(str::trim_start) {
                tracing::debug!(target: "sparkfox_ag_providers", chunk = %data, "sse chunk received");
                if data == "[DONE]" {
...
mod tests {
    ...
    use sparkfox_ag_types::llm::LlmEvent;
    ...
    fn tool_images_ride_in_follow_up_user_message() {
        ...
        let compat = sparkfox_ag_config::compat::ProviderCompat::openai_defaults();
