...
use sparkfox_ag_types::llm::{LlmEvent, LlmRequest, ThinkingConfig};
    ...
    while let Some(chunk) = stream.next().await {
        ...
        loop {
            ...
            if let Some(payload) = event_data {
                ...
                tracing::debug!(target: "sparkfox_ag_providers", chunk = %inner, "bedrock event chunk");
...
/// Build AwsCredentials from sparkfox-ag-config's BedrockConfig
pub fn credentials_from_config(bc: &sparkfox_ag_config::config::BedrockConfig) -> AwsCredentials {
...
mod tests {
    ...
    use sparkfox_ag_types::tool::ToolDef;
