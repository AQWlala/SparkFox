...
use sparkfox_ag_agent::output::protocol_sink::ProtocolSink;
use sparkfox_ag_agent::output::terminal::TerminalSink;
use sparkfox_ag_agent::session;
use sparkfox_ag_config::auth;
use sparkfox_ag_config::config::{self, CliArgs, Config, McpServerConfig, TransportType};
use sparkfox_ag_mcp::manager::McpManager;
use sparkfox_ag_mcp::tool_proxy::register_single_server_tools;
use sparkfox_ag_protocol::commands::{ApprovalScope, ProtocolCommand};
use sparkfox_ag_protocol::events::ProtocolEvent;
use sparkfox_ag_protocol::reader::spawn_stdin_reader;
use sparkfox_ag_protocol::writer::{ProtocolEmitter, ProtocolWriter};
...
struct Cli {
    ...
    /// Log level filter (e.g. "info", "debug", "info,sparkfox_ag_providers=debug")
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let _log_guard = {
        ...
        if resolved.enabled {
            match sparkfox_ag_config::logging::create_file_layer(&resolved) {
                Ok((layer, guard)) => {
    ...
    if let Some(report) = engine.shutdown_processes().await
        && report.sessions.iter().any(|session| {
            matches!(
                &session.outcome,
                sparkfox_sh_process_runtime::ProcessOutcome::Lost { cleanup, .. } if !cleanup.reaped
            )
        })
    {
        tracing::error!(
            target: "sparkfox_ag_cli",
...
async fn repl_loop(
    engine: &mut sparkfox_ag_agent::engine::AgentEngine,
    ...
    loop {
        ...
        match engine.execute_turn(input, "").await {
            ...
            Err(sparkfox_ag_agent::engine::AgentError::UserAborted) => break,
            Err(e) => {
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "sparkfox_ag_mcp", %name, %transport, ?command, "AddMcpServer received");
                let config =
                ...
                tracing::info!(target: "sparkfox_ag_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        ...
                        let advertised_tool_names: Vec<String> = mgr
                            ...
                            .collect();
                        tracing::info!(target: "sparkfox_ag_mcp", %name, tools = advertised_tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "sparkfox_ag_mcp", %name, error = %e, "mcp server connection failed");
    ...
    loop {
        ...
        match cmd {
                            ...
                            Some(sub_cmd) = cmd_rx.recv() => {
                                match sub_cmd {
                                    ...
                                    ProtocolCommand::SetConfig { model, thinking, thinking_budget, effort, compaction } => {
                                        ...
                                        let _ = writer.emit(&sparkfox_ag_protocol::events::ProtocolEvent::Info {
                                            msg_id: String::new(),
                                    ...
                                    ProtocolCommand::Ping => {
                                        let _ = writer.emit(&sparkfox_ag_protocol::events::ProtocolEvent::Pong);
                                    }
                                    _ => {
                                        tracing::debug!(target: "sparkfox_ag_protocol", "ignoring command during active message processing");
                                    }
                                }
                            }
                        }
            ...
            ProtocolCommand::InitHistory { text } => {
                tracing::debug!(target: "sparkfox_ag_protocol", chars = text.len(), "InitHistory received");
            }
            ProtocolCommand::SetMode { mode } => {
                ...
                tracing::debug!(target: "sparkfox_ag_protocol", mode = %mode_str, "SetMode applied");
            }
