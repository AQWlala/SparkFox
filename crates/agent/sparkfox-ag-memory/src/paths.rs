...
const MEMORY_DIR_ENV: &str = "sparkfox_ag_memory_DIR";
...
///
/// Resolution order:
///   1. `sparkfox_ag_memory_DIR` environment variable (explicit override)
///   2. `app_config_dir()` from `sparkfox-ag-config` (platform-aware default)
///
...
pub fn memory_base_dir() -> Option<PathBuf> {
    ...
    sparkfox_ag_config::config::app_config_dir()
}
