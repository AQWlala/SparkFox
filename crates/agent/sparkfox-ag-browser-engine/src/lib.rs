//! sparkfox-ag-browser-engine —— 进程内自研 Rust CDP 浏览器引擎（Chromium-only，不打包
//! Playwright/Node）。
//!
...
/// 公共 API 再导出：调用方用 `sparkfox_ag_browser_engine::{BrowserEngine, Capabilities, …}`，
...
/// 注入。也可经 `sparkfox_ag_browser_engine::firewall::*` 全路径访问。
...
/// 启动注入实现**持久登录**。crypto 复用 `sparkfox_be_common`（裁决⑦不另起第二套栈）。
pub use vault::{
...
pub struct EngineConfig {
    ...
    /// 默认 `std::env::temp_dir()/sparkfox-browser-data`。
...
impl Default for EngineConfig {
    fn default() -> Self {
        Self {
            data_dir: std::env::temp_dir().join("sparkfox-browser-data"),
...
/// **[纯逻辑] 解析并发冷启上限**（env `SPARKFOX_BROWSER_MAX_CONCURRENT_LAUNCHES`）。
...
fn launch_semaphore() -> &'static tokio::sync::Semaphore {
    ...
    SEM.get_or_init(|| {
        let permits =
            parse_max_concurrent_launches(std::env::var("SPARKFOX_BROWSER_MAX_CONCURRENT_LAUNCHES").ok().as_deref());
...
///
/// 数据目录布局：chrome 下到 `<data_dir>/sparkfox-browser/<version>/...`；专属
...
mod tests {
    ...
    fn engine_config_default_is_headless_with_temp_data_dir() {
        ...
        assert!(c.data_dir.ends_with("sparkfox-browser-data"));
    ...
    //   set SPARKFOX_CHROME_BINARY 指向系统 Chrome/Edge 的 chrome.exe（或走下载兜底），
    //   cargo nextest run -p sparkfox-ag-browser-engine -- --ignored launch_navigate_screenshot
    // 跑完核对任务管理器无残留 chrome（Builder kill_on_drop / 清理网应自动清）。
    #[tokio::test]
    #[ignore = "需本机/打包 chrome，手动跑：set SPARKFOX_CHROME_BINARY 后 -- --ignored"]
    ...
    //   set SPARKFOX_CHROME_BINARY 后 cargo nextest run -p sparkfox-ag-browser-engine -- --ignored
    //   dirty_profile_reopen_is_clean
    #[tokio::test]
    #[ignore = "需本机 chrome，手动跑：脏 profile reopen 循环"]
    async fn dirty_profile_reopen_is_clean() {
        let data_dir = std::env::temp_dir().join("sparkfox-ag-browser-reopen-test");
