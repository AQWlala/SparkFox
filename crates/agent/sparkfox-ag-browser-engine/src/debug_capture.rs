...
pub fn redact_debug_text(s: &str) -> String {
    // First pass: sparkfox_sh_redact known patterns + high-entropy (via redact_yaml which does both).
...
/// More aggressive than sparkfox_sh_redact's SECRET_ASSIGNMENT_REGEX (which only matches 4 keywords).
