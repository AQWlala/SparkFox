...
//!    `is_unique_local`/`is_unicast_link_local` 的 stable 版，按本仓 `sparkfox-be-knowledge` SSRF
...
//!
//! **跨域判定**用 eTLD+1（[`is_cross_origin`]）：复用 `sparkfox-be-secret` 的 PSL 机器
...
///
/// **P3-D1（裁决⑤）**：加 `allow_etld1`/`deny_etld1` 两个 eTLD+1 域名策略字段（复用 `sparkfox_be_secret`
...
/// `BrowserTool::ensure_secret_store_and_firewall`（`sparkfox-ag-browser/tool.rs`）从 per-pet vault 加载的
...
pub struct FirewallConfig {
    ...
    /// 条目应是 eTLD+1（`x.com`），但调用方传任意 host/origin 也安全——内部用 [`sparkfox_be_secret::etld_plus_one`]
...
/// 无 stable std 谓词，按本仓 `sparkfox-be-knowledge::source_url::forbidden_ip` 同款最小位掩码补。
...
pub fn is_cross_origin(current_origin: &str, target_url: &str) -> bool {
    use sparkfox_be_secret::{etld_plus_one, host_of};
...
pub fn build_post_preview(
    ...
    let host = sparkfox_be_secret::host_of(target_url).unwrap_or_default();
...
///
/// 复用 [`sparkfox_be_secret::etld_plus_one`]（同一 PSL 机器，co.uk 等多级后缀正确）解析目标域。`deny`/`allow`
...
fn domain_policy(config: &FirewallConfig, req: &RequestInfo<'_>) -> Option<FirewallDecision> {
    use sparkfox_be_secret::etld_plus_one;
...
///
/// **实现侧（facade / 网关）**：把本 trait 接到 GW2 的同一 pending 审批通道（`sparkfox_ag_browser_confirm`
...
///
/// 与 [`domain_policy`]/`FirewallConfig.allow_etld1` 的 [`sparkfox_be_secret::etld_plus_one`] 同款 PSL
...
fn registrable_domain_for_trust(target: &str) -> Option<String> {
    ...
    if let Some(host) = sparkfox_be_secret::host_of(target)
        && ip_literal_of_host(&host).is_some()
    {
        return None;
    }
    sparkfox_be_secret::etld_plus_one(target)
}
...
mod tests {
    ...
    //
    // 复用 sparkfox_be_secret::etld_plus_one 同一 PSL 机器解析目标域。GatePost = 出口到未授权域升审批
