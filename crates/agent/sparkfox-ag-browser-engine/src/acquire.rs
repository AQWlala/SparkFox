...
//! （ENOENT / npm 不走代理），故此处直接用 `sparkfox_be_net::http_client`（代理感知）
//! 下载 + `.part`→rename + zip 解压，全部自包含、不依赖外部 node / npm / PATH。
//!
//! 注：下载 / 解压 / `no_window_command` / `strip_quarantine` 的写法是
//! `sparkfox-be-app::provision::install` 同款的**复刻**而非引用——后者位于 backend
...
pub const CHROME_BINARY_ENV: &str = "SPARKFOX_CHROME_BINARY";

/// 数据目录下安放下载浏览器的子目录名。布局：
/// `<data_dir>/sparkfox-browser/<version>/chrome-<platform>/...`。
const BROWSER_SUBDIR: &str = "sparkfox-browser";
...
/// env 覆写：`SPARKFOX_CHROME_BINARY` 指向的绝对路径，存在即用。两种 [`ChromeSource`] 下
...
fn cft_chrome_path(
    ...
    // 数据目录（运行时已下载）：<data>/sparkfox-browser/<version>/chrome-<platform>/...
    let cand = data_dir
        .join(BROWSER_SUBDIR)
...
///
/// - env 覆写（`SPARKFOX_CHROME_BINARY`）在两种 source 下都最高优先；
...
/// 下载钉死版本 chrome 到 `<data_dir>/sparkfox-browser/<version>/`，解压，
/// mac 上去 quarantine。代理感知（`sparkfox_be_net::http_client`）、`.part`→rename。
...
async fn fetch_chrome_url(platform: &str) -> Result<String, BrowserError> {
    let client = sparkfox_be_net::http_client();
    let json = client
        .get(KNOWN_GOOD_URL)
...
mod tests {
    ...
    /// 手动跑 `cargo nextest run -p sparkfox-ag-browser-engine acquire::detects_real -- --ignored`。
    ...
    /// 联网集成冒烟（~150MB 下载）：手动跑
    /// `cargo nextest run -p sparkfox-ag-browser-engine acquire:: -- --ignored`。
