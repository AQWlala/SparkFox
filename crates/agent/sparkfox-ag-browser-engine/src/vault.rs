...
//! 账号写在磁盘上。故 vault **AES-256-GCM 加密**，**复用 [`sparkfox_be_common`] 的
//! [`encrypt_string`](sparkfox_be_common::encrypt_string) /
//! [`decrypt_string`](sparkfox_be_common::decrypt_string)**（DESIGN §4/§16 裁决⑦：**不另起第二套 crypto
//! 栈**——与 `sparkfox-be-secret` 凭据 vault 同一实现）。key = 调用方传入的 **32 字节机器绑定密钥**（app
//! data-dir 层 provision 的 `encryption_key`，全后端 `[u8; 32]` 一路穿透的同一把；本 crate **不**自造
//! 机器绑定方案，与 `sparkfox-be-secret` 同约定）。机器绑定 key + 加密落盘 → vault 文件即便被拷走，换台机器
...
/// AES-256-GCM key 的字节数（与 [`sparkfox_be_common`] / `sparkfox-be-secret` 同 = 32）。
...
///
/// 序列化 [`StorageState`] → JSON → `sparkfox_be_common::encrypt_string`（AES-256-GCM，机器绑定 `key`）→
...
pub fn save_storage_state(
    ...
    let ciphertext = sparkfox_be_common::encrypt_string(&json, key)
...
///
/// 读 `vault_path` → `sparkfox_be_common::decrypt_string`（机器绑定 `key`，GCM 认证）→
...
pub fn load_storage_state(vault_path: &Path, key: &[u8]) -> Option<StorageState> {
    ...
    let ciphertext = match std::fs::read_to_string(vault_path) {
        ...
        Err(e) => {
            tracing::warn!(
                target: "sparkfox_ag_browser_engine::vault",
    ...
    let plaintext = match sparkfox_be_common::decrypt_string(&ciphertext, key) {
        Ok(p) => p,
        Err(e) => {
...
mod tests {
    ...
    fn vault_file_is_ciphertext_not_plaintext() {
        ...
        // 文件应是 base64 密文（sparkfox_be_common::encrypt_string 产物）——非空、可解回。
    ...
    fn load_valid_ciphertext_of_non_storage_state_json_is_none() {
        ...
        let bogus = sparkfox_be_common::encrypt_string("[1,2,3]", &test_key()).expect("encrypt bogus");
