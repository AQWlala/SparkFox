                ...
                match msg {
                    Ok(WsMessage::Text(text)) => {
                        if let Err(e) = reg_for_loop.dispatch_message(&text) {
                            // 单条畸形消息不应拖垮连接；记录后继续。
                            tracing::warn!(target: "sparkfox_ag_browser_engine::transport", error = %e, "dropped malformed CDP message");
                        }
                    }
                            ...
                            match std::str::from_utf8(text) {
                                Ok(s) => {
                                    if let Err(e) = reg_for_loop.dispatch_message(s) {
                                        tracing::warn!(target: "sparkfox_ag_browser_engine::transport", error = %e, "dropped malformed CDP message (pipe)");
                                    }
                                }
                                Err(e) => {
                                    tracing::warn!(target: "sparkfox_ag_browser_engine::transport", error = %e, "non-utf8 CDP pipe frame; dropped");
                                }
                            }
                        }
                    }
                    Err(e) => {
                        tracing::warn!(target: "sparkfox_ag_browser_engine::transport", error = %e, "pipe read error; closing connection");
                        break;
                    }
                }
            }
                        ...
                        match serde_json::from_value::<EventAttachedToTarget>(ev.params.clone()) {
                            Ok(attached) => {
                                if let Err(e) = conn.handle_attached(&attached).await {
                                    tracing::warn!(target: "sparkfox_ag_browser_engine::transport", error = %e, "handle_attached failed");
                                }
                            }
                            Err(e) => {
                                tracing::warn!(target: "sparkfox_ag_browser_engine::transport", error = %e, "failed to parse attachedToTarget");
                            }
                        }
                    }
