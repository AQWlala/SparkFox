...
impl InjectionManager {
    ...
    async fn create_isolated_worlds_for_existing_frames(&self) -> Result<(), InjectError> {
        ...
        for fid in frame_ids {
            ...
            {
                tracing::warn!(
                    target: "sparkfox_ag_browser_engine::injected",
...
mod tests {
    ...
    //   cargo nextest run -p sparkfox-ag-browser-engine -- --ignored inject_aria_snapshot_smoke
    ...
    async fn inject_aria_snapshot_smoke() {
        ...
        let cfg = LaunchConfig {
            chrome_path: chrome,
            user_data_dir: std::env::temp_dir().join("sparkfox-inject-smoke-profile"),
            headful: false,
        };
