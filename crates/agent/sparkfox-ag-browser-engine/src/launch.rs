//! 托管启动 Chromium：经 [`sparkfox_sh_process_runtime::ChildProcessBuilder`] spawn 解析到的 chrome，传随机
...
pub async fn launch_chrome(
    ...
    if let Err(e) = crate::profile::scrub_crash_markers(&config.user_data_dir) {
        tracing::warn!(
            target: "sparkfox_ag_browser_engine::launch",
...
/// **Unix**：`--remote-debugging-pipe` 启动。建两条匿名管道,经 [`sparkfox_sh_process_runtime::ChildProcessBuilder::inherit_fds`]
...
async fn launch_chrome_pipe(
    ...
    let mut builder = sparkfox_sh_process_runtime::ChildProcessBuilder::new(&config.chrome_path);
    builder
        .args(args)
...
async fn launch_chrome_ws(
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            let _ = sparkfox_sh_process_runtime::kill_process_tree(&mut child).await;
            return Err(BrowserError::Other(format!(
...
mod tests {
    ...
    fn args_include_port_user_data_dir_and_hardening() {
        let dir = Path::new("/tmp/sparkfox-udd");
        ...
        assert!(
            args.iter().any(|a| a == "--user-data-dir=/tmp/sparkfox-udd"),
