...
impl Drop for CdpBackend {
    ...
    fn drop(&mut self) {
        ...
        std::thread::spawn(move || {
            ...
            tracing::debug!(
                target: "sparkfox_ag_browser_engine::profile",
                dir = %dir.display(),
            ...
            match crate::storage_state::StorageState::from_json(value) {
                Ok(state) => {
                    if let Err(e) = backend.restore_cookies(&state).await {
                        tracing::warn!(
                            target: "sparkfox_ag_browser_engine::backend::cdp",
                            error = %e,
...
impl CdpBackend {
    ...
    pub async fn capture_index_db(
        ...
        if let Some(ex) = result.get("exceptionDetails") {
            ...
            tracing::debug!(
                target: "sparkfox_ag_browser_engine::storage_state",
...
async fn handle_paused_request(
    ...
    let target_host = sparkfox_be_secret::host_of(&url);
...
mod tests {
    ...
    fn unique_tmp_dir(tag: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("sparkfox-facts-{tag}-{}", std::process::id()));
...
/// 交错冒烟见 `tests/op_mutex_concurrency.rs`（`#[ignore]`，需 `SPARKFOX_CHROME_BINARY`）。
#[cfg(test)]
mod op_mutex_tests {
    use std::sync::Arc;
    use std::time::Duration;
