//! **多 origin localStorage 自动遍历恢复端到端集成**（`#[ignore]`，需 `SPARKFOX_CHROME_BINARY`）。
//!
...
//!   cargo nextest run -p sparkfox-ag-browser-engine --run-ignored all -E 'test(restore_all_origins)'

mod common;

use sparkfox_ag_browser_engine::BrowserEngine;
use sparkfox_ag_browser_engine::storage_state::{LocalStorageItem, OriginStorage, StorageState};
...
#[tokio::test]
#[ignore = "需 SPARKFOX_CHROME_BINARY（真 Chrome）：multi-origin localStorage auto-restore"]
