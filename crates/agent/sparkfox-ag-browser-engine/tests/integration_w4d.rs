...
//!   cargo nextest run -p sparkfox-ag-browser-engine --run-ignored all -E 'test(w4d)'
...
use sparkfox_ag_browser_engine::storage_state::{SameSite, StorageState, StorageStateCookie};
use sparkfox_ag_browser_engine::{
    load_storage_state, save_storage_state, shared_storage_state_path, storage_state_path,
};
use sparkfox_ag_browser_engine::BrowserEngine;
...
fn login_cookie(name: &str, value: &str) -> StorageStateCookie {
    StorageStateCookie {
        ...
        priority: sparkfox_ag_browser_engine::storage_state::Priority::Medium,
        source_scheme: sparkfox_ag_browser_engine::storage_state::SourceScheme::NonSecure,
        source_port: -1,
        partition_key: None,
    }
}
...
async fn seed_local_storage(backend: &sparkfox_ag_browser_engine::backend::CdpBackend, pairs: &[(&str, &str)]) {
...
async fn read_local_storage(backend: &sparkfox_ag_browser_engine::backend::CdpBackend, key: &str) -> Option<String> {
...
///
///   set SPARKFOX_CHROME_BINARY=...\chrome.exe
///   cargo nextest run -p sparkfox-ag-browser-engine --run-ignored all -E 'test(shared_identity)'
#[tokio::test]
#[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 --run-ignored all -E 'test(shared_identity)'"]
