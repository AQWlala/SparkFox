...
//!   cargo nextest run -p sparkfox-ag-browser-engine --run-ignored all -E 'test(upload) | test(download_action) | test(pdf) | test(extract)'
//! 跑完核对任务管理器无残留 chrome（Builder kill_on_drop 应自动清）。

use std::time::Duration;

use sparkfox_ag_browser_engine::progress::Progress;
use sparkfox_ag_browser_engine::{ActSpec, BrowserEngine, ObserveOpts};
...
async fn upload_file_sets_file_input_via_set_file_input_files() {
    ...
    let tmp = std::env::temp_dir().join(format!("sparkfox-upload-{}.txt", std::process::id()));
...
#[tokio::test]
#[ignore = "需本机 chrome + 显示器（headful）：set SPARKFOX_CHROME_BINARY 后 --run-ignored all"]
