//! **IndexedDB storage_state 捕获端到端集成**（`#[ignore]`，需 `SPARKFOX_CHROME_BINARY`）。
//!
...
//!   cargo nextest run -p sparkfox-ag-browser-engine --run-ignored all -E 'test(capture_index_db)'
...
#[tokio::test]
#[ignore = "需 SPARKFOX_CHROME_BINARY（真 Chrome）：IndexedDB capture 集成"]
async fn capture_index_db_collects_records() {
    ...
    let decoded =
        sparkfox_ag_browser_engine::decode_binary_sentinel(data_field).expect("decode base64 sentinel");
...
#[tokio::test]
#[ignore = "需 SPARKFOX_CHROME_BINARY（真 Chrome）：IndexedDB restore 集成"]
async fn restore_index_db_writes_back() {
    use sparkfox_ag_browser_engine::storage_state::{
        IdbDatabase, IdbStore, IndexedDbDump, OriginStorage, StorageState,
    };
    use sparkfox_ag_browser_engine::encode_binary_sentinel;
