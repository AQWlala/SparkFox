...
//!   cargo nextest run -p sparkfox-ag-browser-engine --run-ignored all -E 'test(observe_inject_contract)'
...
use sparkfox_ag_browser_engine::injected::InjectionManager;
use sparkfox_ag_browser_engine::launch::{launch_chrome, LaunchConfig};
use sparkfox_ag_browser_engine::transport::Connection;
...
async fn observe_inject_contract_iframe() {
    // 1) resolve chrome（env SPARKFOX_CHROME_BINARY > 打包 > 数据目录 > 下载兜底）+ launch headless。
    ...
    let cfg = LaunchConfig {
        chrome_path: chrome,
        user_data_dir: std::env::temp_dir().join("sparkfox-observe-contract-profile"),
        headful: false,
    };
    ...
    let cr = conn
        .send::<CreateTargetParams>(sparkfox_ag_browser_engine::transport::ROOT_SESSION, &create)
        .await
        .expect("createTarget");
...
use sparkfox_ag_browser_engine::{BrowserEngine, ObserveOpts};
