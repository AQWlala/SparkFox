...
use sparkfox_ag_types::llm::ThinkingConfig;
...
pub struct ToolsConfig {
    ...
    /// 可用 `sparkfox_ag_mcp::tool_proxy::canonical_mcp_display_name` 统一生成。
    ...
    match std::fs::read_to_string(path) {
        Ok(content) => {
            let canonical = match canonicalize_legacy_agent_delegation_settings(&content) {
                ...
                Err(error) => {
                    tracing::warn!(target: "sparkfox_ag_config", path = %path.display(), error = %error, "failed to inspect config for legacy Agent delegation settings");
                    None
                }
            };
            ...
            match parse_config_file_content(&source) {
                Ok(config) => Ok(config),
                Err(error) => {
                    tracing::warn!(target: "sparkfox_ag_config", path = %path.display(), error = %error, "failed to parse config file");
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "sparkfox_ag_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "sparkfox_ag_config", path = %path.display(), "config file created");
    Ok(())
}
...
mod tests {
    ...
    fn browser_config_default_source_is_managed() {
        ...
        // 引擎侧 `ChromeSource::from_source_str("managed")` == Managed（见 sparkfox-ag-browser-engine::acquire）。
