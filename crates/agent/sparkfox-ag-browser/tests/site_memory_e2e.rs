//! **P7A: Site-memory real-Chrome smoke test** (`#[ignore]`, requires SPARKFOX_CHROME_BINARY).
//!
...
//!
//! Run:
//!   export SPARKFOX_CHROME_BINARY="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
//!   cargo nextest run -p sparkfox-ag-browser --run-ignored all -E 'test(site_memory_real_chrome)'

use std::sync::Arc;

use sparkfox_ag_browser::site_memory::{InMemorySink, SiteMemoryStore};
use sparkfox_ag_browser::BrowserTool;
use sparkfox_ag_tools::Tool;
use serde_json::json;

fn isolated_data_dir() -> std::path::PathBuf {
    std::env::temp_dir().join("sparkfox-p7a-site-memory-smoke")
}
...
#[tokio::test]
#[ignore = "requires SPARKFOX_CHROME_BINARY + network access to example.com"]
