...
//! path. The engine stays LLM-free (no vision-model call in sparkfox-ag-browser-engine). The
//! coordination lives here in the facade (`sparkfox-ag-browser`).
//!
...
///
/// - The real adapter is `SessionVisualLocator` in sparkfox-ag-agent's bootstrap: it reuses the
///   session `LlmProvider` (NOT sparkfox-ag-computer — sparkfox-ag-browser stays free of that dep) and sends
