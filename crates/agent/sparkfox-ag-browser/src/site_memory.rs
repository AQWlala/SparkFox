...
///
/// Reuses the same PSL machinery as the firewall (`sparkfox_be_secret::etld_plus_one`),
...
pub fn key_for(url: &str) -> Option<String> {
    ...
    let host = sparkfox_be_secret::host_of(url)?;
    if sparkfox_ag_browser_engine::firewall::ip_literal_of_host(&host).is_some() {
        return None;
    }
    sparkfox_be_secret::etld_plus_one(url)
}
...
impl FileSiteMemorySink {
    ...
    pub fn new(root: impl Into<PathBuf>) -> Self {
        ...
        if let Err(e) = std::fs::create_dir_all(&root) {
            tracing::warn!(
                target: "sparkfox_ag_browser::site_memory", error = %e, dir = %root.display(),
    ...
    fn path_for(&self, etld1: &str) -> Option<PathBuf> {
        if !is_safe_etld1_filename(etld1) {
            tracing::warn!(
                target: "sparkfox_ag_browser::site_memory", key = %etld1,
    ...
    fn read_file(path: &Path) -> Vec<SiteMemoryEntry> {
        match std::fs::read(path) {
            Ok(bytes) => serde_json::from_slice(&bytes).unwrap_or_else(|e| {
                tracing::warn!(
                    target: "sparkfox_ag_browser::site_memory", error = %e, path = %path.display(),
    ...
    fn write_file(path: &Path, entries: &[SiteMemoryEntry]) {
        ...
        match serde_json::to_vec_pretty(entries) {
            ...
            Err(e) => tracing::warn!(
                target: "sparkfox_ag_browser::site_memory", error = %e,
