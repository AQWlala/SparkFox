...
//! (`sparkfox-ag-browser-engine`). Mirrors `sparkfox-ag-computer::ComputerTool`: an honest
...
//! [`sparkfox_ag_browser_engine::ActSpec`] and dispatches it through
...
//! [`sparkfox_ag_browser_engine::TypeInput::Secret`] — it never reaches the LLM).
...
use sparkfox_ag_browser_engine::progress::Progress;
use sparkfox_ag_browser_engine::{ActSpec, BrowserEngine, BrowserError, Capabilities, Effect, TypeInput};
use sparkfox_ag_browser_engine::{ChromeSource, EngineConfig, Observation, create_engine};
use sparkfox_ag_browser_engine::{load_storage_state, save_storage_state, shared_storage_state_path};
use sparkfox_ag_config::config::BrowserConfig;
use sparkfox_ag_protocol::ToolApprovalManager;
use sparkfox_ag_protocol::events::ToolCategory;
use sparkfox_ag_tools::Tool;
use sparkfox_ag_types::tool::{JsonSchema, ToolResult};
use sparkfox_be_secret::SecretStore;
...
/// resolved by `sparkfox_be_secret::shared_vault_path`; browser identity
...
pub struct BrowserSecretSource {
    ...
    pub key: [u8; sparkfox_be_secret::KEY_SIZE],
}
...
pub struct BrowserTool {
    ...
    /// 灌进 [`sparkfox_ag_browser_engine::EngineConfig::user_data_dir`]（配 `ephemeral_profile=true` → 引擎 Drop
    ...
    /// （[`sparkfox_ag_browser_engine::download::download_dir`]）——**绝不**落用户真实 Downloads。
    ///
    ...
    /// build 期固化的 bundled Chrome 资源目录（Tauri resource dir 下），由 sparkfox-be-app
    ...
    firewall_override: Mutex<Option<sparkfox_ag_browser_engine::FirewallConfig>>,
    ...
    /// （`sparkfox-ag-cli` json-stream）创建并经 `engine.set_approval_manager` 安到引擎；`set_mode`（前端/网关切
    ...
    /// [`ToolApprovalManager::session_bypasses_approval`]，facade 不复制它。`sparkfox-ag-browser` 本就依赖
    /// `sparkfox-ag-protocol`，故此句柄零新增跨 crate 依赖。
    ...
    /// 引擎时灌进 [`sparkfox_ag_browser_engine::EngineConfig::evaluate_full_power`]，由引擎层 evaluate 门
    ...
    /// [`sparkfox_ag_browser_engine::EngineConfig::evaluate_persistent_login`]，由引擎层 evaluate 互斥门消费。
    ...
    known_secret_values: sparkfox_ag_browser_engine::KnownSecretValues,
    ...
    /// (GW2 `sparkfox_ag_browser_confirm` channel) via [`Self::with_approval_gate`].
...
impl BrowserTool {
    ...
    pub fn new(config: &BrowserConfig) -> Self {
        let data_dir = sparkfox_ag_config::config::app_config_dir()
    ...
    pub fn with_data_dir(data_dir: PathBuf, headful: bool) -> Self {
        ...
        Self {
            ...
            workspace_dir: None,
            // PKG-1: 默认无 bundled Chrome（CLI / BrowserConfig-only / 测试）。sparkfox-be-app 解析
    ...
    /// vault degrades to an empty store — see `sparkfox_be_secret::load_secret_store`). Then
    /// derives the [`sparkfox_ag_browser_engine::FirewallConfig`] for this session: the secret's
    ...
    fn ensure_secret_store_and_firewall(&self) {
        ...
        // pre-injected store (tests via `with_secret_store`) is left as-is.
        {
            ...
            {
                *guard = Some(sparkfox_be_secret::load_secret_store(&src.vault_path, src.key));
            }
        }
        ...
        let fw = sparkfox_ag_browser_engine::FirewallConfig {
            allow_etld1,
            ..sparkfox_ag_browser_engine::FirewallConfig::default()
        };
        ...
        let result = create_engine(EngineConfig {
            ...
            egress_approver: self.approval_gate.as_ref().map(|g| {
                Arc::new(crate::approval::GateEgressApprover::new(g.clone()))
                    as Arc<dyn sparkfox_ag_browser_engine::firewall::EgressApprover>
            }),
    ...
    fn persist_login_key(&self) -> Option<[u8; sparkfox_be_secret::KEY_SIZE]> {
            ...
            match engine.capture_storage_state().await {
                Ok(state) => {
                    ...
                    if let Err(e) = save_storage_state(&state, &vault_path, &key) {
                        tracing::warn!(
                            target: "sparkfox_ag_browser::persist_login",
                            error = %e,
    ...
    async fn do_observe(&self, input: &Value) -> ToolResult {
        let mut opts = sparkfox_ag_browser_engine::ObserveOpts::default();
    ...
    /// 1. **给 LLM**：base64 包成 [`sparkfox_ag_types::tool::ToolImage`] 经 `with_images` 回传——LLM 看图**内容**
    ///    （多模态），与 P0 行为一致。
    ...
    async fn do_screenshot(&self) -> ToolResult {
        ...
        match engine.screenshot().await {
            Ok(png) => {
                ...
                let img = sparkfox_ag_types::tool::ToolImage {
                ...
                let saved = match persist_screenshot(&path, &png) {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            target: "sparkfox_ag_browser::screenshot",
                            error = %e,
                            dir = %base.display(),
    ...
    fn resolve_type_input(&self, text: &str) -> Result<TypeInput, ToolResult> {
        ...
        match resolved {
            Some(value) => {
                ...
                tracing::debug!(
                    target: "sparkfox_ag_browser::secret",
                    name = %name,
        ...
        let spec = match action {
            ...
            "scroll" => {
                ...
                let target = match input.get("ref").and_then(|v| v.as_str()) {
                    Some(r) => sparkfox_ag_browser_engine::ScrollTarget::Element { r#ref: r.to_string() },
                    None => sparkfox_ag_browser_engine::ScrollTarget::Viewport,
                };
    ...
    ///
    /// Maps [`sparkfox_ag_browser_engine::ActResult`] to a [`ToolResult`]: the human
    /// `message` is the text, and a soft `success == false` is surfaced as a
    /// (non-error) note so the model can re-observe/retry. Engine
    /// [`sparkfox_ag_browser_engine::BrowserError`]s become non-panicking tool errors.
    ///
    /// **F2 verify-after-act**: the engine's [`sparkfox_ag_browser_engine::Effect`]
            ...
            Err(e) => {
                    ...
                    && self.action_is_click_or_type(&spec)
                {
                    match self.attempt_visual_fallback(&engine, action, input, &spec).await {
                        ...
                        Err(fallback_err) => {
                            tracing::debug!(
                                target: "sparkfox_ag_browser::visual_fallback",
                                original_error = %e,
    ...
    ///   [`sparkfox_ag_browser_engine::nav::current_entry_is_post`] 填充）。snapshot 不存在或 observe 取不到
    ...
    fn build_action_context(&self, input: &Value) -> ActionContext {
        ...
        // （[`sparkfox_ag_browser_engine::firewall::is_cross_origin`]）判跨域——与 E5 网络层同款 eTLD+1 判定，
        // 不另造逻辑。
        //
    ...
    /// URL → 返 `false`）。两侧都有 → 复用 [`sparkfox_ag_browser_engine::firewall::is_cross_origin`]（同款 PSL
    ...
    fn cross_origin_post_signal(&self, input: &Value) -> bool {
        ...
        sparkfox_ag_browser_engine::firewall::is_cross_origin(&current_origin, target_url)
    }
    ...
    fn redline_gate(&self, action: &str, input: &Value) -> Option<ToolResult> {
        ...
        match redline::enforce_redline(tier, bypass, confirmed) {
            ...
            Err(e) => {
                tracing::warn!(
                    target: "sparkfox_ag_browser::redline",
                    action = %action,
                    tier = ?tier,
    ...
    async fn redline_gate_with_takeover(&self, action: &str, input: &Value) -> Option<ToolResult> {
        ...
        tracing::info!(
            target: "sparkfox_ag_browser::takeover",
            action = %action,
    ...
    fn som_rects_css(&self) -> Option<Vec<sparkfox_ag_browser_engine::CssRect>> {
        let guard = self.last_snapshot.lock().expect("last_snapshot poisoned");
        let snap = guard.as_ref()?;
        if snap.boxes.is_empty() {
            return None;
        }
        let rects: Vec<sparkfox_ag_browser_engine::CssRect> = snap
            .entries
            .iter()
    ...
    async fn som_locate_point(
        ...
        instruction: &str,
        dpr: f64,
        rects_css: &[sparkfox_ag_browser_engine::CssRect],
...
/// `downloads/`（E4 [`sparkfox_ag_browser_engine::download::DOWNLOAD_SUBDIR`]）平级隔离，**绝不**指向
...
/// **F1: parse a scroll `direction` string** into [`sparkfox_ag_browser_engine::ScrollDir`].
/// `None` for missing/invalid (caller turns that into a parameter error before launch).
fn parse_scroll_dir(s: Option<&str>) -> Option<sparkfox_ag_browser_engine::ScrollDir> {
    use sparkfox_ag_browser_engine::ScrollDir;
    match s? {
...
/// [`sparkfox_ag_browser_engine::WaitCondition`]. Accepts `condition` ∈
...
fn parse_wait_condition(input: &Value) -> Result<sparkfox_ag_browser_engine::WaitCondition, String> {
    use sparkfox_ag_browser_engine::WaitCondition;
    let kind = input
        .get("condition")
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
    ...
    fn describe(&self, input: &Value) -> String {
        ...
        let detail = match action {
            "navigate" => {
                ...
                format!("navigate to {:?}", sparkfox_ag_tools::truncate_utf8(url, 80))
            }
            ...
            "type" => {
                ...
                if parse_secret_ref(text).is_some() {
                    ...
                    format!("type {:?} into [ref={ref}]", sparkfox_ag_tools::truncate_utf8(text, 40))
                }
            }
            "set_value" => {
                ...
                if parse_secret_ref(value).is_some() {
                    ...
                    format!("set [ref={ref}] = {:?}", sparkfox_ag_tools::truncate_utf8(value, 40))
                }
            }
            ...
            "scroll_to_text" => {
                let text = input.get("text").and_then(|v| v.as_str()).unwrap_or("");
                format!("scroll to text {:?}", sparkfox_ag_tools::truncate_utf8(text, 40))
            }
            "get_page_text" => "get page text".to_string(),
            "search_page" => {
                let q = input.get("query").and_then(|v| v.as_str()).unwrap_or("");
                format!("search page for {:?}", sparkfox_ag_tools::truncate_utf8(q, 40))
            }
            "find_elements" => {
                let sel = input.get("selector").and_then(|v| v.as_str()).unwrap_or("");
                format!("find elements {:?}", sparkfox_ag_tools::truncate_utf8(sel, 60))
            }
            ...
            "download" => {
                ...
                format!(
                    ...
                    sparkfox_ag_tools::truncate_utf8(url, 60)
                )
            }
            ...
            "open_link_new_tab" => {
                let url = input.get("url").and_then(|v| v.as_str()).unwrap_or("");
                format!("open {:?} in a new tab", sparkfox_ag_tools::truncate_utf8(url, 60))
            }
...
mod tests {
    use super::*;
    use sparkfox_ag_browser_engine::Capabilities;
    ...
    // SPARKFOX_CHROME_BINARY (or rely on the download fallback) then:
    //   cargo nextest run -p sparkfox-ag-browser -- --ignored navigate_then_observe_real
    ...
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 -- --ignored"]
    ...
    use sparkfox_ag_browser_engine::{ElementEntry, Observation, SnapshotGen};
    ...
    use sparkfox_ag_protocol::ToolApprovalManager;
    use sparkfox_ag_protocol::commands::SessionMode;
    ...
    fn ensure_secret_store_and_firewall_loads_vault_and_derives_allowlist() {
        ...
        let key = [0x42u8; sparkfox_be_secret::KEY_SIZE];
        let vault_path = sparkfox_be_secret::shared_vault_path(dir.path());
        let mut store = sparkfox_be_secret::SecretStore::new(key);
        store.register("pw", "the-secret", vec!["x.com".into()]).unwrap();
        store.register("tok", "ghp", vec!["github.com".into()]).unwrap();
        sparkfox_be_secret::save_secret_store(&store, &vault_path).expect("save vault");
        ...
        let def = sparkfox_ag_browser_engine::FirewallConfig::default();
    ...
    fn ensure_secret_store_and_firewall_no_source_keeps_default_firewall() {
        ...
        assert_eq!(fw, sparkfox_ag_browser_engine::FirewallConfig::default(), "firewall == default when no secrets");
    }
    ...
    fn ensure_secret_store_and_firewall_missing_vault_degrades_gracefully() {
        ...
        let vault_path = sparkfox_be_secret::shared_vault_path(dir.path());
        let t = BrowserTool::with_data_dir(dir.path().join("bdata"), false)
            .secret_source(BrowserSecretSource { vault_path, key: [0x42; sparkfox_be_secret::KEY_SIZE] });
        t.ensure_secret_store_and_firewall();
    ...
    fn build_act_spec_parses_every_action() {
        ...
        assert!(matches!(
            ...
            ActSpec::Scroll { target: sparkfox_ag_browser_engine::ScrollTarget::Viewport, .. }
        ));
        assert!(matches!(
            ok("scroll", json!({"action":"scroll","direction":"up","ref":"f0e1"})),
            ActSpec::Scroll { target: sparkfox_ag_browser_engine::ScrollTarget::Element { .. }, .. }
        ));
        assert!(matches!(
    ...
    //
    //   set SPARKFOX_CHROME_BINARY 后:
    //   cargo nextest run -p sparkfox-ag-browser -- --run-ignored --ignored facade_act_click_and_secret_type_real
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 -- --run-ignored"]
    ...
    //
    //   set SPARKFOX_CHROME_BINARY 后:
    //   cargo nextest run -p sparkfox-ag-browser --run-ignored all -E 'test(g2_download_lands_in_session_workspace_real)'
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 -- --run-ignored"]
    ...
    // 普通会话不被 facade 门误拦）。本机 Windows：set SPARKFOX_CHROME_BINARY 后
    //   cargo nextest run -p sparkfox-ag-browser --run-ignored all -E 'test(f1sec_)'
    // ════════════════════════════════════════════════════════════════════════
    ...
    //
    //   set SPARKFOX_CHROME_BINARY 后:
    //   cargo nextest run -p sparkfox-ag-browser --run-ignored all -E 'test(n2_screenshot_lands_in_workspace_real)'
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 -- --run-ignored"]
    ...
    fn extract_model_defaults_to_none_and_builder_sets_it() {
        ...
        let data_dir = std::env::temp_dir().join("sparkfox-test-extract-model-wiring");
    ...
    async fn resume_invalidates_pre_takeover_refs() {
        let data_dir = std::env::temp_dir().join("sparkfox-test-re-observe-flag");
    ...
    async fn irreversible_under_bypass_released_only_by_confirmed_takeover() {
        ...
        let data_dir = std::env::temp_dir().join("sparkfox-test-takeover-redline");
    ...
    #[async_trait]
    impl sparkfox_ag_browser_engine::BrowserEngine for FakeRecordingEngine {
        fn capabilities(&self) -> Capabilities {
            Capabilities { browser_ready: true, headful: false, display_available: false, engine: "fake".into() }
        }
        async fn navigate(&self, _url: &str, _new_tab: bool) -> Result<sparkfox_ag_browser_engine::NavResult, sparkfox_ag_browser_engine::BrowserError> {
            Ok(sparkfox_ag_browser_engine::NavResult {
                final_url: "https://test.example.com".into(),
                http_status: Some(200),
                redirected: false,
                load_state: sparkfox_ag_browser_engine::LoadState::Load,
            })
        }
        async fn screenshot(&self) -> Result<Vec<u8>, sparkfox_ag_browser_engine::BrowserError> {
            Err(sparkfox_ag_browser_engine::BrowserError::Unsupported { capability: "screenshot".into(), hint: "fake".into() })
        }
        async fn rendered_html(&self) -> Result<String, sparkfox_ag_browser_engine::BrowserError> {
            Err(sparkfox_ag_browser_engine::BrowserError::Unsupported { capability: "rendered_html".into(), hint: "fake".into() })
        }
        async fn observe(&self, _opts: &sparkfox_ag_browser_engine::ObserveOpts) -> Result<sparkfox_ag_browser_engine::Observation, sparkfox_ag_browser_engine::BrowserError> {
            Err(sparkfox_ag_browser_engine::BrowserError::Unsupported { capability: "observe".into(), hint: "fake".into() })
        }
        async fn act(
            &self,
            _spec: &sparkfox_ag_browser_engine::ActSpec,
            _progress: &sparkfox_ag_browser_engine::progress::Progress,
        ) -> Result<sparkfox_ag_browser_engine::ActResult, sparkfox_ag_browser_engine::BrowserError> {
            Ok(sparkfox_ag_browser_engine::ActResult {
                success: true,
                message: "clicked".into(),
                effect: sparkfox_ag_browser_engine::Effect { changed: true, before_anchor: None, after_anchor: None },
            })
        }
        async fn debug_snapshot(&self) -> Result<sparkfox_ag_browser_engine::DebugSnapshot, sparkfox_ag_browser_engine::BrowserError> {
            Err(sparkfox_ag_browser_engine::BrowserError::Unsupported { capability: "debug".into(), hint: "fake".into() })
        }
    }
    ...
    async fn do_act_falls_back_to_visual_when_enabled_and_anchor_fails() {
        ...
        assert!(!t.action_is_click_or_type(&ActSpec::Scroll {
            target: sparkfox_ag_browser_engine::ScrollTarget::Viewport,
            direction: sparkfox_ag_browser_engine::ScrollDir::Down,
            amount: None,
        }));
    ...
    async fn som_locate_point_maps_picked_label_to_css_center() {
        ...
        use sparkfox_ag_browser_engine::CssRect;
    ...
    fn som_rects_css_keeps_only_clickable_entries() {
        use sparkfox_ag_browser_engine::{CssRect, ElementEntry, Observation, SnapshotGen};
