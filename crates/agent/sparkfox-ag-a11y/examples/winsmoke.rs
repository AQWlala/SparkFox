...
//!
//! Run with:  cargo run -p sparkfox-ag-a11y --example winsmoke
//! Optional:  cargo run -p sparkfox-ag-a11y --example winsmoke -- <pid>
//!
...
fn main() {
    ...
    use sparkfox_ag_a11y::{ElementAction, ObserveOpts, Snapshot, Target};
    ...
    let _ = std::fs::write(&tmp, format!("SparkFox TextPattern 验证 Hello {MARKER}\n第二行 line two\n"));
    ...
    if let Some(e) = doc {
        let text = "SparkFox SetValue 测试".to_string();
    ...
    let stale = engine.invoke(
        &Target::Ref(1),
        sparkfox_ag_a11y::SnapshotGen(snap.generation.0.wrapping_sub(1)),
        ElementAction::Focus,
    );
