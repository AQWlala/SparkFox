...
fn merge_process_path(
    ...
    // This crate owns bundled-toolchain source discovery; sparkfox-sh-process-runtime owns the shared,
    ...
    sparkfox_sh_process_runtime::merge_process_path(sources)
...
#[cfg(unix)]
const PATH_PROBE_BEGIN: &str = "__SPARKFOX_PATH_BEGIN__";
#[cfg(unix)]
const PATH_PROBE_END: &str = "__SPARKFOX_PATH_END__";

/// Shell snippet that prints the live `$PATH` wrapped in our markers.
#[cfg(unix)]
const PATH_PROBE_SNIPPET: &str = "printf '__SPARKFOX_PATH_BEGIN__%s__SPARKFOX_PATH_END__' \"$PATH\"";
