...
//!   and `[package.metadata.sparkfox-be-runtime] bun_version` from Cargo.toml.
...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=SPARKFOX_EMBED_BUN");
    ...
    // sets SPARKFOX_EMBED_BUN=1 to actually embed.
    if std::env::var("SPARKFOX_EMBED_BUN").as_deref() != Ok("1") {
...
fn read_bun_version() -> String {
    ...
    parsed
        ...
        .and_then(|m| m.get("sparkfox-be-runtime"))
        .and_then(|r| r.get("bun_version"))
        .and_then(|v| v.as_str())
        .expect("[package.metadata.sparkfox-be-runtime] bun_version missing")
        .to_string()
}
