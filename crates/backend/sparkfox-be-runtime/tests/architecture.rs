...
fn runtime_production_source() -> String {
    fs::read_dir(runtime_src_dir())
        .expect("read sparkfox-be-runtime source directory")
...
fn bundled_runtime_contains_no_process_ownership_primitives() {
    ...
    assert!(
        remaining.is_empty(),
        "sparkfox-be-runtime must remain limited to bundled-toolchain and PATH support: {remaining:?}"
    );
}

#[test]
fn path_merge_is_delegated_to_sparkfox_sh_process_runtime() {
    let source = production_source(&runtime_src_dir().join("shell_env.rs"));
    assert!(
        source.contains("sparkfox_sh_process_runtime::merge_process_path"),
        "shell_env must delegate ordered PATH merging to sparkfox_sh_process_runtime::merge_process_path"
    );
    assert!(
