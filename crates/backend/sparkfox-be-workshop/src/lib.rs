//! `sparkfox-be-workshop` — the 创意工坊 (Creative Workshop) domain: an
//! infinite-canvas AI visual-creation workspace.
//!
//! Two backend crates back the domain: this one owns **canvases + assets**
//! (index rows in `sparkfox-be-db`, canvas bodies + asset binaries on disk under
//! the data dir), while `sparkfox-be-creation` owns the generation task queue.
//!
//! Deliberately mirrors the `sparkfox-be-public-agent` crate shape: `fsio` (atomic
