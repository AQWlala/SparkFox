...
use sparkfox_be_ai_agent::capability::prompt_pipeline::{PromptCtx, PromptPipeline};
use sparkfox_be_ai_agent::factory::acp_assembler::{AcpSessionParams, WorkspaceInfo, assemble_acp_params};
use sparkfox_be_ai_agent::manager::acp::{
...
use sparkfox_be_ai_agent::session::ModelId;
use sparkfox_be_ai_agent::{AcpBuildExtra, AcpSkillManager, AgentRuntimeState};
...
fn fixture_skill_manager() -> Arc<AcpSkillManager> {
    ...
    let paths = Arc::new(sparkfox_be_extension::resolve_skill_paths(tmp.path(), tmp.path()));
