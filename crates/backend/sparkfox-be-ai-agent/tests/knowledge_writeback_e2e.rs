...
use sparkfox_ag_agent::knowledge_tools::{
...
/// `sparkfox_be_realtime` ships no public no-op broadcaster, so define a local one
...
async fn staged_write_tool_with_mount_prefixed_path_lands_in_inbox_not_nested() {
    ...
    let sink: Arc<dyn KnowledgeWritebackSink> =
        Arc::new(sparkfox_be_ai_agent::LiveKnowledgeWritebackSink { service: svc.clone() });
...
async fn search_read_write_handle_loop_updates_original_in_direct_mode() {
    ...
    let retrieval: Arc<dyn KnowledgeRetrievalSink> =
        Arc::new(sparkfox_be_ai_agent::LiveKnowledgeRetrievalSink { service: svc.clone() });
