...
use sparkfox_be_ai_agent::manager::nomi::NomiAgentManager;
use sparkfox_be_ai_agent::runtime_registry::AgentRuntimeFactory;
use sparkfox_be_ai_agent::types::{AgentRuntimeBuildOptions, NomiResolvedConfig, SendMessageData};
use sparkfox_be_ai_agent::*;
use sparkfox_be_ai_agent::{SkillIndex, build_system_instructions_with_skills_index};
use sparkfox_be_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
use serde_json::json;
...
impl AgentRuntimeControl for TypedMockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), sparkfox_be_ai_agent::AgentSendError> {
        Ok(())
    }
...
fn make_sparkfox_ag_config() -> NomiResolvedConfig {
    NomiResolvedConfig {
...
async fn sparkfox_ag_agent_kill_succeeds() {
    let agent = NomiAgentManager::new("conv-1".into(), "/proj".into(), make_sparkfox_ag_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
        .await
        .unwrap();
...
#[tokio::test]
async fn sparkfox_ag_agent_confirm_succeeds() {
    let agent = NomiAgentManager::new("conv-1".into(), "/proj".into(), make_sparkfox_ag_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
        .await
        .unwrap();
...
#[tokio::test]
async fn sparkfox_ag_agent_metadata() {
    let agent = NomiAgentManager::new("conv-abc".into(), "/work".into(), make_sparkfox_ag_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
        .await
        .unwrap();
