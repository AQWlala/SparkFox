...
use sparkfox_be_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use sparkfox_be_ai_agent::manager::acp::AcpAgentManager;
use sparkfox_be_ai_agent::registry::AgentRegistry;
use sparkfox_be_ai_agent::{AgentRuntimeHandle, AgentStreamEvent, AgentRuntimeControl};
use sparkfox_be_common::ConversationStatus;
use sparkfox_be_db::{SqliteAgentMetadataRepository, init_database_memory};
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = sparkfox_be_ai_agent::AcpBuildExtra {
    ...
    let skill_paths = std::sync::Arc::new(sparkfox_be_extension::resolve_skill_paths(
        tmp_skills.path(),
        tmp_skills.path(),
    ));
    let skill_manager = sparkfox_be_ai_agent::AcpSkillManager::new(skill_paths);
    ...
    let params = Arc::new(
        assemble_acp_params(
            ...
            metadata,
            sparkfox_be_common::CommandSpec {
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: sparkfox_be_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), sparkfox_be_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(sparkfox_be_common::AgentKillReason::IdleTimeout)).unwrap();
