...
use sparkfox_be_ai_agent::AcpSessionSyncService;
use sparkfox_be_ai_agent::AcpSkillManager;
use sparkfox_be_ai_agent::factory::{AgentFactoryDeps, build_agent_factory};
use sparkfox_be_ai_agent::registry::AgentRegistry;
use sparkfox_be_ai_agent::types::AgentRuntimeBuildOptions;
use sparkfox_be_common::{AgentType, ConversationId, ProviderWithModel, encrypt_string};
use sparkfox_be_db::{
...
fn make_factory(
...
) -> sparkfox_be_ai_agent::runtime_registry::AgentRuntimeFactory {
    let tmp = tempfile::TempDir::new().unwrap();
    let skill_paths = Arc::new(sparkfox_be_extension::resolve_skill_paths(tmp.path(), tmp.path()));
    build_agent_factory(AgentFactoryDeps {
        ...
        agent_registry,
        acp_agent_service,
        data_dir: PathBuf::from("/tmp/sparkfox-test"),
        work_dir: PathBuf::from("/tmp/sparkfox-test"),
        backend_binary_path: Arc::new(PathBuf::from("/tmp/sparkfox-test/nomicore")),
