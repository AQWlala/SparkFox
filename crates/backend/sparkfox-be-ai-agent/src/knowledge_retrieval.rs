...
//! `LiveKnowledgeCompleter`: the trait lives in `sparkfox-ag-agent`, this is the
//! backend impl wired in at app startup.

use std::sync::Arc;

use async_trait::async_trait;
use sparkfox_ag_agent::knowledge_tools::{KnowledgeHit, KnowledgeRetrievalSink};
...
impl KnowledgeRetrievalSink for LiveKnowledgeRetrievalSink {
    async fn search(
        ...
        Ok(hits
            .into_iter()
            .map(|h| KnowledgeHit {
                handle: sparkfox_be_knowledge::encode_doc_handle(&h.kb_id, &h.rel_path),
                kb_id: h.kb_id,
                kb_name: h.kb_name,
                rel_path: h.rel_path,
                heading: h.heading,
    ...
    async fn read_document(
        ...
        let (kb_id, rel_path) =
            sparkfox_be_knowledge::decode_doc_handle(handle).ok_or_else(|| format!("invalid handle: {handle}"))?;
