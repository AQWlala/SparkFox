...
use sparkfox_ag_agent::companion_tools::{CompanionMemorySink, CompanionSkillSink};
use sparkfox_ag_agent::requirement_tools::RequirementSink;
use sparkfox_be_api_types::{
    BrowserMcpConfig, ComputerMcpConfig, GatewayMcpConfig, OpenMcpConfig, RequirementMcpConfig,
};
use sparkfox_be_common::{AgentType, AppError, ExecutionAuthority};
use sparkfox_be_db::{
...
/// frozen forever. Implemented by `sparkfox-be-companion::CompanionService`.
...
/// stays free of the `sparkfox-be-public-agent` config types (which depend on this
...
pub struct PublicAgentRuntime {
    ...
    pub knowledge_base_ids: Vec<sparkfox_be_common::KnowledgeBaseId>,
    ...
    pub model: sparkfox_be_common::ProviderWithModel,
}

/// Resolves a public agent's LIVE runtime + records inbound turns for audit.
/// Implemented by `sparkfox_be_public_agent::PublicAgentService`. Read at every agent
...
pub struct AgentFactoryDeps {
    ...
    /// `SPARKFOX_WORK_DIR` override must not split the two roots.
    ...
    pub knowledge_mcp_config: Option<sparkfox_be_api_types::KnowledgeMcpConfig>,
    ...
    pub knowledge_retrieval: Option<Arc<dyn sparkfox_ag_agent::knowledge_tools::KnowledgeRetrievalSink>>,
    ...
    pub knowledge_writeback: Option<Arc<dyn sparkfox_ag_agent::knowledge_tools::KnowledgeWritebackSink>>,
...
fn validate_runtime_user_id(user_id: &str) -> Result<(), AppError> {
    sparkfox_be_common::UserId::parse(user_id)
        .map(|_| ())
