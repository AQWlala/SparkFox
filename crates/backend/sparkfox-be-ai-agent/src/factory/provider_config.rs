...
//! `sparkfox_ag_config::config::Config`, and a standalone one-shot LLM completion
...
use sparkfox_be_common::{AppError, ProviderId};
...
pub(crate) fn image_support_override(provider_id: &str, model: &str) -> Option<bool> {
    if sparkfox_be_common::VisionUnsupportedRegistry::global().is_unsupported(provider_id, model) {
        Some(false)
    } else {
        None
    }
...
pub(crate) struct ResolvedProviderFields {
    ...
    pub bedrock_config: Option<sparkfox_ag_config::config::BedrockConfig>,
...
pub(crate) async fn resolve_provider_fields(
    ...
    let api_key = sparkfox_be_common::decrypt_string(&row.api_key_encrypted, encryption_key)?;
...
mod tests {
    ...
    async fn drain_text_response_concatenates_deltas() {
        ...
        tx.send(LlmEvent::Done {
            ...
            usage: sparkfox_ag_types::message::TokenUsage::default(),
        })
        .await
        .unwrap();
...
mod fallback_tests {
    use super::*;
    use sparkfox_be_db::models::Provider;
    use sparkfox_be_db::{CreateProviderParams, DbError, UpdateProviderParams};
    ...
    fn provider(id: &str, enabled: bool, models: &[&str], model_enabled: Option<&str>) -> Provider {
        Provider {
            ...
            name: id.into(),
            base_url: String::new(),
            api_key_encrypted: sparkfox_be_common::encrypt_string("sk-test", &[0u8; 32])
