...
use sparkfox_be_api_types::RemoteBuildExtra;
use sparkfox_be_common::{AppError, RemoteAgentProtocol};
use tracing::warn;
...
pub(super) async fn build(
    ...
    let auth_token = row
        ...
        .as_deref()
        .filter(|t| !t.is_empty())
        .map(|encrypted| sparkfox_be_common::decrypt_string(encrypted, &deps.encryption_key))
        .transpose()
        .inspect_err(|e| {
    ...
    let device_identity = match (row.device_id.as_deref(), row.device_private_key.as_deref()) {
        ...
        (Some(device_id), Some(encrypted)) => {
            let private_b64 = sparkfox_be_common::decrypt_string(encrypted, &deps.encryption_key)?;
    ...
    if let Some(device_token) = issued_device_token {
        let encrypted = sparkfox_be_common::encrypt_string(&device_token, &deps.encryption_key)?;
