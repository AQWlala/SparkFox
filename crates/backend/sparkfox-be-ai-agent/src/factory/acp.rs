...
use sparkfox_be_api_types::{AcpBuildExtra, SessionMcpServer, SessionMcpTransport};
use sparkfox_be_common::{AppError, CommandSpec};
...
pub(super) async fn build(
    ...
    let (command, args, mut env, cwd) = (
        ...
        meta.env
            .iter()
            .map(|e| sparkfox_be_common::EnvVar {
                name: e.name.clone(),
                value: e.value.clone(),
            })
            .collect::<Vec<_>>(),
    ...
    if meta.backend.as_deref() == Some("claude") {
        ...
        if !cc_switch_env.is_empty() {
            ...
            for (name, value) in &cc_switch_env {
                env.push(sparkfox_be_common::EnvVar {
                    name: name.clone(),
                    value: value.clone(),
                });
            }
...
mod tests {
    ...
    fn make_row(
        id: &sparkfox_be_common::McpServerId,
        name: &str,
        transport_type: &str,
    ...
    fn row_to_sdk_stdio_roundtrip() {
        let row = make_row(
            &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000001").unwrap(),
            "ctx7",
            "stdio",
    ...
    fn row_to_sdk_http_with_headers() {
        let row = make_row(
            &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000002").unwrap(),
            "remote",
            "http",
    ...
    fn row_to_sdk_unknown_transport_type_errors() {
        let row = make_row(&sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000003").unwrap(), "bad", "websocket", "{}", true, false);
        assert!(row_to_sdk_mcp_server(&row).is_err());
    }

    #[test]
    fn row_to_sdk_invalid_json_errors() {
        let row = make_row(&sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000004").unwrap(), "bad", "stdio", "not-json", true, false);
        assert!(row_to_sdk_mcp_server(&row).is_err());
    }

    #[test]
    fn row_to_sdk_stdio_missing_command_errors() {
        let row = make_row(&sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000005").unwrap(), "bad", "stdio", r#"{"args":[]}"#, true, false);
        assert!(row_to_sdk_mcp_server(&row).is_err());
    }
    ...
    impl IMcpServerRepository for MockRepo {
        async fn list(&self) -> Result<Vec<McpServerRow>, sparkfox_be_db::DbError> {
            if self.fail {
                Err(sparkfox_be_db::DbError::Init("simulated".into()))
            } else {
                Ok(self.rows.clone())
            }
        }
        async fn find_by_id(&self, _id: &sparkfox_be_common::McpServerId) -> Result<Option<McpServerRow>, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn find_by_name(
            &self,
            _name: &str,
        ) -> Result<Option<McpServerRow>, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn list_by_ids_any(
            &self,
            ids: &[sparkfox_be_common::McpServerId],
        ) -> Result<Vec<McpServerRow>, sparkfox_be_db::DbError> {
            if self.fail {
                return Err(sparkfox_be_db::DbError::Init("simulated".into()));
            }
            Ok(ids
                .iter()
        ...
        async fn create(
            &self,
            _params: sparkfox_be_db::CreateMcpServerParams<'_>,
        ) -> Result<McpServerRow, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn update(
            &self,
            _id: &sparkfox_be_common::McpServerId,
            _params: sparkfox_be_db::UpdateMcpServerParams<'_>,
        ) -> Result<McpServerRow, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn delete(&self, _id: &sparkfox_be_common::McpServerId) -> Result<(), sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn batch_upsert(
            &self,
            _servers: &[sparkfox_be_db::CreateMcpServerParams<'_>],
        ) -> Result<Vec<McpServerRow>, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn update_status(
            ...
            _id: &sparkfox_be_common::McpServerId,
            _status: &str,
            _last_connected: Option<sparkfox_be_common::TimestampMs>,
    ...
    async fn load_user_mcp_servers_skips_disabled_and_builtin() {
        ...
        let repo: Arc<dyn IMcpServerRepository> = Arc::new(MockRepo {
            rows: vec![
                make_row(
                    &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000010").unwrap(),
                    "user-enabled",
                    "stdio",
                ...
                make_row(
                    &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000011").unwrap(),
                    "user-disabled",
                    "stdio",
                    r#"{"command":"npx","args":[],"env":{}}"#,
                    false,
                ...
                make_row(
                    &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000012").unwrap(),
                    "builtin",
                    "stdio",
    ...
    async fn load_user_mcp_servers_skips_malformed_rows_but_keeps_others() {
        ...
        let repo: Arc<dyn IMcpServerRepository> = Arc::new(MockRepo {
            rows: vec![
                make_row(
                    &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000020").unwrap(),
                    "good",
                    "stdio",
                    r#"{"command":"npx","args":[],"env":{}}"#,
                    true,
                ...
                make_row(&sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000021").unwrap(), "bad", "stdio", "not-json", true, false),
            ],
            fail: false,
        });
        let servers = load_user_mcp_servers(repo.as_ref(), None, "conv-1", &caps).await;
    ...
    async fn load_user_mcp_servers_uses_selected_snapshot_over_enabled_state() {
        ...
        let repo: Arc<dyn IMcpServerRepository> = Arc::new(MockRepo {
            rows: vec![
                make_row(
                    &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000030").unwrap(),
                    "enabled",
                    "stdio",
                    r#"{"command":"npx","args":[],"env":{}}"#,
                    true,
                ...
                make_row(
                    &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000031").unwrap(),
                    "disabled-picked",
                    "stdio",
        ...
        let selected = vec![
            sparkfox_be_common::McpServerId::parse(
    ...
    async fn load_user_mcp_servers_skips_rows_unsupported_by_capabilities() {
        ...
        let repo: Arc<dyn IMcpServerRepository> = Arc::new(MockRepo {
            rows: vec![make_row(
                &sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000040").unwrap(),
                "stdio-only",
                "stdio",
                r#"{"command":"npx","args":[],"env":{}}"#,
                true,
