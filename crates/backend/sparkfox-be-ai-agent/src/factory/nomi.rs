...
use sparkfox_ag_agent::session::{Session, SessionManager};
use sparkfox_ag_config::config::{McpServerConfig, TransportType};
use sparkfox_be_api_types::{GatewayMcpConfig, NomiBuildExtra, SessionMcpServer, SessionMcpTransport};
...
use sparkfox_be_db::IMcpServerRepository;
use sparkfox_be_db::ISettingsRepository;
use sparkfox_be_db::models::McpServerRow;
...
pub(super) async fn build(
    ...
    if public_agent_id.is_some() {
        overrides.exposure = overrides
            .exposure
            .stricter(sparkfox_be_api_types::ExposureMode::PublicService);
    }
    ...
    let platform_gateway_entitled = is_instance_owner
        ...
        && !matches!(
            overrides.exposure,
            sparkfox_be_api_types::ExposureMode::PublicService
        );
    ...
    let knowledge_write_policy = sparkfox_be_knowledge::resolve_write_policy(
    ...
    let knowledge_write_enabled = !matches!(
        ...
        sparkfox_be_knowledge::WriteMode::Disabled
    );
    let knowledge_writeback_staged = matches!(
        knowledge_write_policy.mode,
        sparkfox_be_knowledge::WriteMode::Staged { .. }
    );
    ...
    let accept_owned =
        |mut session: sparkfox_ag_agent::session::Session| -> Option<sparkfox_ag_agent::session::Session> {
            if !sparkfox_ag_agent::session::session_belongs_to(
    ...
    let computer_use_default = read_bool_pref(
        &deps,
        PREF_COMPUTER_USE,
        cfg!(feature = "computer-use") || env_flag("SPARKFOX_COMPUTER_USE"),
    )
    .await;
    ...
    // regardless. `SPARKFOX_BROWSER_USE` env forces it on for feature-less parity/testing.
    let browser_use_default = read_bool_pref(
        &deps,
        PREF_BROWSER_USE,
        cfg!(feature = "browser-use") || env_flag("SPARKFOX_BROWSER_USE"),
    )
    .await;
    ...
    let browser_full_power_default = read_bool_pref(
        ...
        env_flag("SPARKFOX_BROWSER_FULL_POWER"),
    )
    .await;
    ...
    let browser_secret_vault = if browser_use_enabled {
        Some(crate::types::BrowserSecretVault {
            vault_path: sparkfox_be_secret::shared_vault_path(&deps.data_dir),
    ...
    let config = NomiResolvedConfig {
        ...
        goal: overrides.goal.clone().map(|g| {
            sparkfox_ag_agent::goal::runtime::GoalSpec::new(
                g.objective,
    ...
    let knowledge_kb_ids: Vec<sparkfox_be_common::KnowledgeBaseId> = match public_runtime_state.as_ref() {
    ...
    let knowledge_write_bases: Vec<(sparkfox_be_common::KnowledgeBaseId, String)> = overrides
        .knowledge_mounts
        .iter()
...
/// `SystemSettingsResponse::default().language` in `sparkfox-be-api-types`.
...
/// (`sparkfox_be_knowledge::context::build_knowledge_context`,
...
fn append_knowledge_context(
    ...
    use sparkfox_be_knowledge::context::{
...
pub(crate) fn should_install_embedded_agent_execution(
    ...
    exposure: sparkfox_be_api_types::ExposureMode,
) -> bool {
    !has_platform_gateway
        && is_instance_owner
        && sparkfox_be_api_types::exposure_clamp(exposure)
...
pub(crate) fn resolve_native_write_root(
    ...
    let is_local_desktop = !is_channel && matches!(exposure, sparkfox_be_api_types::ExposureMode::Private);
    if is_local_desktop {
        return None;
    }
...
pub(crate) fn apply_exposure_clamp(overrides: &mut NomiBuildExtra) -> bool {
    match sparkfox_be_api_types::exposure_clamp(overrides.exposure) {
        None => false,
        Some(clamp) => {
...
pub(crate) fn resolve_bedrock_config(
    json: Option<&str>,
) -> Option<sparkfox_ag_config::config::BedrockConfig> {
    let bc: sparkfox_be_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
    Some(sparkfox_ag_config::config::BedrockConfig {
...
async fn load_user_mcp_servers(
    ...
    selected_ids: Option<&[sparkfox_be_common::McpServerId]>,
    conversation_id: &str,
...
mod tests {
    ...
    fn secondary_nomi_session_is_model_only() {
        let mut overrides = NomiBuildExtra {
            ...
            mcp_server_ids: Some(vec![sparkfox_be_common::McpServerId::new()]),
            ...
            knowledge_mounts: vec![sparkfox_be_api_types::KnowledgeMountInfo {
                id: sparkfox_be_common::KnowledgeBaseId::new(),
    ...
    /// factory/acp.rs.
    struct MockSettingsRepo(Result<Option<sparkfox_be_db::models::SystemSettings>, ()>);

    #[async_trait::async_trait]
    impl ISettingsRepository for MockSettingsRepo {
        async fn get_settings(
            &self,
        ) -> Result<Option<sparkfox_be_db::models::SystemSettings>, sparkfox_be_db::DbError> {
            self.0
                .clone()
                .map_err(|_| sparkfox_be_db::DbError::Init("simulated".into()))
        }
        ...
        ) -> Result<sparkfox_be_db::models::SystemSettings, sparkfox_be_db::DbError> {
            unimplemented!("not exercised by the language tests")
        }
    }

    fn settings_row(language: &str) -> sparkfox_be_db::models::SystemSettings {
        sparkfox_be_db::models::SystemSettings {
            id: 1,
    ...
    fn settings_repo(
        result: Result<Option<sparkfox_be_db::models::SystemSettings>, ()>,
    ...
    fn resolve_mcp_servers_adds_gateway_when_process_config_present() {
        ...
        assert_eq!(env.len(), 1);
        let bootstrap: sparkfox_be_api_types::ScopedMcpChildBootstrap<
            sparkfox_be_api_types::GatewayCapabilityClaims,
        > = serde_json::from_str(
    ...
    fn embedded_agent_execution_requires_trusted_no_gateway_host() {
        use sparkfox_be_api_types::ExposureMode;
    ...
    fn public_service_exposure_clamps_session() {
        ...
        let mut o = NomiBuildExtra {
            exposure: sparkfox_be_api_types::ExposureMode::PublicService,
            gateway_mcp_config: Some(gateway_config(41237, "/usr/bin/nomicore", "owner")),
            computer_use: Some(true),
            browser_use: Some(true),
    ...
    fn nomi_build_extra_deserializes_public_service_exposure() {
        ...
        assert_eq!(extra.exposure, sparkfox_be_api_types::ExposureMode::PublicService);
        // 缺省不回归
        let d: NomiBuildExtra = serde_json::from_value(serde_json::json!({})).unwrap();
        assert_eq!(d.exposure, sparkfox_be_api_types::ExposureMode::Private);
    }
    ...
    fn append_knowledge_context_renders_mounts_and_writeback() {
        use sparkfox_be_api_types::KnowledgeMountInfo;
    ...
    fn channel_write_opt_in_threads_from_extra_into_write_policy() {
        ...
        use sparkfox_be_knowledge::{KnowledgeBinding, WriteMode, WriteSurface, resolve_write_policy};
