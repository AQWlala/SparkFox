...
use sparkfox_be_api_types::AgentMetadata;
...
fn resolve_mcp_servers(
    ...
    {
        let kb_ids: Vec<sparkfox_be_common::KnowledgeBaseId> = config
            .knowledge_mounts
            .iter()
            .map(|m| m.id.clone())
            .collect();
...
/// (`sparkfox_be_knowledge::context::build_knowledge_context`, `PromptSection`
...
fn knowledge_mcp_server(
    cfg: &sparkfox_be_api_types::KnowledgeMcpConfig,
    user_id: &str,
    conversation_id: &str,
    workspace_path: &str,
    ...
    let env = vec![EnvVariable::new(
        sparkfox_be_api_types::KnowledgeMcpConfig::ENV_CAPABILITY.to_owned(),
        child
            .bootstrap_json()
            .expect("validated knowledge bootstrap serializes"),
    )];
    let stdio = McpServerStdio::new(
        sparkfox_be_api_types::KnowledgeMcpConfig::SERVER_NAME,
        &child.binary_path,
    )
...
mod tests {
    ...
    fn test_issuer() -> std::sync::Arc<sparkfox_be_common::LoopbackCapabilityIssuer> {
        std::sync::Arc::new(sparkfox_be_common::LoopbackCapabilityIssuer::random().unwrap())
    }
    ...
    fn knowledge_config(port: u16, binary: &str) -> sparkfox_be_api_types::KnowledgeMcpConfig {
        sparkfox_be_api_types::KnowledgeMcpConfig::from_issuer(port, test_issuer(), binary.into())
    }
    ...
    fn knowledge_mount(id: &str) -> sparkfox_be_api_types::KnowledgeMountInfo {
        sparkfox_be_api_types::KnowledgeMountInfo {
            id: sparkfox_be_common::KnowledgeBaseId::parse(id).expect("canonical knowledge base test id"),
            name: "领域知识".into(),
            description: "团队约定".into(),
    ...
    fn knowledge_mcp_injected_only_with_config_and_bound_bases() {
        ...
        // Case A: Some(config) + 1 mount → the "sparkfox-be-knowledge" server is
        ...
        let kb_server = servers
            .iter()
            .find(|s| match s {
                McpServer::Stdio(s) => s.name == sparkfox_be_api_types::KnowledgeMcpConfig::SERVER_NAME,
                _ => false,
            })
        ...
        let baked = env_val(sparkfox_be_api_types::KnowledgeMcpConfig::ENV_CAPABILITY)
        ...
        assert!(
            !servers_b.iter().any(|s| matches!(
                s,
                McpServer::Stdio(s) if s.name == sparkfox_be_api_types::KnowledgeMcpConfig::SERVER_NAME
            )),
