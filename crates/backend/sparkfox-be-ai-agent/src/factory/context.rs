...
use sparkfox_be_common::{AgentType, AppError, ConversationId, validate_prefixed_id};
...
mod tests {
    ...
    fn managed_workspace_path_rebases_under_current_work_dir() {
        let work_dir =
            std::env::temp_dir().join(format!("sparkfox-factory-rebase-{}", sparkfox_be_common::generate_id()));
