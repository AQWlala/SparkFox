...
impl BackendOutputSink {
    ...
    fn reflow_citations(&self, full_text: &str) {
        ...
        for fname in sparkfox_ag_memory::distill::parse_citation_filenames(full_text) {
            if let Err(e) = sparkfox_ag_memory::store::bump_memory_usage(dir, &fname, now) {
...
mod tests {
    ...
    fn citation_reflow_bumps_cited_file_on_stream_end() {
        use sparkfox_ag_memory::store::{read_memory, write_memory};
        use sparkfox_ag_memory::types::{MemoryEntry, MemoryType};
