use sparkfox_be_common::{AppError, CommandSpec, ErrorChain};
use sparkfox_sh_process_runtime::ChildProcessBuilder as CmdBuilder;
use std::path::Path;
use std::sync::Arc;
...
impl CliAgentProcess {
    ...
    pub async fn spawn_for_sdk(config: CommandSpec, data_dir: &Path) -> Result<Self, AppError> {
        let proxy_env =
            sparkfox_be_net::proxy::child_proxy_env(config.env.iter().map(|e| e.name.as_str()));
    ...
    fn agent_spawn_env(data_dir: &Path) -> Vec<(String, String)> {
        ...
        // `sparkfox_be_runtime::enhance_process_path` during startup; children
    ...
    /// to `sparkfox_be_runtime::resolve_command_in`, which honours `PATHEXT` on
    ...
    fn find_native_claude() -> Option<String> {
        ...
        for dir in std::env::split_paths(&path_var) {
            ...
            if let Some(found) = sparkfox_be_runtime::resolve_command_in("claude", &dir) {
