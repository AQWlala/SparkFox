...
use sparkfox_be_common::Confirmation;
...
pub trait MockAgentRuntime: AgentRuntimeControl {
    ...
    async fn mode(&self) -> Result<sparkfox_be_api_types::AgentModeResponse, AppError> {
        Ok(sparkfox_be_api_types::AgentModeResponse {
            mode: "default".into(),
            initialized: false,
        })
    }
...
impl AgentRuntimeHandle {
    ...
    pub fn get_confirmations(&self) -> Vec<sparkfox_be_common::Confirmation> {
        match self {
    ...
    pub async fn get_mode(&self) -> Result<sparkfox_be_api_types::AgentModeResponse, AppError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::Nomi(m) => m.mode().await,
            Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Ok(sparkfox_be_api_types::AgentModeResponse {
                mode: "default".into(),
                initialized: false,
            }),
            #[cfg(any(test, feature = "test-support"))]
    ...
    /// [`sparkfox_be_common::normalize_keys_to_snake_case`] so keys land as
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AppError> {
        match self {
            Self::Acp(m) => {
                ...
                sparkfox_be_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
