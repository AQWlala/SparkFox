...
use sparkfox_be_api_types::TryConnectCustomAgentResponse;
use sparkfox_be_common::{CommandSpec, EnvVar};
...
mod tests {
    ...
    async fn probe_returns_fail_cli_when_command_missing() {
        ...
        let resp = try_connect_custom_agent("sparkfox-definitely-does-not-exist-xyz", &[], &HashMap::new(), &tmp).await;
        match resp {
