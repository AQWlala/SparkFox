...
//! `sparkfox_ag_memory::distill`. Here we gate on an opt-in flag, redact the
...
/// avoids surprising users with unexpected spend. sparkfox-ag-config has no memory
/// section today, so an env flag is the lowest-risk gate (same pattern as the
/// `SPARKFOX_COMPUTER_USE` / `SPARKFOX_BROWSER_USE` host flags).
const DISTILL_ENABLED_ENV: &str = "SPARKFOX_MEMORY_DISTILL";
...
pub async fn run_distill(cfg: Arc<Config>, dir: PathBuf, transcript: String) {
    ...
    // Write on a blocking thread (synchronous fs in sparkfox-ag-memory).
