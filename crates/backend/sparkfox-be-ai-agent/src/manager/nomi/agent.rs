...
use sparkfox_ag_agent::bootstrap::AgentBootstrap;
use sparkfox_ag_agent::companion_tools::{
...
use sparkfox_ag_agent::engine::AgentEngine;
use sparkfox_ag_agent::knowledge_tools::{KnowledgeReadTool, KnowledgeSearchTool, KnowledgeWriteTool};
use sparkfox_ag_agent::output::OutputSink;
use sparkfox_ag_agent::requirement_tools::{RequirementCompleteTool, RequirementSink, RequirementUpdateStatusTool};
use sparkfox_ag_agent::cron_tools::{CronCreateTool, CronDeleteTool, CronListTool, CronSink};
use sparkfox_ag_agent::session::Session;
use sparkfox_ag_config::config::{CliArgs, Config};
use sparkfox_ag_mcp::manager::McpManager;
use sparkfox_ag_protocol::commands::SessionMode;
#[cfg(feature = "browser-use")]
use sparkfox_ag_protocol::events::ToolCategory;
use sparkfox_ag_protocol::{ToolApprovalManager, ToolApprovalResult};
use sparkfox_ag_types::message::ContentBlock;
use sparkfox_be_api_types::{AgentModeResponse, SlashCommandItem};
...
fn apply_provider_context_budget(config: &mut Config, context_limit: Option<u64>) {
    config.compact.context_window = sparkfox_ag_config::compact::resolve_context_window(
        context_limit,
        config.compact.context_window,
    );
    config.max_tokens =
        sparkfox_ag_config::compact::fit_context_budget(&mut config.compact, config.max_tokens);
}
...
pub struct NomiAgentManager {
    ...
    loopback_capability_leases: sparkfox_be_common::LoopbackCapabilityLeaseSet,
    ...
    distill_cfg: Arc<sparkfox_ag_config::config::Config>,
    ...
    knowledge_auto_rag: Option<(
        Arc<dyn sparkfox_ag_agent::knowledge_tools::KnowledgeRetrievalSink>,
        Vec<sparkfox_be_common::KnowledgeBaseId>,
    )>,
}
...
pub(crate) fn should_register_knowledge_search(
    has_sink: bool,
    kb_ids: &[sparkfox_be_common::KnowledgeBaseId],
) -> bool {
...
pub(crate) fn should_register_knowledge_write(
    has_sink: bool,
    bases: &[(sparkfox_be_common::KnowledgeBaseId, String)],
) -> bool {
...
pub(crate) fn prepend_knowledge_context(
    hits: &[sparkfox_ag_agent::knowledge_tools::KnowledgeHit],
    content: String,
) -> String {
    if hits.is_empty() {
        return content;
...
/// Normalize the engine's [`StopReason`](sparkfox_ag_types::message::StopReason) into
...
pub(crate) fn map_engine_stop_reason(
    reason: sparkfox_ag_types::message::StopReason,
) -> TurnStopReason {
    use sparkfox_ag_types::message::StopReason;
    match reason {
...
impl NomiAgentManager {
    pub async fn new(
        ...
        knowledge_retrieval_sink: Option<Arc<dyn sparkfox_ag_agent::knowledge_tools::KnowledgeRetrievalSink>>,
        knowledge_kb_ids: Vec<sparkfox_be_common::KnowledgeBaseId>,
        knowledge_prelude: Option<String>,
        knowledge_writeback_sink: Option<Arc<dyn sparkfox_ag_agent::knowledge_tools::KnowledgeWritebackSink>>,
        knowledge_write_bases: Vec<(sparkfox_be_common::KnowledgeBaseId, String)>,
        ...
        let distill_dir: Option<PathBuf> = if companion_sink.is_some() {
            ...
            sparkfox_ag_memory::paths::auto_memory_dir(std::path::Path::new(&workspace))
        };
        ...
        if let Some(sink) = knowledge_writeback_sink {
            if register_knowledge_write {
                let mode = if knowledge_writeback_staged {
                    sparkfox_ag_agent::knowledge_tools::WriteMode::Staged { scope: conversation_id.clone() }
                } else {
                    sparkfox_ag_agent::knowledge_tools::WriteMode::Direct
                };
                let bound_kb_ids: Vec<sparkfox_be_common::KnowledgeBaseId> =
        ...
        let accepted_turn = 'accepted: {
            ...
            let result = loop {
                ...
                if let Ok(agent_result) = &r
                    && agent_result.stop_reason == sparkfox_ag_types::message::StopReason::EndTurn
                {
                ...
                if let Ok(agent_result) = &r {
                    ...
                    if agent_result.stop_reason == sparkfox_ag_types::message::StopReason::MaxTokens {
    ...
    pub fn confirm(&self, _msg_id: &str, call_id: &str, data: Value, always_allow: bool) -> Result<(), AppError> {
        ...
        if is_cancel {
            ...
            let scope = if always_allow {
                sparkfox_ag_protocol::commands::ApprovalScope::Always
            } else {
                sparkfox_ag_protocol::commands::ApprovalScope::Once
            };
...
mod tests {
    ...
    use sparkfox_ag_protocol::events::ToolCategory;
    use sparkfox_ag_providers::{LlmProvider, ProviderError};
    use sparkfox_ag_tools::{registry::ToolRegistry, Tool};
    use sparkfox_ag_types::llm::{LlmEvent, LlmRequest};
    use sparkfox_ag_types::message::{ContentBlock, Role, StopReason};
    use sparkfox_ag_types::tool::ToolResult;
    ...
    fn make_test_config() -> NomiResolvedConfig {
        NomiResolvedConfig {
            ...
            session_directory: std::env::temp_dir().join("sparkfox-test-sessions"),
            session_mode: None,
    ...
    #[async_trait::async_trait]
    impl sparkfox_ag_agent::knowledge_tools::KnowledgeRetrievalSink for HangingKnowledgeSink {
        async fn search(
            &self,
            _kb_ids: &[sparkfox_be_common::KnowledgeBaseId],
            _query: &str,
            _limit: usize,
        ) -> Result<Vec<sparkfox_ag_agent::knowledge_tools::KnowledgeHit>, String> {
            self.started.add_permits(1);
    ...
    async fn cancel_interrupts_hung_knowledge_preparation() {
        ...
        let started = Arc::new(tokio::sync::Semaphore::new(0));
        let retrieval: Arc<dyn sparkfox_ag_agent::knowledge_tools::KnowledgeRetrievalSink> =
            Arc::new(HangingKnowledgeSink {
                started: Arc::clone(&started),
            });
        let mut agent = make_agent_with_provider(provider.clone());
        agent.knowledge_auto_rag = Some((retrieval, vec![sparkfox_be_common::KnowledgeBaseId::new()]));
        let agent = Arc::new(agent);
        let mut rx = agent.subscribe();
    ...
    async fn send_message_does_not_auto_continue_after_max_turns() {
        ...
        let agent = make_agent_with_provider_and_max_turns(provider.clone(), Some(1));
        {
            ...
            engine.registry_mut().register(Box::new(
                sparkfox_ag_tools::tool_search::ToolSearchTool::new(deferred_state),
            ));
        }
        let mut rx = agent.subscribe();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_returns_correct_type() {
    ...
    #[async_trait::async_trait]
    impl sparkfox_ag_agent::companion_tools::CompanionMemorySink for StubCompanionSink {
    ...
    async fn companion_session_never_distills() {
        ...
        let sink: Arc<dyn sparkfox_ag_agent::companion_tools::CompanionMemorySink> = Arc::new(StubCompanionSink);
        let agent = NomiAgentManager::new(
            "conv-companion".into(),
            "/project".into(),
            make_test_config(),
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_initial_status_is_pending() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_subscribe_returns_receiver() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_kill_succeeds() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_kill_with_reason_succeeds() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_kill_running_turn_cancels_turn_token() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_kill_idle_turn_does_not_leave_stale_stop_signal() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_kill_is_durable_and_rejects_raced_turn_admission() {
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_confirmations_initially_empty() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_get_slash_commands_does_not_wait_for_engine_lock() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn sparkfox_ag_agent_check_approval_returns_false_by_default() {
        let agent = NomiAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None, None, None, None, Vec::new(), None, None, Vec::new(), false, None)
            .await
            .unwrap();
    ...
    fn nomi_provider_connection_error_is_user_llm_provider_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(sparkfox_be_api_types::AgentErrorCode::UserLlmProviderConfigError)
        );
        assert_eq!(
            send_error.ownership(),
            Some(sparkfox_be_api_types::AgentErrorOwnership::UserLlmProvider)
        );
    ...
    fn nomi_api_connection_error_is_user_llm_provider_network_error() {
        ...
        assert_eq!(
            send_error.code(),
            Some(sparkfox_be_api_types::AgentErrorCode::UserLlmProviderNetworkError)
        );
        assert_eq!(
            send_error.ownership(),
            Some(sparkfox_be_api_types::AgentErrorOwnership::UserLlmProvider)
...
mod knowledge_search_gate_tests {
    ...
    use sparkfox_be_common::KnowledgeBaseId;
...
mod knowledge_prelude_tests {
    ...
    fn prepend_knowledge_context_formats_hits_and_passthrough_when_empty() {
        use sparkfox_ag_agent::knowledge_tools::KnowledgeHit;
        assert_eq!(prepend_knowledge_context(&[], "hi".to_string()), "hi");

        let hits = vec![KnowledgeHit {
            kb_id: sparkfox_be_common::KnowledgeBaseId::new(),
            handle: "h".into(),
            kb_name: "Docs".into(),
