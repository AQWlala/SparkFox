    ...
    fn start_tick_watchdog(self: &Arc<Self>) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let gap_ms = sparkfox_be_common::now_ms()
                    .saturating_sub(last)
                    .max(0) as u64;
    ...
    async fn handle_incoming_text(&self, text: &str) {
        ...
        match frame {
            ...
            IncomingFrame::Event(evt) => {
                ...
                if evt.event == "tick" {
                    self.last_tick.store(sparkfox_be_common::now_ms(), Ordering::Relaxed);
                    return;
                }
...
mod tests {
    ...
    /// `SPARKFOX_OPENCLAW_TEST_URL=ws://127.0.0.1:18789 cargo test
    /// -p sparkfox-be-ai-agent real_gateway_handshake -- --ignored --nocapture`
    #[tokio::test]
    ...
    async fn real_gateway_session_methods() {
        ...
        let requested_key = format!("sparkfox-real-smoke-{}", uuid::Uuid::new_v4());
    ...
    async fn real_gateway_chat_send_contract() {
        ...
        let reset: super::super::protocol::SessionsResetResponse = connection
            .request(
                "sessions.reset",
                json!({
                    "key": format!("sparkfox-real-chat-{}", uuid::Uuid::new_v4()),
                    "reason": "new"
                }),
            )
            .await
        ...
        let ack: Value = connection
            .request(
                ...
                json!({
                    ...
                    "message": "Reply with exactly SPARKFOX_REMOTE_OK",
    ...
    fn real_gateway_auth() -> Option<AuthConfig> {
        let token = std::env::var("SPARKFOX_OPENCLAW_TEST_TOKEN").ok();
        let password = std::env::var("SPARKFOX_OPENCLAW_TEST_PASSWORD").ok();
    ...
    fn real_gateway_url() -> String {
        std::env::var("SPARKFOX_OPENCLAW_TEST_URL")
            .expect("set SPARKFOX_OPENCLAW_TEST_URL to run real OpenClaw Gateway tests")
    }
}
