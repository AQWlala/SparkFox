...
//! (`sparkfox-ag-browser-engine`). Same late-wire layering as [`crate::knowledge_completer`]:
//! the trait ([`sparkfox_be_knowledge::PageFetcher`]) lives in the knowledge crate,
...
//!
//! **Why a browser backend at all** — the default [`sparkfox_be_knowledge::HttpFetcher`]
...
//! discipline as `sparkfox_ag_browser::BrowserTool`).
//!
...
use sparkfox_be_common::AppError;
use sparkfox_be_knowledge::PageFetcher;
use sparkfox_be_knowledge::source_url::{FETCH_MAX_BYTES, FetchedPage, html_to_markdown, truncate_to_bytes};
use tokio::sync::Mutex;

use sparkfox_ag_browser_engine::firewall::FirewallConfig;
use sparkfox_ag_browser_engine::{BrowserEngine, EngineConfig, create_engine};
...
mod tests {
    ...
    //   cargo nextest run -p sparkfox-be-ai-agent --features browser-use --run-ignored all \
    ...
    async fn js_rendered_page_is_captured_after_render() {
        ...
        let (_t, static_md) =
            sparkfox_be_knowledge::source_url::html_to_markdown(&html);
        assert!(
