...
use sparkfox_be_db::{IModelProfileRepository, IProviderRepository, models::Provider};
use regex::Regex;
...
impl ProviderHealthCheckService {
    ...
    pub async fn health_check(
        ...
        let task = req
            ...
            .unwrap_or_else(|| {
                sparkfox_be_api_types::derive_tasks_and_traits(&row.platform, model)
                    .0
                    .first()
                    .copied()
        ...
        let api_key = sparkfox_be_common::decrypt_string(&row.api_key_encrypted, &self.encryption_key)?;
        run_modality_probe(
            row.id,
            row.platform,
            model.to_owned(),
...
mod tests {
    ...
    fn openai_model_probe_is_used_for_custom_openai_compatible_configs() {
        let config = NomiResolvedConfig {
            ...
            context_limit: None,
            compat_overrides: crate::types::NomiCompatOverrides::default(),
            session_directory: PathBuf::from("/tmp/sparkfox-health"),
            session_mode: None,
            extra_mcp_servers: HashMap::new(),
            loopback_capability_leases: Default::default(),
            bedrock_config: None,
