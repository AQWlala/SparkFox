...
use sparkfox_be_db::models::RemoteAgentRow;
use sparkfox_be_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
use sha2::Digest;
use tracing::warn;
...
impl RemoteAgentService {
    ...
    pub async fn create(&self, req: CreateRemoteAgentRequest) -> Result<RemoteAgentResponse, AppError> {
        ...
        let row = self
            .repo
            .create(sparkfox_be_db::CreateRemoteAgentParams {
                name: &req.name,
    ...
    pub async fn update(&self, id: &RemoteAgentId, req: UpdateRemoteAgentRequest) -> Result<RemoteAgentResponse, AppError> {
        ...
        let mut row = self.repo.update(id, params).await.map_err(|e| match e {
            sparkfox_be_db::DbError::NotFound(msg) => AppError::NotFound(msg),
...
fn db_err(e: sparkfox_be_db::DbError) -> AppError {
...
mod tests {
    ...
    fn sample_remote_agent_row() -> RemoteAgentRow {
        RemoteAgentRow {
            id: sparkfox_be_common::RemoteAgentId::new(),
            name: "Remote".into(),
