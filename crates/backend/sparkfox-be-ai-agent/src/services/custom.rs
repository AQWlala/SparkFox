...
use sparkfox_be_common::{AppError, generate_prefixed_id};
use sparkfox_be_db::UpsertAgentMetadataParams;
use tracing::warn;
...
async fn probe_or_reject(req: &CustomAgentUpsertRequest, data_dir: &Path) -> Result<(), AppError> {
    ...
    if std::env::var("SPARKFOX_BYPASS_PROBE").is_ok() {
        tracing::warn!("SPARKFOX_BYPASS_PROBE set — skipping custom agent probe. Test-only.");
        return Ok(());
    }
