...
use sparkfox_be_db::ISkillTagRepository;
...
/// (intentionally unlike `sparkfox-be-preset`'s `decode_str_list`, which 500s on
...
async fn set_skill_tags(
    ...
    state
        .skill_tag_repo
        .upsert(&sparkfox_be_db::UpsertSkillTagParams {
            skill_name: &name,
...
mod tests {
    ...
    struct InMemorySkillTagRepo {
        rows: std::sync::Mutex<Vec<sparkfox_be_db::SkillTagRow>>,
    }
    #[async_trait::async_trait]
    impl sparkfox_be_db::ISkillTagRepository for InMemorySkillTagRepo {
        async fn get_all(&self) -> Result<Vec<sparkfox_be_db::SkillTagRow>, sparkfox_be_db::DbError> {
            Ok(self.rows.lock().unwrap().clone())
        }
        async fn upsert(
            &self,
            p: &sparkfox_be_db::UpsertSkillTagParams<'_>,
        ) -> Result<sparkfox_be_db::SkillTagRow, sparkfox_be_db::DbError> {
            let row = sparkfox_be_db::SkillTagRow {
        ...
        async fn delete(&self, name: &str) -> Result<bool, sparkfox_be_db::DbError> {
            let mut g = self.rows.lock().unwrap();
            let before = g.len();
