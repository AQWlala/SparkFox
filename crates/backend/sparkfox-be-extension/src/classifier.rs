...
//!
//! These traits live in `sparkfox-be-extension` (not `sparkfox-be-preset`) so
//! `skill_routes` can depend on them without pulling `sparkfox-be-preset` into
//! the dependency graph; the concrete implementation ships from
//! `sparkfox-be-preset::PresetService`.

use sparkfox_be_api_types::PresetSource;
...
///
/// Implemented by `sparkfox_be_preset::PresetService`; depended on by
