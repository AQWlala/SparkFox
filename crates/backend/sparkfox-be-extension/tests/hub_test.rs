...
use sparkfox_be_extension::hub::{HubIndexManager, HubInstaller};
use sparkfox_be_extension::registry::ExtensionRegistry;
use sparkfox_be_extension::state::ExtensionStateStore;
use sparkfox_be_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![sparkfox_be_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: sparkfox_be_extension::types::ExtensionSource::Env,
    }];
