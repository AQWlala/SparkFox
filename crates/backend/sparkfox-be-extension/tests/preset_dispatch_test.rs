...
use sparkfox_be_api_types::{ApiResponse, PresetSource};
use sparkfox_be_common::AppError;
use sparkfox_be_extension::classifier::{PresetClassifier, PresetRuleDispatcher};
use sparkfox_be_extension::external_paths::ExternalPathsManager;
use sparkfox_be_extension::skill_routes::{SkillRouterState, skill_routes};
use sparkfox_be_extension::skill_service::SkillPaths;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let state = SkillRouterState {
        ...
        skill_tag_repo: {
            ...
            Arc::new(sparkfox_be_db::SqliteSkillTagRepository::new(db.pool().clone()))
        },
