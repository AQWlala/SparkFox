...
use sparkfox_be_ai_agent::RequirementSink;
use sparkfox_be_api_types::{CreateRequirementRequest, RequirementStatus};
use sparkfox_be_common::RequirementCreator;
...
fn parse_req_id(requirement_id: &str) -> Result<String, String> {
    sparkfox_be_common::RequirementId::try_from(requirement_id.trim())
        .map(sparkfox_be_common::RequirementId::into_string)
