...
use dashmap::DashMap;
use sparkfox_be_ai_agent::AgentStreamEvent;
use sparkfox_be_ai_agent::TurnStopReason;
use sparkfox_be_ai_agent::registry::AgentRegistry;
use sparkfox_be_ai_agent::runtime_registry::AgentRuntimeRegistry;
use sparkfox_be_ai_agent::types::AgentRuntimeBuildOptions;
use sparkfox_be_api_types::{AutoWorkState, AutoWorkTargetKind, Requirement, RequirementStatus, SendMessageRequest};
use sparkfox_be_common::{AppError, ConversationId, TerminalId, UserId};
use sparkfox_be_conversation::ConversationService;
use sparkfox_be_db::IConversationRepository;
use sparkfox_be_terminal::{LifecycleKind, TerminalDriver};
    ...
    loop {
        ...
        let result = match kind {
            AutoWorkTargetKind::Conversation => {
                ...
                let turn_started_ms = sparkfox_be_common::now_ms();
...
async fn inject_and_wait(
    ...
    let model = sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row(&row)?;
    let delegation_policy =
        sparkfox_be_conversation::runtime_options::delegation_policy_from_conversation_row(&row)?;
...
async fn submit_terminal_prompt(
    ...
    match sparkfox_be_terminal::encode_submit_chunks(prompt, true) {
        sparkfox_be_terminal::SubmitChunks::PasteThenCr { paste, cr } => {
            driver.write_input(terminal_id, &paste).await?;
            sleep(sparkfox_be_terminal::TERMINAL_SUBMIT_DELAY).await;
            driver.write_input(terminal_id, &cr).await?;
        }
        sparkfox_be_terminal::SubmitChunks::Single(bytes) => {
...
async fn wait_terminal_turn_end(
    ...
    terminal_id: &str,
    req_id: String,
    lifecycle_rx: Option<broadcast::Receiver<sparkfox_be_terminal::TerminalLifecycleEvent>>,
) -> TerminalTurnEnd {
    let mut renew = interval(LEASE_RENEW_INTERVAL);
    renew.tick().await; // consume the immediate first tick
...
async fn parse_agent_type(registry: &AgentRegistry, agent_type_str: &str) -> sparkfox_be_common::AgentType {
    if registry.find_builtin_by_backend(agent_type_str).await.is_some() {
        return sparkfox_be_common::AgentType::Acp;
    }
    serde_json::from_value::<sparkfox_be_common::AgentType>(serde_json::Value::String(agent_type_str.to_owned()))
        .unwrap_or(sparkfox_be_common::AgentType::Acp)
}

#[cfg(test)]
mod tests {
    use super::*;
    use sparkfox_be_terminal::TerminalDescription;
    use sparkfox_be_terminal::error::TerminalError;
    ...
    fn terminal_submit_chunks_keeps_cr_out_of_the_paste_burst() {
        ...
        // by the shared encoder (`sparkfox_be_terminal::encode_submit_chunks`).
        use sparkfox_be_terminal::{encode_submit_chunks, SubmitChunks};
    ...
    impl TerminalDriver for RecordingDriver {
        ...
        fn subscribe_lifecycle(
            &self,
            _id: &str,
        ) -> Option<tokio::sync::broadcast::Receiver<sparkfox_be_terminal::TerminalLifecycleEvent>> {
            None
        }
    }
    ...
    async fn lifecycle_turn_end_event_resolves_immediately() {
        ...
        use sparkfox_be_terminal::TerminalLifecycleEvent;
        ...
        tx.send(TerminalLifecycleEvent {
            terminal_id: sparkfox_be_common::TerminalId::new(),
