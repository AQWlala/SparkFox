...
use sparkfox_be_db::models::{RequirementRowUpdate, RequirementTagRow};
use sparkfox_be_db::{
...
pub struct RequirementService {
    ...
    conversation_service: Option<sparkfox_be_conversation::ConversationService>,
...
impl RequirementService {
    ...
    pub fn with_conversation_service(
        mut self,
        cs: sparkfox_be_conversation::ConversationService,
    ...
    pub async fn create(&self, mut req: CreateRequirementRequest) -> Result<Requirement, AppError> {
        ...
        let new_row = sparkfox_be_db::models::NewRequirementRow {
    ...
    pub async fn delete_many(&self, ids: &[String]) -> Result<u64, AppError> {
        ...
        for id in ids {
            ...
            match self.repo.delete(id).await {
                ...
                Err(sparkfox_be_db::DbError::NotFound(_)) => {}
    ...
    pub async fn user_interrupt(&self, id: &str, conversation_id: &str, tag: &str) -> Result<(), AppError> {
        ...
        match self.repo.pause_tag(tag, "user_interrupted", Some(id), now_ms()).await {
            Ok(()) => self.emitter.emit_tag_paused(&sparkfox_be_api_types::TagPausedPayload {
                tag: tag.to_string(),
    ...
    pub fn emit_autowork_state(&self, state: &sparkfox_be_api_types::AutoWorkState) {
    ...
    /// `sparkfox_be_terminal::terminal_autowork_capable`, the SAME logic the launch
    ...
    pub async fn ensure_terminal_autowork_eligible(&self, terminal_id: &str) -> Result<(), AppError> {
        ...
        let desc = driver
            ...
            .ok_or_else(|| AppError::NotFound(format!("terminal {terminal_id}")))?;
        let is_agent =
            sparkfox_be_terminal::terminal_autowork_capable(&desc.command, &desc.args, desc.backend.as_deref());
        if !is_agent {
    ...
    /// deletion paths (`sparkfox-be-conversation` / `sparkfox-be-terminal` delete) with
...
/// is no FK cascade, so the deletion path drives this explicitly. Wired in `sparkfox-be-app` via
/// `ConversationService::with_delete_hook`.
#[async_trait::async_trait]
impl sparkfox_be_common::OnConversationDelete for RequirementService {
    async fn on_conversation_deleted(&self, _user_id: &str, conversation_id: &str) {
            ...
            .await
        {
            warn!(
                conversation_id,
                error = %sparkfox_be_common::ErrorChain(&e),
...
/// `term_*` owner domain. Wired in `sparkfox-be-app` via
/// `TerminalService::with_delete_hook`.
#[async_trait::async_trait]
impl sparkfox_be_common::OnTerminalDelete for RequirementService {
...
mod tests {
    use super::*;
    use sparkfox_be_common::{ConversationId, RequirementId, TerminalId, UserId};
    use sparkfox_be_db::{
    ...
    fn attachment_ref(source_path: String, file_name: &str) -> sparkfox_be_api_types::NewAttachmentRef {
        sparkfox_be_api_types::NewAttachmentRef {
            source_path,
    ...
    impl TerminalDriver for MockDriver {
        async fn write_input(
            ...
            _id: &str,
            _bytes: &[u8],
        ) -> Result<(), sparkfox_be_terminal::error::TerminalError> {
            Ok(())
        }
        ...
        async fn describe(
            &self,
            _id: &str,
        ) -> Result<Option<sparkfox_be_terminal::TerminalDescription>, sparkfox_be_terminal::error::TerminalError> {
            if !self.exists {
                return Ok(None);
            }
        ...
        async fn read_autowork(
            &self,
            _id: &str,
        ) -> Result<Option<String>, sparkfox_be_terminal::error::TerminalError> {
