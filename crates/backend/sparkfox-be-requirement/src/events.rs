...
use sparkfox_be_api_types::{AutoWorkState, Requirement, RequirementDeletedPayload, TagPausedPayload, WebSocketMessage};
...
impl RequirementEventEmitter {
    ...
    pub fn emit_autowork_changed(&self, state: &AutoWorkState) {
        let target_valid = match state.kind {
            sparkfox_be_api_types::AutoWorkTargetKind::Conversation => {
                ConversationId::try_from(state.target_id.as_str()).is_ok()
            }
            sparkfox_be_api_types::AutoWorkTargetKind::Terminal => {
