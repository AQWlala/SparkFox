...
use sparkfox_be_api_types::{AttachmentDto, NewAttachmentRef};
use sparkfox_be_common::{AppError, AttachmentId, RequirementId, generate_id, now_ms};
use sparkfox_be_db::IAttachmentRepository;
use sparkfox_be_db::models::AttachmentRow;
use sparkfox_be_file::path_safety::{has_traversal, validate_path};
use tracing::warn;
...
mod tests {
    use super::*;
    use sparkfox_be_db::{SqliteAttachmentRepository, init_database_memory};
    use std::sync::Arc;
    ...
    async fn store() -> (AttachmentStore, tempfile::TempDir, tempfile::TempDir) {
        ...
        let repo: Arc<dyn sparkfox_be_db::IAttachmentRepository> =
    ...
    async fn ingest_rejects_non_image_and_traversal_and_outside_root() {
        ...
        let err = store
            ...
            .await
            .unwrap_err();
        assert!(matches!(err, sparkfox_be_common::AppError::BadRequest(_) | sparkfox_be_common::AppError::Forbidden(_)));
