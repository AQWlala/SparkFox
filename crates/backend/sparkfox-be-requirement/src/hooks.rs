...
//!
//! AutoWork (this crate) drives turn execution; IDMM (the `sparkfox-be-idmm` crate)
//! supervises a session for stalls. To let AutoWork ensure a session is being
//! supervised while a turn runs — WITHOUT this crate depending on `sparkfox-be-idmm`
//! (which would be a cycle, since idmm conceptually sits above requirement) —
//! AutoWork defines this trait and holds an optional handle. `sparkfox-be-idmm`
//! implements it; `sparkfox-be-app` injects the implementation at assembly time.

use sparkfox_be_api_types::AutoWorkTargetKind;

/// Implemented by `sparkfox-be-idmm::IdmmManager`. AutoWork calls
