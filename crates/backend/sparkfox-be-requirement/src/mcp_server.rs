...
mod tests {
    ...
    use sparkfox_be_api_types::{AutoWorkTargetKind, CreateRequirementRequest};
    use sparkfox_be_common::{ConversationId, TerminalId};
    use sparkfox_be_db::{SqliteRequirementRepository, init_database_memory};
    ...
    async fn service_with_claim(
        ...
        let repo: Arc<dyn sparkfox_be_db::IRequirementRepository> =
    ...
    fn child_for(
        ...
        kind: AutoWorkTargetKind,
        owner_id: &str,
    ) -> sparkfox_be_api_types::RequirementMcpChildConfig {
    ...
    async fn post_tool(
        server: &RequirementMcpServer,
        child: &sparkfox_be_api_types::RequirementMcpChildConfig,
        tool: &str,
        args: Value,
    ) -> (u16, Value) {
