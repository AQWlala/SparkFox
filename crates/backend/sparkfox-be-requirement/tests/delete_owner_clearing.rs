...
use sparkfox_be_api_types::{AutoWorkTargetKind, CreateRequirementRequest, RequirementStatus};
use sparkfox_be_common::OnTerminalDelete;
use sparkfox_be_common::{TerminalId, UserId};
use sparkfox_be_db::{
...
use sparkfox_be_requirement::{RequirementEventEmitter, RequirementService};
use sparkfox_be_terminal::{TerminalEventEmitter, TerminalService};

#[derive(Default)]
struct NoopBroadcaster;
impl EventBroadcaster for NoopBroadcaster {
    fn broadcast(&self, _event: sparkfox_be_api_types::WebSocketMessage<serde_json::Value>) {}
}
...
async fn deleting_terminal_clears_requirement_owner_via_hook() {
    ...
    // Terminal service wired exactly as `sparkfox-be-app::build_terminal_state` does:
