        ...
        .layer(middleware::from_fn(
            |mut request: axum::extract::Request, next: axum::middleware::Next| async {
                request.extensions_mut().insert(CurrentUser {
                    id: sparkfox_be_common::UserId::parse(
...
fn t13_1_authorization_header_takes_priority() {
    ...
    assert_eq!(
        sparkfox_be_auth::extract_token_from_headers(&headers),
...
fn t13_3_no_token_returns_none() {
    ...
    assert_eq!(sparkfox_be_auth::extract_token_from_headers(&headers), None);
}
