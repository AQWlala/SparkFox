use sparkfox_be_common::constants::{COOKIE_NAME, CSRF_COOKIE_NAME, SESSION_MAX_AGE_SECONDS};
...
impl CookieConfig {
    ...
    ///
    /// - `SPARKFOX_HTTPS=true` → Secure flag, SameSite=Strict
    /// - Otherwise → no Secure flag, SameSite=Lax (for remote HTTP access)
    pub fn from_env() -> Self {
        let https = std::env::var("SPARKFOX_HTTPS")
