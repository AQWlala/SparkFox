...
use sparkfox_be_ai_agent::AgentRegistry;
use sparkfox_be_ai_agent::runtime_handle::AgentRuntimeHandle;
...
use sparkfox_be_common::{ConversationArtifactId, PaginatedResult, TimestampMs, now_ms};
use sparkfox_be_conversation::ConversationService;
use sparkfox_be_conversation::response_middleware::{CronCreateParams, CronUpdateParams};
use sparkfox_be_db::{
...
use sparkfox_be_cron::busy_guard::CronBusyGuard;
use sparkfox_be_cron::events::CronEventEmitter;
use sparkfox_be_cron::executor::JobExecutor;
use sparkfox_be_cron::scheduler::CronScheduler;
use sparkfox_be_cron::service::CronService;
use sparkfox_be_cron::skill_file::{has_skill_file, write_raw_skill_file};
use sparkfox_be_cron::types::JobStatus;
...
async fn init_database_memory() -> Result<sparkfox_be_db::Database, sparkfox_be_db::DbError> {
    sparkfox_be_db::init_database_memory_with_owner(
        sparkfox_be_common::UserId::parse(TEST_USER_ID.to_owned()).expect("canonical fixture owner"),
    )
    .await
}
...
impl sparkfox_be_ai_agent::runtime_registry::AgentRuntimeRegistry for StubAgentRuntimeRegistry {
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<sparkfox_be_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<String, sparkfox_be_db::models::ConversationRow>>,
}
...
impl StubConvRepo {
    ...
    fn upsert_artifact_row(&self, mut artifact: sparkfox_be_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<sparkfox_be_db::ConversationArtifactRow> {
...
impl IConversationRepository for StubConvRepo {
    async fn get(
        ...
        let row = if id == CONV_MODE {
            // conv_mode
            sparkfox_be_db::models::ConversationRow {
                id: id.to_owned(),
            ...
            sparkfox_be_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "anthropic",
                    "agent_name": "Nomi",
                    "workspace": "/tmp/sparkfox-workspace",
                    "session_mode": "default",
                    "current_model_id": "claude-sonnet-4-20250514"
                })
                .to_string(),
    ...
    async fn update(
        ...
        let row = rows
            .entry(id.to_owned())
            .or_insert_with(|| sparkfox_be_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: TEST_USER_ID.into(),
                name: "stub".into(),
                r#type: "default".into(),
    ...
    async fn list_artifacts(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<sparkfox_be_db::ConversationArtifactRow>, sparkfox_be_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<sparkfox_be_db::ConversationArtifactRow>, sparkfox_be_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        artifact: &sparkfox_be_db::ConversationArtifactRow,
    ) -> Result<sparkfox_be_db::ConversationArtifactRow, sparkfox_be_db::DbError> {
        let mut guard = self.artifacts.lock().unwrap();
...
async fn setup_with_conv_repo() -> (
    ...
    // exercise the same string-only ID contract as production.
    {
        ...
        for id in [
            ...
            real_conv_repo
                .create(&sparkfox_be_db::models::ConversationRow {
                    id: id.to_owned(),
                    user_id: TEST_USER_ID.into(),
    ...
    let data_dir = std::env::temp_dir().join(format!("sparkfox-be-cron-test-{}", now_ms()));
    ...
    let runtime_registry: Arc<dyn sparkfox_be_ai_agent::runtime_registry::AgentRuntimeRegistry> =
...
async fn cron_crud_run_history_and_skill_boundaries_are_owner_scoped() {
    ...
    assert!(matches!(
        ...
        Err(sparkfox_be_cron::error::CronError::JobNotFound(_))
    ));
    assert!(
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(
        err,
        sparkfox_be_cron::error::CronError::JobNotFound(_)
    ));
}
...
async fn cj9_update_schedule_type() {
    ...
    assert!(matches!(
        updated.schedule,
        sparkfox_be_cron::types::CronSchedule::Cron { .. }
    ));
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(sparkfox_be_db::ConversationArtifactRow {
...
async fn sk4_save_empty_skill() {
    ...
    assert!(matches!(
        err,
        sparkfox_be_cron::error::CronError::InvalidSkillContent(_)
    ));
}
...
async fn sc5_invalid_cron_expression() {
    ...
    assert!(matches!(
        err,
        sparkfox_be_cron::error::CronError::InvalidCronExpression(_)
    ));
}
...
async fn sc7_every_zero_interval() {
    ...
    assert!(matches!(
        err,
        sparkfox_be_cron::error::CronError::InvalidSchedule(_)
    ));
}
...
async fn oc2_init_cleans_jobs_with_missing_conversation() {
    ...
    repo.update(
        TEST_USER_ID,
        &missing.id,
        &sparkfox_be_db::UpdateCronJobParams {
...
async fn icron_service_create_job() {
    ...
    use sparkfox_be_conversation::response_middleware::ICronService;
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = sparkfox_be_db::UpdateCronJobParams {
