...
use sparkfox_be_ai_agent::runtime_registry::AgentRuntimeRegistry;
use sparkfox_be_ai_agent::types::AgentRuntimeBuildOptions;
#[cfg(test)]
use sparkfox_be_ai_agent::types::SendMessageData;
use sparkfox_be_ai_agent::{AgentRegistry, AgentStreamEvent};
use sparkfox_be_api_types::{CreateConversationRequest, SendMessageRequest};
...
use sparkfox_be_conversation::ConversationService;
use sparkfox_be_db::models::MessageRow;
use sparkfox_be_db::{ConversationRowUpdate, IConversationRepository};
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<sparkfox_be_db::models::ConversationRow>, CronError> {
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: sparkfox_be_common::now_ms(),
        };
...
impl JobExecutor {
    ...
    async fn execute_inner(
        ...
        // `sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row`.
        ...
        let model = match sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row(
            &conversation_row,
        ) {
            Ok(model) => model,
            Err(error) => {
        ...
        let delegation_policy = match sparkfox_be_conversation::runtime_options::delegation_policy_from_conversation_row(&conversation_row) {
            Ok(policy) => policy,
            Err(error) => {
                error!(
                    job_id = %job.id,
    ...
    fn spawn_skill_suggest_flow(
        &self,
        agent: sparkfox_be_ai_agent::AgentRuntimeHandle,
...
fn resolve_model(job: &CronJob) -> Option<ProviderWithModel> {
    ...
    if sparkfox_be_common::ProviderId::try_from(config.backend.as_str()).is_err() {
        return None;
    }
...
mod tests {
    ...
    use sparkfox_be_ai_agent::runtime_handle::{AgentRuntimeHandle, AgentRuntimeControl, MockAgentRuntime};
    use sparkfox_be_ai_agent::protocol::events::FinishEventData;
    use sparkfox_be_api_types::{AgentModeResponse, WebSocketMessage};
    use sparkfox_be_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use sparkfox_be_db::{
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(sparkfox_be_db::SqliteAgentMetadataRepository::new(
            db.pool().clone(),
        ));
    ...
    fn make_executor_for_busy_tests(guard: Arc<CronBusyGuard>) -> JobExecutor {
        ...
        impl AgentRuntimeRegistry for StubAgentRuntimeRegistry {
            ...
            ) -> Result<AgentRuntimeHandle, sparkfox_be_common::AppError> {
                Err(sparkfox_be_common::AppError::Internal("stub".into()))
            }
            fn terminate(
                &self,
                _: &str,
                _: Option<sparkfox_be_common::AgentKillReason>,
            ) -> Result<(), sparkfox_be_common::AppError> {
                Ok(())
            }
            ...
            fn collect_idle_runtimes(&self, _: sparkfox_be_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            async fn get(
                &self,
                _id: &str,
            ) -> Result<Option<sparkfox_be_db::models::ConversationRow>, sparkfox_be_db::DbError>
            {
                Ok(None)
            }
            async fn create(
                &self,
                _row: &sparkfox_be_db::models::ConversationRow,
            ...
            async fn delete(&self, _id: &str) -> Result<(), sparkfox_be_db::DbError> {
                Ok(())
            }
            ...
            ) -> Result<PaginatedResult<sparkfox_be_db::models::ConversationRow>, sparkfox_be_db::DbError>
            {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
            ...
            async fn list_by_cron_job(
                ...
                _user_id: &str,
                _cron_job_id: &str,
            ) -> Result<Vec<sparkfox_be_db::models::ConversationRow>, sparkfox_be_db::DbError> {
                Ok(vec![])
            }
            ...
            async fn get_messages(
                ...
                _page_size: u32,
                _order: SortOrder,
            ) -> Result<PaginatedResult<sparkfox_be_db::models::MessageRow>, sparkfox_be_db::DbError>
            {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
            ...
            async fn insert_message(
                &self,
                _message: &sparkfox_be_db::models::MessageRow,
            ) -> Result<(), sparkfox_be_db::DbError> {
                Ok(())
            }
            ...
            async fn get_message_by_msg_id(
                ...
                _msg_id: &str,
                _msg_type: &str,
            ) -> Result<Option<sparkfox_be_db::models::MessageRow>, sparkfox_be_db::DbError> {
                Ok(None)
            }
            async fn search_messages(
                ...
                _page: u32,
                _page_size: u32,
            ) -> Result<PaginatedResult<MessageSearchRow>, sparkfox_be_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
        ...
        impl sparkfox_be_realtime::UserEventSink for StubBroadcaster {
            fn send_to_user(&self, _: &str, _: WebSocketMessage<serde_json::Value>) {}
        }

        struct StubSkillResolver;
        #[async_trait::async_trait]
        impl sparkfox_be_conversation::skill_resolver::SkillResolver for StubSkillResolver {
            ...
            async fn resolve_skills(
                &self,
                _names: &[String],
            ) -> Vec<sparkfox_be_conversation::skill_resolver::ResolvedAgentSkill> {
                Vec::new()
            }
            ...
            async fn link_workspace_skills(
                ...
                _rel_dirs: &[&str],
                _skills: &[sparkfox_be_conversation::skill_resolver::ResolvedAgentSkill],
            ) -> usize {
                0
            }
        }
        ...
        let agent_metadata_repo: Arc<dyn sparkfox_be_db::IAgentMetadataRepository> =
            Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn sparkfox_be_db::IAcpSessionRepository> =
        ...
        let conv_service = Arc::new(ConversationService::new(
            ...
            acp_session_repo,
            Arc::new(sparkfox_be_conversation::NoExecutionConversationBoundary),
        ));
    ...
    impl AgentRuntimeControl for RecordingAgent {
        ...
        ) -> Result<(), sparkfox_be_ai_agent::AgentSendError> {
            self.send_calls.fetch_add(1, Ordering::Relaxed);
            self.sent_messages.write().await.push(data);
            Ok(())
        }

        async fn cancel(&self) -> Result<(), sparkfox_be_common::AppError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), sparkfox_be_common::AppError> {
            Ok(())
        }
    }
    ...
    impl MockAgentRuntime for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, sparkfox_be_common::AppError> {
        ...
        async fn set_mode(&self, mode: &str) -> Result<(), sparkfox_be_common::AppError> {
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(
            ...
            Ok(Some(sparkfox_be_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: USER_ID.into(),
    ...
    struct MissingWorkspaceConversationRepo {
        row: sparkfox_be_db::models::ConversationRow,
        updates: Mutex<Vec<ConversationRowUpdate>>,
        inserted_messages: Mutex<Vec<sparkfox_be_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<sparkfox_be_db::models::MessageRow> {
    ...
    impl sparkfox_be_realtime::UserEventSink for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn insert_message(
            &self,
            message: &sparkfox_be_db::models::MessageRow,
        ) -> Result<(), sparkfox_be_db::DbError> {
        ...
        ) -> Result<ConversationArtifactRow, sparkfox_be_db::DbError> {
    ...
    where
        B: sparkfox_be_realtime::UserEventSink + 'static,
    {
        struct StubSkillResolver;
    ...
    #[async_trait::async_trait]
    impl sparkfox_be_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<sparkfox_be_db::models::AcpSessionRow>, sparkfox_be_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &sparkfox_be_db::CreateAcpSessionParams<'_>,
        ) -> Result<sparkfox_be_db::models::AcpSessionRow, sparkfox_be_db::DbError> {
            Err(sparkfox_be_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: &str,
            _session_id: &str,
        ) -> Result<bool, sparkfox_be_db::DbError> {
            Ok(false)
        }
        ...
        async fn delete(&self, _conversation_id: &str) -> Result<bool, sparkfox_be_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<sparkfox_be_db::PersistedSessionState>, sparkfox_be_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: &str,
            _params: &sparkfox_be_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, sparkfox_be_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl sparkfox_be_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(
            &self,
        ) -> Result<Vec<sparkfox_be_db::models::AgentMetadataRow>, sparkfox_be_db::DbError> {
            Ok(Vec::new())
        }
        async fn get(
            &self,
            _id: &str,
        ) -> Result<Option<sparkfox_be_db::models::AgentMetadataRow>, sparkfox_be_db::DbError> {
            Ok(None)
        }
        ...
        async fn upsert(
            &self,
            _params: &sparkfox_be_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<sparkfox_be_db::models::AgentMetadataRow, sparkfox_be_db::DbError> {
            Err(sparkfox_be_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &sparkfox_be_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<sparkfox_be_db::models::AgentMetadataRow>, sparkfox_be_db::DbError> {
            Ok(None)
        }
        ...
        async fn delete(&self, _id: &str) -> Result<bool, sparkfox_be_db::DbError> {
            Ok(false)
        }
    }
}
