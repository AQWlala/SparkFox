...
use sparkfox_be_db::{CRON_RUN_HISTORY_LIMIT, CronJobRunRow, ICronRepository, UpdateCronJobParams};
...
impl CronService {
    ...
    async fn resolve_preset_config(
        &self,
        config: &mut sparkfox_be_api_types::CronAgentConfigDto,
    ) -> Result<(), CronError> {
        ...
        let snapshot = service
            .resolve(
                &preset_id,
                sparkfox_be_api_types::PresetTarget::Cron,
                None,
                sparkfox_be_api_types::PresetOverrides::default(),
            )
            .await?;
    ...
    async fn validate_nomi_job_model(
        ...
        match nomi_model_check(agent_type, execution_mode, conversation_id) {
            ...
            NomiModelCheck::AgentConfig => {
                validate_sparkfox_ag_agent_backend(agent_type, agent_backend)
            }
            NomiModelCheck::BoundConversation => {
                ...
                match self.executor.get_conversation_row(conversation_id).await {
                    Ok(Some(row)) => {
                        match sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row(&row) {
                            Ok(Some(_)) => Ok(()),
...
impl sparkfox_be_common::OnConversationDelete for CronService {
...
#[async_trait::async_trait]
impl sparkfox_be_conversation::response_middleware::ICronService for CronService {
    async fn create_job(
        &self,
        user_id: &str,
        conversation_id: &str,
        params: &sparkfox_be_conversation::response_middleware::CronCreateParams,
    ) -> sparkfox_be_conversation::response_middleware::CronCommandResult {
        if sparkfox_be_common::ConversationId::try_from(conversation_id).is_err() {
            return sparkfox_be_conversation::response_middleware::CronCommandResult {
                success: false,
        ...
        match self.add_job(user_id, req).await {
            ...
            Err(e) => sparkfox_be_conversation::response_middleware::CronCommandResult {
                success: false,
                message: e.to_string(),
            },
        }
    ...
    async fn update_job(
        ...
        user_id: &str,
        conversation_id: &str,
        params: &sparkfox_be_conversation::response_middleware::CronUpdateParams,
    ) -> sparkfox_be_conversation::response_middleware::CronCommandResult {
    ...
    async fn delete_job(
        ...
        match self.remove_job(user_id, job_id).await {
            Ok(()) => sparkfox_be_conversation::response_middleware::CronCommandResult {
                success: true,
...
fn build_agent_config_from_conversation(
...
) -> Result<(String, Option<sparkfox_be_api_types::CronAgentConfigDto>), sparkfox_be_common::AppError> {
    ...
    let model_resolved =
        sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row(row)?;
    ...
    let full_auto_mode = agent_type_enum
        ...
        .to_owned();
    let agent_config = sparkfox_be_api_types::CronAgentConfigDto {
        backend,
...
#[cfg(test)]
fn validate_sparkfox_ag_agent_config(
    agent_type: &str,
    agent_config: Option<&sparkfox_be_api_types::CronAgentConfigDto>,
) -> Result<(), CronError> {
    validate_sparkfox_ag_agent_backend(
        agent_type,
        agent_config.map(|config| config.backend.as_str()),
    )
}

fn validate_sparkfox_ag_agent_backend(
    agent_type: &str,
    agent_backend: Option<&str>,
) -> Result<(), CronError> {
    if agent_type != "nomi" {
...
fn clamp_model_only_cron_config(config: &mut sparkfox_be_api_types::CronAgentConfigDto) {
...
mod tests {
    ...
    // -- validate_sparkfox_ag_agent_config ----------------------------------------

    fn agent_cfg_dto(backend: &str) -> sparkfox_be_api_types::CronAgentConfigDto {
        sparkfox_be_api_types::CronAgentConfigDto {
    ...
    fn validate_nomi_accepts_valid_config() {
        ...
        assert!(validate_sparkfox_ag_agent_config("nomi", Some(&cfg)).is_ok());
    }

    #[test]
    fn validate_nomi_rejects_missing_config() {
        let err = validate_sparkfox_ag_agent_config("nomi", None).unwrap_err();
    ...
    fn validate_nomi_rejects_empty_backend() {
        ...
        let err = validate_sparkfox_ag_agent_config("nomi", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }
    ...
    fn validate_nomi_ignores_non_nomi_type() {
        ...
        assert!(validate_sparkfox_ag_agent_config("acp", None).is_ok());
        let cfg = agent_cfg_dto("");
        assert!(validate_sparkfox_ag_agent_config("claude", Some(&cfg)).is_ok());
    }
