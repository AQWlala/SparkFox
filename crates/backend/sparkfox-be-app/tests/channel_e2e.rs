...
async fn seed_telegram_channel(repo: &std::sync::Arc<dyn sparkfox_be_db::IChannelRepository>) {
    use sparkfox_be_common::now_ms;
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn sparkfox_be_db::IChannelRepository> =
        std::sync::Arc::new(sparkfox_be_db::SqliteChannelRepository::new(pool));
    let pairing_svc = sparkfox_be_channel::pairing::PairingService::new(
        repo.clone(),
...
async fn enable_disable_plugin_lifecycle() {
    ...
    let channel_id = telegram["plugin_id"]
        ...
        .to_owned();
    sparkfox_be_common::ChannelId::parse(channel_id.clone()).expect("canonical channel id");
