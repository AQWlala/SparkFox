...
//! `sparkfox_be_app::create_router_with_states` against an in-memory DB.
...
use sparkfox_be_app::{ModuleStates, build_module_states, create_router_with_states};
use sparkfox_be_db::init_database_memory;
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("SPARKFOX_BUILTIN_SKILLS_PATH");
    }
    ...
    sparkfox_be_extension::materialize_if_needed(&data_dir, sparkfox_be_extension::builtin_skills_corpus(), "test-fixture")
        .await
