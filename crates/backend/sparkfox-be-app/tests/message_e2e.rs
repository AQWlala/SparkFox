...
async fn insert_message(
    services: &sparkfox_be_app::AppServices,
    conv_id: &str,
    content: &str,
    created_at: i64,
) -> String {
    let repo = sparkfox_be_db::SqliteConversationRepository::new(services.database.pool().clone());
    let message_id = MessageId::new().into_string();
    let msg = sparkfox_be_db::models::MessageRow {
        id: message_id.clone(),
    ...
    sparkfox_be_db::IConversationRepository::insert_message(&repo, &msg)
        .await
        .unwrap();
    message_id
}

async fn update_conversation_workspace(services: &sparkfox_be_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = sparkfox_be_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn upsert_artifact(services: &sparkfox_be_app::AppServices, artifact: sparkfox_be_db::ConversationArtifactRow) -> String {
    let repo = sparkfox_be_db::SqliteConversationRepository::new(services.database.pool().clone());
    sparkfox_be_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap()
        .id
}
...
async fn seed_cron_job(services: &sparkfox_be_app::AppServices, id: &str) {
    sqlx::query(
...
async fn t2_1c_send_message_accepts_interior_whitespace_workspace() {
    ...
    let workspace = temp.path().join("Application Support").join("Nomi").join("conversations").join("sparkfox-temp-1");
