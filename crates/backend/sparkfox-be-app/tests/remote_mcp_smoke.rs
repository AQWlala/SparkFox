...
use sparkfox_be_common::CompanionId;
...
async fn mcp_endpoint_is_mounted_and_token_gated() {
    ...
    services
        ...
        .insert_token(companion_id.clone(), sparkfox_be_auth::token_sha256_hex(token));
...
async fn rest_v1_stream_endpoint_emits_result_frame() {
    ...
    services
        .companion_token_validator
        .insert_token(test_companion_id(), sparkfox_be_auth::token_sha256_hex(token));
...
fn remote_surface_projection_is_correct() {
    ...
    assert!(!remote.contains(&"sparkfox_ag_agent_run"));
    assert!(!remote.contains(&"sparkfox_ag_agent_result"));
