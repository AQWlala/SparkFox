...
//! integration level in `sparkfox-be-office/tests/`.
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use sparkfox_be_office::error::OfficeError;
    use sparkfox_be_office::types::DocType;
    use sparkfox_be_office::{ProcessHandle, ProcessSpawner};
    ...
    let bc: Arc<dyn sparkfox_be_realtime::UserEventSink> = Arc::new(NoopBroadcaster);
