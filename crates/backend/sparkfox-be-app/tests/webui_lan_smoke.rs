...
async fn webui_lan_start_smoke() {
    ...
    // these tests run in parallel, so a shared set_var("SPARKFOX_DATA_DIR") would
    ...
    let cli = sparkfox_be_app::cli::Cli::parse_from(["sparkfox-desktop-test", "--data-dir", &data_dir]);
    let merged_path = std::env::var("PATH").unwrap_or_default();

    let started = sparkfox_be_app::DesktopServer::start(&cli, &merged_path, Some(spa_dir), None).await;
...
async fn webui_lan_spa_deep_link_serves_app_shell() {
    ...
    let (server, _keep) =
        sparkfox_be_app::DesktopServer::start(&cli, &merged_path, Some(spa_dir), None)
            .await
...
async fn webui_lan_without_app_shell_fails_instead_of_serving_partial_qr_flow() {
    ...
    let (server, _keep) = sparkfox_be_app::DesktopServer::start(&cli, &merged_path, None, None)
        .await
        .expect("DesktopServer::start failed");
...
async fn webui_lan_dev_proxy_serves_live_frontend() {
    ...
    let (server, _keep) =
        sparkfox_be_app::DesktopServer::start(&cli, &merged_path, None, Some(dev_url))
            .await
            .expect("DesktopServer::start failed");
...
async fn webui_enable_preserves_user_set_username_and_reports_persisted_state() {
    ...
    let (server, _keep) = sparkfox_be_app::DesktopServer::start(&cli, &merged_path, Some(spa_dir), None)
        .await
        .expect("DesktopServer::start failed");
