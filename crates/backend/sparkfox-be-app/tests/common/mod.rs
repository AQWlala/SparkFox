//! Shared test helpers for sparkfox-be-app E2E tests.
#![allow(dead_code)]
...
use sparkfox_be_ai_agent::{AgentRuntimeHandle, AgentRuntimeControl, MockAgentRuntime, InMemoryAgentRuntimeRegistry};
use sparkfox_be_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use sparkfox_be_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use sparkfox_be_file::FileService;
use sparkfox_be_system::VersionCheckService;
...
pub async fn build_app_with_skill_paths(root: &std::path::Path) -> (axum::Router, AppServices, SkillPaths) {
    ...
    states.skill = SkillRouterState {
        ...
        skill_tag_repo: std::sync::Arc::new(sparkfox_be_db::SqliteSkillTagRepository::new(
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await;
    states.shell.shell_service = std::sync::Arc::new(sparkfox_be_shell::ShellService::new(std::sync::Arc::new(
        sparkfox_be_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                sparkfox_be_ai_agent::types::AgentRuntimeBuildOptions,
            ) -> futures_util::future::BoxFuture<'static, Result<AgentRuntimeHandle, sparkfox_be_common::AppError>>
            + Send
            + Sync,
    ...
    let runtime_registry: std::sync::Arc<dyn sparkfox_be_ai_agent::AgentRuntimeRegistry> =
...
impl AgentRuntimeControl for NoopMockAgent {
    fn agent_type(&self) -> sparkfox_be_common::AgentType {
        sparkfox_be_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<sparkfox_be_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> sparkfox_be_common::TimestampMs {
        sparkfox_be_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<sparkfox_be_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: sparkfox_be_ai_agent::types::SendMessageData,
    ...
    fn kill(&self, _reason: Option<sparkfox_be_common::AgentKillReason>) -> Result<(), sparkfox_be_common::AppError> {
        Ok(())
    }
}
...
pub async fn setup_and_login(
    ...
    let hash = sparkfox_be_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
