...
use sparkfox_be_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use sparkfox_be_common::{decrypt_string, now_ms};
use sparkfox_be_db::{IChannelRepository, SqliteChannelRepository};
...
async fn eq20_enable_extension_channel_persists_config_and_exposes_status() {
    ...
    let row = repo
        ...
        .unwrap();
    sparkfox_be_common::ChannelId::parse(row.id.clone()).expect("extension channel id is canonical");
    assert!(row.enabled);
...
async fn eq21_disable_extension_channel_updates_status() {
    ...
    let channel_id = status_json["data"]
        ...
        .to_owned();
    sparkfox_be_common::ChannelId::parse(channel_id.clone()).expect("canonical extension channel id");
