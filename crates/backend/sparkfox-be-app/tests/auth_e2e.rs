...
//! auth routes) via `sparkfox_be_app::create_router`, covering test-plan items
...
use sparkfox_be_app::{AppConfig, AppServices};
...
async fn t12_3_session_cookie_attributes() {
    ...
    let hash = sparkfox_be_auth::hash_password("StrongP@ss1").unwrap();
...
async fn installation_control_plane_uses_canonical_owner_identity() {
    ...
    let installation_owner =
        sparkfox_be_db::installation_owner_id(services.database.pool()).await.unwrap();
    assert_eq!(
...
async fn t14_2_setup_then_login() {
    ...
    let hash = sparkfox_be_auth::hash_password("Admin@Pass1").unwrap();
    services
        .user_repo
        .set_system_user_credentials("admin", &hash)
        .await
...
async fn full_auth_flow_e2e() {
    ...
    // 2. Setup user
    let hash = sparkfox_be_auth::hash_password("Initial@Pass1").unwrap();
    services
        .user_repo
        .set_system_user_credentials("admin", &hash)
        .await
