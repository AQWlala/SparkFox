//! E2E tests for the Platform Gateway MCP server (`sparkfox-be-gateway`).
//!
...
struct Gateway {
    config: sparkfox_be_api_types::GatewayMcpConfig,
...
impl Gateway {
    fn from_services(services: &sparkfox_be_app::AppServices) -> Self {
        let cfg = services
            .gateway_mcp_config
            .as_ref()
...
async fn seed_user_and_conversation(services: &sparkfox_be_app::AppServices, user_id: &str, conv_id: &str) {
...
async fn seed_provider(services: &sparkfox_be_app::AppServices, provider_id: &str, model: &str) {
    sqlx::query(
...
async fn gw_issuer_rejects_missing_user_identity_before_bridge_spawn() {
    ...
    assert_eq!(
        gw.config
            ...
            .unwrap_err(),
        sparkfox_be_common::LoopbackCapabilityError::InvalidIdentity
    );
}
