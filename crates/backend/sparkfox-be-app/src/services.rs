...
use sparkfox_be_api_types::{GatewayMcpConfig, RequirementMcpConfig};
use sparkfox_be_auth::{
    AuthPolicy, CompanionTokenValidator, CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret,
};
use sparkfox_be_common::OnConversationDelete;
use sparkfox_be_conversation::runtime_state::ConversationRuntimeStateService;
use sparkfox_be_conversation::{
...
use sparkfox_be_realtime::{BroadcastEventBus, WebSocketManager};
use sparkfox_be_terminal::{TerminalEventEmitter, TerminalLifecycleServer, TerminalService};
...
pub struct AppServices {
    ...
    pub managed_model_service: Arc<sparkfox_be_system::ManagedModelService>,
    /// Keeps the authenticated loopback OpenAI-compatible listener alive.
    pub(crate) _managed_model_server: sparkfox_be_system::ManagedModelServer,
    /// Keeps the immediate + periodic managed catalog refresh loop alive.
    pub(crate) _managed_model_refresh_task: sparkfox_be_system::ManagedModelRefreshTask,
    ...
    pub requirement_service: Arc<sparkfox_be_requirement::RequirementService>,
    ...
    pub skill_paths: Arc<sparkfox_be_extension::SkillPaths>,
    ...
    pub(crate) _requirement_mcp_server: Option<sparkfox_be_requirement::RequirementMcpServer>,
    ...
    pub(crate) _gateway_mcp_server: Option<sparkfox_be_gateway::GatewayMcpServer>,
    ...
    pub(crate) _knowledge_mcp_server: Option<sparkfox_be_knowledge::KnowledgeMcpServer>,
    ...
    pub companion_service: Arc<sparkfox_be_companion::CompanionService>,
    ...
    pub public_agent_service: Arc<sparkfox_be_public_agent::PublicAgentService>,
    ...
    pub workshop_service: Arc<sparkfox_be_workshop::WorkshopService>,
    ...
    pub creation_service: Arc<sparkfox_be_creation::CreationService>,
    ...
    pub knowledge_service: Arc<sparkfox_be_knowledge::KnowledgeService>,
}
...
impl AppServices {
    ...
    pub(crate) async fn inject_gateway_deps(&self, deps: Arc<sparkfox_be_gateway::GatewayDeps>) {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        sparkfox_ag_computer::set_host_app_label("SparkFox");
        ...
        let (managed_model_service, managed_model_server) =
            sparkfox_be_system::start_and_provision_free_model_with_preferences(
                provider_repo.clone(),
                Some(Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(
                    database.pool().clone(),
                ))),
                encryption_key,
            )
        ...
        let managed_model_refresh_task = {
            ...
            sparkfox_be_system::ManagedModelRefreshTask::start_with_success_hook(
                ...
                move |status| {
                    ...
                    async move {
                        ...
                        let models = status
                            ...
                            .collect::<Vec<_>>();
                        match sparkfox_be_system::seed_missing_inferred_profiles(
                            profile_repo.as_ref(),
                            provider_id,
                            sparkfox_be_system::FREE_MODEL_PLATFORM,
                            &models,
                        )
                        .await
                        {
        ...
        let execution_conversation_boundary: Arc<dyn ExecutionConversationBoundary> = Arc::new(
            RepositoryExecutionConversationBoundary::new(Arc::new(
                sparkfox_be_db::SqliteAgentExecutionRepository::new(database.pool().clone()),
            )),
        );
        ...
        let skill_paths = Arc::new(sparkfox_be_extension::resolve_skill_paths(
            &app_resource_dir,
            &data_dir,
        ));
        ...
        let requirement_repo: Arc<dyn sparkfox_be_db::IRequirementRepository> = Arc::new(
            sparkfox_be_db::SqliteRequirementRepository::new(database.pool().clone()),
        );
        let requirement_emitter = sparkfox_be_requirement::RequirementEventEmitter::new(
            event_bus.clone(),
        ...
        let webhook_repo_for_notifier: Arc<dyn sparkfox_be_db::IWebhookRepository> = Arc::new(
            sparkfox_be_db::SqliteWebhookRepository::new(database.pool().clone()),
        );
        let tag_setting_repo_for_notifier: Arc<dyn sparkfox_be_db::ITagSettingRepository> = Arc::new(
            sparkfox_be_db::SqliteTagSettingRepository::new(database.pool().clone()),
        );
        let completion_notifier = sparkfox_be_webhook::CompletionNotifierImpl::new(
            tag_setting_repo_for_notifier,
            webhook_repo_for_notifier,
            Arc::new(sparkfox_be_webhook::DefaultWebhookSender::new()),
        )
        .into_arc();
        let attachment_repo: Arc<dyn sparkfox_be_db::IAttachmentRepository> = Arc::new(
            sparkfox_be_db::SqliteAttachmentRepository::new(database.pool().clone()),
        );
        let attachment_store = Arc::new(sparkfox_be_requirement::AttachmentStore::new(
            data_dir.clone(),
            attachment_repo,
        ));
        let requirement_service = Arc::new(
            sparkfox_be_requirement::RequirementService::new(requirement_repo, requirement_emitter)
                .with_completion_notifier(completion_notifier)
                .with_attachment_store(attachment_store),
        );
        let requirement_sink =
            sparkfox_be_requirement::RequirementServiceSink::into_arc(requirement_service.clone());
        ...
        let (requirement_mcp_server, requirement_mcp_config) =
            match sparkfox_be_requirement::RequirementMcpServer::start().await {
                Ok(srv) => {
        ...
        let (gateway_mcp_server, gateway_mcp_config) =
            match sparkfox_be_gateway::GatewayMcpServer::start().await {
                Ok(srv) => {
                    let config = srv.issuer_config(
                        backend_binary_path.to_string_lossy().to_string(),
        ...
        let open_mcp_config =
            cfg!(target_os = "windows").then(|| sparkfox_be_api_types::OpenMcpConfig {
        ...
        // (`sparkfox-ag-a11y` implements macOS AX / Windows UIA / Linux AT-SPI backends).
        ...
        let computer_mcp_config =
            cfg!(feature = "computer-use").then(|| sparkfox_be_api_types::ComputerMcpConfig {
                binary_path: backend_binary_path.to_string_lossy().to_string(),
            });
        ...
        let browser_mcp_config =
            cfg!(feature = "browser-use").then(|| sparkfox_be_api_types::BrowserMcpConfig {
                binary_path: backend_binary_path.to_string_lossy().to_string(),
            });
        ...
        let knowledge_repo: Arc<dyn sparkfox_be_db::IKnowledgeRepository> = Arc::new(
            sparkfox_be_db::SqliteKnowledgeRepository::new(database.pool().clone()),
        );
        let knowledge_service = Arc::new(sparkfox_be_knowledge::KnowledgeService::new(
            knowledge_repo,
            &data_dir,
            sparkfox_be_knowledge::KnowledgeEventEmitter::new(
                event_bus.clone(),
                authoritative_user_id.clone(),
            ),
        ));
        ...
        knowledge_service.set_completer(Arc::new(sparkfox_be_ai_agent::LiveKnowledgeCompleter {
            provider_repo: provider_repo.clone() as Arc<dyn sparkfox_be_db::IProviderRepository>,
            encryption_key,
        ...
        // boundary (`sparkfox-be-knowledge` never depends on the browser engine) holds:
        // `BrowserFetcher` lives in `sparkfox-be-ai-agent` and is injected here as a
        ...
        #[cfg(feature = "browser-use")]
        {
            ...
            sparkfox_ag_browser_engine::profile::gc_stale_profiles(
                &data_dir.join("browser-data").join("profiles"),
                std::time::Duration::from_secs(3600),
            );
            let browser_fetcher =
                sparkfox_be_ai_agent::BrowserFetcher::new(data_dir.join("knowledge-browser"));
        ...
        knowledge_service.register_connector(Arc::new(
            sparkfox_be_knowledge::connector_feishu::FeishuConnector::new(),
        ));
        let connector_cred_repo: Arc<dyn sparkfox_be_db::IConnectorCredentialRepository> = Arc::new(
            sparkfox_be_db::SqliteConnectorCredentialRepository::new(database.pool().clone()),
        );
        ...
        let (knowledge_mcp_server, knowledge_mcp_config) =
            match sparkfox_be_knowledge::KnowledgeMcpServer::start().await {
                Ok(mut srv) => {
        ...
        terminal_service.with_title_completer(Arc::new(sparkfox_be_ai_agent::LiveTerminalTitleCompleter {
        ...
        let companion_completer: Arc<dyn sparkfox_be_companion::learner::CompanionCompleter> =
            Arc::new(sparkfox_be_companion::learner::LiveCompanionCompleter {
                provider_repo: provider_repo.clone() as Arc<dyn sparkfox_be_db::IProviderRepository>,
                encryption_key,
                workspace: data_dir.clone(),
            });
        let companion_service = sparkfox_be_companion::CompanionService::start(
            &data_dir,
            event_bus.clone(),
        ...
        let public_agent_service = sparkfox_be_public_agent::PublicAgentService::start(&data_dir);
        ...
        let workshop_service = sparkfox_be_workshop::WorkshopService::start(
            &data_dir,
            Arc::new(sparkfox_be_db::SqliteWorkshopRepository::new(database.pool().clone())),
        );
        ...
        let creation_http = sparkfox_be_net::http_client();
        let creation_asset_bridge = Arc::new(crate::workshop_bridge::WorkshopAssetBridge::new(
            data_dir.clone(),
            Arc::new(sparkfox_be_db::SqliteWorkshopRepository::new(database.pool().clone())),
        ));
        let creation_adapters = sparkfox_be_creation::default_adapters(creation_http.clone());
        let creation_service = sparkfox_be_creation::CreationService::builder(Arc::new(
            sparkfox_be_db::SqliteCreationTaskRepository::new(database.pool().clone()),
        ))
        .with_http(creation_http.clone())
        .with_provider_repo(
            Arc::new(sparkfox_be_db::SqliteProviderRepository::new(database.pool().clone())),
            encryption_key,
        )
        ...
        if let Ok(seed) = std::env::var("sparkfox_be_companion_TOKEN") {
            let seed = seed.trim();
            if !seed.is_empty() && companion_token_validator.resolve(seed).is_none() {
                match companion_service.default_companion_id().await {
                    Some(default_id) => {
                        let default_id = sparkfox_be_common::CompanionId::parse(default_id)
                            .map_err(|error| anyhow::anyhow!("default companion has invalid id: {error}"))?;
                        let hash = sparkfox_be_auth::token_sha256_hex(seed);
                        if let Err(e) = companion_token_repo
                            .upsert_for_companion(&default_id, &hash)
                            .await
                        {
                            tracing::warn!("failed to persist sparkfox_be_companion_TOKEN seed: {e}");
                        }
                        companion_token_validator.insert_token(default_id.clone(), hash);
                        tracing::info!(
                            "Remote access token seeded from sparkfox_be_companion_TOKEN, bound to default companion {default_id}"
                        );
                    }
                    None => tracing::warn!(
                        "sparkfox_be_companion_TOKEN set but no companion exists to bind it to; create a companion first"
                    ),
                }
            }
        }
        ...
        let provider_repo_for_services: Arc<dyn IProviderRepository> =
            provider_repo.clone() as Arc<dyn sparkfox_be_db::IProviderRepository>;
        ...
        let factory = build_agent_factory(AgentFactoryDeps {
            ...
            client_prefs: Some(Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(
                database.pool().clone(),
            ))
                as Arc<dyn sparkfox_be_db::IClientPreferenceRepository>),
            ...
            settings_repo: Some(Arc::new(sparkfox_be_db::SqliteSettingsRepository::new(
                database.pool().clone(),
            )) as Arc<dyn sparkfox_be_db::ISettingsRepository>),
            ...
            cron_sink_factory: Some(Arc::new(|user_id: &str, conversation_id: &str| {
                sparkfox_be_cron::sink::cron_sink_for(
                    user_id.to_string(),
            ...
            knowledge_retrieval: Some(Arc::new(sparkfox_be_ai_agent::LiveKnowledgeRetrievalSink {
            ...
            knowledge_writeback: Some(Arc::new(sparkfox_be_ai_agent::LiveKnowledgeWritebackSink {
                service: knowledge_service.clone(),
            })),
            companion_prompt: Some(
                companion_service.clone() as Arc<dyn sparkfox_be_ai_agent::CompanionPromptProvider>
            ),
            public_agent_provider: Some(
                public_agent_service.clone() as Arc<dyn sparkfox_be_ai_agent::PublicAgentProvider>
            ),
        });
...
/// Ensure every provider model has an authoritative [`sparkfox_be_db::ModelProfileRow`].
