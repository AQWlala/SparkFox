...
pub use sparkfox_be_auth::AuthPolicy;
// Re-export the build channel so in-process hosts (the Tauri desktop shell)
// reach it through their existing `sparkfox-be-app` dependency without adding a
// direct `sparkfox-be-common` dep.
pub use sparkfox_be_common::channel;
pub use router::{
...
/// In-process server entry used by embedded hosts (Tauri desktop, `sparkfox-web`)
