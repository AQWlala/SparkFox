...
use sparkfox_be_app::cli::{Cli, Command};
use sparkfox_be_app::{AppServices, bootstrap, commands};
...
fn main() -> Result<ExitCode> {
    ...
    // parse a uniform CLI, but bypass `sparkfox_be_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `sparkfox_be_runtime::init`
