//! Bridge wiring the 生成引擎 (`sparkfox-be-creation`) to the 创意工坊 asset store
//! (`sparkfox-be-workshop`'s data dir + `sparkfox-be-db` index), without either domain
...
use sparkfox_be_common::{WorkshopAssetId, now_ms};
use sparkfox_be_creation::{AssetSink, AssetSource, CreationError, LoadedAsset, PersistAsset};
use sparkfox_be_db::{IWorkshopRepository, WorkshopAssetRow};
use sparkfox_be_workshop::WORKSHOP_REL_DIR;
...
mod tests {
    use super::*;
    use sparkfox_be_common::{WorkshopCanvasId, WorkshopNodeId};
    use sparkfox_be_db::{SqliteWorkshopRepository, init_database_memory};
    use serde_json::json;
    ...
    async fn bridge() -> (WorkshopAssetBridge, tempfile::TempDir, sparkfox_be_db::Database) {
