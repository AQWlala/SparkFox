...
use sparkfox_be_db::backup_bundle::{
...
async fn create_offline_backup(
    ...
    work_dir: &Path,
    output: &Path,
) -> Result<sparkfox_be_db::backup_bundle::BackupManifest> {
    ...
    let database = sparkfox_be_db::open_database_for_backup(&database_path)
        .await
...
async fn database_contains_encrypted_values(database: &sparkfox_be_db::Database) -> Result<bool> {
    const PROBES: &[&str] = &[
    ...
    for query in PROBES {
        let present: i64 = sparkfox_be_db::sqlx::query_scalar(query)
...
) -> Result<sparkfox_be_db::backup_bundle::RestoreOutcome> {
...
mod tests {
    ...
    use sparkfox_be_common::ConversationId;
    use sparkfox_be_db::backup_bundle::verify_backup_bundle;
    use std::fs;
    ...
    async fn command_roundtrip_preserves_ids_and_rotates_generation() {
        ...
        let database = sparkfox_be_db::init_database(&database_path).await.unwrap();
        let installation_owner =
            sparkfox_be_db::installation_owner_id(database.pool()).await.unwrap();
        ...
        let restored = sparkfox_be_db::init_database(&destination.join("nomifun-backend.db"))
            .await
            .unwrap();
        let restored_id: String = sparkfox_be_db::sqlx::query_scalar(
    ...
    async fn backup_refuses_a_contended_server_lock() {
        ...
        let source = root.path().join("source");
        fs::create_dir(&source).unwrap();
        let database = sparkfox_be_db::init_database(&source.join("nomifun-backend.db"))
            .await
            .unwrap();
        database.close().await;
