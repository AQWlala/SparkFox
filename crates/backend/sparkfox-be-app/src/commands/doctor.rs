...
//! `sparkfox_be_runtime::init` + `enhance_process_path` for `Doctor` as it
...
use sparkfox_be_ai_agent::{AgentRegistry, UnavailableReason};
use sparkfox_be_db::{IAgentMetadataRepository, SqliteAgentMetadataRepository, init_database};
...
fn print_environment(merged_path: &str, data_dir: &Path) {
    ...
    if let Some(p) = std::env::var_os("SPARKFOX_BUN_PATH") {
        println!("  SPARKFOX_BUN_PATH: {}", PathBuf::from(p).display());
    }
    println!();
}

fn print_snapshot(snapshot: &[(sparkfox_be_api_types::AgentMetadata, Option<UnavailableReason>)]) {
