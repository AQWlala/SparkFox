...
//! `SPARKFOX_URL` / `sparkfox_be_companion_TOKEN` env vars (the token is a
...
fn resolve_endpoint(url: Option<String>, token: Option<String>) -> Result<(String, String), String> {
    let base = url
        .or_else(|| std::env::var("SPARKFOX_URL").ok())
        .map(|u| u.trim_end_matches('/').to_owned())
        .filter(|u| !u.is_empty())
        .unwrap_or_else(|| DEFAULT_URL.to_owned());
    let token = token
        .or_else(|| std::env::var("sparkfox_be_companion_TOKEN").ok())
        .filter(|t| !t.trim().is_empty())
        .ok_or_else(|| {
            "no access token: pass --token or set sparkfox_be_companion_TOKEN (mint one in the \
...
pub async fn run_call(name: &str, args: Option<&str>, url: Option<String>, token: Option<String>) -> ExitCode {
        ...
        .await
    {
        ...
        Err(e) => {
            eprintln!("request to {base}/v1/tools/{name} failed: {e} (is NomiFun running and SPARKFOX_URL correct?)");
            ExitCode::from(1)
        }
    }
}
