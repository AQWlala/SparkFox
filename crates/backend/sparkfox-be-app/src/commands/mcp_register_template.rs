...
pub fn knowledge_register_template(nomicore_path: &str) -> RegisterTemplate {
    ...
    let claude_cmd = format!(
        "claude mcp add sparkfox-be-knowledge --scope user -- {} mcp-knowledge-stdio",
        shell_quoted
    );
    let claude_json = format!(
        "{{\n  \"mcpServers\": {{\n    \"sparkfox-be-knowledge\": {{\n      \"command\": \"{}\",\n      \"args\": [\"mcp-knowledge-stdio\"]\n    }}\n  }}\n}}",
        json_escaped
    );
    let codex_toml = format!(
        "[mcp_servers.sparkfox-be-knowledge]\ncommand = {}\nargs = [\"mcp-knowledge-stdio\"]",
        toml_escaped
    );
    RegisterTemplate {
        claude_cmd,
...
mod tests {
    ...
    fn templates_are_command_only_and_secret_free() {
        ...
        assert_eq!(
            json["mcpServers"]["sparkfox-be-knowledge"]["command"],
    ...
    fn quoting_preserves_windows_path() {
        ...
        assert_eq!(json["mcpServers"]["sparkfox-be-knowledge"]["command"], path);
