...
pub async fn run_server(env: ServerEnvironment, services: AppServices) -> Result<ExitCode> {
    ...
    // Port failover (shared with desktop LAN + sparkfox-web): bind a bounded-scan
    ...
    let idle_scanner_handle =
        sparkfox_be_ai_agent::start_idle_scanner(services.agent_runtime_registry.clone(), shutdown_rx, None, None);
