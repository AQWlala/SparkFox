...
//! registry (`sparkfox_be_gateway::Registry`) filtered by this session's permission
...
//! danger-tier × surface gate. Adding/renaming a capability in `sparkfox-be-gateway`
...
use sparkfox_be_gateway::{Registry, Surface};
use sparkfox_be_common::{LoopbackCapabilityError, LoopbackSessionKind};
...
impl GatewayStdioServer {
    ...
    /// `sparkfox_be_gateway::CallerCtx::surface`): an IM channel platform marks an
    ...
    fn visible_tool_specs(
        claims: &GatewayCapabilityClaims,
    ) -> Vec<sparkfox_be_gateway::ToolSpec> {
...
mod tests {
    use super::*;
    use sparkfox_be_api_types::GatewayCapabilityScope;
    use sparkfox_be_common::LoopbackSessionBinding;
    ...
    fn profile_filter_hides_domains_outside_allow_list() {
        ...
        assert!(!names.contains(&"sparkfox_ag_mcp_add_server"));
    }
    ...
    fn ordinary_conversation_hides_top_level_creation_but_companion_keeps_it() {
        ...
        claims.scope.companion_id = Some(
            sparkfox_be_common::CompanionId::parse(
    ...
    fn secondary_session_keeps_user_tools_and_never_sees_owner_tools() {
        let mut claims = test_claims();
        claims.user_id = sparkfox_be_common::UserId::parse(
