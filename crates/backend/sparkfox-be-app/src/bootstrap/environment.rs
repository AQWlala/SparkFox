...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment> {
    ...
    // Export the *actual* log dir so `sparkfox_be_system::sysinfo::resolve_log_dir`
    ...
    // SPARKFOX_WORK_DIR export below.
    // SAFETY: called at the very start of boot, before any service initialization
    // or env reads; the only reader of SPARKFOX_LOG_DIR is sysinfo, much later.
    unsafe {
        std::env::set_var("SPARKFOX_LOG_DIR", &log_dir);
    }
    ...
    unsafe {
        std::env::set_var("SPARKFOX_WORK_DIR", &work_dir);
    }
    ...
    let auth_policy = if cli.local {
        sparkfox_be_auth::AuthPolicy::NoAuth
    } else {
        sparkfox_be_auth::AuthPolicy::Required
    };
    ...
    // database via the normal migration path. See sparkfox_be_common::factory_reset.
    match sparkfox_be_common::factory_reset::apply_pending_reset(&config.data_dir, &config.work_dir) {
    ...
    unsafe {
        std::env::set_var("SPARKFOX_STORAGE_GENERATION", &storage_generation);
    }
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database> {
    ...
    let database = sparkfox_be_db::init_database(&db_path).await?;
