...
/// the same binary. When `SPARKFOX_BUILTIN_SKILLS_PATH` is set, skip
...
pub(super) async fn materialize_builtin_skills(data_dir: &Path) -> Result<()> {
    let skip = std::env::var(sparkfox_be_extension::BUILTIN_SKILLS_ENV_VAR)
        .map(|v| !v.is_empty())
        .unwrap_or(false);
    if skip {
        return Ok(());
    ...
    let materialize_version = sparkfox_be_extension::builtin_skills_materialize_version(env!("CARGO_PKG_VERSION"));
    sparkfox_be_extension::materialize_if_needed(
        data_dir,
        sparkfox_be_extension::builtin_skills_corpus(),
        &materialize_version,
    )
    .await
