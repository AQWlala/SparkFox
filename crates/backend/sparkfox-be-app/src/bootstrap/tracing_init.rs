...
const NOMI_TARGETS: &[&str] = &[
    ...
    "sparkfox_ag_compact",
    "sparkfox_ag_mcp",
    "sparkfox_ag_providers",
    "sparkfox_ag_protocol",
    "sparkfox_ag_tools",
    "sparkfox_ag_skills",
    "sparkfox_ag_memory",
];
...
pub fn init_tracing(log_dir: &Path, log_level: Option<&str>) -> LogGuards {
    ...
    let nomi_resolved = sparkfox_be_ai_agent::sparkfox_ag_config::logging::ResolvedLogging {
        enabled: true,
        level: nomi_level,
        dir: log_dir.to_path_buf(),
    };
    let (nomi_layer, nomi_guard) =
        sparkfox_be_ai_agent::sparkfox_ag_config::logging::create_file_layer(&nomi_resolved).expect("failed to create nomi log layer");
