...
pub async fn rewrite_path_prefixes(
    pool: &sparkfox_be_db::SqlitePool,
    old_root: &str,
    new_root: &str,
) -> Result<u64, sparkfox_be_db::sqlx::Error> {
    ...
    for (old, new) in prefix_variants(old_root, new_root) {
        ...
        for (table, column) in TEXT_COLUMN_REWRITES {
            ...
            affected += sparkfox_be_db::sqlx::query(&sql)
                .bind(&old)
                .bind(&new)
                .execute(&mut *tx)
                .await?
        ...
        affected += sparkfox_be_db::sqlx::query(
...
mod tests {
    use super::*;
    use sparkfox_be_common::{ConversationId, TerminalId};
    use sparkfox_be_db::sqlx;
    ...
    async fn insert_kb(pool: &sparkfox_be_db::SqlitePool, id: &str, root_path: &str) {
        sqlx::query(
    ...
    async fn kb_root(pool: &sparkfox_be_db::SqlitePool, id: &str) -> String {
    ...
    async fn insert_conversation(pool: &sparkfox_be_db::SqlitePool, id: &str, extra: &str) {
    ...
    async fn conversation_extra(pool: &sparkfox_be_db::SqlitePool, id: &str) -> serde_json::Value {
    ...
    async fn insert_terminal(pool: &sparkfox_be_db::SqlitePool, id: &str, cwd: &str) {
        let installation_owner = sparkfox_be_db::installation_owner_id(pool).await.unwrap();
        sqlx::query(
    ...
    async fn terminal_cwd(pool: &sparkfox_be_db::SqlitePool, id: &str) -> String {
    ...
    async fn marker_gates_and_finalizes() {
        ...
        let db = sparkfox_be_db::init_database(&data_dir.join("nomifun-backend.db")).await.unwrap();
