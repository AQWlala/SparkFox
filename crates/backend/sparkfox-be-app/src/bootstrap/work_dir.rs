...
/// `data_dir`, see [`sparkfox_be_common::dir_config`]) → `SPARKFOX_WORK_DIR` env
/// (when non-empty) → `--data-dir` fallback.
///
/// The persisted choice sits **above** the env var on purpose: `relaunch()`
/// restarts the whole process and the child can inherit the `SPARKFOX_WORK_DIR`
...
pub(crate) fn resolve_work_dir(cli_work_dir: Option<PathBuf>, data_dir: &Path) -> PathBuf {
    ...
    if let Some(persisted) = sparkfox_be_common::dir_config::persisted_work_dir(data_dir) {
        return persisted;
    }
    std::env::var("SPARKFOX_WORK_DIR")
        .ok()
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
...
mod tests {
    use super::*;
    use sparkfox_be_common::{dir_config, now_ms};

    fn temp_data_dir(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("sparkfox-wdres-{tag}-{}", now_ms()));
