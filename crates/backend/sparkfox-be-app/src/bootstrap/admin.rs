...
//!
//! The standalone web host (`sparkfox-web`) runs the backend in authenticated
...
//!   * **Pre-seed (opt-in):** when the operator supplies `SPARKFOX_ADMIN_PASSWORD`
//!     (optionally `SPARKFOX_ADMIN_USERNAME`), this function provisions the admin
...
pub async fn ensure_admin_credentials(services: &AppServices, opts: AdminBootstrap) -> Result<bool> {
    ...
    if has_users {
        if opts.password.is_some() {
            tracing::info!(
                "admin bootstrap: an admin already exists; ignoring SPARKFOX_ADMIN_PASSWORD. \
    ...
    let Some(password) = opts.password else {
        tracing::info!(
            "admin bootstrap: no SPARKFOX_ADMIN_PASSWORD set — the first WebUI visitor \
    ...
    sparkfox_be_auth::validate_username(&username)
        .map_err(|e| anyhow::anyhow!("admin bootstrap: invalid admin username '{username}': {e}"))?;
    sparkfox_be_auth::validate_password(&password)
        .map_err(|e| anyhow::anyhow!("admin bootstrap: SPARKFOX_ADMIN_PASSWORD rejected: {e}"))?;

    // bcrypt is CPU-bound — hash off the async runtime.
    let pw_for_hash = password.clone();
    let hash = tokio::task::spawn_blocking(move || sparkfox_be_auth::hash_password(&pw_for_hash))
        .await
    ...
    if provisioned {
        tracing::info!(%username, "admin bootstrap: pre-seeded admin from SPARKFOX_ADMIN_PASSWORD");
    } else {
