...
//!
//! Every host (the desktop shell's embedded backend, `sparkfox-web`, the
...
//! — the same dependency sparkfox-be-db's migrate lock uses) and is released by
                ...
                if Instant::now() >= deadline {
                    ...
                    bail!(
                        "data directory {} is already in use by another running NomiFun backend{} — \
                         ...
                         and retry, or point this one at its own directory via SPARKFOX_DATA_DIR / --data-dir",
                        data_dir.display(),
                        if holder.is_empty() {
                            String::new()
                        } else {
...
mod tests {
    ...
    fn second_lock_on_same_dir_fails_until_first_released() {
        ...
        assert!(
            msg.contains("SPARKFOX_DATA_DIR"),
