//! Bridges `sparkfox-be-channel`'s [`AssetResolver`] to the workshop asset store,
//! so channel replies can upload AI-generated images. Kept in `sparkfox-be-app`
//! (not `sparkfox-be-channel`) so the channel crate has no workshop dependency —
//! same layering as `CompanionChannelAgentProfile`.

use std::sync::Arc;

use sparkfox_be_channel::message_service::AssetResolver;
use sparkfox_be_channel::types::{MediaKind, OutgoingMedia};
use sparkfox_be_workshop::WorkshopService;
