...
/// The default data directory shared by ALL hosts (desktop shell, `sparkfox-web`,
...
/// `SPARKFOX_DATA_DIR` env / `--data-dir` flag remain the escape hatch for an
/// isolated sandbox. Concurrent use of one dir is prevented by the exclusive
/// server lock (see `bootstrap::server_lock`).
///
/// This is only the *unset* default — it does NOT consult `SPARKFOX_DATA_DIR`.
...
pub fn default_data_dir() -> PathBuf {
    dirs::data_local_dir()
        ...
        .join(nomi_leaf(&sparkfox_be_common::channel::dir_suffix()))
}
...
/// only `default_data_dir`'s unset default uses it (explicit `SPARKFOX_DATA_DIR`
/// is taken verbatim by clap, channel-agnostic).
fn nomi_leaf(suffix: &str) -> String {
    format!("Nomi{suffix}")
}

/// Reject empty `--data-dir` / `SPARKFOX_DATA_DIR` values. clap's env binding
/// takes an empty env var (a common `.env` slip) literally, which would
/// resolve the data dir to `""` — scattering a `./logs` dir into the CWD
/// before failing cryptically. `SPARKFOX_WORK_DIR` already gets the same
/// non-empty filter in `bootstrap::work_dir`.
pub fn parse_non_empty_path(s: &str) -> Result<PathBuf, String> {
    if s.trim().is_empty() {
        return Err(
            "must not be empty (unset SPARKFOX_DATA_DIR instead of setting it to an empty string)"
                .into(),
        );
    }
    Ok(PathBuf::from(s))
...
pub struct Cli {
    ...
    #[arg(long, default_value_t = String::from(sparkfox_be_common::constants::DEFAULT_HOST))]
    pub host: String,

    /// Port number to listen on.
    #[arg(long, default_value_t = sparkfox_be_common::constants::DEFAULT_PORT)]
    pub port: u16,

    /// Data directory for database and file storage.
    #[arg(long, env = "SPARKFOX_DATA_DIR", default_value_os_t = default_data_dir(), value_parser = parse_non_empty_path)]
    pub data_dir: PathBuf,

    /// Working directory for conversation workspaces.
    /// Falls back to SPARKFOX_WORK_DIR env, then to data-dir.
    #[arg(long)]
    ...
    /// Log level filter (e.g. "info", "debug", "info,sparkfox_be_mcp=trace").
    #[arg(long)]
...
pub enum Command {
    ...
    /// Endpoint/token from `--url`/`--token` or `SPARKFOX_URL` /
    /// `sparkfox_be_companion_TOKEN`.
    Call {
        /// Capability name, e.g. `nomi_cron_list` (see `nomicore tools`).
        name: String,
        /// JSON arguments object (default `{}`).
        args: Option<String>,
        /// Instance base URL (default `$SPARKFOX_URL` or http://127.0.0.1:25808).
        #[arg(long)]
        url: Option<String>,
        /// Per-companion access token (default `$sparkfox_be_companion_TOKEN`).
        #[arg(long)]
        token: Option<String>,
    },
...
mod tests {
    ...
    fn default_data_dir_is_per_user_nomifun_nomi() {
        ...
        // (`env = "SPARKFOX_DATA_DIR"`) and is not exercised here to keep the
