...
use sparkfox_be_ai_agent::{agent_routes, remote_agent_routes};
use sparkfox_be_assets::{AssetRouterState, asset_routes};
use sparkfox_be_preset::preset_routes;
...
use sparkfox_be_channel::channel_routes;
use sparkfox_be_companion::{companion_public_routes, companion_routes};
use sparkfox_be_public_agent::public_agent_routes;
use sparkfox_be_workshop::{workshop_public_routes, workshop_routes};
use sparkfox_be_creation::creation_routes;
use sparkfox_be_conversation::{conversation_ops_routes, conversation_routes};
use sparkfox_be_cron::cron_routes;
use sparkfox_be_extension::{extension_routes, hub_routes, skill_routes};
use sparkfox_be_file::file_routes;
use sparkfox_be_idmm::idmm_routes;
use sparkfox_be_knowledge::knowledge_routes;
use sparkfox_be_mcp::mcp_routes;
use sparkfox_be_office::{office_proxy_routes, office_routes};
use sparkfox_be_agent_execution::{agent_execution_routes, agent_execution_template_routes};
use sparkfox_be_realtime::{UserEventEnvelope, WebSocketManager, WsHandlerState, ws_upgrade_handler};
use sparkfox_be_requirement::requirement_routes;
use sparkfox_be_shell::shell_routes;
use sparkfox_be_system::{connection_test_routes, system_routes};
use sparkfox_be_terminal::terminal_routes;
use sparkfox_be_webhook::webhook_routes;

use sparkfox_be_secret::secret_routes;
...
async fn forward_instance_events(
    mut receiver: tokio::sync::broadcast::Receiver<sparkfox_be_api_types::WebSocketMessage<serde_json::Value>>,
...
pub async fn create_router(services: &AppServices) -> Router {
    ...
    let gateway_deps = Arc::new(sparkfox_be_gateway::GatewayDeps {
        ...
        provider_repo: Arc::new(sparkfox_be_db::SqliteProviderRepository::new(
        ...
        workshop_repo: Arc::new(sparkfox_be_db::SqliteWorkshopRepository::new(
            services.database.pool().clone(),
        )),
        ...
        client_pref_repo: Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(
            services.database.pool().clone(),
        )),
        ...
        browser_registry: Some(
            ...
            sparkfox_be_gateway::browser_registry::BrowserRegistry::default_for_browser_use()
        ...
        computer_registry: Some(sparkfox_be_gateway::computer_registry::ComputerRegistry::new()),
    });
    ...
    let _channel_watchdog = chan_mgr.spawn_watchdog(
        chan_factory,
        sparkfox_be_channel::manager::WatchdogConfig::default(),
    );
    tracing::info!(
        elapsed_ms = boot.elapsed().as_millis(),
    ...
    let router = router.nest(
        "/mcp",
        sparkfox_be_public::public_mcp_router(
            gateway_deps.clone(),
    ...
    let router = router.nest(
        ...
        sparkfox_be_public::public_mcp_router(
            gateway_deps.clone(),
            services.companion_token_validator.clone(),
            Some(sparkfox_be_public::AGENT_PROFILE_DOMAINS),
        ),
    );
    ...
    let router = router.nest(
        "/v1",
        sparkfox_be_public::public_rest_router(
            gateway_deps,
            services.companion_token_validator.clone(),
        ),
    );
...
mod realtime_bridge_tests {
    ...
    use sparkfox_be_realtime::{BroadcastEventBus, EventBroadcaster, UserEventSink, WebSocketManager, WsOutbound};
    use serde_json::json;
    use std::sync::Arc;
    use tokio::sync::mpsc;
...
pub fn create_router_with_all_state(
    ...
    let model_failover_authenticated = protect_instance_owner(
        model_failover_routes(ModelFailoverRouterState {
            client_prefs: Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(
                services.database.pool().clone(),
            )),
        }),
        &auth_mw_state,
    ...
    let browser_login_authenticated = {
        let browser_data_dir = sparkfox_ag_config::config::app_config_dir()
    ...
    let router = router.layer(DefaultBodyLimit::max(sparkfox_be_common::constants::BODY_LIMIT));
