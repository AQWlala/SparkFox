...
//!
//! Lives in sparkfox-be-app (not sparkfox-be-auth) because the mint-time model-availability
//! guard needs `CompanionService`, and sparkfox-be-companion already depends on
//! sparkfox-be-auth (so auth must not depend back on companion).
...
use sparkfox_be_api_types::ApiResponse;
use sparkfox_be_auth::require_local_trust_middleware;
use sparkfox_be_common::{AppError, CompanionId};
use sparkfox_be_db::ICompanionTokenRepository;

#[derive(Clone)]
pub struct CompanionTokenRouterState {
    pub companion_service: Arc<sparkfox_be_companion::CompanionService>,
    pub provider_repo: Arc<dyn sparkfox_be_db::IProviderRepository>,
    pub token_repo: Arc<dyn ICompanionTokenRepository>,
    pub token_validator: Arc<sparkfox_be_auth::CompanionTokenValidator>,
}
...
async fn companion_model_resolvable(
    ...
    profile: &sparkfox_be_companion::CompanionProfileConfig,
) -> bool {
...
async fn mint(
    ...
    let token = sparkfox_be_auth::generate_random_hex_secret();
    let hash = sparkfox_be_auth::token_sha256_hex(&token);
