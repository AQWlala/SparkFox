...
use sparkfox_be_ai_agent::{AgentRouterState, AgentService, RemoteAgentRouterState, RemoteAgentService};
use sparkfox_be_preset::{BuiltinPresetRegistry, PresetRouterState, PresetService};
use sparkfox_be_auth::extract_token_from_ws_headers;
use sparkfox_be_channel::ChannelRouterState;
use sparkfox_be_common::{OnConversationDelete, OnTerminalDelete};
use sparkfox_be_conversation::{ConversationRouterState, ConversationService};
use sparkfox_be_cron::{CronEventEmitter, CronRouterState};
use sparkfox_be_db::{
...
use sparkfox_be_file::{FileRouterState, FileService, FileWatchService, SnapshotService};
use sparkfox_be_idmm::{IdmmManager, IdmmRouterState};
use sparkfox_be_knowledge::KnowledgeRouterState;
use sparkfox_be_mcp::{
...
use sparkfox_be_agent_execution::{AgentExecutionEngine, AgentExecutionEngineConfig};
use sparkfox_be_companion::CompanionRouterState;
use sparkfox_be_public_agent::PublicAgentRouterState;
use sparkfox_be_workshop::WorkshopRouterState;
use sparkfox_be_creation::CreationRouterState;
use sparkfox_be_realtime::{NoopMessageRouter, WsHandlerState};
use sparkfox_be_requirement::RequirementRouterState;
use sparkfox_be_shell::ShellRouterState;
use sparkfox_be_system::{
...
use sparkfox_be_terminal::TerminalRouterState;
use sparkfox_be_webhook::WebhookRouterState;

use sparkfox_be_secret::SecretRouterState;
...
pub struct ChannelMessageLoopComponents {
    pub message_loop: sparkfox_be_channel::message_loop::ChannelMessageLoop,
    pub message_rx: tokio::sync::mpsc::Receiver<sparkfox_be_channel::types::ChannelIncoming>,
    pub confirm_rx: tokio::sync::mpsc::Receiver<(String, String)>,
    pub manager: Arc<sparkfox_be_channel::manager::ChannelManager>,
    pub plugin_factory: Arc<sparkfox_be_channel::manager::PluginFactory>,
}
...
pub async fn build_module_states(services: &AppServices) -> (ModuleStates, ChannelMessageLoopComponents) {
    ...
    sparkfox_be_cron::sink::set_process_cron_service(cron.cron_service.clone());
    tracing::info!(
        elapsed_ms = boot.elapsed().as_millis(),
...
pub fn build_system_state(services: &AppServices) -> SystemRouterState {
    ...
    let client_pref_repo: Arc<dyn sparkfox_be_db::IClientPreferenceRepository> =
    ...
    SystemRouterState {
        ...
        model_profile_service: sparkfox_be_system::ModelProfileService::new(
            services.model_profile_repo.clone(),
        ),
...
pub fn build_conversation_state(
    ...
    cron_service: Option<Arc<sparkfox_be_cron::service::CronService>>,
    ...
    let skill_resolver = Arc::new(sparkfox_be_conversation::skill_resolver::ExtensionSkillResolver::new(
    ...
    conversation_service.with_mcp_server_repo(Arc::new(sparkfox_be_db::SqliteMcpServerRepository::new(
    ...
    conversation_service.with_delete_hook(Arc::new(
        sparkfox_be_ai_agent::runtime_registry::NomiSessionFilesCascade {
            data_dir: services.data_dir.clone(),
...
pub fn build_file_state(services: &AppServices) -> FileRouterState {
    ...
    // (custom SPARKFOX_DATA_DIR on another drive).
    if !allowed_roots.iter().any(|r| r == &services.data_dir) {
        allowed_roots.push(services.data_dir.clone());
    }
    let browse_roots = sparkfox_be_file::browse::default_browse_roots();
...
pub fn build_mcp_state(services: &AppServices) -> McpRouterState {
    let pool = services.database.pool().clone();
    let repo: Arc<dyn sparkfox_be_db::IMcpServerRepository> = Arc::new(sparkfox_be_db::SqliteMcpServerRepository::new(pool));
    ...
    let oauth_token_repo: Arc<dyn sparkfox_be_db::IOAuthTokenRepository> = Arc::new(
        sparkfox_be_db::SqliteOAuthTokenRepository::new(services.database.pool().clone()),
    );
    ...
    McpRouterState {
        ...
        oauth_service: sparkfox_be_mcp::McpOAuthService::new_dynamic(oauth_token_repo),
    }
}
...
struct CompanionChannelAgentProfile {
    companion_service: Arc<sparkfox_be_companion::CompanionService>,
    channel_settings: Arc<sparkfox_be_channel::channel_settings::ChannelSettingsService>,
    public_agent_service: Arc<sparkfox_be_public_agent::PublicAgentService>,
...
impl sparkfox_be_channel::message_service::ChannelAgentProfile for CompanionChannelAgentProfile {
    async fn channel_companion_id(&self, platform: &str) -> Option<String> {
        ...
        if let Some(plugin) = sparkfox_be_channel::types::PluginType::from_str_opt(platform)
    ...
    async fn companion_model(&self, companion_id: &str) -> Option<sparkfox_be_common::ProviderWithModel> {
    ...
    async fn ensure_companion_session(&self, companion_id: &str) -> Option<String> {
        match self.companion_service.create_companion_thread(companion_id, None).await {
            Ok(thread) => match sparkfox_be_common::ConversationId::try_from(thread.conversation_id.as_str()) {
    ...
    async fn public_agent_model(&self, id: &str) -> Option<sparkfox_be_common::ProviderWithModel> {
        ...
        if let Ok(cfg) = self.public_agent_service.get(id).await {
            if cfg.model.is_configured() {
                ...
                return Some(sparkfox_be_common::ProviderWithModel {
                    provider_id,
        ...
        let (provider_id, model) = sparkfox_be_ai_agent::resolve_default_model(&self.provider_repo).await?;
        Some(sparkfox_be_common::ProviderWithModel {
            provider_id,
            model: model.clone(),
...
pub async fn build_channel_state(
    ...
    let pool = services.database.pool().clone();
    let repo: Arc<dyn sparkfox_be_db::IChannelRepository> = Arc::new(sparkfox_be_db::SqliteChannelRepository::new(pool));
    let encryption_key = services.encryption_key;
    ...
    let manager = Arc::new(sparkfox_be_channel::manager::ChannelManager::new(
        repo.clone(),
        services.event_bus.clone(),
        owner_user_id.clone(),
        encryption_key,
    ...
    let pairing_service = Arc::new(sparkfox_be_channel::pairing::PairingService::new(
        repo.clone(),
        services.event_bus.clone(),
        owner_user_id.clone(),
    ));
    ...
    let _pairing_cleanup = sparkfox_be_channel::pairing::PairingService::start_cleanup_timer(repo.clone());

    let session_manager = Arc::new(sparkfox_be_channel::session::SessionManager::new(repo.clone()));

    let plugin_factory: Arc<sparkfox_be_channel::manager::PluginFactory> =
        Arc::new(Box::new(sparkfox_be_channel::plugins::create_plugin));

    // Build channel settings service for per-plugin agent/model configuration
    let pref_pool = services.database.pool().clone();
    let pref_repo: Arc<dyn sparkfox_be_db::IClientPreferenceRepository> =
        Arc::new(SqliteClientPreferenceRepository::new(pref_pool));
    let channel_settings = Arc::new(sparkfox_be_channel::channel_settings::ChannelSettingsService::new(
        pref_repo,
    ));
    ...
    let action_executor = Arc::new(
        sparkfox_be_channel::action::ActionExecutor::new(
        ...
        .with_requirement_creator(Some(
            sparkfox_be_requirement::RequirementServiceSink::creator_arc(
                services.requirement_service.clone(),
            ),
        )),
    );

    let conv_repo: Arc<dyn sparkfox_be_db::IConversationRepository> = Arc::new(
        sparkfox_be_db::SqliteConversationRepository::new(services.database.pool().clone()),
    );
    let skill_resolver = Arc::new(sparkfox_be_conversation::skill_resolver::ExtensionSkillResolver::new(
        services.skill_paths.clone(),
    ));
    let agent_metadata_repo: Arc<dyn sparkfox_be_db::IAgentMetadataRepository> = Arc::new(
        sparkfox_be_db::SqliteAgentMetadataRepository::new(services.database.pool().clone()),
    );
    let acp_session_repo: Arc<dyn sparkfox_be_db::IAcpSessionRepository> = Arc::new(
        sparkfox_be_db::SqliteAcpSessionRepository::new(services.database.pool().clone()),
    );
    ...
    conversation_svc.with_mcp_server_repo(Arc::new(sparkfox_be_db::SqliteMcpServerRepository::new(
        services.database.pool().clone(),
    )));
    ...
    let channel_agent_profile: Arc<dyn sparkfox_be_channel::message_service::ChannelAgentProfile> =
    ...
    let message_service = Arc::new(
        sparkfox_be_channel::message_service::ChannelMessageService::new(
            conversation_svc,
            services.agent_runtime_registry.clone(),
            Arc::clone(&channel_settings),
            repo.clone(),
    ...
    let message_loop = sparkfox_be_channel::message_loop::ChannelMessageLoop::new(
        action_executor,
        message_service,
        Arc::clone(&session_manager),
        manager.clone() as Arc<dyn sparkfox_be_channel::stream_relay::ChannelSender>,
    );
...
pub fn build_requirement_state(services: &AppServices) -> (RequirementRouterState, IdmmRouterState) {
    ...
    let conv_repo: Arc<dyn sparkfox_be_db::IConversationRepository> =
    ...
    let terminal_driver: Arc<dyn sparkfox_be_terminal::TerminalDriver> = services.terminal_service.clone();
    let terminal_repo: Arc<dyn sparkfox_be_db::ITerminalRepository> =
        Arc::new(sparkfox_be_db::SqliteTerminalRepository::new(pool.clone()));
    ...
    let idmm_handle: Arc<dyn sparkfox_be_requirement::IdmmHandle> = Arc::new(idmm_state.service.manager().clone());
    ...
    let deps = Arc::new(sparkfox_be_requirement::AutoWorkRunnerDeps {
    ...
    let auto_work_runner = Arc::new(sparkfox_be_requirement::AutoWorkRunner::new(deps));
...
pub fn build_webhook_state(services: &AppServices) -> WebhookRouterState {
    let pool = services.database.pool().clone();
    let webhook_repo: Arc<dyn sparkfox_be_db::IWebhookRepository> =
        Arc::new(sparkfox_be_db::SqliteWebhookRepository::new(pool.clone()));
    let tag_setting_repo: Arc<dyn sparkfox_be_db::ITagSettingRepository> =
        Arc::new(sparkfox_be_db::SqliteTagSettingRepository::new(pool));
    let sender: Arc<dyn sparkfox_be_webhook::WebhookSender> = Arc::new(sparkfox_be_webhook::DefaultWebhookSender::new());
    let service = sparkfox_be_webhook::WebhookService::new(webhook_repo, tag_setting_repo, sender);
...
pub fn build_agent_execution_engine(
    ...
    preset_service: Arc<sparkfox_be_preset::PresetService>,
...
pub fn build_secret_state(services: &AppServices) -> SecretRouterState {
    let encryption_key = services.encryption_key;
    let service = sparkfox_be_secret::SecretService::new(services.data_dir.clone(), encryption_key);
...
pub fn build_idmm_state(
    ...
    conv_repo: Arc<dyn sparkfox_be_db::IConversationRepository>,
    terminal_driver: Arc<dyn sparkfox_be_terminal::TerminalDriver>,
) -> IdmmRouterState {
    let pool = services.database.pool().clone();
    let provider_repo: Arc<dyn IProviderRepository> = Arc::new(SqliteProviderRepository::new(pool.clone()));
    let client_prefs: Arc<dyn sparkfox_be_db::IClientPreferenceRepository> =
        Arc::new(SqliteClientPreferenceRepository::new(pool.clone()));
    ...
    let completer: Arc<dyn sparkfox_be_idmm::Completer> = Arc::new(sparkfox_be_idmm::LiveCompleter {
        provider_repo,
        encryption_key,
        workspace: services.data_dir.clone(),
    });
    let sidecar = Arc::new(sparkfox_be_idmm::SidecarClient::new(completer, client_prefs.clone()));

    let probe_deps = Arc::new(sparkfox_be_idmm::ProbeDeps {
        conversation_service: conv_service,
        conversation_repo: conv_repo,
        terminal_driver,
        runtime_registry: services.agent_runtime_registry.clone(),
    ...
    let loop_deps = Arc::new(sparkfox_be_idmm::LoopDeps {
        sidecar: sidecar.clone(),
        emitter: sparkfox_be_idmm::IdmmEventEmitter::new(services.event_bus.clone()),
        records: records.clone(),
    });
    let manager = IdmmManager::new(loop_deps, probe_deps.clone(), probe_deps.clone());
    let service = Arc::new(sparkfox_be_idmm::IdmmService::new(
        probe_deps,
        client_prefs,
        sidecar,
        manager,
...
fn spawn_idmm_record_janitor(records: Arc<dyn IIdmmInterventionRepository>) {
    tokio::spawn(async move {
        ...
        loop {
            ticker.tick().await;
            let cutoff = sparkfox_be_common::now_ms() - sparkfox_be_db::TTL_MS;
            match records
                .sweep_all_owners(cutoff, sparkfox_be_db::PER_USER_ACTIVITY_CAP)
                .await
            {
...
pub fn build_companion_state(
    services: &AppServices,
    channel_manager: Arc<sparkfox_be_channel::manager::ChannelManager>,
    preset_service: Arc<sparkfox_be_preset::PresetService>,
    ...
    .with_runtime_state(services.conversation_runtime_state.clone());
    conv_service.with_mcp_server_repo(Arc::new(sparkfox_be_db::SqliteMcpServerRepository::new(
        services.database.pool().clone(),
    )));
...
struct CompanionKnowledgeCleanup {
    knowledge: Arc<sparkfox_be_knowledge::KnowledgeService>,
}

#[async_trait::async_trait]
impl sparkfox_be_companion::service::CompanionCleanupHook for CompanionKnowledgeCleanup {
...
/// gone). Lives in `sparkfox-be-app` so `sparkfox-be-conversation` / `sparkfox-be-terminal`
...
struct CompanionChannelModelSync {
    manager: Arc<sparkfox_be_channel::manager::ChannelManager>,
}

#[async_trait::async_trait]
impl sparkfox_be_companion::service::CompanionCleanupHook for CompanionChannelModelSync {
    async fn on_companion_deleted(&self, companion_id: &str) {
...
pub fn build_cron_state(
    ...
    let pool = services.database.pool().clone();
    let cron_repo: Arc<dyn sparkfox_be_db::ICronRepository> = Arc::new(sparkfox_be_db::SqliteCronRepository::new(pool.clone()));
    ...
    let busy_guard = Arc::new(sparkfox_be_cron::busy_guard::CronBusyGuard::new());
    let executor = Arc::new(sparkfox_be_cron::executor::JobExecutor::new(
        services.authoritative_user_id.clone(),
        services.agent_runtime_registry.clone(),
        conv_repo,
    ...
    let scheduler = Arc::new(sparkfox_be_cron::scheduler::CronScheduler::new(Arc::new(
    ...
    let cron_service = Arc::new(sparkfox_be_cron::service::CronService::new(
        services.authoritative_user_id.clone(),
        cron_repo,
        scheduler,
        executor,
...
pub fn build_office_state(services: &AppServices) -> OfficeRouterState {
    ...
    let spawner: Arc<dyn sparkfox_be_office::ProcessSpawner> = Arc::new(sparkfox_be_office::DefaultProcessSpawner);
...
pub fn build_shell_state(services: &AppServices) -> ShellRouterState {
    ...
    ShellRouterState {
        shell_service: Arc::new(sparkfox_be_shell::ShellService::new(Arc::new(
            sparkfox_be_shell::DefaultSystemOpener,
        ))),
        stt_service: Arc::new(sparkfox_be_shell::SttService::new_dynamic()),
        client_pref_service,
...
#[derive(Default)]
struct CronServiceTickRef(std::sync::Mutex<Option<Arc<sparkfox_be_cron::service::CronService>>>);
...
pub async fn build_extension_states(
    ...
    let skill_paths = sparkfox_be_extension::resolve_skill_paths(&app_resource_dir, &skill_data_dir);

    let ext_paths_mgr = Arc::new(ExternalPathsManager::new(&skill_data_dir).await);

    let skill_tag_repo: Arc<dyn sparkfox_be_db::ISkillTagRepository> =
        Arc::new(sparkfox_be_db::SqliteSkillTagRepository::new(services.database.pool().clone()));
    let builtin_skill_tags = Arc::new(sparkfox_be_extension::skill_service::load_builtin_skill_tags());
...
mod tests {
    ...
    use crate::AppConfig;
    use sparkfox_be_extension::{ExtensionSource, ScanPath};
    ...
    fn every_production_conversation_service_uses_shared_private_event_and_execution_boundaries() {
        ...
        let cron_executor_start = production_source
            .find("sparkfox_be_cron::executor::JobExecutor::new(")
        ...
        let cron_service_start = production_source
            .find("sparkfox_be_cron::service::CronService::new(")
