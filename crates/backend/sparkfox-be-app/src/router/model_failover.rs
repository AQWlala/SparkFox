...
//! `agent.model_failover`; the storage helpers live in `sparkfox_be_conversation`
//! (`get_global_failover_config` / `set_global_failover_config`). This module is
//! a thin authenticated router around them — `sparkfox-be-app` is the only crate that
//! depends on both `sparkfox-be-conversation` and the SQLite client-pref repo, so the
//! route is registered here rather than in the agent-listing router (which lives
//! in `sparkfox-be-ai-agent`, below conversation in the dependency graph).
...
use sparkfox_be_api_types::{ApiResponse, ModelFailoverConfig};
use sparkfox_be_auth::CurrentUser;
use sparkfox_be_common::AppError;
use sparkfox_be_conversation::model_failover::{get_global_failover_config, set_global_failover_config};
