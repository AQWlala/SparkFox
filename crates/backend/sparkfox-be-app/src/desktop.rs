...
use sparkfox_be_auth::generate_random_hex_secret;
...
use sparkfox_be_auth::AuthPolicy;
...
pub struct DesktopServer {
    ...
    terminal_service: Arc<sparkfox_be_terminal::TerminalService>,
...
impl DesktopServer {
    ...
    pub async fn start_lan(self: &Arc<Self>) -> WebUiStatus {
        ...
        if !has_password {
            let pw = sparkfox_be_auth::generate_password(20);
            let pw_for_hash = pw.clone();
            let hashed = match tokio::task::spawn_blocking(move || {
                sparkfox_be_auth::hash_password(&pw_for_hash)
            })
            .await
            {
                Ok(Ok(h)) => h,
...
/// so the desktop LAN listener, `sparkfox-web`, and the `nomicore` bin fail over
/// identically.
