...
pub struct ActionExecutor {
    ...
    requirement_creator: Option<Arc<dyn sparkfox_be_common::RequirementCreator>>,
}
...
impl ActionExecutor {
    ...
    pub fn with_requirement_creator(
        mut self,
        creator: Option<Arc<dyn sparkfox_be_common::RequirementCreator>>,
    ) -> Self {
    ...
    async fn resolve_action_session(
        ...
        internal_user_id: &str,
        channel_id: &str,
    ) -> Result<sparkfox_be_db::models::ChannelSessionRow, ChannelError> {
        let agent_config = self.settings.get_agent_config(action.context.platform).await?;
        self.session_mgr
...
mod action_tests {
    ...
    use sparkfox_be_db::{DbError, IChannelRepository, IClientPreferenceRepository, UpdatePluginStatusParams};
    ...
    async fn chat_continue_carries_existing_conversation_binding() {
        let (executor, repo) = setup();
        repo.add_authorized_user("tg_42", "telegram");
        let bound_conversation_id = sparkfox_be_common::ConversationId::new();
