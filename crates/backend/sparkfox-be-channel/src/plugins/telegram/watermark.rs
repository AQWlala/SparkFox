...
impl FileWatermarkStore {
    ...
    /// `sparkfox_be_runtime::init(&data_dir)` before the channel manager starts,
    ...
    pub fn for_app_data() -> Option<Self> {
        let runtime_root = sparkfox_be_runtime::runtime_root()?;
...
impl WatermarkStore for FileWatermarkStore {
    ...
    fn save(&self, bot_id: &str, update_id: i64) {
        ...
        // the dedup window. Same pattern as sparkfox-be-companion's
