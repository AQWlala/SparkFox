...
use sparkfox_be_ai_agent::{AgentStreamEvent, AgentRuntimeRegistry};
...
use sparkfox_be_common::{AgentType, ConversationSource, MessagePosition, MessageType};
...
/// Implemented in `sparkfox-be-app` over `CompanionService` + `ChannelSettingsService`
...
///
/// Defined here (not in `sparkfox-be-workshop`) so `sparkfox-be-channel` stays free of
/// a workshop dependency; the concrete impl lives in `sparkfox-be-app`
    ...
    async fn dispatch_to_conversation(
        ...
        self.conversation_svc
            ...
            .map_err(|e| match e {
                ...
                sparkfox_be_common::AppError::Conflict(_) => ChannelError::ConversationBusy,
    ...
    pub fn process_stream_event(event: &AgentStreamEvent) -> Option<StreamAction> {
        match event {
            ...
            AgentStreamEvent::ToolCall(data) => {
                ...
                if matches!(
                    data.status,
                    sparkfox_be_ai_agent::protocol::events::ToolCallStatus::Completed
            ...
            AgentStreamEvent::AcpPermission(data) => match data {
                sparkfox_be_ai_agent::protocol::events::AcpPermissionEventData::Request(req) => {
                ...
                sparkfox_be_ai_agent::protocol::events::AcpPermissionEventData::Confirmation(conf) => {
                    confirmation_to_decision(conf)
                }
            },
            AgentStreamEvent::Permission(value) => serde_json::from_value::<sparkfox_be_common::Confirmation>(value.clone())
                .ok()
...
/// Maps a `sparkfox_be_common::Confirmation` to a `Decision` action.
///
...
fn confirmation_to_decision(conf: &sparkfox_be_common::Confirmation) -> Option<StreamAction> {
...
mod tests {
    ...
    fn make_history_message(
        ...
        MessageResponse {
            id: id.into(),
            conversation_id: sparkfox_be_common::ConversationId::new().into_string(),
            msg_id: Some(id.into()),
            r#type: msg_type,
            content,
            position,
