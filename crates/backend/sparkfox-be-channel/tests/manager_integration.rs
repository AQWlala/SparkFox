...
use sparkfox_be_channel::plugin::{ChannelPlugin, PluginCallbacks};
...
use sparkfox_be_common::decrypt_string;
...
async fn send_message_not_running_fails() {
    let (mgr, _repo, _bc) = setup().await;
    let channel_id = sparkfox_be_common::ChannelId::new().into_string();
