...
use sparkfox_be_ai_agent::runtime_handle::{AgentRuntimeHandle, AgentRuntimeControl};
...
use sparkfox_be_ai_agent::{AgentSendError, AgentStreamEvent, MockAgentRuntime, AgentRuntimeRegistry};
use sparkfox_be_api_types::WebSocketMessage;
use sparkfox_be_channel::channel_settings::ChannelSettingsService;
use sparkfox_be_channel::message_service::ChannelMessageService;
use sparkfox_be_channel::types::PluginType;
use sparkfox_be_common::{AgentKillReason, AgentType, AppError, ConversationStatus, TimestampMs};
use sparkfox_be_conversation::ConversationService;
use sparkfox_be_conversation::skill_resolver::{ResolvedAgentSkill, SkillResolver};
use sparkfox_be_db::models::ChannelSessionRow;
use sparkfox_be_db::{
...
async fn seed_channel_models(pool: &sparkfox_be_db::SqlitePool) {
...
struct TestStack {
    ...
    runtime: Arc<sparkfox_be_conversation::runtime_state::ConversationRuntimeStateService>,
    channel_repo: Arc<SqliteChannelRepository>,
    installation_owner: String,
}

async fn build_stack(pool: sparkfox_be_db::SqlitePool) -> TestStack {
    let installation_owner = sparkfox_be_db::installation_owner_id(&pool).await.unwrap();
    seed_channel_models(&pool).await;
    let runtime_registry: Arc<dyn AgentRuntimeRegistry> = Arc::new(RecordingAgentRuntimeRegistry::new());
    let runtime = Arc::new(sparkfox_be_conversation::runtime_state::ConversationRuntimeStateService::default());
...
async fn wait_until_idle(svc: &Arc<ConversationService>, conversation_id: &str) {
    use sparkfox_be_api_types::ConversationRuntimeStateKind;
    for _ in 0..500 {
...
async fn last_user_text_none_for_unknown_conversation() {
    ...
    let missing = sparkfox_be_common::ConversationId::new();
...
#[async_trait]
impl sparkfox_be_channel::message_service::ChannelAgentProfile for StubProfile {
    async fn companion_model(&self, _companion_id: &str) -> Option<sparkfox_be_common::ProviderWithModel> {
        None
    }
...
async fn seed_companion_session(
    ...
    let req = sparkfox_be_api_types::CreateConversationRequest {
        r#type: AgentType::Nomi,
        name: Some(format!("和 {companion_id} 聊天")),
        model: Some(sparkfox_be_common::ProviderWithModel {
...
async fn bind_channel_to_companion(repo: &Arc<SqliteChannelRepository>, channel_id: &str, companion_id: &str) {
    use sparkfox_be_db::IChannelRepository;
    let now = sparkfox_be_common::now_ms();
    repo.upsert_plugin(&sparkfox_be_db::models::ChannelPluginRow {
...
async fn companion_without_model_refuses_turn() {
    use sparkfox_be_channel::error::ChannelError;
...
struct PublicStubProfile {
    servable: bool,
    model: Option<sparkfox_be_common::ProviderWithModel>,
}

#[async_trait]
impl sparkfox_be_channel::message_service::ChannelAgentProfile for PublicStubProfile {
    async fn companion_model(&self, _companion_id: &str) -> Option<sparkfox_be_common::ProviderWithModel> {
        None
    }
    async fn channel_companion_id(&self, _platform: &str) -> Option<String> {
    ...
    async fn public_agent_model(&self, _id: &str) -> Option<sparkfox_be_common::ProviderWithModel> {
        self.model.clone()
    }
}
...
async fn public_agent_bound_platform_builds_clamped_session() {
    ...
    let model = sparkfox_be_common::ProviderWithModel {
