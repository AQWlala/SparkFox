...
use sparkfox_be_api_types::{ConversationRuntimeStateKind, ListMessagesQuery, WebSocketMessage};
use sparkfox_be_channel::action::{ActionExecutor, MessageResult};
...
use sparkfox_be_channel::message_loop::ChannelMessageLoop;
use sparkfox_be_channel::pairing::PairingService;
use sparkfox_be_channel::session::SessionManager;
use sparkfox_be_channel::stream_relay::{ChannelSender, MessageRecorder};
use sparkfox_be_channel::types::{
...
use sparkfox_be_db::models::{ChannelUserRow, ChannelPluginRow};
use sparkfox_be_db::{
...
async fn unauthorized_user_gets_pairing_response() {
    ...
    let repo: Arc<dyn sparkfox_be_db::IChannelRepository> =
        Arc::new(sparkfox_be_db::SqliteChannelRepository::new(pool.clone()));
    let bus = Arc::new(sparkfox_be_realtime::BroadcastEventBus::new(64));

    let pref_repo: Arc<dyn sparkfox_be_db::IClientPreferenceRepository> =
        Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(pool));
...
struct Harness {
    ...
    pending_decisions: Arc<sparkfox_be_channel::pending_decision::PendingDecisionStore>,
}
...
async fn chat_continue_sends_continue_prompt_to_agent() {
    ...
    assert_eq!(messages, vec![
        ...
        sparkfox_be_channel::action::CONTINUE_PROMPT.to_string()
    ]);
}
...
use sparkfox_be_channel::pending_decision::PendingDecision;
use sparkfox_be_channel::types::DecisionOption;
