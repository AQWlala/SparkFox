...
mod weixin_tests {
    ...
    use sparkfox_be_channel::manager::{ChannelManager, EnableChannelSpec, PluginFactory};
    use sparkfox_be_channel::plugin::ChannelPlugin;
    use sparkfox_be_channel::plugins::weixin::WeixinPlugin;
    use sparkfox_be_channel::types::{PluginConfig, PluginCredentials, PluginStatus, PluginType};
    ...
    async fn disable_without_db_row_returns_error() {
        let (manager, _repo, _bc) = setup().await;
        let missing_channel = sparkfox_be_common::ChannelId::new();
    ...
    async fn is_plugin_running_false_when_not_enabled() {
        let (manager, _repo, _bc) = setup().await;
        assert!(!manager.is_plugin_running(sparkfox_be_common::ChannelId::new().as_str()));
    }
    ...
    fn login_event_qr_serializes_correctly() {
        use sparkfox_be_channel::plugins::weixin::weixin_login_stream;
