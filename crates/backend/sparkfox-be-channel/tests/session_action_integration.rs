...
use sparkfox_be_common::{ChannelUserId, now_ms};
use sparkfox_be_db::models::{ChannelUserRow, ChannelPluginRow};
use sparkfox_be_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn sparkfox_be_db::IClientPreferenceRepository> =
        Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
// sparkfox-be-db sqlite_channel.rs::update_session_conversation_persists.
