...
use sparkfox_be_common::{AppError, ProviderId, decrypt_string};
...
mod tests {
    use super::*;
    use sparkfox_be_common::encrypt_string;
    use sparkfox_be_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];

    async fn setup() -> (ModelFetchService, sparkfox_be_db::Database) {
    ...
    async fn fetch_models_minimax_returns_hardcoded() {
        ...
        assert!(
            resp.models
                .contains(&sparkfox_be_api_types::ModelInfo::Id("MiniMax-M3".into()))
        );
        assert!(
            resp.models
                .contains(&sparkfox_be_api_types::ModelInfo::Id("MiniMax-Text-01".into()))
        );
    }
