...
use sparkfox_be_api_types::{GitHubReleaseAsset, UpdateCheckRequest, UpdateCheckResult, UpdateReleaseInfo};
use sparkfox_be_common::AppError;
use serde::Deserialize;

const DEFAULT_REPO: &str = "nomifun/sparkfox-be-app";
...
fn resolve_repo(from_request: Option<&str>) -> String {
    ...
    if let Ok(v) = std::env::var("SPARKFOX_GITHUB_REPO")
        && !v.is_empty()
    {
        return v;
    }
