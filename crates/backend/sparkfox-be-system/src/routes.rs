...
pub struct SystemRouterState {
    ...
    /// the next boot consumes). See `sparkfox_be_common::factory_reset`.
...
/// [`sparkfox_be_api_types::resolve_models`] authority.
async fn resolve_model_profiles(
    ...
    let providers = state.provider_service.list().await?;
    let profiles = state.model_profile_service.list().await?;
    let models = sparkfox_be_api_types::resolve_models(&providers, &profiles, req.task, &req.required_traits);
...
/// `sparkfox_be_common::factory_reset`); the client should restart the app right
...
async fn factory_reset(State(state): State<SystemRouterState>) -> Result<Json<ApiResponse<()>>, AppError> {
    let marker = sparkfox_be_common::factory_reset::ResetMarker::new(sparkfox_be_common::factory_reset::ResetScope::Full);
    sparkfox_be_common::factory_reset::write_marker(&state.data_dir, &marker)?;
...
/// by `bootstrap::work_dir::resolve_work_dir` (see `sparkfox_be_common::dir_config`).
...
async fn set_work_dir(
    ...
    if sparkfox_be_common::workspace_path_has_edge_whitespace_segment(&path) {
    ...
    sparkfox_be_common::dir_config::set_work_dir(&state.data_dir, &path)?;
    tracing::info!(
        target: "system",
