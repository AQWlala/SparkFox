use sparkfox_be_api_types::SystemInfoResponse;
...
///
/// Priority: `SPARKFOX_CACHE_DIR` env → `dirs::cache_dir()/nomifun`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("SPARKFOX_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
///
/// Priority: `SPARKFOX_WORK_DIR` env → `dirs::data_dir()/nomifun`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("SPARKFOX_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("SPARKFOX_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
fn resolve_storage_generation() -> String {
    std::env::var("SPARKFOX_STORAGE_GENERATION")
