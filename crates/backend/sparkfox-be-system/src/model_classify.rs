...
//! delegating to the shared engine [`sparkfox_be_api_types::infer_generation_capabilities`].
//!
//! The result is a **suggested default**: the user may override it. This module
//! is the backend hookpoint reused by the future `sparkfox-be-creation` engine and
//! any endpoint that wants to annotate a fetched model list; it never mutates
//! provider rows.
//!
//! [`ModelFetchService`]: crate::ModelFetchService

use sparkfox_be_api_types::{ModelInfo, ModelType, infer_generation_capabilities};
