...
fn unique_data_dir(tag: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join(format!("sparkfox-wdroute-{tag}-{}", sparkfox_be_common::now_ms()));
...
async fn setup(data_dir: std::path::PathBuf) -> axum::Router {
    ...
    let state = SystemRouterState {
        ...
        model_profile_service: sparkfox_be_system::ModelProfileService::new(std::sync::Arc::new(
            sparkfox_be_db::SqliteModelProfileRepository::new(db.pool().clone()),
        )),
...
async fn persists_valid_work_dir_and_creates_it() {
    ...
    assert_eq!(
        sparkfox_be_common::dir_config::persisted_work_dir(&data_dir).as_deref(),
        Some(target.as_path())
    );
...
async fn rejects_relative_work_dir() {
    ...
    assert!(sparkfox_be_common::dir_config::persisted_work_dir(&data_dir).is_none());
