...
//! layered externally in sparkfox-be-app (mirrors the knowledge / webhook routes).
//!
...
use sparkfox_be_api_types::{ApiResponse, RegisterSecretRequest, SecretListItem};
...
mod tests {
    ...
    fn router_with_user(dir: &std::path::Path) -> Router {
        ...
        secret_routes(SecretRouterState::new(svc))
            ...
            // sparkfox-be-app; tests attach the extension so the Extension extractor resolves).
