//! `sparkfox-be-secret` — credential vault for the native browser-use engine.
//!
//! Provides a [`SecretStore`] that:
//!
//! * **encrypts** each registered value at rest with AES-256-GCM, reusing the
//!   shared implementation in [`sparkfox_be_common`] (`encrypt_string` /
...
// sparkfox-be-app). Gated so pure-logic consumers don't pull axum / auth / api-types.
#[cfg(feature = "web")]
mod routes;
#[cfg(feature = "web")]
pub mod service;
...
impl SecretStore {
    ...
    pub fn register(&mut self, name: &str, value: &str, allowed_origins: Vec<String>) -> Result<(), SecretError> {
        ...
        let ciphertext =
            sparkfox_be_common::encrypt_string(value, &self.key).map_err(|e| SecretError::Crypto(e.to_string()))?;
    ...
    pub fn resolve(&self, name: &str, current_origin: &str) -> Option<SecretValue> {
        ...
        match sparkfox_be_common::decrypt_string(&record.ciphertext, &self.key) {
...
mod tests {
    ...
    // ---- nonce randomness (inherited from sparkfox-be-common, asserted here) ----
