...
use sparkfox_be_ai_agent::{AcpPermissionEventData, AcpPermissionOptionKind, AcpToolCallKind, AgentStreamEvent, TurnStopReason};
use sparkfox_be_ai_agent::runtime_registry::AgentRuntimeRegistry;
use sparkfox_be_api_types::{ConfirmRequest, IdmmTargetKind, SendMessageRequest};
...
use sparkfox_be_db::{IConversationRepository, SortOrder};
use sparkfox_be_terminal::TerminalDriver;
use tokio::sync::mpsc;
...
fn pending_signal_from_page(extra: &str, messages: &[sparkfox_be_db::models::MessageRow]) -> Option<(SessionSignal, i64)> {
...
impl SessionProbe for ConversationProbe {
    ...
    fn observe(&self, idle_threshold: Duration) -> mpsc::Receiver<SessionSignal> {
        ...
        tokio::spawn(async move {
            ...
            let mut work_epoch_ms = sparkfox_be_common::now_ms();
                        ...
                        Ok(ev) => {
                            ...
                            if let Some(sig) = sig {
                                if matches!(sig, SessionSignal::Working) {
                                    saw_activity = true;
                                    work_epoch_ms = sparkfox_be_common::now_ms();
                                }
    ...
    async fn fallback_model(&self) -> Option<(String, String)> {
        let row = self
            ...
            .await
            .ok()??;
        let pm = sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row(&row)
            .ok()??;
...
fn map_lifecycle_event(kind: sparkfox_be_terminal::LifecycleKind) -> Option<SessionSignal> {
    use sparkfox_be_terminal::LifecycleKind;
    match kind {
...
impl SessionProbe for TerminalProbe {
                ...
                tokio::select! {
                    ...
                    lifecycle_ev = async {
                        match lifecycle_rx.as_mut() {
                            ...
                            None => std::future::pending::<Result<sparkfox_be_terminal::TerminalLifecycleEvent, tokio::sync::broadcast::error::RecvError>>().await,
                        }
                    } => {
                        match lifecycle_ev {
                            Ok(ev) => {
                                ...
                                let sig = match ev.kind {
                                    sparkfox_be_terminal::LifecycleKind::TurnEnd => Some(terminal_turn_end_signal(&detector)),
    ...
    async fn inject(&self, action: &WakeAction) -> Result<(), AppError> {
        ...
        let is_agent = self
            ...
            .map(|b| sparkfox_be_terminal::enhance::resolve_agent_family(&b, &[], Some(&b)).is_some())
            .unwrap_or(false);
        // Track the payload's lines so the CLI's echo of them isn't re-detected.
        self.note_injection(&text);
        match sparkfox_be_terminal::encode_submit_chunks(&text, is_agent) {
            sparkfox_be_terminal::SubmitChunks::Single(bytes) => self
                .driver
                .write_input(self.terminal_id.as_str(), &bytes)
                .await
                .map_err(|e| AppError::Internal(format!("terminal inject failed: {e}"))),
            sparkfox_be_terminal::SubmitChunks::PasteThenCr { paste, cr } => {
                self.driver
                    .write_input(self.terminal_id.as_str(), &paste)
                    .await
                    .map_err(|e| AppError::Internal(format!("terminal inject failed: {e}")))?;
                tokio::time::sleep(sparkfox_be_terminal::TERMINAL_SUBMIT_DELAY).await;
                self.driver
...
mod tests {
    use super::*;
    use sparkfox_be_api_types::{AgentErrorCode, AgentErrorOwnership, AgentStreamErrorData};
    use sparkfox_be_common::MessageId;
    ...
    fn map_agent_event_finish_cancelled_is_user_cancel_not_done() {
        ...
        let ev = AgentStreamEvent::Finish(sparkfox_be_ai_agent::FinishEventData {
            session_id: None,
    ...
    fn confirmation(command_type: &str) -> Confirmation {
        use sparkfox_be_common::ConfirmationOption;
        Confirmation {
            id: "c1".into(),
            call_id: "call-1".into(),
    ...
    fn idmm_single_line_stays_raw_plus_cr() {
        use sparkfox_be_terminal::{encode_submit_chunks, SubmitChunks};
    ...
    use sparkfox_be_terminal::{TerminalLifecycleEvent, LifecycleKind};
    use sparkfox_be_terminal::TerminalDriver as TerminalDriverTrait;
    use sparkfox_be_terminal::error::TerminalError as TermError;
    ...
    impl TerminalDriverTrait for FakeDriver {
        ...
        async fn describe(&self, _id: &str) -> Result<Option<sparkfox_be_terminal::TerminalDescription>, TermError> {
            unimplemented!()
        }
    ...
    impl TerminalDriverTrait for CapturingDriver {
        ...
        async fn describe(&self, _id: &str) -> Result<Option<sparkfox_be_terminal::TerminalDescription>, TermError> {
            Ok(Some(sparkfox_be_terminal::TerminalDescription {
    ...
    fn msg_row(position: &str, hidden: bool, r#type: &str, text: &str) -> sparkfox_be_db::models::MessageRow {
    ...
    fn msg_row_status(
        ...
        text: &str,
        status: Option<&str>,
    ) -> sparkfox_be_db::models::MessageRow {
    ...
    struct StubConvRepo {
        row: Option<sparkfox_be_db::models::ConversationRow>,
        messages: Vec<sparkfox_be_db::models::MessageRow>,
    }

    #[async_trait]
    impl IConversationRepository for StubConvRepo {
        async fn get(&self, _id: &str) -> Result<Option<sparkfox_be_db::models::ConversationRow>, sparkfox_be_db::DbError> {
            Ok(self.row.clone())
        }
        async fn create(&self, _row: &sparkfox_be_db::models::ConversationRow) -> Result<String, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn update(
            &self,
            _id: &str,
            _updates: &sparkfox_be_db::ConversationRowUpdate,
        ...
        ) -> Result<sparkfox_be_common::PaginatedResult<sparkfox_be_db::models::ConversationRow>, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        ...
        async fn get_messages(
            ...
            _page_size: u32,
            _order: SortOrder,
        ) -> Result<sparkfox_be_common::PaginatedResult<sparkfox_be_db::models::MessageRow>, sparkfox_be_db::DbError> {
        ...
        async fn insert_message(&self, _message: &sparkfox_be_db::models::MessageRow) -> Result<(), sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn update_message(
            &self,
            _id: &str,
            _updates: &sparkfox_be_db::MessageRowUpdate,
        ) -> Result<(), sparkfox_be_db::DbError> {
            unimplemented!()
        }
        async fn delete_messages_by_conversation(&self, _conv_id: &str) -> Result<(), sparkfox_be_db::DbError> {
            unimplemented!()
        }
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<sparkfox_be_common::PaginatedResult<sparkfox_be_db::MessageSearchRow>, sparkfox_be_db::DbError> {
            unimplemented!()
        }
    }

    fn conv_row(extra: &str) -> sparkfox_be_db::models::ConversationRow {
