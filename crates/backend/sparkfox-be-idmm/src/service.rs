...
use sparkfox_be_api_types::{IdmmConfig, IdmmSettings, IdmmState, IdmmTargetKind, InterventionRecord};
use sparkfox_be_common::{AppError, ConversationId, IdmmInterventionId, TerminalId, UserId};
use sparkfox_be_conversation::ConversationService;
use sparkfox_be_db::models::IdmmInterventionRow;
use sparkfox_be_db::{IClientPreferenceRepository, IConversationRepository, IIdmmInterventionRepository};
...
impl IdmmService {
    ...
    async fn sidecar_backup_resolvable(&self, kind: IdmmTargetKind, target_id: &str, cfg: &IdmmConfig) -> bool {
        ...
        {
            return sparkfox_be_conversation::runtime_options::provider_model_from_conversation_row(&row)
        ...
        for r in rows {
            match r.key.as_str() {
                PREF_BACKUP_PROVIDER => {
                    sparkfox_be_common::ProviderId::parse(&r.value).map_err(|error| {
    ...
    pub async fn set_settings(&self, settings: &IdmmSettings) -> Result<(), AppError> {
        ...
        let provider = settings.backup_provider_id.as_deref().map(|provider_id| {
            sparkfox_be_common::ProviderId::parse(provider_id).map_err(|error| {
...
// `sparkfox-be-app/tests/idmm_e2e.rs` against a real in-memory database (per
