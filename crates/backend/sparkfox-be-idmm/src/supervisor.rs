...
use dashmap::DashMap;
use sparkfox_be_api_types::{AutoWorkTargetKind, IdmmConfig, IdmmState, IdmmTargetKind, InterventionRecord};
use sparkfox_be_common::{AppError, IdmmInterventionId, UserId, now_ms};
use sparkfox_be_db::IIdmmInterventionRepository;
use sparkfox_be_db::models::IdmmInterventionRow;
...
#[async_trait::async_trait]
impl sparkfox_be_requirement::IdmmHandle for IdmmManager {
...
impl sparkfox_be_conversation::ConversationSupervisionHook for IdmmManager {
...
impl sparkfox_be_terminal::TerminalSupervisionHook for IdmmManager {
...
mod tests {
    ...
    use sparkfox_be_api_types::{IdmmConfig, WatchTier};
    use sparkfox_be_db::DbError;
    use sparkfox_be_db::models::ClientPreference;
    use std::sync::Mutex;
    use tokio::sync::mpsc;
    ...
    impl SessionProbe for MockProbe {
        ...
        async fn inject(&self, action: &WakeAction) -> Result<(), sparkfox_be_common::AppError> {
            self.injected.lock().unwrap().push(action.clone());
            Ok(())
        }
        async fn snapshot_context(&self, _max: usize) -> Result<String, sparkfox_be_common::AppError> {
            Ok("ctx".into())
        }
        fn is_alive(&self) -> bool {
            true
        }
        async fn describe(&self) -> Result<SessionDescription, sparkfox_be_common::AppError> {
    ...
    #[async_trait]
    impl sparkfox_be_db::IClientPreferenceRepository for MockPrefs {
    ...
    impl sparkfox_be_realtime::UserEventSink for NullBroadcaster {
        fn send_to_user(
            &self,
            _user_id: &str,
    ...
    struct RecordingRepo {
        inserted: Mutex<Vec<sparkfox_be_db::models::IdmmInterventionRow>>,
        /// When true, `insert` returns Err to exercise the fail-open path.
        fail: bool,
    }
    #[async_trait]
    impl sparkfox_be_db::IIdmmInterventionRepository for RecordingRepo {
        async fn insert(&self, row: &sparkfox_be_db::models::IdmmInterventionRow) -> Result<(), DbError> {
        ...
        async fn list_for_target(
            ...
            _id: &str,
            _limit: i64,
        ) -> Result<Vec<sparkfox_be_db::models::IdmmInterventionRow>, DbError> {
    ...
    fn sidecar_cfg() -> IdmmConfig {
        ...
        c.fault_watch.base.max_retries = 1;
        c.fault_watch.base.budget.min_interval_secs = 0;
        c.fault_watch.base.bypass_model = sparkfox_be_api_types::BypassModelRef {
        ...
        c.decision_watch.base.max_retries = 1;
        c.decision_watch.base.budget.min_interval_secs = 0;
        c.decision_watch.base.bypass_model = sparkfox_be_api_types::BypassModelRef {
            provider_id: Some(TEST_PROVIDER_ID.into()),
            model: Some("m".into()),
        };
        c.decision_watch.strategy.categories.option_decision.allow_unmarked_pick = false;
    ...
    async fn has_pending_decision_true_when_decision_watch_on_and_text_is_decision() {
        use sparkfox_be_requirement::IdmmHandle;
