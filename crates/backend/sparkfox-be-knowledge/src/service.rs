...
use sparkfox_be_api_types::{ConnectorCredentialSummary, ConnectorSyncState, KnowledgeMountInfo, KnowledgeSource, KnowledgeSourceEntry, KnowledgeSourceMode, KnowledgeTag, UpdateKnowledgeTagRequest};
use sparkfox_be_common::{
    AppError, CompanionId, ConnectorCredentialId, ConversationId, KnowledgeBaseId, TerminalId,
    TimestampMs, now_ms,
};
use sparkfox_be_db::models::{CreateKnowledgeTagParams, KnowledgeBaseRow, KnowledgeBindingRow};
use sparkfox_be_db::{IConnectorCredentialRepository, IKnowledgeRepository};
...
pub struct KnowledgeService {
    ...
    /// rendering backend (`BrowserFetcher`, late-wired from `sparkfox-be-ai-agent`)
    ...
    /// `BrowserFetcher`, late-wired from `sparkfox-be-ai-agent` when the `browser-use`
...
impl KnowledgeService {
    ...
    /// Late-wire the production LLM completer (see `sparkfox-be-ai-agent`'s
    ...
    /// `BrowserFetcher` from `sparkfox-be-ai-agent`, wired by the app layer when the
    ...
    async fn load_credential(&self, id: &ConnectorCredentialId) -> Result<ConnectorCredential, AppError> {
        ...
        let plaintext = sparkfox_be_common::decrypt_string(&row.payload_encrypted, &key)?;
    ...
    pub async fn create_credential(
        ...
        let encrypted = sparkfox_be_common::encrypt_string(&plaintext, &key)?;
        ...
        Fut: Future<Output = ()>,
    {
        ...
        let redacted_user = sparkfox_sh_redact::redact_secrets_owned(req.user_text);
        let redacted_assistant = sparkfox_sh_redact::redact_secrets_owned(req.assistant_text);
        ...
        for candidate in out.candidates {
            ...
            let mut content = sparkfox_sh_redact::redact_secrets_owned(content.to_owned());
    ...
    async fn merge_direct_turn_writeback(
        ...
        let merged = sparkfox_sh_redact::redact_secrets_owned(out.content.trim().to_owned());
...
impl sparkfox_be_common::OnConversationDelete for KnowledgeService {
...
fn canonical_target_id(kind: &str, target_id: &str) -> Result<String, AppError> {
    let invalid = |error: sparkfox_be_common::PrefixedIdError| {
...
impl KnowledgeService {
    ...
    pub async fn update_tag(
        ...
        use sparkfox_be_db::models::UpdateKnowledgeTagParams;
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl sparkfox_be_db::IKnowledgeRepository for FailingUpdateRepo {
        async fn insert_base(&self, row: &KnowledgeBaseRow) -> Result<(), sparkfox_be_db::DbError> {
            self.0.insert_base(row).await
        }
        async fn update_base(&self, row: &KnowledgeBaseRow) -> Result<(), sparkfox_be_db::DbError> {
            Err(sparkfox_be_db::DbError::NotFound(format!("simulated persist failure for {}", row.id)))
        }
        async fn delete_base(&self, id: &str) -> Result<(), sparkfox_be_db::DbError> {
            self.0.delete_base(id).await
        }
        async fn get_base(&self, id: &str) -> Result<Option<KnowledgeBaseRow>, sparkfox_be_db::DbError> {
            self.0.get_base(id).await
        }
        async fn list_bases(&self) -> Result<Vec<KnowledgeBaseRow>, sparkfox_be_db::DbError> {
        ...
        async fn get_binding(
            ...
            kind: &str,
            id: &str,
        ) -> Result<Option<(KnowledgeBindingRow, Vec<String>)>, sparkfox_be_db::DbError> {
        ...
        async fn delete_binding(&self, kind: &str, id: &str) -> Result<(), sparkfox_be_db::DbError> {
            self.0.delete_binding(kind, id).await
        }
        async fn list_bindings_using_kb(&self, kb_id: &str) -> Result<Vec<KnowledgeBindingRow>, sparkfox_be_db::DbError> {
            self.0.list_bindings_using_kb(kb_id).await
        }
        async fn list_knowledge_tags(&self) -> Result<Vec<sparkfox_be_db::models::KnowledgeTagRow>, sparkfox_be_db::DbError> {
            self.0.list_knowledge_tags().await
        }
        async fn create_knowledge_tag(&self, params: sparkfox_be_db::models::CreateKnowledgeTagParams) -> Result<(), sparkfox_be_db::DbError> {
            self.0.create_knowledge_tag(params).await
        }
        async fn update_knowledge_tag(&self, key: &str, params: sparkfox_be_db::models::UpdateKnowledgeTagParams) -> Result<(), sparkfox_be_db::DbError> {
            self.0.update_knowledge_tag(key, params).await
        }
        async fn delete_knowledge_tag(&self, key: &str) -> Result<(), sparkfox_be_db::DbError> {
    ...
    impl sparkfox_be_realtime::UserEventSink for RecordingBroadcaster {
        fn send_to_user(
            &self,
            _user_id: &str,
            event: sparkfox_be_api_types::WebSocketMessage<serde_json::Value>,
        ) {
    ...
    async fn connector_sync_writes_snapshots_persists_cursor_and_trashes_deletions() {
        ...
        use sparkfox_be_db::models::ConnectorCredentialRow;
