...
use sparkfox_be_common::{CompanionId, ConversationId, KnowledgeBindingId, TerminalId, TimestampMs};
use sparkfox_be_db::models::{CreateKnowledgeTagParams, KnowledgeBaseRow, KnowledgeBindingRow, KnowledgeTagRow, UpdateKnowledgeTagParams};
use sparkfox_be_db::{DbError, IKnowledgeRepository};
...
pub(crate) fn make_service(data_dir: &Path) -> KnowledgeService {
    let owner_id = sparkfox_be_common::UserId::new();
