...
use sparkfox_be_api_types::{ApiResponse, ConnectorCredentialSummary, CreateKnowledgeTagRequest, KnowledgeSource, KnowledgeTag, UpdateKnowledgeTagRequest};
use sparkfox_be_auth::CurrentUser;
use sparkfox_be_common::{AppError, ConnectorCredentialId, KnowledgeBaseId};
...
fn model_override(
    ...
    match (provider_id, model) {
        (Some(provider_id), Some(model)) => {
            let provider_id = sparkfox_be_common::ProviderId::parse(provider_id).map_err(|error| {
...
mod tests {
    ...
    fn test_app(data_dir: &std::path::Path) -> Router {
        ...
        knowledge_routes(KnowledgeRouterState::new(service)).layer(Extension(CurrentUser {
            id: sparkfox_be_common::UserId::new(),
            username: "u1".into(),
        }))
    }
