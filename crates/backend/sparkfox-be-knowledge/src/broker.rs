...
use sparkfox_be_common::{LoopbackCapabilityLease, generate_id};
...
mod platform {
    ...
    use sparkfox_be_api_types::KnowledgeMcpConfig;
    use sparkfox_be_common::LoopbackCapabilityLease;
    ...
    const RUNTIME_DIR_PREFIX: &str = "sparkfox-be-knowledge-broker";
    ...
    mod tests {
        ...
        use sparkfox_be_common::{LoopbackCapabilityIssuer, LoopbackSessionKind};
        ...
        async fn wait_until_revoked(
            ...
            renewal: &sparkfox_be_common::LoopbackCapabilityRenewalRequest,
        ) {
            tokio::time::timeout(std::time::Duration::from_secs(2), async {
                loop {
                    if issuer
                        .renew::<sparkfox_be_api_types::KnowledgeCapabilityScope>(
        ...
        async fn broker_resolves_isolated_workspaces_and_unbound_read_only_scope() {
            ...
            let kb_a_id = sparkfox_be_common::KnowledgeBaseId::parse(kb_a).unwrap();
            let kb_b_id = sparkfox_be_common::KnowledgeBaseId::parse(kb_b).unwrap();
            service
                .set_binding(
                    "workpath",
        ...
        async fn symlink_cwd_is_canonical_and_eof_or_restart_revokes() {
            ...
            service
                .set_binding(
                    ...
                    KnowledgeBinding {
                        enabled: true,
                        kb_ids: vec![sparkfox_be_common::KnowledgeBaseId::parse(kb).unwrap()],
                        ..KnowledgeBinding::default()
                    },
                )
                .await
...
mod platform {
    ...
    const PIPE_PREFIX: &str = r"\\.\pipe\sparkfox-be-knowledge-broker";
    ...
    mod tests {
        ...
        fn windows_broker_owner_only_sddl_and_pipe_namespace_are_exact() {
            ...
            assert_eq!(name, format!(r"\\.\pipe\sparkfox-be-knowledge-broker-{sid}"));
