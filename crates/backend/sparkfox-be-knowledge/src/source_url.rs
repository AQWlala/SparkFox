...
/// browser-rendering backend (`BrowserFetcher`) lives in `sparkfox-be-ai-agent`
...
/// `sparkfox-be-gateway::tools_knowledge`) still refer to `UrlFetcher`; it is now
...
impl HttpFetcher {
    ...
    async fn send(&self, url: &Url, addrs: &[SocketAddr]) -> Result<reqwest::Response, AppError> {
        ...
        let mut builder = sparkfox_be_net::proxy::apply_detected_proxy(reqwest::Client::builder())
        ...
        client
            .get(url.clone())
            .header(reqwest::header::USER_AGENT, "sparkfox-be-knowledge/1.0")
            .send()
            .await
            .map_err(|e| {
                if e.is_timeout() {
