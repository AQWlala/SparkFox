...
//!
//! ## Shape (mirrors `sparkfox-be-requirement::mcp_server::RequirementMcpServer`)
//!
...
mod tests {
    ...
    /// (recipe from sparkfox-be-ai-agent's `knowledge_search_e2e`). Returns the
    ...
    async fn build_service() -> (Arc<KnowledgeService>, tempfile::TempDir) {
        ...
        let installation_owner = sparkfox_be_db::installation_owner_id(db.pool())
            .await
    ...
    fn conversation_child<I: AsRef<str>>(
        ...
        kb_ids: &[I],
        allow_write: bool,
    ) -> sparkfox_be_api_types::KnowledgeMcpChildConfig {
        let kb_ids = kb_ids
            .iter()
    ...
    async fn post_renew(
        ...
        u16,
        Option<sparkfox_be_common::LoopbackCapabilityAccess<KnowledgeCapabilityClaims>>,
    ) {
    ...
    async fn renewal_rejects_registry_authorization_with_invalid_knowledge_scope() {
        ...
        let claims = KnowledgeCapabilityClaims::issue(
            TEST_OWNER_ID,
            sparkfox_be_common::LoopbackSessionBinding::conversation(
    ...
    async fn tampered_cross_session_expired_and_write_escalation_fail_closed() {
        ...
        forged.session = sparkfox_be_common::LoopbackSessionBinding::conversation(
