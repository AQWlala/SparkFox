...
use sparkfox_be_common::constants::UPLOAD_MAX_SIZE;
use sparkfox_be_auth::CurrentUser;
...
pub fn file_routes(state: FileRouterState) -> Router {
    ...
    // We first disable the global `DefaultBodyLimit` that `sparkfox-be-app`
...
fn to_zip_entry(e: sparkfox_be_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let (mode, reason) = match info.mode {
        SnapshotMode::GitRepo => (sparkfox_be_api_types::SnapshotMode::GitRepo, None),
        SnapshotMode::Snapshot => (sparkfox_be_api_types::SnapshotMode::Snapshot, None),
        SnapshotMode::Disabled { reason } => (sparkfox_be_api_types::SnapshotMode::Disabled, Some(reason)),
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = sparkfox_be_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, sparkfox_be_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, sparkfox_be_api_types::SnapshotMode::Snapshot);
    ...
    fn snapshot_info_response_disabled_mode_carries_reason() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, sparkfox_be_api_types::SnapshotMode::Disabled);
        assert!(r.branch.is_none());
    ...
    fn compare_response_conversion() {
        use sparkfox_be_common::FileChangeOperation;
