...
use sparkfox_be_common::{generate_prefixed_id, now_ms};
...
mod tests {
    ...
    fn migrates_smart_collaboration_key_once_without_dual_read() {
        ...
        let companion_id = sparkfox_be_common::CompanionId::new().into_string();
        std::fs::write(
            &path,
            serde_json::json!({
    ...
    fn marker_blocks_migration_even_with_legacy_artifacts() {
        ...
        std::fs::write(
            ...
            sparkfox_be_common::CompanionId::new().into_string(),
        )
        .unwrap();
