...
impl CompanionStore {
    ...
    pub async fn insert_memory_scoped(
        ...
        let content = sparkfox_sh_redact::redact_secrets(content);
        let now = now_ms();
    ...
    pub async fn update_memory(
        ...
        let redacted: Option<String> = match content {
            Some(c) => {
                ...
                Some(sparkfox_sh_redact::redact_secrets(trimmed).into_owned())
            }
            None => None,
        };
...
// ---------------------------------------------------------------------------
// 自进化：技能注册表 / 挖矿统计 / 反馈回流
// 正文以磁盘 SKILL.md 为事实源（见 sparkfox-be-extension::skill_service）；这里只存
...
mod tests {
    ...
    async fn learn_run_dump_and_exists() {
        let store = CompanionStore::open_memory().await.unwrap();
        let run_id = sparkfox_be_common::CompanionLearnRunId::new().into_string();
