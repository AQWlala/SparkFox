...
use sparkfox_be_ai_agent::CompanionMemorySink;
use sparkfox_be_api_types::CreateConversationRequest;
use sparkfox_be_common::{AppError, ProviderWithModel};
...
pub(crate) async fn active_thread_ptr(store: &CompanionStore, companion_id: &str) -> Result<Option<String>, AppError> {
    sparkfox_be_common::CompanionId::try_from(companion_id)
    ...
    value
        .map(|conversation_id| {
            sparkfox_be_common::ConversationId::try_from(conversation_id.as_str())
...
pub(crate) async fn set_active_thread_ptr(
    ...
    match conversation_id {
        Some(conversation_id) => {
            sparkfox_be_common::ConversationId::try_from(conversation_id)
...
pub async fn build_companion_system_prompt(
    ...
    // `sparkfox-be-ai-agent::factory::nomi` (read from system settings). Re-adding a
    ...
    if let Some(platform) = channel_platform.filter(|p| !p.is_empty()) {
        system.push_str(&format!(
            "\n\n主人此刻正通过 {platform} 远程和你说话。此刻你是一个通过 IM 陪主人聊天、答疑、出主意的对话助手：\
             ...
             也可以用 sparkfox_ag_memory_* 维护你的长期记忆。\
    ...
    system.push_str(
        "\n\n## 知识沉淀技巧\n\
         ...
         - 分工边界：全局记忆（sparkfox_ag_memory_*）只放轻量的个人事实与偏好；知识库放成体系、可检索的领域资料。闲聊琐事不要建库。",
    );
...
fn compute_desired_workspace_dir(
    ...
    match profile.seq {
        Some(seq) => {
            let seg = sparkfox_be_common::sanitize_dir_segment(&profile.name);
...
mod workspace_path_tests {
    ...
    fn plan_foreign_temp_cwd_left_untouched() {
        ...
        let ws = Path::new("/ws");
        assert_eq!(
            plan_workspace_reconcile("/data/conversations/sparkfox-temp-9", desired, legacy, ws),
            WorkspaceAction::Leave
        );
    }
}
...
pub struct CompanionThreads {
    ...
    pub runtime_registry: Arc<dyn sparkfox_be_ai_agent::AgentRuntimeRegistry>,
}
...
impl CompanionThreads {
    ...
    pub async fn create(&self, companion_id: &str, title: Option<String>) -> Result<CompanionThread, AppError> {
        ...
        let req = CreateConversationRequest {
            r#type: sparkfox_be_common::AgentType::Nomi,
    ...
    pub async fn list(&self, companion_id: &str) -> Result<Vec<CompanionThread>, AppError> {
        ...
        for t in threads.drain(..) {
                ...
                .await
            {
                ...
                Ok(resp) if resp.r#type == sparkfox_be_common::AgentType::Nomi => pruned.push(t),
    ...
    pub async fn set_model(
        ...
        self.conversations
            .update(
                ...
                conversation_id,
                sparkfox_be_api_types::UpdateConversationRequest {
                    name: None,
                    pinned: None,
...
fn mirror_memory_to_nomi(dir: &std::path::Path, kind: &str, content: &str) -> std::io::Result<()> {
    ...
    use sparkfox_ag_memory::types::{MemoryEntry, MemoryFrontmatter, MemoryType};
    ...
    let path = sparkfox_ag_memory::store::write_memory(dir, &entry)
    ...
    let _ = sparkfox_ag_memory::index::append_index_entry(&dir.join("MEMORY.md"), &title, filename, &title);
    Ok(())
}
    ...
    async fn save(&self, conversation_id: &str, kind: &str, content: &str, tags: &[String]) -> Result<String, String> {
        ...
        if let Some(dir) = bridge_dir.filter(|d| !d.trim().is_empty()) {
            if let Err(e) = mirror_memory_to_nomi(std::path::Path::new(&dir), kind, content) {
                tracing::warn!(target: "sparkfox_be_companion", error = %e, "companion→nomi memory bridge write failed");
            }
        }
...
mod tests {
    ...
    async fn companion_system_prompt_does_not_force_a_reply_language() {
        ...
        // (sparkfox-be-ai-agent::factory::nomi), so baking it here would freeze it
    ...
    async fn companion_system_prompt_uses_profile_name_persona_and_memories() {
        ...
        let today = format_date(sparkfox_be_common::now_ms());
