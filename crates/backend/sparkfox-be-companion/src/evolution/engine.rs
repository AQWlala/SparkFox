...
use sparkfox_be_common::{AppError, generate_id, now_ms};
use sparkfox_be_extension::constants::SKILL_MANIFEST_FILE;
use sparkfox_be_extension::skill_service::{self, SkillDraftInput, SkillPaths, SkillScope};
...
impl EvolutionEngine {
    ...
    pub async fn draft_from_episode(
        ...
        sparkfox_be_common::CompanionId::try_from(owner)
...
mod tests {
    ...
    async fn make_engine_with(dir: &std::path::Path, completer: Arc<dyn CompanionCompleter>) -> (EvolutionEngine, String) {
        ...
        config.evolve.model = Some(sparkfox_be_common::ProviderWithModel {
    ...
    async fn evolve_falls_back_to_learn_model_when_unconfigured() {
        ...
        let (engine, _cid) = make_engine(dir.path(), draft, true).await;
        {
            ...
            cfg.learn.model = Some(sparkfox_be_common::ProviderWithModel {
                provider_id: sparkfox_be_common::ProviderId::new().into_string(),
                model: "test-model".into(),
                use_model: None,
