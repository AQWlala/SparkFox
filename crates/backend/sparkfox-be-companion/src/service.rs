...
use sparkfox_be_common::{AppError, CompanionId, ProviderId, ProviderUsage, ProviderUsageFeature};
use serde::Serialize;
...
use sparkfox_be_extension::skill_service::{self, SkillPaths, SkillScope};
use sparkfox_be_extension::constants::SKILL_MANIFEST_FILE;
...
impl CompanionService {
    ...
    pub async fn start(
        ...
        bus: Arc<sparkfox_be_realtime::BroadcastEventBus>,
        owner_id: &str,
    ...
    pub fn attach_companion(
        &self,
        conversations: Arc<sparkfox_be_conversation::ConversationService>,
        runtime_registry: Arc<dyn sparkfox_be_ai_agent::AgentRuntimeRegistry>,
    ) {
    ...
    pub fn memory_sink(&self) -> Arc<dyn sparkfox_be_ai_agent::CompanionMemorySink> {
    ...
    pub fn skill_sink(&self) -> Arc<dyn sparkfox_be_ai_agent::CompanionSkillSink> {
    ...
    pub async fn apply_preset_snapshot(
        ...
        if snapshot.target != sparkfox_be_api_types::PresetTarget::Companion {
    ...
    pub async fn add_memory(&self, kind: &str, content: &str, tags: &[String], scope: MemoryScope) -> Result<CompanionMemory, AppError> {
        ...
        // `sparkfox_ag_memory_save` path, which previously inserted blindly.
    ...
    pub async fn decide_companion_skill(
        ...
        if accept {
            ...
            let fid = sparkfox_be_common::CompanionEvolutionFeedbackId::new().into_string();
            let sig = if skill.signature.is_empty() { None } else { Some(skill.signature.as_str()) };
            self.store
                .record_feedback(&fid, name, sig, "reject", reason, sparkfox_be_common::now_ms())
                .await?;
...
#[async_trait::async_trait]
impl sparkfox_be_ai_agent::CompanionPromptProvider for CompanionService {
...
mod tests {
    ...
    fn provider_fixture(sequence: u64) -> String {
        ...
        sparkfox_be_common::ProviderId::try_from(raw.as_str()).unwrap().into_string()
    }
    ...
    async fn service(data_dir: &std::path::Path) -> Arc<CompanionService> {
        CompanionService::start(
            ...
            Arc::new(sparkfox_be_extension::skill_service::resolve_skill_paths(data_dir, data_dir)),
        )
        .await
        .unwrap()
    }
    ...
    async fn start_rejects_missing_authoritative_owner() {
        ...
        let result = CompanionService::start(
            ...
            "  ",
            Arc::new(NoopCompleter),
            Arc::new(sparkfox_be_extension::skill_service::resolve_skill_paths(
                dir.path(),
                dir.path(),
            )),
        )
    ...
    async fn providers_in_use_detects_shared_learn_model() {
        ...
        let hits = svc.providers_in_use(&provider_id).await;
        assert!(hits.iter().any(|u| matches!(u.feature, sparkfox_be_common::ProviderUsageFeature::DesktopCompanion)));
    }
    ...
    async fn accepting_create_skill_promotes_draft_and_is_idempotent() {
        ...
        let input = sparkfox_be_extension::skill_service::SkillDraftInput {
            name: "demo".into(),
    ...
    async fn recent_events_returns_newest_window() {
        let dir = tempfile::tempdir().unwrap();
        let svc = service(dir.path()).await;
        let base = sparkfox_be_common::now_ms();
        for i in 0..5 {
    ...
    async fn companion_prompt_provider_builds_only_for_bound_companion() {
        use sparkfox_be_ai_agent::CompanionPromptProvider;
        let dir = tempfile::tempdir().unwrap();
        let svc = service(dir.path()).await;
