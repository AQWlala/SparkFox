...
use sparkfox_be_common::{AppError, ProviderWithModel, now_ms};
...
/// keeps `sparkfox-be-companion` from hard-depending on conversation internals and
/// lets tests run without a live `ConversationService` (mirrors the
/// `IdmmHandle` seam in `sparkfox-be-requirement`).
...
mod tests {
    ...
    fn conversation_fixture() -> String {
        sparkfox_be_common::ConversationId::try_from(
