...
fn scope_from_parts(
    ...
    match (scope_kind, scope_companion_id) {
        ...
        (Some("companion"), Some(companion_id)) => {
            sparkfox_be_common::CompanionId::try_from(companion_id).map_err(|error| {
...
async fn weekly_digest(
    ...
    let since_ms = sparkfox_be_common::now_ms() - days * 86_400_000;
...
async fn list_day_digests(
    ...
    let digests = if let Some(mmdd) = q.on_day.filter(|s| s.len() == 4) {
        let today = crate::store::local_day(sparkfox_be_common::now_ms());
...
struct ApplyPresetRequest {
    ...
    #[serde(default)]
    overrides: sparkfox_be_api_types::PresetOverrides,
}
...
async fn apply_preset(
    ...
    let snapshot = presets
        .resolve(
            &req.preset_id,
            sparkfox_be_api_types::PresetTarget::Companion,
            req.locale.as_deref(),
            req.overrides,
        )
        .await?;
...
/// Binary serve of one companion's figure (the sparkfox-be-assets Response template,
