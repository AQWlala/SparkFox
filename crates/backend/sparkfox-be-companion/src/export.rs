...
//! the knowledge-base exporter (`sparkfox-be-knowledge/src/export.rs`):
...
use sparkfox_be_common::{AppError, TimestampMs, now_ms};
...
mod tests {
    ...
    fn memory_fixture(sequence: u64) -> String {
        ...
        sparkfox_be_common::CompanionMemoryId::try_from(raw.as_str()).unwrap().into_string()
    }
    ...
    async fn memory_bundle_roundtrip_full_fidelity_and_dedup() {
        ...
        let mut originals = vec![
            ...
            raw_memory(&memory_fixture(3), "knowledge", "cargo test -p sparkfox-be-companion 是门禁", "active"),
        ];
        for m in &originals {
            store_a.insert_memory_raw(m).await.unwrap();
        }
        store_a.set_state("mood", "happy").await.unwrap();
        let learn_run_id = sparkfox_be_common::CompanionLearnRunId::new().into_string();
        store_a
