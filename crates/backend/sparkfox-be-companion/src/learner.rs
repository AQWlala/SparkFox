...
use sparkfox_be_ai_agent::sparkfox_ag_config;
use sparkfox_be_ai_agent::{one_shot_completion, resolve_provider_config, user_message};
use sparkfox_be_common::{AppError, generate_prefixed_id, now_ms};
...
impl LiveCompanionCompleter {
    async fn resolve(&self, provider_id: &str, model: &str) -> Result<sparkfox_ag_config::config::Config, AppError> {
...
mod tests {
    ...
    async fn make_learner(dir: &std::path::Path, reply: &str) -> (Learner, String) {
        ...
        config.learn.model = Some(sparkfox_be_common::ProviderWithModel {
