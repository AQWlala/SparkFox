use sparkfox_be_api_types::TerminalSessionResponse;
use sparkfox_be_db::TerminalSessionRow;
use std::path::Path;
...
fn resolve_program(program: &str) -> String {
    if !program.is_empty()
        && !program.contains('/')
        && !program.contains('\\')
        && let Some(path) = sparkfox_be_runtime::resolve_command_path(program)
    {
...
pub fn default_login_shell() -> String {
    #[cfg(windows)]
    {
        ...
        for candidate in ["pwsh.exe", "powershell.exe"] {
            if let Some(path) = sparkfox_be_runtime::resolve_command_path(candidate) {
                return path.to_string_lossy().into_owned();
            }
        }
...
/// `is_temporary_workspace` (sparkfox-be-conversation/src/convert.rs).
...
mod tests {
    ...
    fn sample_row() -> TerminalSessionRow {
        TerminalSessionRow {
            id: sparkfox_be_common::TerminalId::new(),
            name: "shell".into(),
            cwd: "/tmp".into(),
            command: "$SHELL".into(),
            args: r#"["-l"]"#.into(),
            ...
            exit_code: None,
            user_id: sparkfox_be_common::UserId::new(),
            pinned: false,
            pinned_at: None,
            autowork: None,
            idmm: None,
    ...
    fn default_shell_prefers_the_first_available_powershell() {
        let expected = ["pwsh.exe", "powershell.exe"]
            .into_iter()
            .find_map(sparkfox_be_runtime::resolve_command_path)
    ...
    fn resolve_command_passes_through_unresolvable_command() {
        ...
        let name = "sparkfox-definitely-not-on-path-xyz-987";
    ...
    fn row_to_response_derives_is_default_workpath() {
        ...
        assert!(!row_to_response(&row, None, work_dir).is_default_workpath);
        row.cwd = "/srv/sparkfox-workspace".into();
        assert!(!row_to_response(&row, None, work_dir).is_default_workpath);
