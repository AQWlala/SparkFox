...
use dashmap::DashMap;
use sparkfox_be_api_types::{CreateTerminalRequest, TerminalSessionResponse};
use sparkfox_be_common::{KnowledgeBaseId, LoopbackCapabilityLeaseSet, OnTerminalDelete, TerminalId, UserId};
use sparkfox_be_db::{CreateTerminalParams, ITerminalRepository};
...
/// `sparkfox-be-runtime::shell_env` repairs `PATH`). Missing `TERM` breaks terminal
...
/// so `sparkfox-be-terminal` need not depend on `sparkfox-be-idmm`; `IdmmManager`
/// implements it and `sparkfox-be-app` injects the impl via
/// [`TerminalService::with_terminal_supervision_hook`]. Mirrors
/// `sparkfox_be_conversation::ConversationSupervisionHook`.
///
...
pub struct TerminalService {
    ...
    knowledge: Arc<std::sync::RwLock<Option<Arc<sparkfox_be_knowledge::KnowledgeService>>>>,
    /// Hooks notified after a terminal row is deleted (registration order),
    /// mirroring `ConversationService::delete_hooks`. Used by `sparkfox-be-app` to
    ...
    knowledge_mcp_config: Arc<std::sync::RwLock<Option<sparkfox_be_api_types::KnowledgeMcpConfig>>>,
    ...
    requirement_mcp_config: Arc<std::sync::RwLock<Option<sparkfox_be_api_types::RequirementMcpConfig>>>,
...
impl TerminalService {
    ...
    /// by `sparkfox-be-app` to wire `RequirementService` so a deleted terminal
    ...
    pub fn with_knowledge_service(&self, service: Arc<sparkfox_be_knowledge::KnowledgeService>) {
    ...
    /// router/driver handles see it too). Called by `sparkfox-be-app` so a
    ...
    pub fn with_knowledge_mcp_config(
        &self,
        config: sparkfox_be_api_types::KnowledgeMcpConfig,
    ...
    fn knowledge_mcp_config(&self) -> Option<sparkfox_be_api_types::KnowledgeMcpConfig> {
    ...
    pub fn with_requirement_mcp_config(&self, config: sparkfox_be_api_types::RequirementMcpConfig) {
        if let Ok(mut g) = self.requirement_mcp_config.write() {
            *g = Some(config);
        }
    }

    fn requirement_mcp_config(&self) -> Option<sparkfox_be_api_types::RequirementMcpConfig> {
    ...
    fn build_enhancement(
        ...
        {
            use sparkfox_be_api_types::KnowledgeMcpConfig as K;
        ...
        if let Some(cfg) = self.requirement_mcp_config() {
            use sparkfox_be_api_types::RequirementMcpConfig as R;
    ...
    fn spawn_pty(
        ...
        let capability_rendered = hook_env.iter().any(|(name, _)| {
            name == sparkfox_be_api_types::KnowledgeMcpConfig::ENV_CAPABILITY
                || name == sparkfox_be_api_types::RequirementMcpConfig::ENV_CAPABILITY
        });
    ...
    fn knowledge_service(&self) -> Option<Arc<sparkfox_be_knowledge::KnowledgeService>> {
    ...
    async fn bind_knowledge(&self, id: &str, cwd: &str, kb_ids: &[KnowledgeBaseId]) {
        ...
        let wp_key =
            sparkfox_be_knowledge::session_workpath_key(std::path::Path::new(cwd), &self.work_dir);
        ...
        let mut binding = ks
            .get_binding(sparkfox_be_knowledge::WORKPATH_BINDING_KIND, &wp_key)
            .await
            .unwrap_or_default();
        binding.enabled = true;
        binding.kb_ids = kb_ids.to_vec();
        if let Err(e) = ks
            .set_binding(sparkfox_be_knowledge::WORKPATH_BINDING_KIND, &wp_key, binding)
            .await
        {
    ...
    async fn sync_knowledge_workspace(
        ...
        let wp_key = sparkfox_be_knowledge::session_workpath_key(cwd_path, &self.work_dir);
        let outcome = ks
        ...
        if let Some(readme) = sparkfox_be_knowledge::build_knowledge_context(
            &outcome.mounts,
            &sparkfox_be_knowledge::KnowledgeContextOptions {
                format: sparkfox_be_knowledge::KnowledgeContextFormat::TerminalReadme,
            ...
            let dir = std::path::Path::new(cwd).join(sparkfox_be_knowledge::KB_MOUNT_REL_DIR);
            if let Err(e) = async {
    ...
    /// `search` filter live in [`sparkfox_be_file::list_workspace_level`]. The exact
    ...
    pub async fn browse_workspace(
        ...
        path: &str,
        search: Option<&str>,
    ) -> Result<Vec<sparkfox_be_api_types::WorkspaceEntry>, sparkfox_be_common::AppError> {
        let row = self
            .repo
            .get_by_id(id)
            .await
            .map_err(|e| {
                sparkfox_be_common::AppError::Internal(format!("Failed to load terminal session: {e}"))
            })?
            .ok_or_else(|| {
                sparkfox_be_common::AppError::NotFound(format!("Terminal session '{id}' not found"))
            })?;
        if row.cwd.trim().is_empty() {
            return Err(sparkfox_be_common::AppError::BadRequest(
                "Terminal session has no working directory".into(),
            ));
        }
        sparkfox_be_file::list_workspace_level(std::path::Path::new(&row.cwd), path, search)
    }
...
mod tests {
    use super::*;
    use crate::submit::SettleReason;
    use sparkfox_be_api_types::WebSocketMessage;
    use sparkfox_be_realtime::{EventBroadcaster, UserEventSink};
    use std::sync::Mutex;
    use std::time::Duration;
    ...
    fn test_kb_id() -> sparkfox_be_common::KnowledgeBaseId {
        sparkfox_be_common::KnowledgeBaseId::parse(TEST_KB_ID).unwrap()
    }

    fn test_issuer() -> Arc<sparkfox_be_common::LoopbackCapabilityIssuer> {
        Arc::new(sparkfox_be_common::LoopbackCapabilityIssuer::random().unwrap())
    }

    fn knowledge_mcp_config(
        port: u16,
        binary: &str,
    ) -> sparkfox_be_api_types::KnowledgeMcpConfig {
        sparkfox_be_api_types::KnowledgeMcpConfig::from_issuer(
            port,
            test_issuer(),
            binary.into(),
        )
    ...
    fn requirement_mcp_config(
        port: u16,
        binary: &str,
    ) -> sparkfox_be_api_types::RequirementMcpConfig {
        sparkfox_be_api_types::RequirementMcpConfig::from_issuer(
            port,
            test_issuer(),
            binary.into(),
        )
    ...
    #[derive(Default)]
    struct MemRepo {
        rows: Mutex<HashMap<String, sparkfox_be_db::TerminalSessionRow>>,
    ...
    impl ITerminalRepository for MemRepo {
        ...
        ) -> Result<sparkfox_be_db::TerminalSessionRow, sparkfox_be_db::DbError> {
            let row = sparkfox_be_db::TerminalSessionRow {
                id: p.id.clone(),
                name: p.name.clone(),
                cwd: p.cwd.clone(),
        ...
        async fn get_by_id(
            &self,
            id: &str,
        ) -> Result<Option<sparkfox_be_db::TerminalSessionRow>, sparkfox_be_db::DbError> {
        ...
        async fn list_by_user(
            &self,
            user_id: &str,
        ) -> Result<Vec<sparkfox_be_db::TerminalSessionRow>, sparkfox_be_db::DbError> {
            Ok(self
                .rows
                .lock()
                .unwrap()
        ...
        async fn update_status(
        ...
        ) -> Result<(), sparkfox_be_db::DbError> {
            if self
                .fail_next_update_status
                .swap(false, std::sync::atomic::Ordering::SeqCst)
            {
                return Err(sparkfox_be_db::DbError::Init(
            ...
            }
            {
                ...
                let row = rows
                    .get_mut(id)
                    .ok_or_else(|| sparkfox_be_db::DbError::NotFound(id.to_string()))?;
        ...
        async fn delete(&self, id: &str) -> Result<(), sparkfox_be_db::DbError> {
            if self
                .fail_next_delete
                .swap(false, std::sync::atomic::Ordering::SeqCst)
            {
        ...
        async fn delete_all(&self) -> Result<u64, sparkfox_be_db::DbError> {
            if self
        ...
        async fn get_idmm(&self, id: &str) -> Result<Option<String>, sparkfox_be_db::DbError> {
            Ok(self.rows.lock().unwrap().get(id).and_then(|row| row.idmm.clone()))
        }

        async fn mark_all_running_exited(&self) -> Result<u64, sparkfox_be_db::DbError> {
            let mut rows = self.rows.lock().unwrap();
            let mut count = 0;
        ...
        async fn save_scrollback(&self, id: &str, data: &[u8]) -> Result<(), sparkfox_be_db::DbError> {
            self.scrollback
                .lock()
                .unwrap()
        ...
        async fn load_scrollback(&self, id: &str) -> Result<Option<Vec<u8>>, sparkfox_be_db::DbError> {
            Ok(self.scrollback.lock().unwrap().get(id).cloned())
        }

        async fn clear_scrollback(&self, id: &str) -> Result<(), sparkfox_be_db::DbError> {
    ...
    impl crate::title::TerminalTitleCompleter for FakeTitler {
        async fn summarize(&self, _content: &str) -> Result<String, sparkfox_be_common::AppError> {
    ...
    #[derive(Default)]
    struct MemKbRepo {
        bases: Mutex<HashMap<String, sparkfox_be_db::models::KnowledgeBaseRow>>,
        ...
        bindings: Mutex<
            HashMap<(String, String), (sparkfox_be_db::models::KnowledgeBindingRow, Vec<String>)>,
        >,
    }

    #[async_trait::async_trait]
    impl sparkfox_be_db::IKnowledgeRepository for MemKbRepo {
        async fn insert_base(
            &self,
            row: &sparkfox_be_db::models::KnowledgeBaseRow,
        ) -> Result<(), sparkfox_be_db::DbError> {
            self.bases
                .lock()
                .unwrap()
        ...
        async fn delete_base(&self, id: &str) -> Result<(), sparkfox_be_db::DbError> {
        ...
        async fn get_base(
            &self,
            id: &str,
        ) -> Result<Option<sparkfox_be_db::models::KnowledgeBaseRow>, sparkfox_be_db::DbError> {
            Ok(self.bases.lock().unwrap().get(id).cloned())
        }
        async fn list_bases(
            &self,
        ) -> Result<Vec<sparkfox_be_db::models::KnowledgeBaseRow>, sparkfox_be_db::DbError> {
        ...
        async fn get_binding(
            ...
            Option<(sparkfox_be_db::models::KnowledgeBindingRow, Vec<String>)>,
            sparkfox_be_db::DbError,
        > {
            Ok(self
                .bindings
                .lock()
        ...
        async fn set_binding(
            ...
            updated_at: sparkfox_be_common::TimestampMs,
        ) -> Result<String, sparkfox_be_db::DbError> {
            ...
            let binding_id = bindings
                ...
                .unwrap_or_else(sparkfox_be_common::KnowledgeBindingId::new);
            let mut row = sparkfox_be_db::models::KnowledgeBindingRow {
            ...
            match target_kind {
                ...
                "conversation" => {
                    row.target_conv_id = Some(sparkfox_be_common::ConversationId::parse(target_id)
                        .map_err(|error| sparkfox_be_db::DbError::Init(error.to_string()))?)
                }
                "terminal" => {
                    row.target_term_id = Some(sparkfox_be_common::TerminalId::parse(target_id)
                        .map_err(|error| sparkfox_be_db::DbError::Init(error.to_string()))?)
                }
                "companion" => {
                    row.target_companion_id = Some(sparkfox_be_common::CompanionId::parse(target_id)
                        .map_err(|error| sparkfox_be_db::DbError::Init(error.to_string()))?)
                }
                _ => {}
            }
        ...
        async fn list_bindings_using_kb(
            &self,
            _kb_id: &str,
        ) -> Result<Vec<sparkfox_be_db::models::KnowledgeBindingRow>, sparkfox_be_db::DbError> {
            Ok(vec![])
        }
        async fn list_knowledge_tags(
            &self,
        ) -> Result<Vec<sparkfox_be_db::models::KnowledgeTagRow>, sparkfox_be_db::DbError> {
            Ok(vec![])
        }
        async fn create_knowledge_tag(
            &self,
            _params: sparkfox_be_db::models::CreateKnowledgeTagParams,
        ) -> Result<(), sparkfox_be_db::DbError> {
            Ok(())
        }
        async fn update_knowledge_tag(
            &self,
            _key: &str,
            _params: sparkfox_be_db::models::UpdateKnowledgeTagParams,
        ) -> Result<(), sparkfox_be_db::DbError> {
            Ok(())
        }
        async fn delete_knowledge_tag(&self, _key: &str) -> Result<(), sparkfox_be_db::DbError> {
            Ok(())
        }
    }
    ...
    fn knowledge_fixture(
    ...
    ) -> (Arc<sparkfox_be_knowledge::KnowledgeService>, tempfile::TempDir) {
        let data_dir = tempfile::TempDir::new().unwrap();
        let repo = MemKbRepo::default();
        repo.bases.lock().unwrap().insert(
            TEST_KB_ID.into(),
            sparkfox_be_db::models::KnowledgeBaseRow {
                id: test_kb_id(),
        ...
        (
            Arc::new(sparkfox_be_knowledge::KnowledgeService::new(
                Arc::new(repo),
                data_dir.path(),
                emitter,
            )),
    ...
    async fn create_with_kb_ids_binds_and_writes_readme() {
        ...
        let wp_key = sparkfox_be_knowledge::session_workpath_key(cwd.path(), &std::env::temp_dir());
    ...
    async fn create_with_kb_ids_binds_under_custom_workpath() {
        ...
        let key = sparkfox_be_knowledge::workpath_key(&cwd.path().to_string_lossy());
        assert_ne!(
            key, "__default__",
    ...
    async fn create_with_kb_ids_preserves_existing_workpath_writeback() {
        ...
        ks.set_binding(
            "workpath",
            &key,
            sparkfox_be_knowledge::KnowledgeBinding {
                enabled: true,
                writeback: true,
    ...
    async fn relaunch_resyncs_knowledge_and_rewrites_readme() {
        ...
        ks.set_binding(
            "terminal",
            &resp.id.to_string(),
            sparkfox_be_knowledge::KnowledgeBinding::default(),
        )
        .await
        .unwrap();
        svc.relaunch(&resp.id).await.unwrap();
    ...
    async fn create_with_kb_ids_without_knowledge_service_still_spawns() {
        ...
        let mut request = req("cat", &[]);
        request.cwd = cwd.path().to_string_lossy().into_owned();
        request.knowledge_base_ids = Some(vec![sparkfox_be_common::KnowledgeBaseId::new()]);
    ...
    async fn submit_text_not_found_when_not_live() {
        let (svc, _bc) = service();
        let missing = sparkfox_be_common::TerminalId::new();
        assert!(matches!(
    ...
    async fn await_turn_settle_turn_end_via_lifecycle_for_agent_backend() {
        ...
        let request = sparkfox_be_api_types::CreateTerminalRequest {
            name: None,
            cwd: std::env::temp_dir().to_string_lossy().into_owned(),
            command: "cat".into(),
            args: vec![],
    ...
    async fn failed_deferred_spawn_is_stable_broadcast_and_relaunchable() {
        ...
        let (svc, bc) = service();
        let mut r = req("sparkfox-command-that-does-not-exist-2c5f4d", &[]);
        r.defer_spawn = true;
        let id = svc.create(TEST_USER_ID, r).await.unwrap().id;
    ...
    async fn non_deferred_spawn_failure_never_leaves_running_row() {
        ...
        assert!(matches!(
            svc.create(
                TEST_USER_ID,
                req("sparkfox-command-that-does-not-exist-0c54db", &[])
            )
            .await
            .unwrap_err(),
    ...
    async fn browse_workspace_unknown_id_is_not_found() {
        ...
        assert!(matches!(
            svc.browse_workspace(missing.as_str(), "", None)
                .await
                .unwrap_err(),
            sparkfox_be_common::AppError::NotFound(_)
        ));
    }
    ...
    async fn browse_workspace_rejects_parent_traversal() {
        ...
        let err = svc
            ...
            .unwrap_err();
        assert!(
            matches!(err, sparkfox_be_common::AppError::BadRequest(_)),
    ...
    async fn build_enhancement_three_way_gate() {
        use sparkfox_be_api_types::KnowledgeMcpConfig as K;
        let (svc, _bc) = service();
        let terminal_id = sparkfox_be_common::TerminalId::new();
        ...
        let terminal_id2 = sparkfox_be_common::TerminalId::new().into_string();
    ...
    async fn spawn_injects_knowledge_mcp_for_claude_when_kb_ids_present() {
        ...
        let enh = TerminalLaunchEnhancement {
            mcp_servers: vec![McpServerSpec {
                name: sparkfox_be_api_types::KnowledgeMcpConfig::SERVER_NAME.into(),
                command: cfg.binary_path.clone(),
                args: vec!["mcp-knowledge-stdio".into()],
                env: HashMap::from([(
                    sparkfox_be_api_types::KnowledgeMcpConfig::ENV_CAPABILITY.into(),
                    "bootstrap".into(),
                )]),
            }],
            lifecycle: None,
        ...
        assert_eq!(
            env.get(sparkfox_be_api_types::KnowledgeMcpConfig::ENV_CAPABILITY)
                .map(String::as_str),
            Some("bootstrap")
        );
    }
    ...
    async fn build_enhancement_requirement_only_no_kb() {
        ...
        let terminal_id = sparkfox_be_common::TerminalId::new().into_string();
    ...
    async fn build_enhancement_knowledge_and_requirement_coexist() {
        use sparkfox_be_api_types::{KnowledgeMcpConfig as K, RequirementMcpConfig as R};
        let (svc, _bc) = service();
        svc.with_knowledge_mcp_config(
