...
//! the provider-backed `LiveTerminalTitleCompleter` lives in `sparkfox-be-ai-agent`,
//! and `sparkfox-be-app` wires it. A terminal with no completer (tests / webui-only)
...
/// `sparkfox-be-ai-agent` (`LiveTerminalTitleCompleter`) over the default provider/
