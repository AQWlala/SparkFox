...
use sparkfox_be_common::TerminalId;
...
pub struct McpServerSpec {
    /// Wire-level server name (e.g. "sparkfox-be-knowledge"). Must be a bare-key-safe
...
impl AgentCli {
    ...
    /// structured turn-end signal (see `sparkfox-be-requirement`'s AutoWork runner —
...
mod tests {
    ...
    fn sample_kb_server() -> McpServerSpec {
        McpServerSpec {
            name: "sparkfox-be-knowledge".into(),
    ...
    fn claude_renderer_writes_mcp_json_outside_cwd_and_returns_additive_argv() {
        ...
        let srv = &doc["mcpServers"]["sparkfox-be-knowledge"];
    ...
    fn codex_renderer_emits_c_overrides_preserving_user_config() {
        ...
        // 形如 -c mcp_servers.sparkfox-be-knowledge.command="..." -c ...args=[...]
        let joined = argv.join(" ");
        assert!(joined.contains(r#"-c mcp_servers.sparkfox-be-knowledge.command="/opt/nomi/nomicore""#));
        assert!(joined.contains(r#"mcp_servers.sparkfox-be-knowledge.args=["mcp-knowledge-stdio"]"#));
    ...
    fn apply_enhancement_dispatches_by_cli_and_noops_unknown() {
        ...
        assert!(out.iter().any(|a| a.starts_with("mcp_servers.sparkfox-be-knowledge")));
        let env: HashMap<_, _> = env.into_iter().collect();
        assert_eq!(
            env.get("NOMI_KB_MCP_CAPABILITY").map(String::as_str),
            Some("scoped-bootstrap")
    ...
    fn codex_skips_non_bare_key_safe_name_and_emits_safe_ones() {
        ...
        assert!(joined.contains("mcp_servers.sparkfox-be-knowledge.command="));
    }
