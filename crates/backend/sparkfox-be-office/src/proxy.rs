...
impl From<ProxyError> for sparkfox_be_common::AppError {
    fn from(err: ProxyError) -> Self {
        match err {
            ProxyError::InvalidCapability => sparkfox_be_common::AppError::Forbidden(err.to_string()),
            ProxyError::Timeout => sparkfox_be_common::AppError::Timeout(err.to_string()),
            ProxyError::ConnectionFailed(msg) => sparkfox_be_common::AppError::BadGateway(msg),
            ProxyError::RequestFailed(msg) => sparkfox_be_common::AppError::BadGateway(msg),
        }
    }
}
...
mod tests {
    ...
    fn proxy_error_invalid_capability_to_forbidden() {
        let err: sparkfox_be_common::AppError = ProxyError::InvalidCapability.into();
        assert!(matches!(err, sparkfox_be_common::AppError::Forbidden(_)));
    }

    #[test]
    fn proxy_error_timeout_to_timeout() {
        let err: sparkfox_be_common::AppError = ProxyError::Timeout.into();
        assert!(matches!(err, sparkfox_be_common::AppError::Timeout(_)));
    }

    #[test]
    fn proxy_error_connection_failed_to_bad_gateway() {
        let err: sparkfox_be_common::AppError = ProxyError::ConnectionFailed("refused".into()).into();
        assert!(matches!(err, sparkfox_be_common::AppError::BadGateway(_)));
    }

    #[test]
    fn proxy_error_request_failed_to_bad_gateway() {
        let err: sparkfox_be_common::AppError = ProxyError::RequestFailed("network error".into()).into();
        assert!(matches!(err, sparkfox_be_common::AppError::BadGateway(_)));
    }
