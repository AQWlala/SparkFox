...
//!
//! 跑：`cargo nextest run -p sparkfox-be-gateway --features browser-use -E 'test(execute_parallel)'`

#![cfg(feature = "browser-use")]

use sparkfox_be_gateway::browser_registry::{tool_result_to_value, BrowserRegistry};
use serde_json::json;
...
//   set SPARKFOX_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe 后：
//   cargo nextest run -p sparkfox-be-gateway --features browser-use --run-ignored all -E 'test(distinct_keys_both_launch)'
#[tokio::test]
#[ignore = "需本机/打包 chrome：并发隔离真机证明（set SPARKFOX_CHROME_BINARY）"]
