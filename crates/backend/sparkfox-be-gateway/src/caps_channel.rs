...
//! field built by `build_module_states` in `sparkfox_be_app::router::state`).
...
async fn enable_plugin(deps: Arc<GatewayDeps>, p: EnablePluginParams) -> Value {
    use sparkfox_be_channel::manager::EnableChannelSpec;
...
async fn test_plugin(deps: Arc<GatewayDeps>, p: TestPluginParams) -> Value {
    use sparkfox_be_channel::types::{PluginConfig, PluginCredentials};
