...
use sparkfox_ag_browser::{ApprovalTier, OUT_OF_BAND_CONFIRMED_KEY};
        ...
        "result": {
            "approval_required": {
                ...
                "how_to": "This action is irreversible (submit / payment / delete / send) and the \
                           ...
                           once they approve, call sparkfox_ag_browser_confirm with this call_id and option \
    ...
    match registry.stash_pending(key, input.clone()) {
        ...
        None => Some(json!({
            "error": "too many browser actions are awaiting approval; resolve or cancel some via \
                      sparkfox_ag_browser_confirm before issuing more irreversible actions"
        })),
    }
}
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    out.push(Capability::new::<NavigateParams, _, _>(
        CapabilityMeta::new("sparkfox_ag_browser_navigate", "browser", "Load a URL in the caller's browser (optionally a new tab).", DangerTier::Write),
        navigate,
    ));
    out.push(Capability::new::<ObserveParams, _, _>(
        CapabilityMeta::new("sparkfox_ag_browser_observe", "browser", "Read the page's accessibility tree (aria snapshot + ref table) to target later. Read-only.", DangerTier::Read),
        observe,
    ));
    out.push(Capability::new::<ActParams, _, _>(
        CapabilityMeta::new("sparkfox_ag_browser_act", "browser", "Run any browser action (click/type/scroll/screenshot/...); irreversible actions are held for out-of-band approval.", DangerTier::Write),
        act,
    ));
    out.push(Capability::new::<ConfirmParams, _, _>(
        CapabilityMeta::new("sparkfox_ag_browser_confirm", "browser", "Resolve a pending out-of-band browser approval (proceed_once / cancel).", DangerTier::Write),
        confirm,
    ));
}
