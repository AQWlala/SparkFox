...
use sparkfox_be_preset::PresetService;
use sparkfox_be_companion::CompanionService;
use sparkfox_be_conversation::ConversationService;
use sparkfox_be_cron::service::CronService;
use sparkfox_be_db::IProviderRepository;
use sparkfox_be_idmm::IdmmService;
use sparkfox_be_knowledge::KnowledgeService;
use sparkfox_be_requirement::{AutoWorkRunner, RequirementService};
use sparkfox_be_system::{ClientPrefService, ModelFetchService, ProviderService, SettingsService};
use sparkfox_be_terminal::TerminalService;
use sparkfox_be_common::{CompanionId, ConversationId, UserId};

/// Everything the gateway tools need to operate the desktop.
///
/// Constructed by `sparkfox-be-app` AFTER `build_module_states` (the
...
/// `sparkfox-be-app/src/router/routes.rs::inject_gateway_deps` (clone from the
...
pub struct GatewayDeps {
    ...
    pub workshop_repo: Arc<dyn sparkfox_be_db::IWorkshopRepository>,
    ...
    pub channel_state: sparkfox_be_channel::ChannelRouterState,
    /// Filesystem service (path-scoped to the configured allowed roots).
    pub file_service: sparkfox_be_file::FileServiceRef,
    /// Shell-open service (OS ShellExecute / `open`).
    pub shell_service: std::sync::Arc<sparkfox_be_shell::ShellService>,
    /// MCP server CRUD (same instance as the `/api/mcp` routes).
    pub mcp_config_service: sparkfox_be_mcp::McpConfigService,
    /// Extension registry + hub + skills (same instances as the extension routes).
    pub extension_registry: sparkfox_be_extension::ExtensionRegistry,
    pub hub_index_manager: sparkfox_be_extension::HubIndexManager,
    pub hub_installer: sparkfox_be_extension::HubInstaller,
    pub skill_paths: sparkfox_be_extension::SkillPaths,
    /// Agent catalog + remote agents (same instances as the agent routes).
    pub agent_service: std::sync::Arc<sparkfox_be_ai_agent::AgentService>,
    pub remote_agent_service: std::sync::Arc<sparkfox_be_ai_agent::RemoteAgentService>,
    /// Client-preference repo backing the global model-failover config.
    pub client_pref_repo: std::sync::Arc<dyn sparkfox_be_db::IClientPreferenceRepository>,
    /// One shared persistent collaboration facade. REST, gateway tools, boot
    /// recovery and scheduling all use this exact instance.
    pub agent_execution_engine: Arc<sparkfox_be_agent_execution::AgentExecutionEngine>,
    ...
    /// gateway exposes no `sparkfox_ag_browser_*` tools. Each companion gets its own
    ...
    /// otherwise the gateway exposes no `sparkfox_ag_computer_*` tools. See
