...
//!    Crate: `sparkfox-be-mcp`, type: `sparkfox_be_mcp::McpConfigService`.
//!
//! - `extension_registry: ExtensionRegistry`
//!    Clone of `states.extension.registry` (from `ExtensionRouterState`).
//!    Crate: `sparkfox-be-extension`, type: `sparkfox_be_extension::ExtensionRegistry`.
//!
//! - `hub_installer: HubInstaller`
//!    Clone of `states.hub.installer` (from `HubRouterState`).
//!    Crate: `sparkfox-be-extension`, type: `sparkfox_be_extension::hub::installer::HubInstaller`.
//!
//! - `hub_index_manager: HubIndexManager`
//!    Clone of `states.hub.index_manager` (from `HubRouterState`).
//!    Crate: `sparkfox-be-extension`, type: `sparkfox_be_extension::hub::index_manager::HubIndexManager`.
//!
//! - `skill_paths: SkillPaths`
//!    Clone of `states.skill.skill_paths` (from `SkillRouterState`).
//!    Crate: `sparkfox-be-extension`, type: `sparkfox_be_extension::skill_service::SkillPaths`.
//!
//! ## SKIPPED tools (listed at the bottom of this file):
//!
//! - `sparkfox_ag_mcp_test_connection` — requires building a `McpServerTransport`
...
use sparkfox_be_common::McpServerId;
...
struct McpEditServerParams {
    /// The MCP server id (from sparkfox_ag_mcp_list_servers).
...
async fn mcp_add_server(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: McpAddServerParams) -> Value {
    use sparkfox_be_api_types::McpTransport;
    ...
    let req = sparkfox_be_api_types::CreateMcpServerRequest {
        name: p.name,
...
async fn mcp_edit_server(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: McpEditServerParams) -> Value {
    ...
    let req = sparkfox_be_api_types::UpdateMcpServerRequest {
        name: None,
        description: p.description,
        transport,
        original_json: None,
...
async fn skill_list(deps: Arc<GatewayDeps>, _ctx: CallerCtx, _p: SkillListParams) -> Value {
    match sparkfox_be_extension::skill_service::list_available_skills(&deps.skill_paths).await {
        Ok(items) => {
...
async fn skill_import(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: SkillImportParams) -> Value {
    ...
    match sparkfox_be_extension::skill_service::import_skill(&deps.skill_paths, path).await {
...
async fn skill_delete(deps: Arc<GatewayDeps>, _ctx: CallerCtx, p: SkillDeleteParams) -> Value {
    match sparkfox_be_extension::skill_service::delete_skill(&deps.skill_paths, &p.name).await {
...
// ══════════════════════════════════════════════════════════════════════════════
//
// 1. `sparkfox_ag_mcp_test_connection` (Read/Write)
