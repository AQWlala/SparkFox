...
use sparkfox_be_api_types::{KnowledgeSource, KnowledgeSourceEntry, KnowledgeSourceMode};
use sparkfox_be_common::KnowledgeBaseId;
use sparkfox_be_knowledge::source_url::truncate_to_bytes;
use sparkfox_be_knowledge::{
...
async fn write_file(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: WriteFileParams) -> Value {
    ...
    match deps.knowledge_service.write_document(req).await {
        Ok(out) => ok(json!({
            ...
            "staged": out.staged,
            "updated": matches!(out.op, sparkfox_be_knowledge::WriteOp::Update),
