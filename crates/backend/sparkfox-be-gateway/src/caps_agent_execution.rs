...
//! [`AgentExecutionEngine`](sparkfox_be_agent_execution::AgentExecutionEngine) as
...
fn caller_conversation_id(ctx: &CallerCtx) -> Result<String, String> {
    ctx.conversation_id
        .clone()
        .map(sparkfox_be_common::ConversationId::into_string)
...
async fn execution_model_authority(
    ...
    owner_id: &str,
    execution_id: &str,
) -> Result<ExecutionModelPool, sparkfox_be_common::AppError> {
    let detail = deps
    ...
    if models.is_empty() {
        return Err(sparkfox_be_common::AppError::Conflict(
...
async fn narrow_execution_model_pool(
    ...
    narrow_model_pool(inherited, Some(explicit)).map_err(sparkfox_be_common::AppError::BadRequest)
}
...
async fn delegate(deps: Arc<GatewayDeps>, ctx: CallerCtx, params: DelegateParams) -> Value {
    ...
    let created = if let Some(template_id) = defaults.template_id.as_deref() {
        ...
        match conversation.as_ref() {
            ...
            None => Err(sparkfox_be_common::AppError::BadRequest(
...
async fn authorize_remote_execution(
    ...
    ctx: &CallerCtx,
    execution_id: &str,
) -> Result<AgentExecutionActor, sparkfox_be_common::AppError> {
    let companion_id = ctx
        .companion_id
        .as_ref()
        .map(|id| id.as_str())
        .ok_or_else(|| {
            sparkfox_be_common::AppError::NotFound(format!("Agent Execution {execution_id}"))
        })?;
    deps.agent_execution_engine
        .get(ctx.user_id.as_str(), execution_id)
        .await?;
    let created = deps
        ...
        .await
        .map_err(|_| sparkfox_be_common::AppError::NotFound(format!("Agent Execution {execution_id}")))?
        .into_iter()
        .next();
    if !created.is_some_and(|event| {
        ...
        return Err(sparkfox_be_common::AppError::NotFound(format!(
...
async fn authorize_execution_caller(
    ...
    if ctx.remote {
        ...
        let conversation_id = caller_conversation_id(ctx)
            .map_err(sparkfox_be_common::AppError::BadRequest)?;
        deps.agent_execution_engine
...
async fn execution_update(
    ...
    let actor = match params.execution_id() {
        ...
        None if ctx.remote => Err(sparkfox_be_common::AppError::BadRequest(
        ...
        None => match caller_conversation_id(&ctx) {
            Ok(conversation_id) => deps
                ...
                .await,
            Err(error) => Err(sparkfox_be_common::AppError::BadRequest(error)),
        },
    };
    ...
    let result: Result<Value, sparkfox_be_common::AppError> = match params {
...
fn to_value<T: serde::Serialize>(value: T) -> Result<Value, sparkfox_be_common::AppError> {
    serde_json::to_value(value)
        .map_err(|error| sparkfox_be_common::AppError::Internal(format!("serialize execution: {error}")))
}
