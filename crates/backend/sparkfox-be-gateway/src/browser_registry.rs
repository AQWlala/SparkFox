...
//! (`sparkfox_be_secret::shared_vault_path` routes to
...
use sparkfox_ag_browser::{BrowserSecretSource, BrowserTool, OUT_OF_BAND_CONFIRMED_KEY};
use sparkfox_ag_config::config::BrowserConfig;
use sparkfox_ag_tools::Tool;
use sparkfox_ag_types::tool::ToolResult;
use sparkfox_be_common::{CompanionId, ConversationId};
...
impl sparkfox_ag_browser::BrowserApprovalGate for GatewayAutoApprovalGate {
    async fn request_approval(&self, _ask: sparkfox_ag_browser::ApprovalAsk) -> sparkfox_ag_browser::ApprovalDecision {
        sparkfox_ag_browser::ApprovalDecision::Approve
    }
}
...
pub struct BrowserRegistry {
    ...
    /// the user approves it via `sparkfox_ag_browser_confirm`. Bounded-ish: capped per the
...
impl BrowserRegistry {
    ...
    pub fn new(config: &BrowserConfig) -> Self {
        let data_dir = sparkfox_ag_config::config::app_config_dir()
            ...
            .unwrap_or_else(|| std::env::temp_dir().join("sparkfox-ag-browser-data"));
        Self {
            data_dir,
    ...
    /// Convenience constructor for `sparkfox-be-app`'s gateway wiring: build the
    /// registry with the default browser config so the app does not need a direct
    /// `sparkfox-ag-config` dependency (the gateway already has one behind this feature).
    ...
    fn slot(&self, key: &str) -> Arc<CompanionBrowser> {
        ...
        if let Some(secret_key) = self.secret_key {
            let vault_path = sparkfox_be_secret::shared_vault_path(&self.data_dir);
    ...
    pub fn classify(&self, key: &str, action: &str, input: &Value) -> sparkfox_ag_browser::ApprovalTier {
    ...
    pub fn stash_pending(&self, key: &str, input: Value) -> Option<String> {
        let call_id = sparkfox_be_common::generate_prefixed_id("browseroob");
...
mod tests {
    ...
    fn all_companions_resolve_the_same_shared_secret_vault() {
        ...
        let path = sparkfox_be_secret::shared_vault_path(&r.data_dir);
    ...
    fn classify_builds_slot_and_returns_a_tier_without_launching() {
        ...
        let r = registry();
        use sparkfox_ag_browser::ApprovalTier;
        assert_eq!(
    ...
    // engines / user-data-dirs, distinct slots). Set SPARKFOX_CHROME_BINARY then:
    //   set SPARKFOX_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe
    //   cargo nextest run -p sparkfox-be-gateway --features browser-use --run-ignored all -E 'test(gateway_browser)'
    ...
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 --run-ignored all -E 'test(gateway_browser)'"]
    ...
    //   set SPARKFOX_CHROME_BINARY=C:\Program Files\Google\Chrome\Application\chrome.exe
    //   cargo nextest run -p sparkfox-be-gateway --features browser-use --run-ignored all -E 'test(gw2_confirmed)'
    #[tokio::test]
    #[ignore = "需本机/打包 chrome：set SPARKFOX_CHROME_BINARY 后 --run-ignored all -E 'test(gw2_confirmed)'"]
