...
use sparkfox_be_common::{AgentType, AppError, CompanionId, ConversationId};
...
async fn create(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CreateConversationParams) -> Value {
    ...
    if agent_type == AgentType::Remote {
        ...
        let remote_agent_id = match sparkfox_be_common::RemoteAgentId::parse(remote_agent_id) {
            Ok(id) => id,
        ...
        match deps.remote_agent_service.get(&remote_agent_id).await {
            Ok(remote) if remote.protocol == sparkfox_be_common::RemoteAgentProtocol::OpenClaw => {}
            Ok(_) => {
                return json!({
