...
async fn handle_tool_request(
    ...
    let Ok(conversation_id) = sparkfox_be_common::ConversationId::parse(&claims.session.session_id) else {
...
mod tests {
    ...
    fn child(
        ...
        user_id: &str,
        conversation_id: &str,
    ) -> sparkfox_be_api_types::GatewayMcpChildConfig {
        server
    ...
    async fn renewal_restores_server_scope_and_revoke_ends_the_lease() {
        ...
        let renewed: sparkfox_be_common::LoopbackCapabilityAccess<GatewayCapabilityClaims> =
    ...
    async fn tampered_cross_conversation_and_expired_claims_are_unauthorized() {
        ...
        forged.session = sparkfox_be_common::LoopbackSessionBinding::conversation(OTHER_CONVERSATION_ID);
        let (status, _) = post_tool(
            server.http_port(),
            Some(&access.token),
    ...
    async fn correctly_signed_terminal_binding_is_unauthorized() {
        ...
        let claims = GatewayCapabilityClaims::issue(
            TEST_OWNER_ID,
            sparkfox_be_common::LoopbackSessionBinding::terminal(
                "term_0190f5fe-7c00-7a00-8000-000000000001",
            ),
            [
                sparkfox_be_api_types::GATEWAY_LIST_TOOLS_OPERATION,
    ...
    async fn tools_call_requires_signed_operation_scope() {
        ...
        claims.allowed_tools = vec![sparkfox_be_api_types::GATEWAY_LIST_TOOLS_OPERATION.into()];
        let (token, _) = server
            .issuer
            .activate(GATEWAY_CAPABILITY_DOMAIN, &claims)
            .unwrap();
