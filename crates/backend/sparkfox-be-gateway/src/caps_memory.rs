...
use sparkfox_be_companion::store::{MemoryFilter, MemoryScope};
...
struct MemoryUpdateParams {
    /// The id of the memory to update (from sparkfox_ag_memory_list).
    id: String,
...
struct MemoryDeleteParams {
    ...
    /// sparkfox_ag_memory_update unless the user explicitly asked to delete.
    id: String,
}
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<MemoryDeleteParams, _, _>(
        CapabilityMeta::new(
            ...
            "memory",
            "Permanently delete a memory. Prefer archiving via sparkfox_ag_memory_update unless the user asked to delete.",
