...
//!
//! Backed by:
//! - `sparkfox_be_ai_agent::AgentService` — installed agent listing, health checks,
//!   custom agent CRUD, enable/disable.
//! - `sparkfox_be_ai_agent::RemoteAgentService` — remote OpenClaw Gateway CRUD,
//!   authentication, pairing, and connection testing.
//! - `sparkfox_be_conversation::model_failover` — global model-failover config read/write
//!   (stored in `client_preferences` key `agent.model_failover`).
//!
//! NEW GatewayDeps fields assumed (parent wires):
//! - `agent_service: Arc<sparkfox_be_ai_agent::AgentService>`
//! - `remote_agent_service: Arc<sparkfox_be_ai_agent::RemoteAgentService>`
//! - `client_pref_repo: Arc<dyn sparkfox_be_db::IClientPreferenceRepository>`
...
use sparkfox_be_common::RemoteAgentId;
...
async fn agent_health_check(deps: Arc<GatewayDeps>, p: AgentHealthCheckParams) -> Value {
    let req = sparkfox_be_api_types::AcpHealthCheckRequest {
        backend: p.backend,
    };
...
async fn agent_custom_create(deps: Arc<GatewayDeps>, p: AgentCustomCreateParams) -> Value {
    ...
    let req = CustomAgentUpsertRequest {
        ...
        env: p
            .env
            .into_iter()
            .map(|e| sparkfox_be_api_types::AgentEnvEntry {
                name: e.name,
                value: e.value,
...
async fn remote_agent_create(deps: Arc<GatewayDeps>, p: RemoteAgentCreateParams) -> Value {
    ...
    let req = sparkfox_be_api_types::CreateRemoteAgentRequest {
        name: p.name,
        protocol,
        url: p.url,
        auth_type,
...
async fn remote_agent_update(deps: Arc<GatewayDeps>, p: RemoteAgentUpdateParams) -> Value {
    ...
    let req = sparkfox_be_api_types::UpdateRemoteAgentRequest {
        name: p.name,
        protocol,
        url: p.url,
        auth_type,
...
async fn model_failover_get(deps: Arc<GatewayDeps>, _p: ModelFailoverGetParams) -> Value {
    let cfg =
        sparkfox_be_conversation::model_failover::get_global_failover_config(&deps.client_pref_repo)
            .await;
    ok(cfg)
}
...
async fn model_failover_set(deps: Arc<GatewayDeps>, p: ModelFailoverSetParams) -> Value {
    ...
    let queue: Vec<sparkfox_be_common::ProviderWithModel> = match p
        .queue
        .into_iter()
    ...
    match sparkfox_be_conversation::model_failover::set_global_failover_config(
...
pub(crate) fn register(out: &mut Vec<Capability>) {
    ...
    out.push(Capability::new::<AgentHealthCheckParams, _, _>(
        CapabilityMeta::new(
            "sparkfox_ag_agent_health_check",
            "agent",
    ...
    out.push(Capability::new::<AgentProviderHealthCheckParams, _, _>(
        CapabilityMeta::new(
            "sparkfox_ag_agent_provider_health_check",
            "agent",
    ...
    out.push(Capability::new::<AgentCustomCreateParams, _, _>(
        CapabilityMeta::new(
            "sparkfox_ag_agent_custom_create",
            "agent",
    ...
    out.push(Capability::new::<AgentCustomUpdateParams, _, _>(
        CapabilityMeta::new(
            "sparkfox_ag_agent_custom_update",
            "agent",
    ...
    out.push(Capability::new::<AgentCustomDeleteParams, _, _>(
        CapabilityMeta::new(
            "sparkfox_ag_agent_custom_delete",
            "agent",
    ...
    out.push(Capability::new::<AgentCustomTryConnectParams, _, _>(
        CapabilityMeta::new(
            "sparkfox_ag_agent_custom_try_connect",
            "agent",
