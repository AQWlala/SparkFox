...
async fn get_info(_deps: Arc<GatewayDeps>, _p: GetInfoParams) -> Value {
    let info = sparkfox_be_system::sysinfo::get_system_info();
    ok(info)
}
...
//     Needs: `deps.version_check_service: sparkfox_be_system::VersionCheckService`
...
//     Method: `sparkfox_be_common::factory_reset::write_marker(&data_dir, &ResetMarker::new(ResetScope::Full))`
