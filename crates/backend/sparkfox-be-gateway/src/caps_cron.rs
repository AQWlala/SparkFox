...
use sparkfox_be_api_types::{ListCronJobsQuery, UpdateConversationRequest};
use sparkfox_be_common::AgentType;
use sparkfox_be_conversation::response_middleware::{
    CronCreateParams as SvcCronCreate, CronUpdateParams as SvcCronUpdate, ICronService,
};
use sparkfox_be_cron::types::cron_job_to_response;
...
async fn list(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CronListParams) -> Value {
    if sparkfox_be_common::UserId::parse(ctx.user_id.as_str()).is_err() {
...
async fn create(deps: Arc<GatewayDeps>, ctx: CallerCtx, p: CronCreateParams) -> Value {
    ...
    let target_conv_id = p
        .conversation_id
        .or_else(|| ctx.conversation_id.clone().map(sparkfox_be_common::ConversationId::into_string));
...
fn command_result(result: sparkfox_be_conversation::response_middleware::CronCommandResult) -> Value {
    if result.success {
