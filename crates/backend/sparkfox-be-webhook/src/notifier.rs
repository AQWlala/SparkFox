//! Completion notifier: implements `sparkfox_be_requirement::CompletionNotifier` by
//! looking up the requirement's tag → bound webhook and sending a notification.
//!
//! Dependency direction: this crate depends on `sparkfox-be-requirement` (for the
//! trait); `sparkfox-be-requirement` does NOT depend on this crate. Mirrors how
//! `sparkfox-be-idmm` implements `sparkfox_be_requirement::IdmmHandle`.

use std::sync::Arc;

use async_trait::async_trait;
use sparkfox_be_api_types::WebhookPlatform;
use sparkfox_be_db::models::RequirementRow;
use sparkfox_be_db::{ITagSettingRepository, IWebhookRepository};
use sparkfox_be_requirement::CompletionNotifier;
...
impl CompletionNotifierImpl {
    ...
    async fn resolve_webhook(&self, tag: &str) -> Option<(sparkfox_be_db::models::WebhookRow, Vec<String>)> {
