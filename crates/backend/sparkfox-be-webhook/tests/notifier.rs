...
use sparkfox_be_db::models::{RequirementRow, TagSettingRow, WebhookRow};
use sparkfox_be_db::{
...
use sparkfox_be_webhook::{CompletionNotifierImpl, WebhookSender};
...
async fn add_webhook(ctx: &Ctx, enabled: bool) -> sparkfox_be_common::WebhookId {
    ctx.webhooks
        .insert(&WebhookRow {
            id: sparkfox_be_common::WebhookId::new(),
            name: "bot".into(),
            platform: "lark".into(),
...
async fn bind_tag(ctx: &Ctx, tag: &str, webhook_id: Option<sparkfox_be_common::WebhookId>) {
    ctx.tags
