...
use sparkfox_be_api_types::{CreateWebhookRequest, UpdateWebhookRequest, UpsertTagSettingRequest, WebhookPlatform};
use sparkfox_be_db::{
    ITagSettingRepository, IWebhookRepository, SqliteTagSettingRepository, SqliteWebhookRepository,
    init_database_memory,
};
use sparkfox_be_webhook::{WebhookSender, WebhookService};
...
impl WebhookSender for MockSender {
    ...
    ) -> Result<(), sparkfox_be_webhook::WebhookError> {
        self.calls.lock().unwrap().push((url.to_string(), title.to_string()));
        if self.fail {
            return Err(sparkfox_be_webhook::WebhookError::Remote("boom".into()));
        }
        Ok(())
    }
}
    ...
    let err = s
        .upsert_tag_setting(
            ...
            UpsertTagSettingRequest {
                webhook_id: Some(Some(sparkfox_be_common::WebhookId::parse("webhook_0190f5fe-7c00-7a00-8000-000000000999").unwrap())),
