...
mod tests {
    ...
    fn test_update_check_request_with_options() {
        let raw = json!({
            ...
            "repo": "nomifun/sparkfox-be-app"
        });
        let req: UpdateCheckRequest = serde_json::from_value(raw).unwrap();
        assert!(req.include_prerelease);
        assert_eq!(req.repo.as_deref(), Some("nomifun/sparkfox-be-app"));
    }
    ...
    fn test_update_check_result_with_update() {
        let result = UpdateCheckResult {
            ...
            latest: Some(UpdateReleaseInfo {
                ...
                html_url: "https://github.com/nomifun/sparkfox-be-app/releases/tag/v2.0.0".into(),
                published_at: Some("2026-04-01T00:00:00Z".into()),
                prerelease: false,
                draft: false,
                assets: vec![GitHubReleaseAsset {
                    name: "sparkfox-2.0.0-darwin-arm64.dmg".into(),
        ...
        assert_eq!(json["latest"]["assets"][0]["name"], "sparkfox-2.0.0-darwin-arm64.dmg");
