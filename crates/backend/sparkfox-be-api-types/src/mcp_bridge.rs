...
//! (`sparkfox-be-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference the
...
pub const REQUIREMENT_CAPABILITY_DOMAIN: &str = "sparkfox-be-requirement-mcp-v2";
pub const KNOWLEDGE_CAPABILITY_DOMAIN: &str = "sparkfox-be-knowledge-mcp-v2";
...
impl RequirementMcpConfig {
    ...
    /// name `mcp__sparkfox-be-requirement__requirement_update_status` (51 chars)
    /// stays within Anthropic's 64-char tool-name limit (see ELECTRON-1JY).
    pub const SERVER_NAME: &'static str = "sparkfox-be-requirement";
...
impl KnowledgeMcpConfig {
    ...
    pub const SERVER_NAME: &'static str = "sparkfox-be-knowledge";
...
impl GatewayMcpConfig {
    ...
    /// Wire-level MCP server name. Kept short so the longest wire-level tool
    /// name `mcp__sparkfox-desktop__nomi_send_to_conversation` (47 chars) stays
    /// within Anthropic's 64-char tool-name limit (see ELECTRON-1JY).
    pub const SERVER_NAME: &'static str = "sparkfox-desktop";
...
/// the nomi engine has (macOS AX / Windows UIA / Linux AT-SPI via `sparkfox-ag-a11y`).
///
...
mod tests {
    ...
    fn gateway_child_binds_operations_identity_surface_profile_and_exclusions() {
        ...
        forged_user.user_id = sparkfox_be_common::UserId::parse(TEST_USER_ID).unwrap();
    ...
    fn gateway_correctly_signed_expired_claims_fail_closed() {
        ...
        let child = cfg
            .issue_for_conversation(TEST_USER_ID, "conv_0190f5fe-7c00-7a00-8abc-012345678901", None, None, None, &[])
            .unwrap();
        let now = sparkfox_be_common::unix_time_secs();
        let expired = cfg
            .issuer
            .renew_at::<GatewayCapabilityScope>(
                GATEWAY_CAPABILITY_DOMAIN,
                &child.bootstrap.renewal,
                now.saturating_sub(sparkfox_be_common::LOOPBACK_CAPABILITY_TTL_SECS + 1),
            )
            .unwrap();
        assert_eq!(
