...
use sparkfox_be_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use sparkfox_be_common::{TimestampMs, now_ms};
use sparkfox_be_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
impl McpOAuthService {
    ...
    pub fn new_dynamic(token_repo: Arc<dyn IOAuthTokenRepository>) -> Self {
        Self {
            token_repo,
            http_client: Arc::new(sparkfox_be_net::http_client),
            pending: Arc::new(Mutex::new(None)),
        }
    }
    ...
    fn build_no_redirect_client() -> Result<reqwest::Client, McpError> {
        sparkfox_be_net::proxy::apply_detected_proxy(reqwest::ClientBuilder::new())
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<sparkfox_be_db::models::OAuthTokenRow>, sparkfox_be_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<sparkfox_be_db::models::OAuthTokenRow, sparkfox_be_db::DbError> {
            unimplemented!()
        }
        ...
        async fn list_authenticated_urls(&self) -> Result<Vec<String>, sparkfox_be_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), sparkfox_be_db::DbError> {
            Err(sparkfox_be_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<sparkfox_be_db::models::OAuthTokenRow>, sparkfox_be_db::DbError> {
            Ok(Some(sparkfox_be_db::models::OAuthTokenRow {
