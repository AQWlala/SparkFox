...
use sparkfox_be_common::{McpServerId, now_ms};
use sparkfox_be_db::{CreateMcpServerParams, IMcpServerRepository, UpdateMcpServerParams};
...
mod tests {
    use super::*;
    use sparkfox_be_api_types::{ImportMcpServerRequest, McpTransport};
    use sparkfox_be_common::{McpServerStatus, TimestampMs};
    use sparkfox_be_db::models::McpServerRow;
    use sparkfox_be_db::{CreateMcpServerParams, DbError, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockMcpServerRepo {
        ...
        async fn find_by_id(&self, id: &sparkfox_be_common::McpServerId) -> Result<Option<McpServerRow>, DbError> {
            let servers = self.servers.lock().unwrap();
        ...
        async fn find_by_id_any(&self, id: &sparkfox_be_common::McpServerId) -> Result<Option<McpServerRow>, DbError> {
            let servers = self.servers.lock().unwrap();
        ...
        async fn list_by_ids_any(&self, ids: &[sparkfox_be_common::McpServerId]) -> Result<Vec<McpServerRow>, DbError> {
            let servers = self.servers.lock().unwrap();
            Ok(servers
                .iter()
        ...
        async fn create(&self, params: CreateMcpServerParams<'_>) -> Result<McpServerRow, DbError> {
            ...
            let row = McpServerRow {
                id: sparkfox_be_common::McpServerId::new(),
        ...
        async fn update(&self, id: &sparkfox_be_common::McpServerId, params: UpdateMcpServerParams<'_>) -> Result<McpServerRow, DbError> {
            let mut servers = self.servers.lock().unwrap();
            let idx = servers
                .iter()
                .position(|s| s.id == *id)
        ...
        async fn delete(&self, id: &sparkfox_be_common::McpServerId) -> Result<(), DbError> {
            let mut servers = self.servers.lock().unwrap();
            let idx = servers
                .iter()
        ...
        async fn update_tools(&self, id: &sparkfox_be_common::McpServerId, tools: Option<&str>) -> Result<(), DbError> {
            let mut servers = self.servers.lock().unwrap();
            let idx = servers
                .iter()
                .position(|s| s.id == *id)
    ...
    async fn persist_test_result_records_success_status_and_tools() {
        ...
        let result = McpConnectionTestResult {
            success: true,
            tools: Some(vec![sparkfox_be_api_types::McpToolResponse {
                name: "read_file".into(),
        ...
        assert_eq!(updated.last_test_status, sparkfox_be_common::McpServerStatus::Connected);
    ...
    async fn persist_test_result_records_error_and_clears_tools() {
        ...
        let updated = svc.get_server(&created.id).await.unwrap();
        assert_eq!(updated.last_test_status, sparkfox_be_common::McpServerStatus::Error);
