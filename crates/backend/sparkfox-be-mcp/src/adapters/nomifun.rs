...
use sparkfox_be_common::McpSource;
...
mod tests {
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn find_by_id(&self, id: &sparkfox_be_common::McpServerId) -> Result<Option<McpServerRow>, sparkfox_be_db::DbError> {
            Ok(self.servers.iter().find(|s| s.id == *id).cloned())
        }

        async fn find_by_name(&self, name: &str) -> Result<Option<McpServerRow>, sparkfox_be_db::DbError> {
        ...
        async fn update_tools(&self, _id: &sparkfox_be_common::McpServerId, _tools: Option<&str>) -> Result<(), sparkfox_be_db::DbError> {
            unimplemented!("not needed for adapter tests")
        }
    }
