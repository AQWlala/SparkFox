//! Integration tests for sparkfox-be-mcp core types.
//!
...
use sparkfox_be_mcp::{McpServer, McpServerTransport, McpTool};
...
fn row(transport_type: &str, transport_config: &str, tools: Option<&str>, status: &str) -> McpServerRow {
    McpServerRow {
        id: sparkfox_be_common::McpServerId::parse("mcp_0190f5fe-7c00-7a00-8000-000000000042").unwrap(),
...
fn tool_fields_preserved_through_conversion() {
    ...
    // Domain -> API response
    let resp: sparkfox_be_api_types::McpToolResponse = tool.clone().into();
