...
/// `sparkfox-be-workshop` (registers each result as a `wsa_` asset), so this crate
/// never depends on `sparkfox-be-workshop` (no dependency cycle).
#[async_trait]
...
/// implemented by the app over `sparkfox-be-workshop`.
#[async_trait]
...
mod tests {
    ...
    use sparkfox_be_db::{SqliteCreationTaskRepository, SqliteProviderRepository};
    ...
    async fn seed_provider(pool: &sparkfox_be_db::SqlitePool, platform: &str) -> String {
        let repo = SqliteProviderRepository::new(pool.clone());
        let encrypted = sparkfox_be_common::encrypt_string("sk-test-key", &TEST_KEY).unwrap();
        let row = repo
            .create(sparkfox_be_db::CreateProviderParams {
                id: None,
                platform,
                name: "Test",
...
mod http_e2e_tests {
    ...
    async fn build(base_url: &str) -> (Arc<CreationService>, String, Arc<CountingSink>, sparkfox_be_db::Database) {
        let db = sparkfox_be_db::init_database_memory().await.unwrap();
        let pool = db.pool().clone();
        // seed a provider row pointed at the mock server
        let prov_repo = SqliteProviderRepository::new(pool.clone());
        let encrypted = sparkfox_be_common::encrypt_string("sk-e2e", &TEST_KEY).unwrap();
