...
//! factory's runtime. The factory (sparkfox-be-ai-agent) owns the trait + the small
//! `PublicAgentRuntime` DTO; this crate depends on the factory (never the
//! reverse — no cycle), exactly like `sparkfox-be-companion` does for its persona
//! provider.
//!
...
use sparkfox_be_ai_agent::{PublicAgentProvider, PublicAgentRuntime};
...
mod tests {
    use super::*;
    use sparkfox_be_common::{KnowledgeBaseId, ProviderId};
