...
use sparkfox_be_common::{AppError, ProviderId, PublicAgentId};
...
impl PublicAgentService {
    ...
    pub async fn apply_preset_snapshot(
        ...
        if snapshot.target != sparkfox_be_api_types::PresetTarget::PublicCompanion {
    ...
    pub async fn providers_in_use(&self, provider_id: &str) -> Vec<sparkfox_be_common::ProviderUsage> {
        ...
        self.list()
            ...
            .map(|a| sparkfox_be_common::ProviderUsage {
                feature: sparkfox_be_common::ProviderUsageFeature::PublicCompanion,
                label: a.name,
