...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(sparkfox_be_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = sparkfox_be_system::ClientPrefService::new(repo);
    ...
    fn multipart_request_with_format(
        ...
        const BOUNDARY: &str = "sparkfox-stt-limit-test";
        let prefix = format!(
    ...
    fn speech_to_text_config_prefers_tools_key_and_supports_legacy_key() {
        ...
        assert!(matches!(
            config.provider,
            sparkfox_be_api_types::SpeechToTextProvider::Openai
        ));
        assert_eq!(
        ...
        assert!(matches!(
            config.provider,
            sparkfox_be_api_types::SpeechToTextProvider::Deepgram
        ));
        assert_eq!(
