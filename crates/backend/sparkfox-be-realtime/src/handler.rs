...
///
/// Injected by `sparkfox-be-app` — wraps `sparkfox_be_auth::extract_token_from_ws_headers`
/// so that `sparkfox-be-realtime` does not depend on `sparkfox-be-auth` directly.
