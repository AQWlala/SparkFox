...
use sparkfox_be_common::{AgentKillReason, AppError, ConversationStatus, TimestampMs, now_ms};
...
use sparkfox_be_db::{IProviderRepository, IConversationRepository, SqliteConversationRepository};
...
fn make_message(conv_id: String, content: &str, offset_ms: i64) -> MessageRow {
    MessageRow {
        id: sparkfox_be_common::MessageId::new().into_string(),
        conversation_id: conv_id,
        msg_id: Some(sparkfox_be_common::MessageId::new().into_string()),
...
async fn t8_6_compact_mode_truncates_large_tool_content_only_for_list_response() {
    ...
    let message_id = sparkfox_be_common::MessageId::new();
...
async fn t9_4_empty_keyword() {
    ...
    assert!(matches!(err, sparkfox_be_common::AppError::BadRequest(_)));
}
...
async fn messages_wrong_user_returns_not_found() {
    ...
    let other_user = sparkfox_be_common::UserId::new();
    let err = svc
