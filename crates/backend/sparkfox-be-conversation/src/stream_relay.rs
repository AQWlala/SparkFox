...
use sparkfox_be_api_types::{AgentErrorCode, ConversationRuntimeSummary, WebSocketMessage};
use sparkfox_be_common::{CompanionId, ErrorChain, MessageId, normalize_keys_to_snake_case, now_ms};

use crate::service::ConversationService;
use sparkfox_be_db::{IConversationRepository, MessageRowUpdate};
use sparkfox_be_db::models::MessageRow;
use sparkfox_be_realtime::UserEventSink;
...
fn turn_writeback_status_label(status: sparkfox_be_knowledge::TurnWritebackStatus) -> &'static str {
    match status {
        sparkfox_be_knowledge::TurnWritebackStatus::Disabled => "disabled",
        sparkfox_be_knowledge::TurnWritebackStatus::NoCompleter => "no_completer",
        sparkfox_be_knowledge::TurnWritebackStatus::NoCandidate => "no_candidate",
        sparkfox_be_knowledge::TurnWritebackStatus::Written => "written",
        sparkfox_be_knowledge::TurnWritebackStatus::Partial => "partial",
        sparkfox_be_knowledge::TurnWritebackStatus::Failed => "failed",
    }
}

fn turn_writeback_phase_label(phase: sparkfox_be_knowledge::TurnWritebackPhase) -> &'static str {
    match phase {
        sparkfox_be_knowledge::TurnWritebackPhase::Extracting => "extracting",
        sparkfox_be_knowledge::TurnWritebackPhase::Writing => "writing",
    }
}

fn turn_writeback_retryable(status: sparkfox_be_knowledge::TurnWritebackStatus) -> bool {
    matches!(
        status,
        sparkfox_be_knowledge::TurnWritebackStatus::NoCompleter
            | sparkfox_be_knowledge::TurnWritebackStatus::Partial
            | sparkfox_be_knowledge::TurnWritebackStatus::Failed
    )
}
...
fn turn_writeback_final_state(
    report: &sparkfox_be_knowledge::TurnWritebackReport,
    status: &str,
    attempt_id: &str,
    started_at: i64,
    finished_at: i64,
...
pub(crate) async fn run_turn_writeback_report(
    service: Arc<sparkfox_be_knowledge::KnowledgeService>,
    mut request: sparkfox_be_knowledge::TurnWritebackRequest,
    repo: Arc<dyn IConversationRepository>,
    user_events: Arc<dyn UserEventSink>,
    user_id: String,
    ...
    match report.status {
        sparkfox_be_knowledge::TurnWritebackStatus::Written
        | sparkfox_be_knowledge::TurnWritebackStatus::Partial => {
            info!(
...
impl StreamRelay {
    ...
    async fn consume_inner(
        ...
        let mut active_acp_tool_calls: HashMap<
            String,
            sparkfox_be_ai_agent::protocol::events::tool_call::AcpToolCallEventData,
        > = HashMap::new();
        let mut terminal_acp_tool_calls: HashSet<String> = HashSet::new();
        let mut active_tool_groups: HashMap<
            String,
            Vec<sparkfox_be_ai_agent::protocol::events::tool_call::ToolGroupEntry>,
        > = HashMap::new();
        let mut active_plan_ids: HashSet<String> = HashSet::new();
        let mut active_agent_status: Option<sparkfox_be_ai_agent::protocol::events::AgentStatusEventData> = None;
            ...
            match recv_result {
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let channel_error = AgentStreamEvent::Error(
                        sparkfox_be_ai_agent::protocol::events::ErrorEventData::legacy(
    ...
    async fn flush_text_segment(&self, segment: &mut TextSegmentState) {
        ...
        if segment.record_created {
            let update = sparkfox_be_db::MessageRowUpdate {
            ...
            if let Some(primary_segment) = text_segments.first() {
                if processed.message != text || hidden {
                    ...
                    for segment in text_segments.iter().skip(1) {
                        let hide_update = sparkfox_be_db::MessageRowUpdate {
                            content: None,
                            status: None,
                            hidden: Some(true),
                        };
    ...
    async fn persist_error_tips(&self, data: &sparkfox_be_ai_agent::protocol::events::ErrorEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_agent_status(&self, data: &sparkfox_be_ai_agent::protocol::events::AgentStatusEventData) {
    ...
    async fn finalize_active_agent_status(
        &self,
        active_status: &mut Option<sparkfox_be_ai_agent::protocol::events::AgentStatusEventData>,
        terminal_status: &str,
    ) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_tool_call(&self, data: &sparkfox_be_ai_agent::protocol::events::tool_call::ToolCallEventData) {
        self.persist_tool_call_with_hidden(data, false).await;
    }

    async fn persist_tool_call_with_hidden(
        &self,
        data: &sparkfox_be_ai_agent::protocol::events::tool_call::ToolCallEventData,
        hidden: bool,
    ) {
    ...
    fn incomplete_tool_reason(event: &AgentStreamEvent) -> Option<&'static str> {
        match event {
            ...
            AgentStreamEvent::Finish(data) => match data.stop_reason {
                Some(sparkfox_be_ai_agent::protocol::events::TurnStopReason::MaxTokens) => Some("max_tokens"),
                Some(sparkfox_be_ai_agent::protocol::events::TurnStopReason::MaxTurnRequests) => {
                    Some("max_turn_requests")
                }
                Some(sparkfox_be_ai_agent::protocol::events::TurnStopReason::Refusal) => Some("refusal"),
                Some(sparkfox_be_ai_agent::protocol::events::TurnStopReason::Cancelled) => Some("cancelled"),
                Some(sparkfox_be_ai_agent::protocol::events::TurnStopReason::EndTurn) => Some("end_turn"),
                None => Some("finish"),
            },
            _ => None,
        }
    ...
    fn plan_terminal_status(event: &AgentStreamEvent) -> &'static str {
        match event {
            AgentStreamEvent::Finish(data)
                if matches!(
                    data.stop_reason,
                    None | Some(sparkfox_be_ai_agent::protocol::events::TurnStopReason::EndTurn)
                ) => "finish",
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_acp_tool_call(&self, data: &sparkfox_be_ai_agent::protocol::events::tool_call::AcpToolCallEventData) {
    ...
    #[tracing::instrument(skip_all)]
    async fn persist_tool_group(&self, entries: &[sparkfox_be_ai_agent::protocol::events::tool_call::ToolGroupEntry]) {
    ...
    pub async fn complete_conversation_with_context(
        ...
        let update = sparkfox_be_db::ConversationRowUpdate {
...
mod tests {
    use super::*;
    use sparkfox_be_ai_agent::protocol::events::{
        ErrorEventData, FinishEventData, PlanEventData, TextEventData, ThinkingEventData,
    };
    use sparkfox_be_common::{ConversationId, MessageId};
    use sparkfox_be_db::DbError;
    use std::sync::Mutex;
    ...
    async fn turn_completed_accumulates_tokens_into_wired_runtime_state() {
        use sparkfox_be_ai_agent::protocol::events::TurnCompletedEventData;
    ...
    async fn run_plan_event_completes_and_hides_its_source_tool() {
        use sparkfox_be_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    ...
    async fn run_terminal_error_closes_preparing_agent_status() {
        use sparkfox_be_ai_agent::protocol::events::AgentStatusEventData;
    ...
    async fn run_marks_active_tool_call_error_when_turn_hits_max_tokens() {
        use sparkfox_be_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
        use sparkfox_be_ai_agent::protocol::events::TurnStopReason;
    ...
    async fn run_suppresses_pre_response_error_when_failover_will_fire() {
        ...
        tx.send(AgentStreamEvent::Error(ErrorEventData::legacy(
            ...
            Some(sparkfox_be_api_types::AgentErrorCode::UserLlmProviderGatewayError),
        )))
        .unwrap();
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        send_error_tx
            .send(AgentSendError::from_app_error(sparkfox_be_common::AppError::BadGateway(
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(sparkfox_be_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_send_error_keeps_existing_stream_error_when_it_arrives_first() {
        ...
        let rx = tx.subscribe();
        let send_error = AgentSendError::from_app_error(sparkfox_be_common::AppError::BadGateway(
            "provider returned 401 invalid api key".into(),
        ));
        tx.send(AgentStreamEvent::Error(ErrorEventData::legacy(
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use sparkfox_be_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_marks_active_acp_tool_failed_when_turn_is_truncated() {
        use sparkfox_be_ai_agent::protocol::events::{TurnStopReason, tool_call::{
            AcpToolCallEventData, AcpToolCallSessionUpdateKind, AcpToolCallStatus, AcpToolCallUpdateData,
        }};
    ...
    async fn run_tool_group_persists_message() {
        use sparkfox_be_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, sparkfox_be_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, sparkfox_be_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: &str) -> Result<Option<sparkfox_be_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, row: &sparkfox_be_db::models::ConversationRow) -> Result<String, DbError> {
            Ok(row.id.clone())
        }
        async fn update(&self, _id: &str, _updates: &sparkfox_be_db::ConversationRowUpdate) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &sparkfox_be_db::ConversationFilters,
        ) -> Result<sparkfox_be_common::PaginatedResult<sparkfox_be_db::models::ConversationRow>, DbError> {
            Ok(sparkfox_be_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<sparkfox_be_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<sparkfox_be_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn get_messages(
            ...
            _page: u32,
            _page_size: u32,
            _order: sparkfox_be_db::SortOrder,
        ) -> Result<sparkfox_be_common::PaginatedResult<MessageRow>, DbError> {
            Ok(sparkfox_be_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
        ...
        async fn update_message(&self, id: &str, updates: &sparkfox_be_db::MessageRowUpdate) -> Result<(), DbError> {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<sparkfox_be_common::PaginatedResult<sparkfox_be_db::MessageSearchRow>, DbError> {
            Ok(sparkfox_be_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
