...
use sparkfox_be_api_types::{ExecutionModelPool, ExecutionModelRef, HealthStatus, ModelHealthStatus};
use sparkfox_be_common::{AgentKillReason, AgentType, ErrorChain, ProviderWithModel, now_ms};
use sparkfox_be_db::{ConversationRowUpdate, UpdateProviderParams};
...
impl ConversationService {
    ...
    pub(crate) async fn resolve_failover_config(
        &self,
        extra_json: &str,
    ) -> Option<sparkfox_be_api_types::ModelFailoverConfig> {
    ...
    pub async fn perform_model_failover(
        &self,
        conversation_id: &str,
        config: &sparkfox_be_api_types::ModelFailoverConfig,
    ...
    pub(crate) async fn strip_images_and_rebuild(
        ...
        sparkfox_be_common::VisionUnsupportedRegistry::global().mark_unsupported(&pm.provider_id, &pm.model);
    ...
    pub async fn idmm_failover_conversation(
        ...
        conversation_id: &str,
        runtime_registry: &Arc<dyn AgentRuntimeRegistry>,
    ) -> Result<bool, sparkfox_be_common::AppError> {
        ...
        let extra_json = match self.conversation_repo().get(conv_id).await {
            ...
            Err(e) => return Err(sparkfox_be_common::AppError::from(e)),
        };
        ...
        let req = sparkfox_be_api_types::SendMessageRequest {
