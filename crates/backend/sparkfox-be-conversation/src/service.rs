...
use sparkfox_be_ai_agent::{AgentRuntimeHandle, AgentRuntimeRegistry};
...
use sparkfox_be_db::models::{ConversationRow, MessageRow};
use sparkfox_be_db::{
...
use sparkfox_be_mcp::{AcpMcpCapabilities, parse_acp_mcp_capabilities};
use sparkfox_be_realtime::UserEventSink;
use sparkfox_be_runtime::resolve_command_path;
...
pub(crate) fn parse_conv_id(id: &str) -> Result<&str, sparkfox_be_common::AppError> {
    ConversationId::try_from(id)
        .map(|_| id)
        .map_err(|_| sparkfox_be_common::AppError::NotFound(format!("conversation {id}")))
}
...
/// One-directional seam letting IDMM (the `sparkfox-be-idmm` crate) arm supervision
/// for a desktop conversation at turn start — WITHOUT this crate depending on
/// `sparkfox-be-idmm` (which sits above it). `sparkfox-be-idmm::IdmmManager` implements
/// it; `sparkfox-be-app` injects the implementation at assembly time via
...
pub struct ConversationService {
    ...
    knowledge_service: Arc<RwLock<Option<Arc<sparkfox_be_knowledge::KnowledgeService>>>>,
    ...
    preset_service: Arc<RwLock<Option<Arc<sparkfox_be_preset::PresetService>>>>,
    ...
    /// as `cron_service`). Wired by `sparkfox-be-app` so a desktop turn arms 智能决策
    ...
    /// 签名:`sparkfox-be-app` 在装配处对 send-loop 实例调用
    /// [`Self::with_failover_deps`]。未注册(两槽为 `None`)即视为故障转移关闭
    /// —— fail-safe,所以不跑故障转移的上下文(测试、纯 webui)无需任何改动。
    failover_provider_repo: Arc<RwLock<Option<Arc<dyn sparkfox_be_db::IProviderRepository>>>>,
    failover_client_prefs: Arc<RwLock<Option<Arc<dyn sparkfox_be_db::IClientPreferenceRepository>>>>,
...
impl ConversationService {
    ...
    pub fn with_preset_service(&self, service: Arc<sparkfox_be_preset::PresetService>) {
    ...
    /// `with_cron_service`). Called by `sparkfox-be-app` so each desktop turn arms
    ...
    /// backs the global failover config. Wired by `sparkfox-be-app` on the
    ...
    pub fn with_failover_deps(
        &self,
        provider_repo: Arc<dyn sparkfox_be_db::IProviderRepository>,
        client_prefs: Arc<dyn sparkfox_be_db::IClientPreferenceRepository>,
    ) {
    ...
    /// `delete()`. Used by `sparkfox-be-app` to wire up `InMemoryAgentRuntimeRegistry`
    ...
    pub(crate) fn failover_deps(
        ...
        Arc<dyn sparkfox_be_db::IProviderRepository>,
        Arc<dyn sparkfox_be_db::IClientPreferenceRepository>,
    )> {
...
impl ConversationService {
    ...
    pub async fn create_from_preset_snapshot(
        ...
        snapshot: sparkfox_be_api_types::ResolvedPresetSnapshot,
    ) -> Result<ConversationResponse, AppError> {
        req.preset_id = None;
    ...
    async fn create_inner(
        ...
        user_id: &str,
        mut req: CreateConversationRequest,
        trusted_snapshot: Option<sparkfox_be_api_types::ResolvedPresetSnapshot>,
        ...
        if resolved_preset_snapshot.is_none() && let Some(preset_id) = preset_id {
            ...
            resolved_preset_snapshot = Some(service
                .resolve(
                    &preset_id,
                    sparkfox_be_api_types::PresetTarget::Conversation,
                    None,
                    preset_overrides,
                )
                .await?);
        ...
        if let Some(snapshot) = resolved_preset_snapshot.as_ref() {
            ...
            if let Some(model) = snapshot.resolved_model.as_ref() {
                if req.r#type == AgentType::Nomi {
                    ...
                    req.model = Some(sparkfox_be_common::ProviderWithModel {
        ...
        let row = sparkfox_be_db::models::ConversationRow {
        ...
        if let Some(snapshot_value) = preset_snapshot_value.as_ref()
            && let Ok(snapshot) = serde_json::from_value::<sparkfox_be_api_types::ResolvedPresetSnapshot>(
        ...
        let response_row = sparkfox_be_db::models::ConversationRow {
            id: new_id.clone(),
            extra: serde_json::to_string(&extra)
                .map_err(|e| AppError::Internal(format!("Failed to serialize extra: {e}")))?,
            ..row
...
impl ConversationService {
    ...
    async fn send_message_inner(
        ...
        let user_msg = sparkfox_be_db::models::MessageRow {
        ...
        tokio::spawn(async move {
            ...
            let mut failover_tried: Vec<sparkfox_be_common::ProviderWithModel> = Vec::new();
            let mut final_turn_writeback: Option<(
                Arc<sparkfox_be_knowledge::KnowledgeService>,
                sparkfox_be_knowledge::TurnWritebackRequest,
                String,
                String,
            )> = None;
            ...
            while let Some((current_send, msg_id)) = pending_send.take() {
                        ...
                        relay = relay.with_failover_suppressor(Arc::new(move |code| {
                            (failover_within_bound
                                ...
                                || (image_retry_available
                                    && code
                                        == sparkfox_be_api_types::AgentErrorCode::UserLlmProviderImageUnsupported)
                        }));
                    }
                }
                ...
                if image_strip_retries_done == 0
                    ...
                    && outcome.terminal.code()
                        == Some(sparkfox_be_api_types::AgentErrorCode::UserLlmProviderImageUnsupported)
                {
    ...
    pub(crate) async fn steer_message_idempotent(
        ...
        let message = sparkfox_be_db::models::MessageRow {
            id: message_id.clone(),
    ...
    fn note_user_cancel(&self, conversation_id: &str) {
        if let Ok(mut stamps) = self.user_cancel_stamps.lock() {
            stamps.insert(conversation_id.to_string(), sparkfox_be_common::now_ms());
        }
    }
...
impl ConversationService {
    ...
    pub(crate) fn build_runtime_options(&self, row: &sparkfox_be_db::models::ConversationRow) -> Result<AgentRuntimeBuildOptions, AppError> {
    ...
    async fn apply_knowledge_mounts(&self, row: &ConversationRow, runtime_options: &mut AgentRuntimeBuildOptions) {
        ...
        let outcome = if target_kind == "conversation" && !preset_binding {
            let wp_key = sparkfox_be_knowledge::session_workpath_key(&workspace, &self.workspace_root);
            service
    ...
    fn build_turn_writeback_request(
        ...
        let surface = if companion {
            sparkfox_be_knowledge::WriteSurface::Companion
        } else if channel_platform.map(str::trim).filter(|s| !s.is_empty()).is_some() {
            sparkfox_be_knowledge::WriteSurface::ExternalChannel
        } else if agent_type == AgentType::Acp {
            sparkfox_be_knowledge::WriteSurface::TerminalAcp
        } else {
            sparkfox_be_knowledge::WriteSurface::RegularChat
        };
        ...
        let request = sparkfox_be_knowledge::TurnWritebackRequest {
            mounts: mounts.clone(),
            binding: sparkfox_be_knowledge::KnowledgeBinding {
                enabled: true,
                writeback,
                writeback_mode,
                writeback_eagerness,
...
fn classify_repo_mcp_status(
    row: &sparkfox_be_db::models::McpServerRow,
...
/// [`sparkfox_be_knowledge::build_knowledge_context`] renders) plus the write-back
...
fn knowledge_mounts_signature(outcome: &sparkfox_be_knowledge::MountOutcome) -> String {
...
mod tests {
    ...
    fn enum_to_db_agent_type() {
        use sparkfox_be_common::AgentType;
    ...
    fn response_with_type(agent_type: sparkfox_be_common::AgentType) -> ConversationResponse {
        ConversationResponse {
            id: sparkfox_be_common::ConversationId::new().into_string(),
            name: "test".into(),
            r#type: agent_type,
            model: None,
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_sparkfox_be_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
