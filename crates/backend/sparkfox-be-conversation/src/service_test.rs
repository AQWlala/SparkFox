...
use sparkfox_be_ai_agent::types::{AgentRuntimeBuildOptions, SendMessageData};
use sparkfox_be_ai_agent::{AgentSendError, AgentRuntimeRegistry};

use crate::response_middleware::{CronCommandResult, CronCreateParams, CronUpdateParams, ICronService};
use sparkfox_be_api_types::AgentErrorCode;
use sparkfox_be_api_types::{
...
use sparkfox_be_knowledge::{KnowledgeBinding, KnowledgeCompleter, KnowledgeEventEmitter, KnowledgeService};
...
async fn init_database_memory() -> Result<sparkfox_be_db::Database, sparkfox_be_db::DbError> {
    sparkfox_be_db::init_database_memory_with_owner(
        sparkfox_be_common::UserId::parse(SQLITE_TEST_OWNER.to_owned())
...
impl IConversationRepository for MockRepo {
    async fn get(&self, id: &str) -> Result<Option<ConversationRow>, sparkfox_be_db::DbError> {
        let rows = self.rows.lock().unwrap();
        Ok(rows.iter().find(|r| r.id == id).cloned())
    }

    async fn create(&self, row: &ConversationRow) -> Result<String, sparkfox_be_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        if rows.iter().any(|existing| existing.id == row.id) {
            return Err(sparkfox_be_db::DbError::Conflict(format!(
                "Conversation {}",
                row.id
            )));
        }
    ...
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), sparkfox_be_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .iter_mut()
            .find(|r| r.id == id)
            .ok_or_else(|| sparkfox_be_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, id: &str) -> Result<(), sparkfox_be_db::DbError> {
        ...
        if rows.len() == len_before {
            return Err(sparkfox_be_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, sparkfox_be_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, sparkfox_be_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, sparkfox_be_db::DbError> {
        Ok(vec![])
    }
    ...
    async fn get_messages(
        ...
        page_size: u32,
        order: SortOrder,
    ) -> Result<PaginatedResult<MessageRow>, sparkfox_be_db::DbError> {
    ...
    async fn get_message(&self, conv_id: &str, message_id: &str) -> Result<Option<MessageRow>, sparkfox_be_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
    ...
    async fn insert_message(&self, message: &MessageRow) -> Result<(), sparkfox_be_db::DbError> {
        self.messages.lock().unwrap().push(message.clone());
        Ok(())
    }

    async fn update_message(&self, id: &str, updates: &MessageRowUpdate) -> Result<(), sparkfox_be_db::DbError> {
        let mut messages = self.messages.lock().unwrap();
        let message = messages
            .iter_mut()
            .find(|message| message.id == id)
            .ok_or_else(|| sparkfox_be_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, conv_id: &str) -> Result<(), sparkfox_be_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, sparkfox_be_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_artifacts(&self, conversation_id: &str) -> Result<Vec<ConversationArtifactRow>, sparkfox_be_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, sparkfox_be_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn mark_skill_suggest_artifacts_saved(
        ...
        cron_job_id: &str,
        updated_at: TimestampMs,
    ) -> Result<Vec<ConversationArtifactRow>, sparkfox_be_db::DbError> {
        let mut artifacts = self.artifacts.lock().unwrap();
    ...
    async fn delete_artifacts_by_conversation(&self, conversation_id: &str) -> Result<(), sparkfox_be_db::DbError> {
        self.artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, sparkfox_be_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
async fn create_rejects_workspace_with_trailing_whitespace_in_request() {
    ...
    let dir = std::env::temp_dir().join(format!("sparkfox-test-{}", sparkfox_be_common::generate_id()));
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, sparkfox_be_api_types::ConversationRuntimeStateKind::Idle);
...
async fn two_user_private_events_are_owner_scoped() {
    use sparkfox_be_db::IUserRepository;

    let (service, user_events, _repository, database) = make_sqlite_projection_service().await;
    let owner_b = sparkfox_be_db::SqliteUserRepository::new(database.pool().clone())
...
async fn delete_rejects_soft_deleted_execution_attempt_transcript() {
    ...
    let event = |event_type: AgentExecutionEventKind| NewAgentExecutionEvent {
        ...
        step_id: None,
        attempt_id: None,
        actor: sparkfox_be_common::AgentExecutionActor::system(),
        payload: "{}".to_owned(),
    };
...
impl AgentRuntimeControl for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            sparkfox_be_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
fn unique_test_dir(label: &str) -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    std::env::temp_dir().join(format!("sparkfox-{label}-{}-{nanos}", std::process::id()))
}
...
async fn send_message_stamps_channel_platform_for_channel_agent_conversation() {
    ...
    // A Channel Agent conversation (see sparkfox-be-channel's
...
async fn idempotent_send_replay_reuses_pending_turn_and_completed_receipt() {
    ...
    sparkfox_be_common::MessageId::parse(&user_messages[0].id)
...
async fn build_runtime_options_rebases_managed_workspace_after_restore() {
    let destination_root =
        std::env::temp_dir().join(format!("sparkfox-rebase-{}", sparkfox_be_common::generate_id()));
...
async fn build_runtime_options_preserves_explicit_custom_workspace() {
    let destination_root =
        std::env::temp_dir().join(format!("sparkfox-custom-{}", sparkfox_be_common::generate_id()));
    let custom_workspace =
        std::env::temp_dir().join(format!("sparkfox-project-{}", sparkfox_be_common::generate_id()));
...
async fn send_message_turn_writeback_runs_after_system_continuation_final_answer() {
    ...
    let knowledge_db = sparkfox_be_db::init_database_memory().await.unwrap();
    let knowledge_owner = sparkfox_be_db::installation_owner_id(knowledge_db.pool()).await.unwrap();
    let knowledge_repo: Arc<dyn sparkfox_be_db::IKnowledgeRepository> = Arc::new(
        sparkfox_be_db::SqliteKnowledgeRepository::new(knowledge_db.pool().clone()),
    );
...
async fn execution_cleanup_does_not_record_a_user_cancel_stamp() {
    ...
    let runtime_registry: Arc<dyn AgentRuntimeRegistry> = Arc::new(MockAgentRuntimeRegistry::new());
    let conv = svc.create(TEST_USER_1, make_create_req()).await.unwrap();
    let since = sparkfox_be_common::now_ms();
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = sparkfox_be_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
use sparkfox_be_common::ProviderWithModel;
use sparkfox_be_db::models::{ClientPreference, Provider};
use sparkfox_be_db::{
...
async fn seed_nomi_failover_conversation(
    ...
    let temp_workspace_id = sparkfox_be_common::generate_prefixed_id("ws");
...
async fn perform_model_failover_returns_none_for_acp_conversation() {
    ...
    let config = sparkfox_be_api_types::ModelFailoverConfig {
        enabled: true,
