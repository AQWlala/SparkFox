...
//!    形状抄 `sparkfox-be-idmm/service.rs` 的多字段 pref 先例),会话级可在
...
use sparkfox_be_api_types::{AgentErrorCode, HealthStatus, ModelFailoverConfig};
use sparkfox_be_common::{AppError, ErrorChain, ProviderId, ProviderWithModel};
use sparkfox_be_db::IClientPreferenceRepository;
...
/// 单厂商失败(限流 / 5xx / 网络 / 配置)。
///
/// 这张 matches 表是 `sparkfox-be-idmm/config.rs::is_provider_fault` 的**就地副本**:
/// 故障转移 seam 在 `sparkfox-be-conversation`,而 `sparkfox-be-idmm` 在其之上,直接依赖
...
impl ProviderAvailability {
    ...
    fn from_provider(provider: &Provider) -> Self {
        ...
        let model_health = provider
            ...
            .as_deref()
            .and_then(|s| {
                serde_json::from_str::<std::collections::HashMap<String, sparkfox_be_api_types::ModelHealthStatus>>(s).ok()
            })
...
mod tests {
    ...
    fn provider(
        ...
        let health_map: HashMap<String, sparkfox_be_api_types::ModelHealthStatus> = model_health
            .iter()
            .map(|(m, s)| {
                (
                    m.to_string(),
                    sparkfox_be_api_types::ModelHealthStatus {
                        status: *s,
                        last_check: None,
                        latency: None,
                        error: None,
