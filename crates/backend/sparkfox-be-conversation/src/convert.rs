...
use sparkfox_be_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
...
use sparkfox_be_db::MessageSearchRow;
use sparkfox_be_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
pub fn row_to_response_with_extra(
    ...
    let preset_snapshot = row
        .preset_snapshot
        .as_deref()
        .map(serde_json::from_str::<sparkfox_be_api_types::ResolvedPresetSnapshot>)
        .transpose()
    ...
    let execution_model_pool = row
        ...
        .as_deref()
        .map(serde_json::from_str::<sparkfox_be_api_types::ExecutionModelPool>)
        .transpose()
...
mod tests {
    use super::*;
    use sparkfox_be_common::{AgentType, ConversationId, ConversationSource, ConversationStatus};
    use serde_json::json;
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        ...
        assert_eq!(
            ...
            sparkfox_be_common::DelegationPolicy::PreferParallel
        );
        assert_eq!(
            item.conversation.execution_model_pool,
            Some(sparkfox_be_api_types::ExecutionModelPool::Range {
                models: vec![sparkfox_be_api_types::ExecutionModelRef {
        ...
        assert_eq!(
            ...
            sparkfox_be_common::DecisionPolicy::AskUser
        );
