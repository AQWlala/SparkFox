...
pub use sparkfox_be_extension::ResolvedAgentSkill;
...
/// Production adapter backed by `sparkfox_be_extension::skill_service`.
pub struct ExtensionSkillResolver {
    paths: Arc<sparkfox_be_extension::SkillPaths>,
}

impl ExtensionSkillResolver {
    pub fn new(paths: Arc<sparkfox_be_extension::SkillPaths>) -> Self {
        Self { paths }
    }
}
...
impl SkillResolver for ExtensionSkillResolver {
    async fn auto_inject_names(&self) -> Vec<String> {
        match sparkfox_be_extension::list_builtin_auto_skills(&self.paths).await {
            Ok(items) => {
    ...
    async fn resolve_skills(&self, names: &[String]) -> Vec<ResolvedAgentSkill> {
        ...
        match sparkfox_be_extension::materialize_skills_for_agent(&self.paths, "workspace-link", names).await {
            Ok(list) => list,
            Err(e) => {
                tracing::warn!(
                    error = %e,
    ...
    async fn link_workspace_skills(&self, workspace: &Path, rel_dirs: &[&str], skills: &[ResolvedAgentSkill]) -> usize {
        ...
        match sparkfox_be_extension::link_workspace_skills(workspace, rel_dirs, skills).await {
            Ok(n) => n,
            Err(e) => {
                tracing::warn!(
