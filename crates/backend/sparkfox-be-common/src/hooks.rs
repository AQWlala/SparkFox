...
//!
//! Hooks defined here let lower-layer crates (e.g. `sparkfox-be-ai-agent`,
//! `sparkfox-be-cron`) react to events owned by higher-layer crates (e.g.
//! `sparkfox-be-conversation`) without forming a dependency cycle.
...
/// crates react to a terminal going away without `sparkfox-be-terminal` depending
/// on them (e.g. `sparkfox-be-requirement` clears the dual-domain
...
/// IM → requirement pipeline). Lets `sparkfox-be-channel` file a message as a
/// requirement without depending on `sparkfox-be-requirement`; the concrete
/// implementor (in `sparkfox-be-requirement`) delegates to `RequirementService`.
