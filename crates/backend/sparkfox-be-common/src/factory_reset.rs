...
/// SQLite DB family (relative to data_dir). Mirrors `sparkfox-be-db`'s on-disk
...
/// Names are kept as literals here so `sparkfox-be-common` need not depend on the
/// domain crates; they mirror those crates' constants:
///   `knowledge`   → `sparkfox_be_knowledge::KB_MANAGED_REL_DIR`
///   `companion`         → `sparkfox_be_companion::PET_*_REL_DIR` (whole tree incl. `memory.db`)
///   `attachments` → `sparkfox_be_requirement` `ATTACHMENTS_REL_DIR`
///   `cron`        → parent of `sparkfox_be_cron::CRON_SKILLS_REL_DIR`
...
/// (mirrors `sparkfox-be-db`'s startup file-op retry for raw OS errors 5/32/33).
...
mod tests {
    ...
    fn no_marker_is_noop() {
        let dir = std::env::temp_dir().join(format!("sparkfox-fr-noop-{}", now_ms()));
    ...
    fn full_reset_wipes_targets_keeps_logs_and_clears_marker() {
        let dir = std::env::temp_dir().join(format!("sparkfox-fr-full-{}", now_ms()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn malformed_marker_still_triggers_reset() {
        let dir = std::env::temp_dir().join(format!("sparkfox-fr-bad-{}", now_ms()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn full_reset_clears_dir_config_override() {
        let dir = std::env::temp_dir().join(format!("sparkfox-fr-dircfg-{}", now_ms()));
        std::fs::create_dir_all(&dir).unwrap();
