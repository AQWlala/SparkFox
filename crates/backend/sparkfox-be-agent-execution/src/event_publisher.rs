...
use sparkfox_be_api_types::{AgentExecutionChangedEvent, WebSocketMessage};
use sparkfox_be_common::now_ms;
use sparkfox_be_db::IAgentExecutionRepository;
...
mod tests {
    ...
    async fn outbox_drain_targets_installation_owner_and_marks_every_event_published() {
        ...
        let installation_owner = sparkfox_be_db::installation_owner_id(database.pool()).await.unwrap();
        sqlx::query(
