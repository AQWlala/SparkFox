...
struct AttemptDelegationOperation<'a> {
    ...
    explicit_steps: Option<&'a [sparkfox_be_api_types::PlannedExecutionStep]>,
}
...
fn attempt_delegation_operation_id(
    ...
    explicit_steps: Option<&[sparkfox_be_api_types::PlannedExecutionStep]>,
...
impl AgentExecutionEngine {
    ...
    pub async fn delegate_from_attempt(
        ...
        requested_model_pool: Option<ExecutionModelPool>,
        explicit_steps: Option<Vec<sparkfox_be_api_types::PlannedExecutionStep>>,
        ...
        if detail.execution.delegation_policy == sparkfox_be_common::DelegationPolicy::Disabled {
            return Err(AppError::Conflict(
    ...
    pub async fn browse_workspace(
        ...
        sparkfox_be_file::list_workspace_level(std::path::Path::new(root), path, search)
    }
    ...
    async fn plan_initial(
        ...
        loop {
            ...
            match result {
                ...
                Err(sparkfox_be_db::DbError::Conflict(_)) => {
    ...
    async fn fail_planning(&self, owner_id: &str, execution_id: &str, reason: &str) {
        loop {
            ...
            .await {
                ...
                Err(sparkfox_be_db::DbError::Conflict(_)) => continue,
                Err(error) => {
...
fn runtime_participants_from_template(
    ...
    for row in rows {
        ...
        if let Some(capability) = row.capability.as_deref() {
            let _: sparkfox_be_api_types::ParticipantCapability =
                decode_json(capability, "template participant capability")?;
        }
        if let Some(constraints) = row.constraints.as_deref() {
            let constraints: sparkfox_be_api_types::ParticipantConstraints =
...
mod tests {
    ...
    use sparkfox_be_api_types::{ExecutionModelPool, ExecutionModelRef};
    use sparkfox_be_common::MAX_AGENT_EXECUTION_PARALLELISM;
