//! `sparkfox-be-public` — the **Remote 前门** (external companion surface).
//!
//! Projects the platform's single capability source of truth
//! (`sparkfox_be_gateway::Registry`) onto a network-reachable, companion-token-
...
//! live in `sparkfox-be-gateway`; adding a capability there makes it appear here
//! automatically (the inheritance guarantee — see the design spec §2.1). It MUST
//! be mounted in-process by `sparkfox-be-app` (the `server.lock` data-dir is
