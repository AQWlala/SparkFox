use sparkfox_be_db::models::ConversationRow;
use sparkfox_be_db::{
...
async fn test_database() -> sparkfox_be_db::Database {
...
async fn schema_guards_owner_authority_idempotency_and_terminal_reopen_boundaries() {
    ...
    let removed_execution_tree_columns: i64 = sparkfox_be_db::sqlx::query_scalar(
    ...
    assert!(
        sparkfox_be_db::sqlx::query("UPDATE conversations SET user_id = 'user_2' WHERE id = ?")
            .bind(&lead)
            .execute(database.pool())
            .await
            .is_err(),
    ...
    sparkfox_be_db::sqlx::query("UPDATE agent_executions SET status = 'running' WHERE id = ?")
        .bind(&first_id)
        .execute(database.pool())
        .await
        .unwrap();
    let current_leads: Vec<String> = sparkfox_be_db::sqlx::query_scalar(
...
async fn repository_and_schema_enforce_active_participant_limit() {
    ...
    assert!(
        ...
        .bind(&execution_id)
        .bind(sparkfox_be_common::now_ms())
        .execute(database.pool())
        .await
        .is_err(),
    ...
    let too_many_for_create: Vec<_> =
        (0..=sparkfox_be_common::MAX_AGENT_EXECUTION_PARTICIPANTS)
    ...
    .await
    .unwrap();
    let active_count: i64 = sparkfox_be_db::sqlx::query_scalar(
    ...
    assert_eq!(
        active_count,
        sparkfox_be_common::MAX_AGENT_EXECUTION_PARTICIPANTS as i64
    );
...
async fn running_attempt_appends_one_flat_dag_and_gates_only_pending_downstream() {
    ...
    let append_event = NewAgentExecutionEvent {
        ...
        step_id: None,
        attempt_id: None,
        actor: sparkfox_be_common::AgentExecutionActor::agent(
            &attempt_conversation,
...
async fn pause_atomically_interrupts_in_flight_work_and_fences_the_old_generation() {
    ...
    let first_expiry = sparkfox_be_common::now_ms() + 60_000;
    repository
    ...
    assert!(
        repository
            .try_acquire_lease(
                ...
                sparkfox_be_common::now_ms() + 60_000,
            )
            .await
            .unwrap()
            .is_none(),
...
async fn resuming_one_of_multiple_waiting_attempts_keeps_the_aggregate_waiting() {
    ...
    assert!(
        repository
            .mark_conversation_cleanup_completed(&pending[0].link_id, sparkfox_be_common::now_ms())
            .await
            .unwrap()
    );
    assert_eq!(
...
async fn manual_retry_clears_dispatch_gate_without_rewriting_prior_attempt() {
    ...
    repository
        ...
        .await
        .unwrap();
    let retry_after = sparkfox_be_common::now_ms() + 60_000;
    repository
        .settle_attempt(
            USER_ID,
            &execution_id,
...
async fn repository_and_schema_enforce_current_dag_limit_and_step_snapshot_immutability() {
    ...
    .await;
    let too_many = (0..=sparkfox_be_common::MAX_AGENT_EXECUTION_STEPS)
...
async fn event_actor_is_authorized_and_on_behalf_user_is_derived_atomically() {
    ...
    let execution_id = repository
        .create_execution_with_participants(
            ...
            &NewAgentExecutionEvent {
                ...
                step_id: None,
                attempt_id: None,
                actor: sparkfox_be_common::AgentExecutionActor::external_agent("companion_1"),
                payload: "{}".to_owned(),
            },
        )
        .await
    ...
    let agent_event = NewAgentExecutionEvent {
        ...
        step_id: None,
        attempt_id: None,
        actor: sparkfox_be_common::AgentExecutionActor::agent(lead_conversation_id.clone(), None),
        payload: "{}".to_owned(),
    };
    repository
        .update_execution(
    ...
    for actor in [
        sparkfox_be_common::AgentExecutionActor::external_agent("companion_spoof"),
        sparkfox_be_common::AgentExecutionActor::Agent {
        ...
        let spoofed = repository
            ...
            .await;
        assert!(matches!(spoofed, Err(sparkfox_be_db::DbError::Conflict(_))));
    }
    ...
    let denied = repository
        ...
        .await;
    assert!(matches!(denied, Err(sparkfox_be_db::DbError::Conflict(_))));
    let unchanged = repository
        .get_execution(USER_ID, &execution_id)
        .await
        .unwrap()
    ...
    let execution_id = repository
        .create_execution_with_participants(
            ...
            &NewAgentExecutionEvent {
                ...
                step_id: None,
                attempt_id: None,
                actor: sparkfox_be_common::AgentExecutionActor::external_agent("companion_root"),
                payload: "{}".to_owned(),
            },
        )
        .await
