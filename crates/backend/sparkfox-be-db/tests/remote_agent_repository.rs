...
async fn repo() -> (Arc<dyn IRemoteAgentRepository>, sparkfox_be_db::Database) {
...
async fn update_status_to_connected_with_timestamp() {
    ...
    let ts = sparkfox_be_common::now_ms();
...
async fn full_crud_lifecycle() {
    ...
    // Update status
    r.update_status(&created.id, "connected", Some(sparkfox_be_common::now_ms()))
        .await
        .unwrap();
