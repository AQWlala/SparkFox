...
async fn insert_provider(database: &sparkfox_be_db::Database, id: &str) {
...
async fn provider_bindings_are_validated_and_delete_is_atomic_after_a_stale_scan() {
    ...
    let owner = sparkfox_be_db::installation_owner_id(database.pool()).await.unwrap();
    ...
    sparkfox_be_db::sqlx::query("DELETE FROM providers WHERE id = 'prov_0190f5fe-7c00-7a00-8000-000000000002'")
        .execute(database.pool())
        .await
        .unwrap();
    ...
    let failover: String = sparkfox_be_db::sqlx::query_scalar(
    ...
    let collaboration_models: String = sparkfox_be_db::sqlx::query_scalar(
    ...
    sparkfox_be_db::sqlx::query("UPDATE conversations SET model = NULL WHERE id = ?")
...
async fn provider_delete_keeps_empty_collaboration_models_preference_as_an_array() {
    ...
    let value: String = sparkfox_be_db::sqlx::query_scalar(
        "SELECT value FROM client_preferences WHERE key = 'nomi.collaborationModels'",
    )
    .fetch_one(database.pool())
    .await
