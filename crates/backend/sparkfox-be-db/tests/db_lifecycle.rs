...
async fn installation_owner_is_a_canonical_user() {
    ...
    sparkfox_be_common::UserId::parse(row.get::<String, _>("id")).unwrap();
...
async fn deterministic_memory_fixture_records_the_requested_canonical_owner() {
    let requested = sparkfox_be_common::UserId::new();
...
async fn installation_owner_has_valid_timestamps() {
    let before = sparkfox_be_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = sparkfox_be_common::now_ms();
...
async fn installation_identity_is_one_immutable_singleton() {
    ...
    let replacement = sparkfox_be_common::UserId::new().into_string();
    sqlx::query(
        "INSERT INTO users (id, username, password_hash, created_at, updated_at) \
