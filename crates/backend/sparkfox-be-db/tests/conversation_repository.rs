...
use sparkfox_be_common::{ConversationArtifactId, ConversationId, CronJobId, MessageId};
...
async fn setup() -> (SqliteConversationRepository, sparkfox_be_db::Database) {
...
fn make_artifact(conv_id: &str) -> sparkfox_be_db::ConversationArtifactRow {
    let cron_job_id = CronJobId::parse(ARTIFACT_CRON_JOB_ID)
        .expect("canonical artifact cron fixture")
        .into_string();
    sparkfox_be_db::ConversationArtifactRow {
...
async fn internal_creation_and_delivery_operations_are_durable_and_immutable() {
    ...
    let accepted_at = sparkfox_be_common::now_ms();
    ...
    assert!(sparkfox_be_common::MessageId::parse(&accepted.message_id).is_ok());
...
async fn assistant_message_projection_is_atomic_idempotent_and_owner_scoped() {
    ...
    let invalid_shape = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(invalid_shape, sparkfox_be_db::DbError::Conflict(_)));
    ...
    let invalid_kind = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(invalid_kind, sparkfox_be_db::DbError::Conflict(_)));
    ...
    let payload_conflict = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(payload_conflict, sparkfox_be_db::DbError::Conflict(_)));
    ...
    let non_owner = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(non_owner, sparkfox_be_db::DbError::NotFound(_)));
    ...
    let missing = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(missing, sparkfox_be_db::DbError::NotFound(_)));
}
...
async fn filter_by_source_and_pinned_combined() {
    ...
    let mut c1 = make_conversation("sparkfox-pinned");
    ...
    c2.pinned = true;
    c2.pinned_at = Some(sparkfox_be_common::now_ms());
    repo.create(&c2).await.unwrap();

    let mut c3 = make_conversation("sparkfox-unpinned");
...
async fn update_nonexistent_conversation_returns_not_found() {
    ...
    let err = repo
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, sparkfox_be_db::DbError::NotFound(_)));
}
