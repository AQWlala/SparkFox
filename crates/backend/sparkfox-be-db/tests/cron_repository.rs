...
use sparkfox_be_db::models::{CronJobRow, CronJobRunRow};
use sparkfox_be_db::{DbError, ICronRepository, SqliteCronRepository, UpdateCronJobParams};
...
async fn init_database_memory() -> Result<sparkfox_be_db::Database, sparkfox_be_db::DbError> {
    sparkfox_be_db::init_database_memory_with_owner(
        sparkfox_be_common::UserId::parse(INSTALLATION_OWNER.to_owned())
            .expect("canonical fixture owner"),
    )
    .await
}

async fn repo() -> (Arc<dyn ICronRepository>, sparkfox_be_db::Database) {
