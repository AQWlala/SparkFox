use sparkfox_be_common::{ConversationId, generate_prefixed_id};
...
use sparkfox_be_db::init_database;
use serde_json::json;
...
async fn complete_bundle_captures_key_companion_and_managed_workspaces_only() {
    ...
    std::fs::create_dir_all(
        work_dir
            ...
            .join("sparkfox-temp-ws_test"),
    )
    .unwrap();
    std::fs::write(
        work_dir
            .join(MANAGED_WORKSPACES_DIR)
            .join("sparkfox-temp-ws_test/output.txt"),
        b"managed workspace",
    )
    .unwrap();
    ...
    assert!(paths.contains(&format!(
        "{BUNDLE_WORK_DIR}/{MANAGED_WORKSPACES_DIR}/sparkfox-temp-ws_test/output.txt"
    )));
...
async fn offline_restore_preserves_entity_ids_and_rotates_dataset_generation() {
    ...
    let database = init_database(&source).await.unwrap();
    let source_owner = sparkfox_be_db::installation_owner_id(database.pool()).await.unwrap();
    let conversation_id = ConversationId::new().into_string();
    sqlx::query(
...
async fn restore_rejects_noncanonical_row_ids_after_checksum_rewrite() {
    ...
    let bundle = dir.path().join("backup.nomifun");
    let database = init_database(&dir.path().join("source.db")).await.unwrap();
    let owner = sparkfox_be_db::installation_owner_id(database.pool())
        .await
        .unwrap();
    sqlx::query(
        "INSERT INTO conversations \
