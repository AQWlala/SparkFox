...
impl IChannelRepository for SqliteChannelRepository {
    ...
    async fn update_user_last_active(&self, id: &str, last_active: sparkfox_be_common::TimestampMs) -> Result<(), DbError> {
    ...
    async fn update_session_activity(
        &self,
        id: &str,
        last_activity: sparkfox_be_common::TimestampMs,
    ) -> Result<(), DbError> {
    ...
    async fn cleanup_expired_pairings(&self, now: sparkfox_be_common::TimestampMs) -> Result<u64, DbError> {
        let result = sqlx::query(
...
mod tests {
    ...
    async fn upsert_plugin_updates_existing() {
        ...
        let updated = ChannelPluginRow {
            ...
            enabled: true,
            updated_at: sparkfox_be_common::now_ms(),
            ..plugin
        };
    ...
    async fn same_platform_user_two_channels_are_independent() {
        ...
        let mk = |id: &str, chan: &str| ChannelUserRow {
            ...
            display_name: None,
            authorized_at: sparkfox_be_common::now_ms(),
            last_active: None,
            session_id: None,
        };
    ...
    async fn update_user_last_active_updates_timestamp() {
        ...
        let new_ts = sparkfox_be_common::now_ms() + 5000;
    ...
    async fn update_session_conversation_persists() {
        let conversation_id = sparkfox_be_common::ConversationId::new().into_string();
    ...
    async fn update_session_conversation_not_found() {
        let (repo, _db) = setup().await;
        let missing_id = sparkfox_be_common::ConversationId::new();
        let err = repo
