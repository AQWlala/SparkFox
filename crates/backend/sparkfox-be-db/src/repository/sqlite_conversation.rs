...
use sparkfox_be_common::{MessageId, PaginatedResult, ProviderWithModel};
...
impl IConversationRepository for SqliteConversationRepository {
    ...
    async fn claim_message_correlation(
        ...
        sparkfox_be_common::ConversationId::parse(conversation_id)
    ...
    async fn list_mcp_server_ids(&self, conversation_id: &str) -> Result<Vec<sparkfox_be_common::McpServerId>, DbError> {
        ...
        ids.into_iter()
            .map(|id| sparkfox_be_common::McpServerId::parse(id).map_err(|e| DbError::Init(e.to_string())))
            .collect()
    }

    async fn set_mcp_server_ids(&self, conversation_id: &str, ids: &[sparkfox_be_common::McpServerId]) -> Result<(), DbError> {
        let mut tx = self.pool.begin().await?;
...
mod tests {
    ...
    async fn init_database_memory() -> Result<crate::Database, crate::DbError> {
        crate::init_database_memory_with_owner(
            sparkfox_be_common::UserId::parse(TEST_INSTALLATION_OWNER.to_owned())
    ...
    fn sample_message(conv_id: impl Into<String>) -> MessageRow {
        let now = sparkfox_be_common::now_ms();
        MessageRow {
            id: sparkfox_be_common::generate_prefixed_id("msg"),
    ...
    async fn insert_mcp_server(pool: &SqlitePool, name: &str) -> sparkfox_be_common::McpServerId {
        let id = sparkfox_be_common::McpServerId::new();
        let now = sparkfox_be_common::now_ms();
        sqlx::query(
    ...
    fn sample_artifact(conversation_id: impl Into<String>, kind: &str, cron_job_id: Option<&str>) -> ConversationArtifactRow {
        ...
        ConversationArtifactRow {
            id: sparkfox_be_common::ConversationArtifactId::new().into_string(),
    ...
    async fn link_lead_and_attempt_conversations(
        ...
        let now = sparkfox_be_common::now_ms();
        let execution_id = sparkfox_be_common::AgentExecutionId::new().into_string();
        let participant_id = sparkfox_be_common::AgentExecutionParticipantId::new().into_string();
        let step_id = sparkfox_be_common::AgentExecutionStepId::new().into_string();
        let attempt_id = sparkfox_be_common::AgentExecutionAttemptId::new().into_string();
        ...
        .bind(sparkfox_be_common::ConversationExecutionLinkId::new().into_string())
    ...
    async fn create_and_get_conversation() {
        ...
        assert!(sparkfox_be_common::ConversationId::parse(conv.id.clone()).is_ok());
    ...
    async fn update_conversation_pinned() {
        ...
        let pin_time = sparkfox_be_common::now_ms();
        repo.update(
            &conv.id,
            &ConversationRowUpdate {
                pinned: Some(true),
    ...
    async fn list_filter_by_pinned() {
        ...
        let mut c1 = sample_conversation(TEST_INSTALLATION_OWNER);
        c1.pinned = true;
        c1.pinned_at = Some(sparkfox_be_common::now_ms());
        repo.create(&c1).await.unwrap();
    ...
    async fn cron_job_id_roundtrips_as_column() {
        ...
        repo.update(
            ...
            &ConversationRowUpdate {
                ...
                updated_at: Some(sparkfox_be_common::now_ms()),
                ..Default::default()
            },
        )
        .await
    ...
    async fn cron_trigger_artifacts_insert_distinct_rows() {
        ...
        assert!(sparkfox_be_common::ConversationArtifactId::parse(a1.id.clone()).is_ok());
        assert!(sparkfox_be_common::ConversationArtifactId::parse(a2.id.clone()).is_ok());
    ...
    async fn get_and_update_artifact_status_by_i64_id() {
        ...
        let updated = repo
            .update_artifact_status(&conv.id, &inserted.id, "dismissed", sparkfox_be_common::now_ms())
            .await
            .unwrap()
            .unwrap();
        ...
        // Missing id → None.
        let missing = repo
            .update_artifact_status(&conv.id, "artifact_019abcdef012-7abc-8abc-0123-456789abcdee", "dismissed", sparkfox_be_common::now_ms())
            .await
            .unwrap();
    ...
    async fn get_messages_pagination() {
        ...
        for i in 0..10 {
            ...
            msg.id = sparkfox_be_common::generate_prefixed_id("msg");
    ...
    async fn search_messages_by_keyword() {
        ...
        msg2.id = sparkfox_be_common::generate_prefixed_id("msg");
