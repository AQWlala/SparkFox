    ...
    for (table, column) in ENTITY_OR_NATURAL_KEY_UNION_COLUMNS {
        ...
        for value in values {
            ...
            if value.starts_with("presettag_") {
                sparkfox_be_common::validate_prefixed_id(&value, "presettag").map_err(|error| {
                    DbError::Init(format!(
...
fn validate_generic_entity_id(value: &str) -> Result<(), String> {
    ...
    sparkfox_be_common::validate_id_prefix(prefix).map_err(|error| error.to_string())?;
    sparkfox_be_common::validate_uuidv7(uuid_body).map_err(|error| error.to_string())?;
    Ok(())
}
