...
//! still can, via the `SPARKFOX_BUILTIN_PRESETS_PATH` env var → disk
//! fallback path.
...
/// `crates/sparkfox-be-app/assets/builtin-presets/`.
static BUILTIN_ASSETS: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../sparkfox-be-app/assets/builtin-presets");
...
/// `SPARKFOX_BUILTIN_PRESETS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinPresetRegistry {
    ...
    ///
    /// If `SPARKFOX_BUILTIN_PRESETS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("SPARKFOX_BUILTIN_PRESETS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "SPARKFOX_BUILTIN_PRESETS_PATH points to missing directory; \
    ...
    /// `SPARKFOX_BUILTIN_PRESETS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // SPARKFOX_BUILTIN_PRESETS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "SPARKFOX_BUILTIN_PRESETS_PATH";
