...
use sparkfox_be_common::{AgentId, AppError, KnowledgeBaseId, PresetId, PresetTagId, ProviderId};
use sparkfox_be_db::{
...
use sparkfox_be_extension::{ExtensionRegistry, ResolvedPreset};

use crate::builtin::{AvatarAsset, BuiltinPreset, BuiltinPresetRegistry};
use sparkfox_be_extension::{PresetClassifier, PresetRuleDispatcher};
...
impl PresetService {
    ...
    fn builtin_response(&self, item: &BuiltinPreset, state: Option<&sparkfox_be_db::PresetUserStateRow>) -> PresetResponse {
...
fn apply_state(response:&mut PresetResponse,state:Option<&sparkfox_be_db::PresetUserStateRow>){if let Some(s)=state{response.enabled=s.enabled;response.auto_selectable=s.auto_selectable;response.preferred_agent_id=s.preferred_agent_id.clone();response.sort_order=s.sort_order;response.last_used_at=s.last_used_at}}
...
fn collect_examples_i18n(rows:&[sparkfox_be_db::PresetExampleRow])->HashMap<String,Vec<String>>{let mut output=HashMap::new();for row in rows.iter().filter(|row|!row.locale.is_empty()){output.entry(row.locale.clone()).or_insert_with(Vec::new).push(row.prompt.clone());}output}
