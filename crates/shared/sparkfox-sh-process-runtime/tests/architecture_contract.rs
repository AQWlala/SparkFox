...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .ancestors()
        .nth(3)
        .expect("sparkfox-sh-process-runtime should live three levels below the workspace root")
        .to_path_buf()
}
...
fn process_runtime_is_registered_and_backend_neutral() {
    ...
    assert!(
        workspace_compact.contains(
            "sparkfox-sh-process-runtime={path=\"crates/shared/sparkfox-sh-process-runtime\"}"
        ),
        "workspace dependencies must register sparkfox-sh-process-runtime"
    );
    assert!(
    ...
    let manifest = read_workspace("crates/shared/sparkfox-sh-process-runtime/Cargo.toml");
    let forbidden = [
        "nomifun-",
        "sparkfox-ag-types",
        "sparkfox-ag-agent",
    ...
    assert!(
        present.is_empty(),
        "sparkfox-sh-process-runtime must remain backend-neutral: {present:?}"
    );
}
...
fn platform_module_is_private_and_owns_os_process_primitives() {
    let lib = without_whitespace(&production_source(
        "crates/shared/sparkfox-sh-process-runtime/src/lib.rs",
    ));
    assert!(lib.contains("modplatform;"));
    assert!(!lib.contains("pubmodplatform;"));

    let src = workspace_root().join("crates/shared/sparkfox-sh-process-runtime/src");
    ...
    assert!(
        escaped.is_empty(),
        "OS ownership primitives escaped sparkfox-sh-process-runtime/src/platform: {escaped:?}"
    );
}
...
fn runtime_path_support_and_tools_depend_on_the_process_runtime() {
    let runtime_manifest = without_whitespace(&read_workspace(
        "crates/backend/sparkfox-be-runtime/Cargo.toml",
    ));
    assert!(
        runtime_manifest.contains("sparkfox-sh-process-runtime.workspace=true"),
        "sparkfox-be-runtime must depend on sparkfox-sh-process-runtime"
    );
    let tools_manifest =
        without_whitespace(&read_workspace("crates/agent/sparkfox-ag-tools/Cargo.toml"));
    assert!(
        tools_manifest.contains("sparkfox-sh-process-runtime.workspace=true"),
        "sparkfox-ag-tools must depend on sparkfox-sh-process-runtime"
    );

    assert!(
        !workspace_root()
            .join("crates/backend/sparkfox-be-runtime/src/spawn.rs")
            .exists(),
        "sparkfox-be-runtime must not retain a child-process facade"
    );
    let shell_env = without_whitespace(&production_source(
        "crates/backend/sparkfox-be-runtime/src/shell_env.rs",
    ));
    assert!(
        shell_env.contains("sparkfox_sh_process_runtime::merge_process_path("),
        "sparkfox-be-runtime PATH merging must delegate to sparkfox-sh-process-runtime"
    );
}
...
fn bootstrap_creates_one_supervisor_and_shares_it_with_all_command_tools() {
    let bootstrap = without_whitespace(&production_source(
        "crates/agent/sparkfox-ag-agent/src/bootstrap.rs",
    ));
    assert_eq!(
        count(
            &bootstrap,
            "sparkfox_sh_process_runtime::ProcessSupervisor::new("
        ),
        1,
        "bootstrap must create exactly one ProcessSupervisor"
    );
    for consumer in [
        "sparkfox_ag_tools::bash::BashTool::new(Arc::clone(&process_supervisor),",
        "sparkfox_ag_tools::exec_command::ExecCommandTool::new(Arc::clone(&process_supervisor),",
        "sparkfox_ag_tools::write_stdin::WriteStdinTool::new(Arc::clone(&process_supervisor),",
    ...
    assert_eq!(
        count(
            &bootstrap,
            "sparkfox_ag_tools::process_store::ProcessStore::new("
        ),
        1,
...
fn mcp_routing_uses_origin_stable_reserved_names_without_a_collision_snapshot() {
    ...
    let proxy = without_whitespace(&production_source(
        "crates/agent/sparkfox-ag-mcp/src/tool_proxy.rs",
    ));
    assert!(
...
fn retired_pty_modules_and_portable_pty_are_test_only() {
    let lib_path = "crates/agent/sparkfox-ag-tools/src/lib.rs";
    ...
    for module in ["pty", "persistent_shell"] {
        ...
        assert!(
            workspace_root()
                .join(format!("crates/agent/sparkfox-ag-tools/src/{module}.rs"))
                .is_file(),
            "expected retired test-only source {module}.rs"
        );
    }

    let manifest = read_workspace("crates/agent/sparkfox-ag-tools/Cargo.toml");
    ...
    assert!(
        ...
        "portable-pty must appear only in sparkfox-ag-tools dev-dependencies, found {sections:?}"
    );
}

#[test]
fn numeric_process_store_contains_metadata_but_no_process_owner() {
    let path = "crates/agent/sparkfox-ag-tools/src/process_store.rs";
