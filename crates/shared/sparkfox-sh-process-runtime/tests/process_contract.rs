...
#[cfg(target_os = "macos")]
use sparkfox_sh_process_runtime::SandboxPolicy;
#[cfg(windows)]
use sparkfox_sh_process_runtime::ShellKind;
...
async fn invalid_executable_is_a_stable_spawn_failure_without_a_session() {
    let supervisor = ProcessSupervisor::new(SupervisorConfig::default());
    let missing = Path::new("/definitely/not/a/sparkfox-executable");
...
async fn windows_invalid_executable_is_a_stable_spawn_failure_without_a_session() {
    let directory = tempfile::tempdir().expect("temporary directory should be created");
    let missing = directory.path().join("definitely-missing-sparkfox-executable.exe");
    let supervisor = ProcessSupervisor::new(SupervisorConfig::default());
...
async fn windows_preserves_complex_unicode_argv_environment_and_cwd() {
    ...
    let env_key = OsString::from("SPARKFOX_WINDOWS_ENV_CASE");
