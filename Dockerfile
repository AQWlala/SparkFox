...
# ============================================================================
# sparkfox-web — headless WebUI server image (no GUI / WebView; runs anywhere)
#   Stage 1  builds the React SPA (ui/dist)
#   Stage 2  compiles the sparkfox-web Rust binary
...
# first reachable browser creates the admin account unless SPARKFOX_ADMIN_PASSWORD
...
COPY --from=ui /app/ui/dist/sparkfox-build.json /src/ui/dist/sparkfox-build.json
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked -p sparkfox-web \
    && cp target/release/sparkfox-web /usr/local/bin/sparkfox-web
# -> /usr/local/bin/sparkfox-web
...
COPY --from=rust /usr/local/bin/sparkfox-web /usr/local/bin/sparkfox-web
COPY --from=ui   /app/ui/dist                    /opt/sparkfox/web

ENV SPARKFOX_WEB_HOST=0.0.0.0 \
    SPARKFOX_WEB_PORT=8787 \
    SPARKFOX_DATA_DIR=/data \
    SPARKFOX_WEB_DIST=/opt/sparkfox/web \
    SHELL=/bin/bash
# Set SPARKFOX_HTTPS=true when a TLS proxy fronts the app (makes cookies Secure).
# Set SPARKFOX_ADMIN_PASSWORD (+ SPARKFOX_ADMIN_USERNAME) to pre-seed the admin
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=40s --retries=3 \
  CMD bun -e "fetch('http://127.0.0.1:'+(process.env.SPARKFOX_WEB_PORT||'8787')+'/').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
CMD ["sparkfox-web"]
