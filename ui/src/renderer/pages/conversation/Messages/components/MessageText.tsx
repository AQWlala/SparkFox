...
import { parseCitations, CitationChip, type CitationData } from '@/renderer/pages/conversation';
...
const MessageText: React.FC<{ message: IMessageText; hideActions?: boolean }> = ({ message, hideActions = false }) => {
  ...
  // F2 引用 chip 集成 —— 解析 LLM 回复中的 [citation:...] 标记
  // 仅对 agent markdown 内容解析引用（用户消息/JSON 不含 citation 标记），
  // 原始文本渲染保留，仅新增引用解析 + chip 渲染。
  const parsedCitations = useMemo<{ text: string; citations: CitationData[] }>(() => {
    if (shouldRenderPlainText || json || typeof data !== 'string') {
      return { text: typeof data === 'string' ? data : '', citations: [] };
    }
    return parseCitations(data);
  }, [data, json, shouldRenderPlainText]);
            ...
            {shouldRenderPlainText ? (
              ...
              <div data-testid='message-text-content'>
                <MarkdownView codeStyle={CODE_STYLE} fontSize={MESSAGE_BODY_FONT_SIZE} lineHeight={MESSAGE_BODY_LINE_HEIGHT}>
                  {parsedCitations.text}
                </MarkdownView>
                {parsedCitations.citations.length > 0 && (
                  <div className='mt-4px flex flex-wrap gap-4px'>
                    {parsedCitations.citations.map((citation, idx) => (
                      <CitationChip
                        key={`${citation.kdoc_id}-${citation.chunk_id}-${idx}`}
                        citation={citation}
                      />
                    ))}
                  </div>
