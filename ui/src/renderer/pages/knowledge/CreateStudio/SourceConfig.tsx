...
// ─── F3 文件拖拽支持类型（spec 2048-2066）───────────────────────────────────
// 知识库创建支持拖入的文件扩展名白名单（local / import 之外的通用文件拖拽校验）
// 注意：.xls（BIFF 格式）v1.0.0 不支持，仅支持 .xlsx（Office Open XML）
...
/**
 * handleFileDrop —— 拖拽文件类型校验
 *
 * @param file 用户拖入的文件
 * @returns true=接受；false=拒绝（已通过 Message.warning 提示原因）
 *
 * 与 spec 2058-2066 行一致；导出供后续拖拽 UI（onDrop / onDragOver）接入。
 */
export const handleFileDrop = (file: File): boolean => {
