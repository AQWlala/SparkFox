...
// F5 检索模式切换（spec 2130-2141）—— 三选：全文 / 语义 / 混合
...
const KnowledgeListPage: React.FC = () => {
  ...
  // F5 检索模式（默认 hybrid，与 spec 2133 行一致）
    ...
    >
      <div className='mx-auto flex w-full max-w-1180px box-border flex-col gap-16px'>
        ...
        {error ? (
          ...
          <>
            {/* F5 检索模式切换（spec 2130-2141）—— 全文 / 语义 / 混合 */}
            <div className='flex items-center gap-8px'>
              <RadioGroup
                value={searchMode}
                onChange={(v: unknown) => setSearchMode(v as SearchMode)}
                type='button'
              >
                <Radio value='fulltext'>{t('knowledge.searchMode.fulltext', { defaultValue: '全文匹配' })}</Radio>
                <Radio value='semantic'>{t('knowledge.searchMode.semantic', { defaultValue: '语义匹配' })}</Radio>
                <Radio value='hybrid'>{t('knowledge.searchMode.hybrid', { defaultValue: '混合检索' })}</Radio>
              </RadioGroup>
            </div>
