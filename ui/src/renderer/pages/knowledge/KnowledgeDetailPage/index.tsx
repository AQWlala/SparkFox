...
import type { ExtractionStatus } from '@renderer/components/extraction/constants';
...
const KnowledgeDetailPage: React.FC = () => {
  ...
  // F4 向量化进度 + 重建索引（spec 2086-2109）—— mock 进度，后续接 IPC 触发真实重建
  const [vectorizationProgress, setVectorizationProgress] = useState(0);
  const [isRebuilding, setIsRebuilding] = useState(false);

  // ─── SAG 5 状态机联动（v1.1.0 第十一波集成 / spec §三 10.11.1） ─────────────
  // PoC：本地 mock SAG 提取状态，演示 5 状态机跳变 + 事件/实体计数
  // 生产环境：从后端 knowledge_event.status + count(*) 查询读取
  const [extractionStatus, setExtractionStatus] = useState<ExtractionStatus>('COMPLETED');
  const [extractionEventCount, setExtractionEventCount] = useState(0);
  const [extractionEntityCount, setExtractionEntityCount] = useState(0);

  const handleToggleExtractionStatus = () => {
    // PoC：循环演示 5 状态机
    const order: ExtractionStatus[] = ['PENDING', 'PARSING', 'PARSED', 'EXTRACTING', 'COMPLETED'];
    const idx = order.indexOf(extractionStatus);
    const next = order[(idx + 1) % order.length];
    setExtractionStatus(next);
    if (next === 'EXTRACTING') {
      setExtractionEventCount(42);
      setExtractionEntityCount(128);
    } else if (next === 'COMPLETED') {
      setExtractionEventCount(56);
      setExtractionEntityCount(167);
    }
  };
  ...
  const handleRebuildIndex = async () => {
    ...
    // v1.1.0 第十一波：重建索引联动 SAG 状态机（PENDING→PARSING→PARSED→EXTRACTING→COMPLETED）
    const sequence: Array<{ status: ExtractionStatus; event?: number; entity?: number }> = [
      { status: 'PENDING' },
      { status: 'PARSING' },
      { status: 'PARSED' },
      { status: 'EXTRACTING', event: 30, entity: 80 },
      { status: 'EXTRACTING', event: 42, entity: 128 },
      { status: 'COMPLETED', event: 56, entity: 167 },
    ];
    for (const step of sequence) {
      setExtractionStatus(step.status);
      if (step.event !== undefined) setExtractionEventCount(step.event);
      if (step.entity !== undefined) setExtractionEntityCount(step.entity);
      setVectorizationProgress(
        step.status === 'PENDING' ? 10 :
        step.status === 'PARSING' ? 30 :
        step.status === 'PARSED' ? 50 :
        step.status === 'EXTRACTING' ? 80 : 100
      );
      ...
      <div className='mx-auto flex w-full max-w-1180px box-border flex-col gap-16px'>
        ...
        <Tabs className='knowledge-detail-tabs' activeTab={activeTab} onChange={(k) => setTab(k)} type='line'>
          ...
          <Tabs.TabPane key='docs' title={t('knowledge.detail.tabDocs', { defaultValue: '文档' })}>
            ...
            {/* F4 向量化进度 + 重建索引（spec 2086-2109）+ v1.1.0 SAG 5 状态机 */}
            <div className='mt-12px flex flex-col gap-8px'>
              <ExtractionProgressCard
                status={extractionStatus}
                event_count={extractionEventCount}
                entity_count={extractionEntityCount}
              />
              ...
              <div className='flex items-center gap-8px'>
                ...
                >
                  {t('knowledge.detail.rebuildIndex', { defaultValue: '重建索引' })}
                </Button>
                {/* PoC 演示按钮：手动切换 SAG 状态机 */}
                <Button
                  size='small'
                  type='secondary'
                  onClick={handleToggleExtractionStatus}
                  title='PoC 演示：循环切换 SAG 5 状态'
                >
                  切换状态（{extractionStatus}）
                </Button>
              </div>
            </div>
          </Tabs.TabPane>
