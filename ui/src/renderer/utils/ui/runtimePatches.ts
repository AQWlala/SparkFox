...
declare global {
  interface Window {
    __SparkSafeResizeObserver__?: boolean;
    __SparkResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__SparkSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__SparkSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__SparkResizeObserverPatched__) {
    ...
    window.__SparkResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__SparkResizeObserverPatched__) {
    ...
    console.__SparkResizeObserverPatched__ = true;
  }
};
