...
const RAIL_WIDTH_STORAGE_KEY = 'sparkfox:rail-width';
...
const Layout: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      ...
      setTimeout(() => {
        window.dispatchEvent(new CustomEvent('sparkfox-open-update-modal', { detail: { source: 'tray' } }));
      }, 100);
    };
    ...
    void (async () => {
      try {
        ...
        if (!cancelled && res?.success && res.data?.updateInfo) {
          ...
          window.dispatchEvent(new CustomEvent('sparkfox-open-update-modal', { detail: { source: 'startup' } }));
                    ...
                    <button
                      ...
                      onClick={() => {
                        window.dispatchEvent(
                          new CustomEvent('sparkfox-open-update-modal', { detail: { source: 'sidebar' } })
                        );
                      }}
                    >
