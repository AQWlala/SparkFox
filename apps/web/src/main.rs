//! `sparkfox-web` — the standalone Web host for the browser deployment.
//!
//! In the unified architecture there is ONE Rust backend (`sparkfox-be-app`, the
...
const ENV_INSECURE_NO_AUTH: &str = "SPARKFOX_WEB_INSECURE_NO_AUTH";
...
struct Args {
    ...
    #[arg(long, env = "SPARKFOX_WEB_HOST", default_value = "127.0.0.1")]
    host: String,
    /// Port to listen on (serves both the API and the SPA).
    #[arg(long, env = "SPARKFOX_WEB_PORT", default_value_t = 8787)]
    port: u16,
    ...
    /// Windows, see `sparkfox_be_app::cli::default_data_dir`) so every host and
    ...
    #[arg(
        long,
        env = "SPARKFOX_DATA_DIR",
        default_value_os_t = sparkfox_be_app::cli::default_data_dir(),
        value_parser = sparkfox_be_app::cli::parse_non_empty_path
    )]
    data_dir: PathBuf,
    /// Directory containing the built SPA (ui/dist).
    #[arg(long, env = "SPARKFOX_WEB_DIST", default_value = "../../ui/dist")]
    dist: PathBuf,
    ...
    /// Can also be enabled via `SPARKFOX_WEB_INSECURE_NO_AUTH=true`.
    #[arg(long)]
    insecure_no_auth: bool,
    /// Initial admin username provisioned on first run (authenticated mode only).
    /// Ignored once an admin exists.
    #[arg(long, env = "SPARKFOX_ADMIN_USERNAME", default_value = "admin")]
    admin_user: String,
    ...
    #[arg(long, env = "SPARKFOX_ADMIN_PASSWORD")]
...
fn validate_static_dist(args: &Args) -> Result<Option<sparkfox_be_app::bootstrap::UiBuildManifest>> {
    validate_static_dist_with_build_id(args, option_env!("SPARKFOX_FRONTEND_BUILD_ID"))
}

fn validate_static_dist_with_build_id(
    args: &Args,
    expected_build_id: Option<&str>,
) -> Result<Option<sparkfox_be_app::bootstrap::UiBuildManifest>> {
    if args.api_only {
        return Ok(None);
    }

    let expected_build_id = expected_build_id.context(
        "this sparkfox-web binary has no exact frontend build identity and cannot serve static assets; run `bun run serve:web` (or build with `--features static-webui`) after `bun run build:ui`",
    )?;

    sparkfox_be_app::bootstrap::validate_webui_dist(
        &args.dist,
...
fn main() -> Result<ExitCode> {
    ...
    if let Some(code) = sparkfox_be_app::commands::run_mcp_stdio_subcommand_if_present() {
        return Ok(code);
    }
    ...
    let mut cli = sparkfox_be_app::cli::Cli::parse_from(["sparkfox-web"]);
    ...
    sparkfox_be_runtime::init(&cli.data_dir);
    // SAFETY: called before the tokio runtime (and its threads) is built.
    let merged_path = unsafe { sparkfox_be_runtime::enhance_process_path() };
...
async fn serve(cli: sparkfox_be_app::cli::Cli, merged_path: String, args: Args) -> Result<ExitCode> {
    ...
    let env = sparkfox_be_app::bootstrap::init_environment(&cli, &merged_path)?;
    let database = sparkfox_be_app::bootstrap::init_data_layer(&env.config).await?;
    let services = sparkfox_be_app::AppServices::from_config(database, &env.config).await?;
    ...
    let needs_first_run_setup = sparkfox_be_app::bootstrap::ensure_admin_credentials(
        &services,
        sparkfox_be_app::bootstrap::AdminBootstrap {
    ...
    let mut app = sparkfox_be_app::create_router(&services).await;
    if !args.api_only {
    ...
    tracing::info!(
        ...
        "sparkfox-web: embedded backend + SPA on one port"
    );
    ...
    let (actual_port, listener) = sparkfox_be_app::bootstrap::bind_with_fallback(ip, args.port).await?;
...
mod tests {
    ...
    fn api_only_flag_disables_static_spa_mode() {
        let args = Args::try_parse_from(["sparkfox-web", "--api-only"]).expect("parse --api-only");
    ...
    fn api_only_mode_does_not_require_a_static_distribution() {
        let args = Args::try_parse_from([
            ...
            "--api-only",
            "--dist",
            "/definitely/missing/sparkfox-ui-dist",
        ])
    ...
    fn static_mode_rejects_a_host_without_an_exact_embedded_build_id() {
        let args = Args::try_parse_from(["sparkfox-web", "--dist", "ui/dist"])
            .expect("parse static args");
    ...
    fn static_webui_feature_accepts_only_its_paired_distribution() {
        ...
        assert_eq!(
            ...
            option_env!("SPARKFOX_FRONTEND_BUILD_ID")
