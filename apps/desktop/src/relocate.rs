...
//! `sparkfox-be-companion/src/migrate.rs`, hardened with a cross-process lock and a
//! live-writer probe:
//!
//! 1. **Gate** — skip when `SPARKFOX_DATA_DIR` is set (checked by the caller in
...
//!    path rewrite (`sparkfox_be_app::bootstrap::rewrite_relocated_paths`).
...
//!
//! Outcomes are queued via `sparkfox_be_app::bootstrap::record_boot_note` and
...
/// is explicitly overridden (`SPARKFOX_DATA_DIR`) OR when this is a non-stable
...
pub fn effective_data_dir(default_dir: PathBuf) -> PathBuf {
    ...
    let env_override = std::env::var_os("SPARKFOX_DATA_DIR").is_some();
    if !should_relocate(env_override, sparkfox_be_app::channel::is_stable()) {
        return default_dir;
    }
...
fn boot_note(level: BootNoteLevel, message: String) {
    eprintln!("sparkfox-desktop: {message}");
...
fn copy_dir_recursive(from: &Path, to: &Path) -> std::io::Result<()> {
    ...
    for entry in std::fs::read_dir(from)? {
        ...
        // (junctions made by sparkfox-be-extension's skill_service via
...
mod tests {
    ...
    /// sparkfox-be-extension's symlink tests).
    #[cfg(windows)]
    ...
    fn env_override_skips_relocation_entirely() {
        ...
        unsafe { std::env::set_var("SPARKFOX_DATA_DIR", r"X:\custom-root") };
        let dir = PathBuf::from(r"X:\custom-root\Nomi");
        let resolved = effective_data_dir(dir.clone());
        unsafe { std::env::remove_var("SPARKFOX_DATA_DIR") };
