...
//! `sparkfox-desktop` — the Tauri shell (replaces the Electron shell).
//!
//! Core idea (the whole point of this rewrite): there is NO spawned backend
//! binary. The unified Rust backend (`sparkfox-be-app`, ex-`nomicore`) is linked
...
//!
//! ┌── sparkfox-desktop (this process) ──────────────────────────┐
//! │  Tauri shell (window/tray/dialog/deep-link/updater)         │
//! │  └─ tokio task: sparkfox_be_app embedded axum on 127.0.0.1:<p>  │
...
use anyhow::Context;
use clap::Parser;
use sparkfox_be_app::{DesktopServer, WebUiStatus};
...
/// the LAN listener. Probes a `sparkfox_webUI_DIST` override, several
...
fn resolve_webui_spa_dir(app: &tauri::App) -> anyhow::Result<Option<PathBuf>> {
    ...
    if let Some(p) = std::env::var_os("sparkfox_webUI_DIST") {
...
///
/// 1. `SPARKFOX_DATA_DIR` env — explicit override; the shell appends `/Nomi`
///    (semantics unchanged since the Electron era).
/// 2. The shared per-host default from `sparkfox_be_app::cli::default_data_dir()`:
...
fn default_data_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("SPARKFOX_DATA_DIR") {
        return PathBuf::from(dir).join("Nomi");
    }
    sparkfox_be_app::cli::default_data_dir()
}
...
fn acquire_keep_awake() -> Result<keepawake::KeepAwake, String> {
    keepawake::Builder::default()
        ...
        .reason("SparkFox keep-awake enabled")
        .app_name("SparkFox")
        .app_reverse_domain("com.sparkfox.desktop")
        .create()
...
fn main() -> std::process::ExitCode {
    ...
    sparkfox_be_runtime::init(&data_dir);
    ...
    let mut cli = sparkfox_be_app::cli::Cli::parse_from(["sparkfox-desktop"]);
    ...
    // At `debug`, the `sparkfox_ag_providers` target logs the outgoing request body and
    // each SSE chunk, and `sparkfox_ag_mcp` logs MCP connect results — exactly what is
    ...
    let app = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let tray_show = MenuItem::with_id(app, "tray-show", "Show SparkFox", true, None::<&str>)?;
...
mod tests {
    ...
    fn production_spa_candidate_rejects_a_manifestless_bundle() {
        ...
        assert!(format!("{error:#}").contains("sparkfox-build.json"));
    }
